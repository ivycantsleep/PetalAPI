...
PUCHAR
ACPIRegLocalCopyString(PUCHAR Destination, PUCHAR Source, ULONG MaxLength)
/*++
...
__*/
{
    ...
    for (i = 0; i < MaxLength; i++)
    {
        ...
        { // Translate blanks to underscores
            Destination[i] = '_';
        }
        else
        {
...
__*/
{
    ...
    RtlCopyMemory(buffer, ACPI_PARAMETERS_REGISTRY_KEY, baseSize);
    buffer += baseSize;
    *buffer++ = '\\';
    ...
    *buffer = 0; // Terminate
    ...
    //
    status = OSOpenLargestSubkey(tableIdKey, &revisionKey, header->OEMRevision);
    if (!NT_SUCCESS(status))
    {
    ...
    status = OSReadRegValue("Action", revisionKey, &action, &bytesRead);
...
--*/
{
    ...
    for (index = 0;; index++)
    {
        ...
        //
        bytesRead = 8 * 1024;
        status = OSReadRegValue(value, RevisionKey, buffer, &bytesRead);
        if (!NT_SUCCESS(status))
        {
        ...
        for (entry = 0; entry < bytesRead; entry += (entryHeader->Length + sizeof(REGISTRY_HEADER)))
        {
            ...
            if (entryHeader->Length == 0)
            {
                ...
                if (entryHeader->Offset != descHeader->Length)
                {
                    ...
                    //
                    table = ExAllocatePoolWithTag(NonPagedPool, entryHeader->Offset, ACPI_SHARED_TABLE_POOLTAG);
                    if (table == NULL)
                    {
            ...
            RtlCopyMemory(((PUCHAR)*Table) + entryHeader->Offset, (PUCHAR)entryHeader + sizeof(REGISTRY_HEADER),
                          entryHeader->Length);
        }
    }
