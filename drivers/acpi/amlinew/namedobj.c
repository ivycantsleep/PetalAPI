...
NTSTATUS LOCAL BankField(PCTXT pctxt, PTERM pterm)
{
    ...
    if (((rc = GetNameSpaceObject((PSZ)pterm->pdataArgs[0].pbDataBuff, pctxt->pnsScope, &pnsBase, NSF_WARN_NOTFOUND)) ==
         STATUS_SUCCESS) &&
        ((rc = GetNameSpaceObject((PSZ)pterm->pdataArgs[1].pbDataBuff, pctxt->pnsScope, &pnsBank, NSF_WARN_NOTFOUND)) ==
         STATUS_SUCCESS))
    {
        if (pnsBase->ObjData.dwDataType != OBJTYPE_OPREGION)
        {
            rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE,
                             ("BankField: %s is not an operation region", pterm->pdataArgs[0].pbDataBuff));
        }
        else if (pnsBank->ObjData.dwDataType != OBJTYPE_FIELDUNIT)
        {
            rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE,
                             ("BankField: %s is not a field unit", pterm->pdataArgs[1].pbDataBuff));
        }
        else if ((rc = CreateNameSpaceObject(pctxt->pheapCurrent, NULL, pctxt->pnsScope, pctxt->powner, &pterm->pnsObj,
                                             0)) == STATUS_SUCCESS)
        {
            pterm->pnsObj->ObjData.dwDataType = OBJTYPE_BANKFIELD;
            pterm->pnsObj->ObjData.dwDataLen = sizeof(BANKFIELDOBJ);

            if ((pterm->pnsObj->ObjData.pbDataBuff = NEWKFOBJ(pctxt->pheapCurrent, pterm->pnsObj->ObjData.dwDataLen)) ==
                NULL)
            {
                rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("BankField: failed to allocate BankField object"));
            }
            else
            {
                PBANKFIELDOBJ pbf;

                MEMZERO(pterm->pnsObj->ObjData.pbDataBuff, pterm->pnsObj->ObjData.dwDataLen);
                pbf = (PBANKFIELDOBJ)pterm->pnsObj->ObjData.pbDataBuff;
                pbf->pnsBase = pnsBase;
                pbf->pnsBank = pnsBank;
                pbf->dwBankValue = (ULONG)pterm->pdataArgs[2].uipDataValue;
                rc = ParseFieldList(pctxt, pterm->pbOpEnd, pterm->pnsObj, (ULONG)pterm->pdataArgs[3].uipDataValue,
...
NTSTATUS LOCAL CreateXField(PCTXT pctxt, PTERM pterm, POBJDATA pdataTarget, PBUFFFIELDOBJ *ppbf)
{
    ...
    if (((rc = ValidateArgTypes(pterm->pdataArgs, "BI")) == STATUS_SUCCESS) &&
        ((rc = CreateNameSpaceObject(pctxt->pheapCurrent, (PSZ)pdataTarget->pbDataBuff, pctxt->pnsScope, pctxt->powner,
                                     &pterm->pnsObj, 0)) == STATUS_SUCCESS))
    {
        ...
        if ((pdata->pbDataBuff = NEWBFOBJ(pctxt->pheapCurrent, pdata->dwDataLen)) == NULL)
        {
            rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("CreateXField: failed to allocate BuffField object"));
        }
        else
        {
...
NTSTATUS LOCAL CreateBitField(PCTXT pctxt, PTERM pterm)
{
    ...
    {
        pbf->FieldDesc.dwByteOffset = (ULONG)(pterm->pdataArgs[1].uipDataValue / 8);
        pbf->FieldDesc.dwStartBitPos = (ULONG)(pterm->pdataArgs[1].uipDataValue - pbf->FieldDesc.dwByteOffset * 8);
...
NTSTATUS LOCAL CreateField(PCTXT pctxt, PTERM pterm)
{
    ...
    {
        if ((rc = CreateXField(pctxt, pterm, &pterm->pdataArgs[3], &pbf)) == STATUS_SUCCESS)
        {
            pbf->FieldDesc.dwByteOffset = (ULONG)(pterm->pdataArgs[1].uipDataValue / 8);
            pbf->FieldDesc.dwStartBitPos = (ULONG)(pterm->pdataArgs[1].uipDataValue - pbf->FieldDesc.dwByteOffset * 8);
    ...
    {
        rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_ARGTYPE, ("CreateField: NoBits must be evaluated to integer type"));
    }
...
NTSTATUS LOCAL Device(PCTXT pctxt, PTERM pterm)
{
    ...
    {                               // next OP exist
        if (DeviceDef[0] == 0x5B && // 5B 82 1A definition #1
            DeviceDef[1] == 0x82 && DeviceDef[2] == 0x1A)
        {
            ProcDefType = 1;
            DeviceOpSize = 28;
            KdPrint(("Try ACPI0007 def #1 \n"));
        }
        else if (DeviceDef[0] == 0x5B && // 5B 82 1B definition #2
                 DeviceDef[1] == 0x82 && DeviceDef[2] == 0x1B)
        {
            ProcDefType = 2;
            DeviceOpSize = 29;
    ...
    if (ProcDefType == 0 || DeviceDef[13] != 'A' || DeviceDef[14] != 'C' || DeviceDef[15] != 'P' ||
        DeviceDef[16] != 'I' || DeviceDef[17] != '0' || DeviceDef[18] != '0' || DeviceDef[19] != '0' ||
        DeviceDef[20] != '7')
    ...
    if (ProcDefType != 0)
    {
        ...
        KdPrint(("ACPI0007 CPU=%x PBlk=%x NextOp=%X,%X,%X,%X \n", DeviceDef[7], dwPBlk, DeviceDef[DeviceOpSize],
                 DeviceDef[DeviceOpSize + 1], DeviceDef[DeviceOpSize + 2], DeviceDef[DeviceOpSize + 3]));
    }
    else
    { // normal Device()
        ...
        else if (rc == AMLIERR_OBJ_ALREADY_EXIST)
        {
            ...
            if (NextOp)
            { // next OP exist
                if (OneByte[0] == 0x5B && OneByte[1] == 0x82)
                {                                           // 0x5B 0x82 Device() Opcode
                    pctxt->pbOp = OneByte + OneByte[2] + 2; // start + pkglength + opcodelength
                }
                else if (TwoByte[0] == 0x5B && TwoByte[1] == 0x82)
                {
                    pctxt->pbOp = TwoByte + (TwoByte[3] << 4) + (TwoByte[2] & 0x0F) + 2; // pkglength magic
                }
                else if (ThreeByte[0] == 0x5B && ThreeByte[1] == 0x82)
                {
                    pctxt->pbOp =
                        ThreeByte + (ThreeByte[4] << (4 + 8)) + (ThreeByte[3] << 4) + (ThreeByte[2] & 0x0F) + 2;
                }
                else if (FourByte[0] == 0x5B && FourByte[1] == 0x82)
                {
                    pctxt->pbOp = FourByte + (FourByte[5] << (4 + 8 + 8)) + (FourByte[4] << (4 + 8)) +
                                  (FourByte[3] << 4) + (FourByte[2] & 0x0F) + 2;
                }
                else
...
NTSTATUS LOCAL InitEvent(PHEAP pheap, PNSOBJ pns)
{
    ...
    if ((pns->ObjData.pbDataBuff = NEWEVOBJ(pheap, pns->ObjData.dwDataLen)) == NULL)
    {
        rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("InitEvent: failed to allocate Event object"));
    }
    else
    {
...
NTSTATUS LOCAL Field(PCTXT pctxt, PTERM pterm)
{
        ...
        STATUS_SUCCESS)
    {
        ...
        {
            rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE,
                             ("Field: %s is not an operation region", pterm->pdataArgs[0].pbDataBuff));
        }
        else if ((rc = CreateNameSpaceObject(pctxt->pheapCurrent, NULL, pctxt->pnsScope, pctxt->powner, &pterm->pnsObj,
                                             0)) == STATUS_SUCCESS)
        {
            ...
            if ((pterm->pnsObj->ObjData.pbDataBuff = NEWFOBJ(pctxt->pheapCurrent, pterm->pnsObj->ObjData.dwDataLen)) ==
                NULL)
            {
                rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("Field: failed to allocate Field object"));
            }
            else
            {
                PFIELDOBJ pfd;
                ...
                pfd->pnsBase = pnsBase;
                rc = ParseFieldList(pctxt, pterm->pbOpEnd, pterm->pnsObj, (ULONG)pterm->pdataArgs[1].uipDataValue,
                                    ((POPREGIONOBJ)pnsBase->ObjData.pbDataBuff)->dwLen);
            }
        }
    }
...
NTSTATUS LOCAL IndexField(PCTXT pctxt, PTERM pterm)
{
    ...
    if (((rc = GetNameSpaceObject((PSZ)pterm->pdataArgs[0].pbDataBuff, pctxt->pnsScope, &pnsIdx, NSF_WARN_NOTFOUND)) ==
         STATUS_SUCCESS) &&
        ((rc = GetNameSpaceObject((PSZ)pterm->pdataArgs[1].pbDataBuff, pctxt->pnsScope, &pnsData, NSF_WARN_NOTFOUND)) ==
         STATUS_SUCCESS))
    {
        if (pnsIdx->ObjData.dwDataType != OBJTYPE_FIELDUNIT)
        {
            rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE,
                             ("IndexField: Index (%s) is not a field unit", pterm->pdataArgs[0].pbDataBuff));
        }
        else if (pnsData->ObjData.dwDataType != OBJTYPE_FIELDUNIT)
        {
            rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE,
                             ("IndexField: Data (%s) is not a field unit", pterm->pdataArgs[1].pbDataBuff));
        }
        else if ((rc = CreateNameSpaceObject(pctxt->pheapCurrent, NULL, pctxt->pnsScope, pctxt->powner, &pterm->pnsObj,
                                             0)) == STATUS_SUCCESS)
        {
            ...
            if ((pterm->pnsObj->ObjData.pbDataBuff = NEWIFOBJ(pctxt->pheapCurrent, pterm->pnsObj->ObjData.dwDataLen)) ==
                NULL)
            {
                rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("IndexField: failed to allocate IndexField object"));
            }
            else
            {
                PINDEXFIELDOBJ pif;
                ...
                pif->pnsIndex = pnsIdx;
                pif->pnsData = pnsData;
                rc = ParseFieldList(pctxt, pterm->pbOpEnd, pterm->pnsObj, (ULONG)pterm->pdataArgs[2].uipDataValue,
                                    0xffffffff);
            }
        }
    }
...
NTSTATUS LOCAL Method(PCTXT pctxt, PTERM pterm)
{
                                    ...
                                    pctxt->powner, &pterm->pnsObj, 0)) == STATUS_SUCCESS)
    {
        ...
        pterm->pnsObj->ObjData.dwDataLen = (ULONG)(FIELD_OFFSET(METHODOBJ, abCodeBuff) + pterm->pbOpEnd - pctxt->pbOp);

        if ((pterm->pnsObj->ObjData.pbDataBuff = NEWMEOBJ(pctxt->pheapCurrent, pterm->pnsObj->ObjData.dwDataLen)) ==
            NULL)
        {
            rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("Method: failed to allocate method buffer"));
        }
        else
        {
...
NTSTATUS LOCAL InitMutex(PHEAP pheap, PNSOBJ pns, ULONG dwLevel)
{
    ...
    if ((pns->ObjData.pbDataBuff = NEWMTOBJ(pheap, pns->ObjData.dwDataLen)) == NULL)
    {
        rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("InitMutex: failed to allocate Mutex object"));
    }
    else
    {
        MEMZERO(pns->ObjData.pbDataBuff, pns->ObjData.dwDataLen);
...
NTSTATUS LOCAL Mutex(PCTXT pctxt, PTERM pterm)
{
    ...
    if ((rc = CreateNameSpaceObject(pctxt->pheapCurrent, (PSZ)pterm->pdataArgs[0].pbDataBuff, pctxt->pnsScope,
                                    pctxt->powner, &pterm->pnsObj, 0)) == STATUS_SUCCESS)
    {
        rc = InitMutex(pctxt->pheapCurrent, pterm->pnsObj, (ULONG)pterm->pdataArgs[1].uipDataValue);
    }
...
NTSTATUS LOCAL OpRegion(PCTXT pctxt, PTERM pterm)
{
                                    ...
                                    pctxt->powner, &pterm->pnsObj, 0)) == STATUS_SUCCESS)
    {
        ...
        if ((pterm->pnsObj->ObjData.pbDataBuff = NEWOROBJ(pctxt->pheapCurrent, pterm->pnsObj->ObjData.dwDataLen)) ==
            NULL)
        {
            rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("OpRegion: failed to allocate OpRegion object"));
        }
        else
        {
            POPREGIONOBJ pop;
            ...
            if (pop->bRegionSpace == REGSPACE_MEM)
            {
                ...
                rc = MapUnmapPhysMem(pctxt, pop->uipOffset, pop->dwLen, &pop->uipOffset);
            }
            else if (pop->bRegionSpace == REGSPACE_IO)
            {
                ...
                else
                {
                    rc = AMLI_LOGERR(AMLIERR_FAILED_ADDR_XLATE,
                                     ("OpRegion: failed to translate IO address %x", pop->uipOffset));
                }
            }
...
NTSTATUS LOCAL PowerRes(PCTXT pctxt, PTERM pterm)
{
                                    ...
                                    pctxt->powner, &pterm->pnsObj, 0)) == STATUS_SUCCESS)
    {
        ...
        if ((pterm->pnsObj->ObjData.pbDataBuff = NEWPROBJ(pctxt->pheapCurrent, pterm->pnsObj->ObjData.dwDataLen)) ==
            NULL)
        {
            rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("PowerRes: failed to allocate PowerRes object"));
        }
        else
        {
            PPOWERRESOBJ ppr;
...
NTSTATUS LOCAL Processor(PCTXT pctxt, PTERM pterm)
{
                                    ...
                                    pctxt->powner, &pterm->pnsObj, 0)) == STATUS_SUCCESS)
    {
        ...
        if ((pterm->pnsObj->ObjData.pbDataBuff = NEWPCOBJ(pctxt->pheapCurrent, pterm->pnsObj->ObjData.dwDataLen)) ==
            NULL)
        {
            rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("Processor: failed to allocate processor object"));
        }
        else
        {
            PPROCESSOROBJ pproc;
