...
NTSTATUS LOCAL ReadObject(PCTXT pctxt, POBJDATA pdataObj, POBJDATA pdataResult)
{
    ...
    case OBJTYPE_FIELDUNIT:
        if ((rc = PushFrame(pctxt, SIG_ACCFIELDUNIT, sizeof(ACCFIELDUNIT), AccFieldUnit, &pafu)) == STATUS_SUCCESS)
        {
    ...
    case OBJTYPE_BUFFFIELD:
        rc = ReadField(pctxt, pdataObj, &((PBUFFFIELDOBJ)pdataObj->pbDataBuff)->FieldDesc, pdataResult);
        break;
...
NTSTATUS LOCAL WriteObject(PCTXT pctxt, POBJDATA pdataObj, POBJDATA pdataSrc)
{
    ...
    case OBJTYPE_BUFFFIELD:
        rc = WriteField(pctxt, pdataObj, &((PBUFFFIELDOBJ)pdataObj->pbDataBuff)->FieldDesc, pdataSrc);
        break;
    ...
    case OBJTYPE_INTDATA:
        rc = CopyObjBuffer((PUCHAR)&pdataObj->uipDataValue, sizeof(ULONG), pdataSrc);
        break;

    case OBJTYPE_STRDATA:
        rc = CopyObjBuffer(pdataObj->pbDataBuff, pdataObj->dwDataLen - 1, pdataSrc);
        break;

    case OBJTYPE_BUFFDATA:
        rc = CopyObjBuffer(pdataObj->pbDataBuff, pdataObj->dwDataLen, pdataSrc);
        break;

    default:
        rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE, ("WriteObject: unexpected target object type (type=%s)",
                                                      GetObjectTypeName(pdataObj->dwDataType)));
    }

    EXIT(3, ("WriteObject=%x (ObjType=%s,DataType=%x,Value=%x,Buff=%x)\n", rc, GetObjectTypeName(pdataObj->dwDataType),
             pdataSrc->dwDataType, pdataSrc->uipDataValue, pdataSrc->pbDataBuff));
    return rc;
} //WriteObject
...
NTSTATUS LOCAL AccFieldUnit(PCTXT pctxt, PACCFIELDUNIT pafu, NTSTATUS rc)
{
    ...
    ULONG dwStage = (rc == STATUS_SUCCESS) ? (pafu->FrameHdr.dwfFrame & FRAMEF_STAGE_MASK) : 3;
    PFIELDUNITOBJ pfu;
    ...
    case 0:
        ...
        if (pdataParent->dwDataType == OBJTYPE_BANKFIELD)
        {
            ...
            rc = PushAccFieldObj(pctxt, WriteFieldObj, pdataBank, &((PFIELDUNITOBJ)pdataBank->pbDataBuff)->FieldDesc,
                                 (PUCHAR)&pbf->dwBankValue, sizeof(ULONG));
            break;
        }
    ...
    case 2:
        ...
        {
            rc = ReadField(pctxt, pafu->pdataObj, &pfu->FieldDesc, pafu->pdata);
        }
        else
        {
            rc = WriteField(pctxt, pafu->pdataObj, &pfu->FieldDesc, pafu->pdata);
        }

        if ((rc == AMLISTA_PENDING) || (&pafu->FrameHdr != (PFRAMEHDR)pctxt->LocalHeap.pbHeapEnd))
        {
            break;
        }
...
NTSTATUS LOCAL ReadField(PCTXT pctxt, POBJDATA pdataObj, PFIELDDESC pfd, POBJDATA pdataResult)
{
    ...
    if ((pfd->dwFieldFlags & ACCTYPE_MASK) <= ACCTYPE_QWORD)
    {
        ...
        case OBJTYPE_UNKNOWN:
            ...
            else
            {
                ...
                if ((pdataResult->pbDataBuff = NEWBDOBJ(gpheapGlobal, pdataResult->dwDataLen)) == NULL)
                {
                    rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM,
                                     ("ReadField: failed to allocate target buffer (size=%d)", pdataResult->dwDataLen));
                    pb = NULL;
                    dwcb = 0;
                }
                else
                {
                    MEMZERO(pdataResult->pbDataBuff, pdataResult->dwDataLen);
        ...
        default:
            rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE, ("ReadField: invalid target data type (type=%s)",
    ...
    else
    {
        rc = AMLI_LOGERR(AMLIERR_INVALID_ACCSIZE,
                         ("ReadField: invalid access size for buffer field (FieldFlags=%x)", pfd->dwFieldFlags));
    }
...
NTSTATUS LOCAL WriteField(PCTXT pctxt, POBJDATA pdataObj, PFIELDDESC pfd, POBJDATA pdataSrc)
{
    ...
    if ((pfd->dwFieldFlags & ACCTYPE_MASK) <= ACCTYPE_QWORD)
    {
        ...
        default:
            rc = AMLI_LOGERR(AMLIERR_UNEXPECTED_OBJTYPE, ("WriteField: invalid source data type (type=%s)\n",
        ...
        if ((rc == STATUS_SUCCESS) &&
            ((rc = PushFrame(pctxt, SIG_WRFIELDLOOP, sizeof(WRFIELDLOOP), WriteFieldLoop, &pwfl)) == STATUS_SUCCESS))
        {
    ...
    {
        rc = AMLI_LOGERR(AMLIERR_INVALID_ACCSIZE,
                         ("WriteField: invalid access size for buffer field (FieldFlags=%x)", pfd->dwFieldFlags));
    }
...
NTSTATUS LOCAL WriteFieldLoop(PCTXT pctxt, PWRFIELDLOOP pwfl, NTSTATUS rc)
{
    ...
    ULONG dwStage = (rc == STATUS_SUCCESS) ? (pwfl->FrameHdr.dwfFrame & FRAMEF_STAGE_MASK) : 1;
    ULONG dwXactionSize;
    ...
    case 0:
        ...
        if (pwfl->dwBuffSize > 0)
        {
            ...
            rc = PushAccFieldObj(pctxt, WriteFieldObj, pwfl->pdataObj, pwfl->pfd, pwfl->pbBuff, dwXactionSize);
...
NTSTATUS LOCAL PushAccFieldObj(PCTXT pctxt, PFNPARSE pfnAcc, POBJDATA pdataObj, PFIELDDESC pfd, PUCHAR pb, ULONG dwcb)
{
    ...
    ENTER(3, ("PushAccFieldObj(pctxt=%x,pfnAcc=%x,pdataObj=%x,FieldDesc=%x,pb=%x,Size=%d)\n", pctxt, pfnAcc, pdataObj,
              pfd, pb, dwcb));

    if ((rc = PushFrame(pctxt, SIG_ACCFIELDOBJ, sizeof(ACCFIELDOBJ), pfnAcc, &pafo)) == STATUS_SUCCESS)
    {
        pafo->pdataObj = pdataObj;
        pafo->pbBuff = pb;
        pafo->pbBuffEnd = pb + dwcb;
        pafo->dwAccSize = ACCSIZE(pfd->dwFieldFlags);
        ASSERT((pafo->dwAccSize == sizeof(UCHAR)) || (pafo->dwAccSize == sizeof(USHORT)) ||
               (pafo->dwAccSize == sizeof(ULONG)));
        pafo->dwcAccesses = (pfd->dwStartBitPos + pfd->dwNumBits + pafo->dwAccSize * 8 - 1) / (pafo->dwAccSize * 8);
...
NTSTATUS LOCAL ReadFieldObj(PCTXT pctxt, PACCFIELDOBJ pafo, NTSTATUS rc)
{
    ...
    ULONG dwStage = (rc == STATUS_SUCCESS) ? (pafo->FrameHdr.dwfFrame & FRAMEF_STAGE_MASK) : 3;
    ...
    Stage0:
        ...
        else
        {
            ...
            {
                pdataParent = &((PFIELDUNITOBJ)pafo->pdataObj->pbDataBuff)->pnsFieldParent->ObjData;

                if (pdataParent->dwDataType == OBJTYPE_INDEXFIELD)
                {
                    PINDEXFIELDOBJ pif = (PINDEXFIELDOBJ)pdataParent->pbDataBuff;

                    rc = PushAccFieldObj(pctxt, WriteFieldObj, &pif->pnsIndex->ObjData,
                                         &((PFIELDUNITOBJ)pif->pnsIndex->ObjData.pbDataBuff)->FieldDesc,
                                         (PUCHAR)&pafo->fd.dwByteOffset, sizeof(ULONG));
                    break;
                }
            }
        }
    ...
    case 1:
        ...
        //
        pafo->FrameHdr.dwfFrame++;
        rc = AccessFieldData(pctxt, pafo->pdataObj, &pafo->fd, &pafo->dwData, TRUE);

        if ((rc != STATUS_SUCCESS) || (&pafo->FrameHdr != (PFRAMEHDR)pctxt->LocalHeap.pbHeapEnd))
        {
            break;
        }
    ...
    case 2:
        ...
        if (pafo->iAccess > 0)
        {
            WriteSystemMem((ULONG_PTR)pafo->pbBuff, pafo->dwAccSize,
                           SHIFTLEFT(pafo->dwData, pafo->iLBits) & pafo->dwDataMask,
        ...
        WriteSystemMem((ULONG_PTR)pafo->pbBuff, pafo->dwAccSize, pafo->dwData,
...
NTSTATUS LOCAL WriteFieldObj(PCTXT pctxt, PACCFIELDOBJ pafo, NTSTATUS rc)
{
    ...
    case 1:
        ...
        //
        pafo->FrameHdr.dwfFrame++;
        dwData1 = ReadSystemMem((ULONG_PTR)pafo->pbBuff, pafo->dwAccSize, pafo->dwDataMask);
        if (pafo->iAccess > 0)
        {
        ...
        if ((rc == AMLISTA_PENDING) || (&pafo->FrameHdr != (PFRAMEHDR)pctxt->LocalHeap.pbHeapEnd))
        {
            break;
        }
...
NTSTATUS LOCAL RawFieldAccess(PCTXT pctxt, ULONG dwAccType, POBJDATA pdataObj, POBJDATA pdataResult)
{
    ...
    {
    case OBJTYPE_FIELD:
        pop = (POPREGIONOBJ)((PFIELDOBJ)pdataParent->pbDataBuff)->pnsBase->ObjData.pbDataBuff;
        break;

    default:
        rc = AMLI_LOGERR(AMLIERR_ASSERT_FAILED,
                         ("RawFieldAccess: invalid field parent type (type=%s)", pdataParent->dwDataType));
        pop = NULL;
    }

    if (rc == STATUS_SUCCESS)
    {
        if (((prsa = FindRSAccess(pop->bRegionSpace)) != NULL) && (prsa->pfnRawAccess != NULL))
        {
#ifdef DEBUGGER
            ...
            {
                rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("RawFieldAccess: failed to allocate Field unit"));
            }
            else
            {
                ...
                rc = prsa->pfnRawAccess(dwAccType, pfuFieldUnit, pdataResult, prsa->uipRawParam, RestartCtxtCallback,
                                        &pctxt->CtxtData);
                if (rc == STATUS_BUFFER_TOO_SMALL)
                {
                    ...
                    {
                        rc = AMLI_LOGERR(AMLIERR_OUT_OF_MEM, ("Buffer: failed to allocate data buffer (size=%d)",
                                                              pdataResult->dwDataValue));
                    }
                    else
                    {
                        ...
                        pdataResult->dwDataType = OBJTYPE_BUFFDATA;
                        rc = prsa->pfnRawAccess(dwAccType, pfuFieldUnit, pdataResult, prsa->uipRawParam,
            ...
            else if (rc != STATUS_SUCCESS)
            {
                rc = AMLI_LOGERR(AMLIERR_RS_ACCESS,
                                 ("RawFieldAccess: RegionSpace %x handler returned error %x", pop->bRegionSpace, rc));
            }
        }
        else
        {
            rc = AMLI_LOGERR(AMLIERR_INVALID_REGIONSPACE,
                             ("RawFieldAccess: no handler for RegionSpace %x", pop->bRegionSpace));
        }
    }
...
NTSTATUS LOCAL AccessFieldData(PCTXT pctxt, POBJDATA pdataObj, PFIELDDESC pfd, PULONG pdwData, BOOLEAN fRead)
{
    ...
    if (pdataObj->dwDataType == OBJTYPE_BUFFFIELD)
    {
        if (fRead)
        {
            rc = ReadBuffField((PBUFFFIELDOBJ)pdataObj->pbDataBuff, pfd, pdwData);
        }
        else
        {
            rc = WriteBuffField((PBUFFFIELDOBJ)pdataObj->pbDataBuff, pfd, *pdwData);
        }
    }
    else //must be OBJTYPE_FIELDUNIT
    {
        ...
        if (pdataParent->dwDataType == OBJTYPE_INDEXFIELD)
        {
            ...
            if (fRead)
            {
                rc = PushAccFieldObj(pctxt, ReadFieldObj, &pif->pnsData->ObjData,
                                     &((PFIELDUNITOBJ)pif->pnsData->ObjData.pbDataBuff)->FieldDesc, (PUCHAR)pdwData,
                                     sizeof(ULONG));
            }
            else
            {
                ...
                dwPreserveMask = ~((SHIFTLEFT(1L, pfd->dwNumBits) - 1) << pfd->dwStartBitPos);
                dwAccMask = SHIFTLEFT(1L, ACCSIZE(pfd->dwFieldFlags) * 8) - 1;
                if (((pfd->dwFieldFlags & UPDATERULE_MASK) == UPDATERULE_PRESERVE) &&
                    ((dwPreserveMask & dwAccMask) != 0))
                {
                    rc = PushPreserveWriteObj(pctxt, &pif->pnsData->ObjData, *pdwData, dwPreserveMask);
                }
                else
                {
                    rc = PushAccFieldObj(pctxt, WriteFieldObj, &pif->pnsData->ObjData,
                                         &((PFIELDUNITOBJ)pif->pnsData->ObjData.pbDataBuff)->FieldDesc, (PUCHAR)pdwData,
                                         sizeof(ULONG));
                }
            }
        }
        else if ((rc = GetFieldUnitRegionObj((PFIELDUNITOBJ)pdataObj->pbDataBuff, &pnsBase)) == STATUS_SUCCESS &&
                 pnsBase != NULL)
        {
...
NTSTATUS LOCAL PushPreserveWriteObj(PCTXT pctxt, POBJDATA pdataObj, ULONG dwData, ULONG dwPreserveMask)
{
    ...
    if ((rc = PushFrame(pctxt, SIG_PRESERVEWROBJ, sizeof(PRESERVEWROBJ), PreserveWriteObj, &ppwro)) == STATUS_SUCCESS)
    {
...
NTSTATUS LOCAL PreserveWriteObj(PCTXT pctxt, PPRESERVEWROBJ ppwro, NTSTATUS rc)
{
    ...
    ULONG dwStage = (rc == STATUS_SUCCESS) ? (ppwro->FrameHdr.dwfFrame & FRAMEF_STAGE_MASK) : 2;
    ...
    case 0:
        ...
        rc = PushAccFieldObj(pctxt, ReadFieldObj, ppwro->pdataObj,
                             &((PFIELDUNITOBJ)ppwro->pdataObj->pbDataBuff)->FieldDesc, (PUCHAR)&ppwro->dwReadData,
                             sizeof(ULONG));
        break;
    ...
    case 1:
        ...
        rc = PushAccFieldObj(pctxt, WriteFieldObj, ppwro->pdataObj,
                             &((PFIELDUNITOBJ)ppwro->pdataObj->pbDataBuff)->FieldDesc, (PUCHAR)&ppwro->dwWriteData,
                             sizeof(ULONG));
        break;
...
NTSTATUS LOCAL AccessBaseField(PCTXT pctxt, PNSOBJ pnsBase, PFIELDDESC pfd, PULONG pdwData, BOOLEAN fRead)
{
    ...
    ENTER(3,
          ("AccessBaseField(pctxt=%x,pnsBase=%x,pfd=%x,pdwData=%x,fRead=%x)\n", pctxt, pnsBase, pfd, pdwData, fRead));
    ...
    fPreserve =
        (BOOLEAN)(((pfd->dwFieldFlags & UPDATERULE_MASK) == UPDATERULE_PRESERVE) && ((~dwDataMask & dwAccMask) != 0));

    if (!fRead && ((pfd->dwFieldFlags & UPDATERULE_MASK) == UPDATERULE_WRITEASONES))
    {
        *pdwData |= ~dwDataMask;
    }
    ...
    default:
        if (((prsa = FindRSAccess(pop->bRegionSpace)) != NULL) && (prsa->pfnCookAccess != NULL))
        {
            if (fRead)
            {
#ifdef DEBUGGER
                ...
                //
                ASSERT(!(pctxt->dwfCtxt & CTXTF_READY));
                rc = prsa->pfnCookAccess(RSACCESS_READ, pnsBase, uipAddr, dwSize, pdwData, prsa->uipCookParam,
                                         RestartCtxtCallback, &pctxt->CtxtData);
#ifdef DEBUGGER
                gDebugger.dwfDebugger = dwOldFlags;
#endif
                ...
                else if (rc != STATUS_SUCCESS)
                {
                    rc = AMLI_LOGERR(
                        AMLIERR_RS_ACCESS,
                        ("AccessBaseField: RegionSpace %x read handler returned error %x", pop->bRegionSpace, rc));
                }
            }
            else
            {
                ...
                // Write access.
                //
                if ((rc = PushFrame(pctxt, SIG_WRCOOKACC, sizeof(WRCOOKACC), WriteCookAccess, &pwca)) == STATUS_SUCCESS)
                {
        ...
        {
            rc = AMLI_LOGERR(AMLIERR_INVALID_REGIONSPACE,
                             ("AccessBaseField: no handler for RegionSpace %x", pop->bRegionSpace));
        }
    }

    EXIT(3, ("AccessBaseField=%x (Value=%x,Addr=%x,Size=%d,DataMask=%x,AccMask=%x)\n", rc, *pdwData, uipAddr, dwSize,
             dwDataMask, dwAccMask));
    return rc;
} //AccessBaseField
...
NTSTATUS LOCAL WriteCookAccess(PCTXT pctxt, PWRCOOKACC pwca, NTSTATUS rc)
{
    ...
    ULONG dwStage = (rc == STATUS_SUCCESS) ? (pwca->FrameHdr.dwfFrame & FRAMEF_STAGE_MASK) : 3;
    KIRQL oldIrql;
    LONG busy;
    ...
    case 0:
        ...
        if (pwca->fPreserve)
        {
            ...
            ASSERT(!(pctxt->dwfCtxt & CTXTF_READY));
            rc = pwca->prsa->pfnCookAccess(RSACCESS_READ, pwca->pnsBase, (ULONG_PTR)pwca->dwAddr, pwca->dwSize,
                                           &pwca->dwDataTmp, pwca->prsa->uipCookParam, RestartCtxtCallback,
                                           &pctxt->CtxtData);
#ifdef DEBUGGER
            gDebugger.dwfDebugger = dwOldFlags;
#endif
            ...
            {
                rc = AMLI_LOGERR(AMLIERR_RS_ACCESS, ("WriteCookAccess: RegionSpace %x read handler returned error %x",
                                                     pop->bRegionSpace, rc));
            }
...
#ifdef DEBUGGER
    {
        ...
        pwca->FrameHdr.dwfFrame++;
        ASSERT(!(pctxt->dwfCtxt & CTXTF_READY));
        rc = pwca->prsa->pfnCookAccess(RSACCESS_WRITE, pwca->pnsBase, (ULONG_PTR)pwca->dwAddr, pwca->dwSize,
                                       &pwca->dwData, pwca->prsa->uipCookParam, RestartCtxtCallback, &pctxt->CtxtData);
#ifdef DEBUGGER
        gDebugger.dwfDebugger = dwOldFlags;
    }
#endif
        ...
        {
            rc = AMLI_LOGERR(AMLIERR_RS_ACCESS,
                             ("WriteCookAccess: RegionSpace %x read handler returned error %x", pop->bRegionSpace, rc));
        }
...
NTSTATUS LOCAL ReadBuffField(PBUFFFIELDOBJ pbf, PFIELDDESC pfd, PULONG pdwData)
{
    ...
    {
        ULONG dwMask = (SHIFTLEFT(1L, pfd->dwNumBits) - 1) << pfd->dwStartBitPos;

        *pdwData = ReadSystemMem((ULONG_PTR)(pbf->pbDataBuff + pfd->dwByteOffset), dwAccSize, dwMask);
    }
    else
    {
...
NTSTATUS LOCAL WriteBuffField(PBUFFFIELDOBJ pbf, PFIELDDESC pfd, ULONG dwData)
{
    ...
    if (pfd->dwByteOffset + dwAccSize <= pbf->dwBuffLen)
    {
        ...
        WriteSystemMem((ULONG_PTR)(pbf->pbDataBuff + pfd->dwByteOffset), dwAccSize, dwData & dwMask, dwMask);
    }
    else
    {
        rc = AMLI_LOGERR(AMLIERR_INDEX_TOO_BIG,
...
VOID LOCAL DumpObject(POBJDATA pdata, PSZ pszName, int iLevel)
{
    ...
    int i;
    char szName1[sizeof(NAMESEG) + 1], szName2[sizeof(NAMESEG) + 1];
    ...
    case OBJTYPE_INTDATA:
        PRINTF("Integer(%s:Value=0x%08x[%d])", pszName, pdata->uipDataValue, pdata->uipDataValue);
        break;
    ...
    case OBJTYPE_BUFFDATA:
        PRINTF("Buffer(%s:Ptr=%x,Len=%d)", pszName, pdata->pbDataBuff, pdata->dwDataLen);
        PrintBuffData(pdata->pbDataBuff, pdata->dwDataLen);
        break;

    case OBJTYPE_PKGDATA:
        PRINTF("Package(%s:NumElements=%d){", pszName, ((PPACKAGEOBJ)pdata->pbDataBuff)->dwcElements);
        ...
        {
            DumpObject(&((PPACKAGEOBJ)pdata->pbDataBuff)->adata[i], NULL, fPrintNewLine ? iLevel + 1 : -1);

            if (!fPrintNewLine && (i < (int)((PPACKAGEOBJ)pdata->pbDataBuff)->dwcElements))
            {
                PRINTF(",");
            }
        }
    ...
    case OBJTYPE_FIELDUNIT:
        PRINTF("FieldUnit(%s:FieldParent=%p,ByteOffset=0x%x,StartBit=0x%x,NumBits=%d,FieldFlags=0x%x)", pszName,
    ...
    case OBJTYPE_METHOD:
        PRINTF("Method(%s:Flags=0x%x,CodeBuff=%p,Len=%d)", pszName, ((PMETHODOBJ)pdata->pbDataBuff)->bMethodFlags,
               ((PMETHODOBJ)pdata->pbDataBuff)->abCodeBuff, pdata->dwDataLen - FIELD_OFFSET(METHODOBJ, abCodeBuff));
        break;
    ...
    case OBJTYPE_OPREGION:
        PRINTF("OpRegion(%s:RegionSpace=%s,Offset=0x%x,Len=%d)", pszName,
               GetRegionSpaceName(((POPREGIONOBJ)pdata->pbDataBuff)->bRegionSpace),
               ((POPREGIONOBJ)pdata->pbDataBuff)->uipOffset, ((POPREGIONOBJ)pdata->pbDataBuff)->dwLen);
        break;

    case OBJTYPE_POWERRES:
        PRINTF("PowerResource(%s:SystemLevel=0x%x,ResOrder=%d)", pszName,
               ((PPOWERRESOBJ)pdata->pbDataBuff)->bSystemLevel, ((PPOWERRESOBJ)pdata->pbDataBuff)->bResOrder);
        break;

    case OBJTYPE_PROCESSOR:
        PRINTF("Processor(%s:ApicID=0x%x,PBlk=0x%x,PBlkLen=%d)", pszName, ((PPROCESSOROBJ)pdata->pbDataBuff)->bApicID,
               ((PPROCESSOROBJ)pdata->pbDataBuff)->dwPBlk, ((PPROCESSOROBJ)pdata->pbDataBuff)->dwPBlkLen);
        break;
    ...
    case OBJTYPE_BUFFFIELD:
        PRINTF("BufferField(%s:Ptr=%x,Len=%d,ByteOffset=0x%x,StartBit=0x%x,NumBits=%d,FieldFlags=0x%x)", pszName,
               ((PBUFFFIELDOBJ)pdata->pbDataBuff)->pbDataBuff, ((PBUFFFIELDOBJ)pdata->pbDataBuff)->dwBuffLen,
    ...
    case OBJTYPE_OBJALIAS:
        PRINTF("ObjectAlias(%s:Alias=%s,Type=%s)", pszName, GetObjectPath(pdata->pnsAlias),
    ...
    case OBJTYPE_BANKFIELD:
        STRCPYN(szName1, (PSZ)(&((PBANKFIELDOBJ)pdata->pbDataBuff)->pnsBase->dwNameSeg), sizeof(NAMESEG));
        STRCPYN(szName2, (PSZ)(&((PBANKFIELDOBJ)pdata->pbDataBuff)->pnsBank->dwNameSeg), sizeof(NAMESEG));
        PRINTF("BankField(%s:Base=%s,BankName=%s,BankValue=0x%x)", pszName, szName1, szName2,
               ((PBANKFIELDOBJ)pdata->pbDataBuff)->dwBankValue);
        break;

    case OBJTYPE_FIELD:
        STRCPYN(szName1, (PSZ)(&((PFIELDOBJ)pdata->pbDataBuff)->pnsBase->dwNameSeg), sizeof(NAMESEG));
        PRINTF("Field(%s:Base=%s)", pszName, szName1);
        break;

    case OBJTYPE_INDEXFIELD:
        STRCPYN(szName1, (PSZ)(&((PINDEXFIELDOBJ)pdata->pbDataBuff)->pnsIndex->dwNameSeg), sizeof(NAMESEG));
        STRCPYN(szName2, (PSZ)(&((PINDEXFIELDOBJ)pdata->pbDataBuff)->pnsData->dwNameSeg), sizeof(NAMESEG));
        PRINTF("IndexField(%s:IndexName=%s,DataName=%s)", pszName, szName1, szName2);
        break;

    default:
        AMLI_ERROR(("DumpObject: unexpected data object type (type=%x)", pdata->dwDataType));
    }
...
BOOLEAN LOCAL NeedGlobalLock(PFIELDUNITOBJ pfu)
{
    ...
    if ((pfu->FieldDesc.dwFieldFlags & FDF_NEEDLOCK) || (pfu->FieldDesc.dwFieldFlags & LOCKRULE_LOCK))
    {
        rc = TRUE;
    }
    else
    {
        ...
        if (pdataParent->dwDataType == OBJTYPE_BANKFIELD)
        {
            pfuParent = (PFIELDUNITOBJ)((PBANKFIELDOBJ)pdataParent->pbDataBuff)->pnsBank->ObjData.pbDataBuff;
        ...
        {
            pfuParent = (PFIELDUNITOBJ)((PINDEXFIELDOBJ)pdataParent->pbDataBuff)->pnsIndex->ObjData.pbDataBuff;
            if (pfuParent->FieldDesc.dwFieldFlags & LOCKRULE_LOCK)
            {
                rc = TRUE;
            }
            ...
            {
                pfuParent = (PFIELDUNITOBJ)((PINDEXFIELDOBJ)pdataParent->pbDataBuff)->pnsData->ObjData.pbDataBuff;
                if (pfuParent->FieldDesc.dwFieldFlags & LOCKRULE_LOCK)
                {
                    rc = TRUE;
                }
...
BOOLEAN LOCAL CheckSystemIOAddressValidity(BOOLEAN fRead, ULONG dwAddr, ULONG dwSize, PULONG pdwData)
{
    ...
    if (gpBadIOAddressList)
    {
        ...
        for (i = 0; gpBadIOAddressList[i].BadAddrSize != 0; i++)
        {
            ...
            //
            if ((dwAddr >= (gpBadIOAddressList[i].BadAddrBegin)) &&
                (dwAddr < ((gpBadIOAddressList[i].BadAddrBegin) + (gpBadIOAddressList[i].BadAddrSize))))
            {
                ...
                {
                    PRINTF("CheckSystemIOAddressValidity: Passing for compatibility reasons on illegal IO address "
                           "(0x%x).\n",
                           dwAddr);
                ...
                else
                {
                    ...
                    if (pACPIDeviceObject)
                    {
                        ...
                        if (Log_WorkItem)
                        {
                            ...
                            pWorkItemContext = (PAMLI_LOG_WORKITEM_CONTEXT)ExAllocatePoolWithTag(
                                NonPagedPool, sizeof(AMLI_LOG_WORKITEM_CONTEXT), PRIV_TAG);
                            if (pWorkItemContext)
                            {
                                ...
                                IoQueueWorkItem(Log_WorkItem, DelayedLogInErrorLog, DelayedWorkQueue,
                            ...
                            else
                            {
                                ...
                                //
                                PRINTF("CheckSystemIOAddressValidity: Failed to allocate contxt block from pool to "
                                       "spin off a logging work item.\n");
                        ...
                        else
                        {
                            ...
                            //
                            PRINTF("CheckSystemIOAddressValidity: Failed to allocate a workitem to spin off delayed "
                                   "logging.\n");
                        }
                    }
                    else
...
VOID DelayedLogInErrorLog(IN PDEVICE_OBJECT DeviceObject, IN PVOID Context)
{
    ...
    LogInErrorLog(((PAMLI_LOG_WORKITEM_CONTEXT)Context)->fRead, ((PAMLI_LOG_WORKITEM_CONTEXT)Context)->Address,
...
VOID LOCAL LogInErrorLog(BOOLEAN fRead, ULONG dwAddr, ULONG ArrayIndex)
{
    ...
    if (gpBadIOErrorLogDoneList)
    {
        ...
        if (!(gpBadIOErrorLogDoneList[ArrayIndex] & (fRead ? READ_ERROR_NOTED : WRITE_ERROR_NOTED)))
        {
            ...
            swprintf(addressRangeBuffer, L"0x%x - 0x%x", gpBadIOAddressList[ArrayIndex].BadAddrBegin,
        ...
        if (status == STATUS_INFO_LENGTH_MISMATCH)
        {
            if (Length)
            {
                ...
                else
                {
                    ...
                    //
                    status = HalQuerySystemInformation(HalQueryAMLIIllegalIOPortAddresses, Length, gpBadIOAddressList,
                                                       &Length);
                    //
                    // Cleanup on failure.
                    //
                    ...
                    {
                        PRINTF("InitIllegalIOAddressListFromHAL: HalQuerySystemInformation failed to get list from "
                               "HAL. Returned(%x).\n",
                               status);
                    ...
                    else
                    {
                        ...
                        if (ArrayLength >= 1)
                        {
                            if ((gpBadIOErrorLogDoneList = (PULONG)MALLOC((ArrayLength * sizeof(ULONG)), PRIV_TAG)) ==
                                NULL)
                            {
                                AMLI_LOGERR(AMLIERR_OUT_OF_MEM,
            ...
            {
                PRINTF("InitIllegalIOAddressListFromHAL: HalQuerySystemInformation (HalQueryIllegalIOPortAddresses) "
                       "returned 0 Length.\n");
            }
        }
        else if (status == STATUS_INVALID_LEVEL)
        {
            PRINTF("InitIllegalIOAddressListFromHAL: HalQuerySystemInformation does not support "
                   "HalQueryIllegalIOPortAddresses returned (STATUS_INVALID_LEVEL).\n");
        }
        else
        {
