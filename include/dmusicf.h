...
#define DMUS_FOURCC_BANDITEM_CHUNK2 mmioFOURCC('b', 'd', '2', 'h')
    ...
    {
        MUSIC_TIME
            lBandTimeLogical; /* Position in track list. Time in the music with which band change is associated. */
...
#define DMUS_FOURCC_WAVEHEADER_CHUNK mmioFOURCC('w', 'a', 'v', 'h')
    ...
    {
        REFERENCE_TIME
            rtReadAhead; /* How far ahead in the stream wave data will be read (in REFERENCE_TIME).  Ignored for one-shot waves.  */
        DWORD
            dwFlags; /* Various flags, including whether this is a streaming wave and whether it can be invalidated. */
    } DMUS_IO_WAVE_HEADER;
...
#define DMUS_WAVETRACKF_PERSIST_CONTROL 0x2
    ...
    {
        long
            lVolume; /* Gain, in 1/100th of dB, to be applied to all waves.  Note:  All gain values should be negative. */
    ...
    {
        long
            lVolume; /* Gain, in 1/100th of dB, to be applied to all waves in wave part.  Note:  All gain values should be negative. */
    ...
    typedef struct _DMUS_IO_WAVE_ITEM_HEADER
    {
        ...
        REFERENCE_TIME
            rtTime; /* Start time, in REFERENCE_TIME, if clock time track, or MUSIC_TIME for music time track. */
        ...
        MUSIC_TIME
            mtLogicalTime; /* If in music track format, this indicates the musical boundary where this belongs. Otherwise, ignored. */
...
#define DMUS_CONTAINER_NOLOADS \
    (1 << 1) /* Contained items are not loaded when the container is loaded.
...
#define DMUS_FOURCC_TRACKREFS_LIST \
    mmioFOURCC('t', 'r', 's', 'l') /* Set of track references within the segment reference. */
...
#define DMUS_SONG_NOFROMSEG \
    0x80000001 /* Special ID for dwSegmentID to indicate transition from nothing (or outside the song) into this segment. */
...
#define DMUS_SEGMENTTRACKF_MOTIF \
    1 /* interpret DMRF as link to style, and use snam as the name of a motif within the style */
 ...
 */
#define DMUS_IO_SCRIPTTRACKF_PREPARE \
    (1                               \
     << 0) /* Fire event in advance of time stamp, at Prepare time.  This is the default because it leaves the script time to change the music happening at the target time. */
...
#define DMUS_FOURCC_LYRICSTRACKEVENTTEXT_CHUNK mmioFOURCC('l', 'y', 'r', 'n')
    ...
    typedef struct _DMUS_IO_LYRICSTRACK_EVENTHEADER
    {
        DWORD dwFlags; /* Reserved leave as 0. */
        DWORD
            dwTimingFlags; /* Combination DMUS_PMSGF_TOOL_* flags.  Determines the precise timing of when the notification happens. Invalid with the flag DMUS_PMSGF_REFTIME, DMUS_PMSGF_MUSICTIME, DMUS_PMSGF_TOOL_FLUSH, or DMUS_PMSGF_LOCKTOREFTIME. */
        MUSIC_TIME lTimeLogical;  /* Position in track list. Time in the music with which the event is associated. */
        MUSIC_TIME lTimePhysical; /* Precise time event will be triggered. Should be close to logical time. */
...
#define DMUS_FOURCC_PARAMCONTROLTRACK_CURVES_CHUNK mmioFOURCC('p', 'r', 'c', 'c')
    ...
    typedef struct _DMUS_IO_PARAMCONTROLTRACK_OBJECTHEADER
    {
        ...
        GUID
            guidTimeFormat; /* Time format to set the object to.  Must be GUID_TIME_REFERNCE or GUID_TIME_MUSIC from medparam.h. */
