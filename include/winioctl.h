...
DEFINE_GUID(GUID_DEVINTERFACE_WRITEONCEDISK, 0x53f5630cL, 0xb6bf, 0x11d0, 0x94, 0xf2, 0x00, 0xa0, 0xc9, 0x1e, 0xfb,
            0x8b);
DEFINE_GUID(GUID_DEVINTERFACE_VOLUME, 0x53f5630dL, 0xb6bf, 0x11d0, 0x94, 0xf2, 0x00, 0xa0, 0xc9, 0x1e, 0xfb, 0x8b);
DEFINE_GUID(GUID_DEVINTERFACE_MEDIUMCHANGER, 0x53f56310L, 0xb6bf, 0x11d0, 0x94, 0xf2, 0x00, 0xa0, 0xc9, 0x1e, 0xfb,
            0x8b);
...
DEFINE_GUID(GUID_DEVINTERFACE_SERENUM_BUS_ENUMERATOR, 0x4D36E978L, 0xE325, 0x11CE, 0xBF, 0xC1, 0x08, 0x00, 0x2B, 0xE1,
            0x03, 0x18);
...
#define IsRecognizedPartition(PartitionType)                                                      \
    (((PartitionType & PARTITION_NTFT) && ((PartitionType & ~0xC0) == PARTITION_FAT_12)) ||       \
     ...
     ((PartitionType) == PARTITION_FAT_12) || ((PartitionType) == PARTITION_FAT_16) ||            \
     ((PartitionType) == PARTITION_IFS) || ((PartitionType) == PARTITION_HUGE) ||                 \
     ((PartitionType) == PARTITION_FAT32) || ((PartitionType) == PARTITION_FAT32_XINT13) ||       \
...
typedef struct _SENDCMDOUTPARAMS
{
    ...
    BYTE bBuffer
        [1]; // Buffer of arbitrary length in which to store the data read from the                                                                                  // drive.
...
#define CHANGER_INIT_ELEM_STAT_WITH_RANGE \
    0x00000002 // The medium-changer has the ability to initialize elements within a specified range.
...
#define CHANGER_STATUS_NON_VOLATILE \
    0x00000010                            // The medium-changer uses non-volatile memory for element status information.
...
#define CHANGER_REPORT_IEPORT_STATE                                         \
    0x00000800 // The medium-changer can determine whether media is present \
               // in the IE Port.
...
#define CHANGER_STORAGE_TRANSPORT \
    0x00008000 // The medium-changer can use a transport as an independent storage element.

#define CHANGER_DRIVE_CLEANING_REQUIRED                                                 \
    0x00010000 // The drives controlled by the medium changer require periodic cleaning \
               // initiated by an application.
#define CHANGER_PREDISMOUNT_EJECT_REQUIRED                                                         \
    0x00020000 // The medium-changer requires a drive eject command to be issued, before a changer \
...
#define CHANGER_PREMOUNT_EJECT_REQUIRED                                          \
    0x00080000 // The medium-changer requires a drive eject command to be issued \
...
#define CHANGER_SERIAL_NUMBER_VALID                                     \
    0x04000000 // The serial number reported in GetProductData is valid \
               // and unique.
...
#define CHANGER_PREDISMOUNT_ALIGN_TO_SLOT \
    0x80000001 // The transport must be prepositioned to the slot prior to ejecting the media.
#define CHANGER_PREDISMOUNT_ALIGN_TO_DRIVE \
    0x80000002 // The transport must be prepositioned to the drive prior to ejecting the media.
#define CHANGER_CLEANER_AUTODISMOUNT \
    0x80000004 // The device will move the cleaner cartridge back into the slot when cleaning has completed.
...
#define CHANGER_RTN_MEDIA_TO_ORIGINAL_ADDR \
    0x80000020 // Media must be returned to the slot from which it originated after a move to another element.
...
#define CHANGER_IEPORT_USER_CONTROL_CLOSE \
    0x80000100                                  // Indicates that user action is necessary to close an opened ieport.
...
#define ELEMENT_STATUS_EXCEPT \
    0x00000004 // Element is in an abnormal state; check ExceptionCode field for more information.
...
#define ELEMENT_STATUS_INVERT \
    0x00400000 // Media in element was inverted (valid only if ELEMENT_STATUS_SVALID bit is set)
...
#define FSCTL_IS_PATHNAME_VALID \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 11, METHOD_BUFFERED, FILE_ANY_ACCESS) // PATHNAME_BUFFER,
...
#define FSCTL_QUERY_FAT_BPB \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 22, METHOD_BUFFERED, FILE_ANY_ACCESS) // FSCTL_QUERY_FAT_BPB_BUFFER
#define FSCTL_REQUEST_FILTER_OPLOCK CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 23, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define FSCTL_FILESYSTEM_GET_STATISTICS \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 24, METHOD_BUFFERED, FILE_ANY_ACCESS) // FILESYSTEM_STATISTICS
#if (_WIN32_WINNT >= 0x0400)
#define FSCTL_GET_NTFS_VOLUME_DATA \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 25, METHOD_BUFFERED, FILE_ANY_ACCESS) // NTFS_VOLUME_DATA_BUFFER
#define FSCTL_GET_NTFS_FILE_RECORD                         \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 26, METHOD_BUFFERED, \
             FILE_ANY_ACCESS) // NTFS_FILE_RECORD_INPUT_BUFFER, NTFS_FILE_RECORD_OUTPUT_BUFFER
#define FSCTL_GET_VOLUME_BITMAP                           \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 27, METHOD_NEITHER, \
             FILE_ANY_ACCESS) // STARTING_LCN_INPUT_BUFFER, VOLUME_BITMAP_BUFFER
#define FSCTL_GET_RETRIEVAL_POINTERS                      \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 28, METHOD_NEITHER, \
             FILE_ANY_ACCESS) // STARTING_VCN_INPUT_BUFFER, RETRIEVAL_POINTERS_BUFFER
...
#define FSCTL_SET_OBJECT_ID \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 38, METHOD_BUFFERED, FILE_SPECIAL_ACCESS) // FILE_OBJECTID_BUFFER
#define FSCTL_GET_OBJECT_ID \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 39, METHOD_BUFFERED, FILE_ANY_ACCESS) // FILE_OBJECTID_BUFFER
#define FSCTL_DELETE_OBJECT_ID CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 40, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define FSCTL_SET_REPARSE_POINT \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 41, METHOD_BUFFERED, FILE_SPECIAL_ACCESS) // REPARSE_DATA_BUFFER,
...
#define FSCTL_SECURITY_ID_CHECK \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 45, METHOD_NEITHER, FILE_READ_DATA) // BULK_SECURITY_TEST_DATA,
#define FSCTL_READ_USN_JOURNAL \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 46, METHOD_NEITHER, FILE_ANY_ACCESS) // READ_USN_JOURNAL_DATA, USN
#define FSCTL_SET_OBJECT_ID_EXTENDED CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 47, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define FSCTL_CREATE_OR_GET_OBJECT_ID \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 48, METHOD_BUFFERED, FILE_ANY_ACCESS) // FILE_OBJECTID_BUFFER
...
#define FSCTL_QUERY_ALLOCATED_RANGES                      \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 51, METHOD_NEITHER, \
             FILE_READ_DATA) // FILE_ALLOCATED_RANGE_BUFFER, FILE_ALLOCATED_RANGE_BUFFER
...
#define FSCTL_WRITE_RAW_ENCRYPTED \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 55, METHOD_NEITHER, FILE_SPECIAL_ACCESS) // ENCRYPTED_DATA_INFO,
#define FSCTL_READ_RAW_ENCRYPTED                          \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 56, METHOD_NEITHER, \
             FILE_SPECIAL_ACCESS) // REQUEST_RAW_ENCRYPTED_DATA, ENCRYPTED_DATA_INFO
#define FSCTL_CREATE_USN_JOURNAL \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 57, METHOD_NEITHER, FILE_ANY_ACCESS) // CREATE_USN_JOURNAL_DATA,
#define FSCTL_READ_FILE_USN_DATA \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 58, METHOD_NEITHER, FILE_ANY_ACCESS) // Read the Usn Record for a file
#define FSCTL_WRITE_USN_CLOSE_RECORD \
    CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 59, METHOD_NEITHER, FILE_ANY_ACCESS) // Generate Close Usn Record
