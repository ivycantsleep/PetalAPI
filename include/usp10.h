...
/////   USP Status Codes
//
#define USP_E_SCRIPT_NOT_IN_FONT MAKE_HRESULT(SEVERITY_ERROR, FACILITY_ITF, 0x200) // Script doesn't exist in font
    ...
    HRESULT WINAPI ScriptFreeCache(SCRIPT_CACHE *psc); //InOut  Cache handle
    ...
    HRESULT WINAPI ScriptItemize(const WCHAR *pwcInChars,         // In   Unicode string to be itemized
    ...
    HRESULT WINAPI ScriptLayout(int cRuns,               // In   Number of runs to process
    ...
    HRESULT WINAPI ScriptShape(HDC hdc,               // In    Optional (see under caching)
    ...
    HRESULT WINAPI ScriptPlace(HDC hdc,                    // In    Optional (see under caching)
                               SCRIPT_CACHE *psc,          // InOut Cache handle
    ...
    HRESULT WINAPI ScriptTextOut(const HDC hdc,              // In     OS handle to device context (required)
                                 SCRIPT_CACHE *psc,          // InOut  Cache handle
    ...
    /////
    HRESULT WINAPI ScriptJustify(const SCRIPT_VISATTR *psva, // In   Collected visual attributes for entire line
                                 const int *piAdvance,       // In   Advance widths from ScriptPlace
    ...
    HRESULT WINAPI ScriptBreak(const WCHAR *pwcChars,      // In   Logical unicode item
    ...
    HRESULT WINAPI ScriptCPtoX(int iCP,                    // In   Logical character position in run
    ...
    HRESULT WINAPI ScriptXtoCP(int iX,                     // In   X offset from left of run
                               int cChars,                 // In   Count of logical codepoints in run
                               int cGlyphs,                // In   Count of glyphs in run
                               const WORD *pwLogClust,     // In   Logical clusters
                               const SCRIPT_VISATTR *psva, // In   Visual glyph attributes
    ...
    HRESULT WINAPI ScriptGetLogicalWidths(const SCRIPT_ANALYSIS *psa, // In   Script analysis from item attributes
                                          int cChars,                 // In   Count of logical codepoints in run
                                          int cGlyphs,                // In   Count of glyphs in run
    ...
    HRESULT WINAPI
    ScriptApplyLogicalWidth(const int *piDx,            // In     Logical dx array to apply
                            int cChars,                 // In     Count of logical codepoints in run
...
#define SGCM_RTL 0x00000001 // Return mirrored glyph for mirrorable Unicode codepoints
    ...
    HRESULT WINAPI ScriptGetCMap(HDC hdc,                 // In    Optional (see notes on caching)
    ...
    HRESULT WINAPI ScriptGetGlyphABCWidth(HDC hdc,           // In    Optional (see notes on caching)
                                          SCRIPT_CACHE *psc, // InOut Address of Cache handle
    ...
    HRESULT WINAPI ScriptGetFontProperties(HDC hdc,                     // In    Optional (see notes on caching)
                                           SCRIPT_CACHE *psc,           // InOut Address of Cache handle
    ...
    HRESULT WINAPI ScriptCacheGetHeight(HDC hdc,           // In    Optional (see notes on caching)
                                        SCRIPT_CACHE *psc, // InOut Address of Cache handle
...
#define SSA_NOKASHIDA 0x80000000 // Internal - Used by GCP to justify the non Arabic glyphs only.
    ...
    HRESULT WINAPI
    ScriptStringAnalyse(HDC hdc,                   //In  Device context (required)
    ...
    HRESULT WINAPI ScriptStringFree(SCRIPT_STRING_ANALYSIS *pssa); //InOut Address of pointer to analysis
    ...
    HRESULT WINAPI ScriptStringCPtoX(SCRIPT_STRING_ANALYSIS ssa, //In  String analysis
    ...
    HRESULT WINAPI ScriptStringXtoCP(SCRIPT_STRING_ANALYSIS ssa, // In
                                     int iX,                     // In
                                     int *piCh,                  // Out
                                     int *piTrailing);           // Out
    ...
    HRESULT WINAPI ScriptStringOut(SCRIPT_STRING_ANALYSIS ssa, //In  Analysis with glyphs
                                   int iX,                     //In
                                   int iY,                     //In
...
#define SIC_NEUTRAL 4    // Treat neutrals as complex
    ...
    HRESULT WINAPI ScriptIsComplex(const WCHAR *pwcInChars, //In  String to be tested
    ...
    HRESULT WINAPI ScriptRecordDigitSubstitution(LCID Locale, // In   LOCALE_USER_DEFAULT or desired locale
...
#define SCRIPT_DIGITSUBSTITUTE_TRADITIONAL 3 // Substitute with traditional digits of the locale
    ...
    HRESULT WINAPI ScriptApplyDigitSubstitution(const SCRIPT_DIGITSUBSTITUTE *psds, // In   Digit substitution settings
