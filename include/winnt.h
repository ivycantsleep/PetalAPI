...
#pragma intrinsic(__rdtsc)
    ...
    VOID __movsb(IN PBYTE Destination, IN PBYTE Source, IN DWORD Count);

    VOID __movsw(IN PWORD Destination, IN PWORD Source, IN DWORD Count);

    VOID __movsd(IN PDWORD Destination, IN PDWORD Source, IN DWORD Count);

    VOID __movsq(IN PULONGLONG Destination, IN PULONGLONG Source, IN DWORD Count);
...
#define UnsignedMultiplyHigh __umulh
    ...
    LONGLONG
    MultiplyHigh(IN LONGLONG Multiplier, IN LONGLONG Multiplicand);

    ULONGLONG
    UnsignedMultiplyHigh(IN ULONGLONG Multiplier, IN ULONGLONG Multiplicand);
...
#pragma intrinsic(__umulh)
    ...
    DWORD64
    __readgsqword(IN DWORD Offset);

    VOID __writegsbyte(IN DWORD Offset, IN BYTE Data);

    VOID __writegsword(IN DWORD Offset, IN WORD Data);

    VOID __writegsdword(IN DWORD Offset, IN DWORD Data);

    VOID __writegsqword(IN DWORD Offset, IN DWORD64 Data);
...
#ifndef ISTAG
#define ISTAG(x) \
    ((x) == IMAGE_SYM_CLASS_STRUCT_TAG || (x) == IMAGE_SYM_CLASS_UNION_TAG || (x) == IMAGE_SYM_CLASS_ENUM_TAG)
#endif
...
#define INS_IMM64(Value, Address, Size, InstPos, ValPos) /* Intel-IA64-Filler */                                    \
    *(PDWORD)Address =                                                                                              \
        (*(PDWORD)Address & ~(((1 << Size) - 1) << InstPos)) |                              /* Intel-IA64-Filler */ \
...
#define PSLIST_ENTRY PSINGLE_LIST_ENTRY
    ...
    NTSYSAPI
    VOID NTAPI RtlInitializeSListHead(IN PSLIST_HEADER ListHead);
    ...
    PSLIST_ENTRY
    NTAPI
    RtlInterlockedPushEntrySList(IN PSLIST_HEADER ListHead, IN PSLIST_ENTRY ListEntry);
    ...
    NTSYSAPI
    WORD NTAPI RtlQueryDepthSList(IN PSLIST_HEADER ListHead);
...
#define RtlEqualMemory(Source1, Source2, Length) ((Length) == RtlCompareMemory(Source1, Source2, Length))
    ...
    NTSYSAPI
    VOID NTAPI RtlCopyMemory32(VOID UNALIGNED *Destination, CONST VOID UNALIGNED *Source, DWORD Length);

#endif

    NTSYSAPI
    VOID NTAPI RtlMoveMemory(VOID UNALIGNED *Destination, CONST VOID UNALIGNED *Source, SIZE_T Length);

    NTSYSAPI
    VOID NTAPI RtlFillMemory(VOID UNALIGNED *Destination, SIZE_T Length, BYTE Fill);

    NTSYSAPI
    VOID NTAPI RtlZeroMemory(VOID UNALIGNED *Destination, SIZE_T Length);
...
#define RtlZeroMemory(Destination, Length) memset((Destination), 0, (Length))
    ...
    FORCEINLINE
    PVOID
    RtlSecureZeroMemory(IN PVOID ptr, IN SIZE_T cnt)
    {
...
#define VER_SET_CONDITION(_m_, _t_, _c_) ((_m_) = VerSetConditionMask((_m_), (_t_), (_c_)))
    ...
    ULONGLONG
    NTAPI
    VerSetConditionMask(IN ULONGLONG ConditionMask, IN DWORD TypeMask, IN BYTE Condition);
    //
...
#define VERIFIER_STOP(Code, Msg, P1, S1, P2, S2, P3, S3, P4, S4)                                                       \
    {                                                                                                                  \
        RtlApplicationVerifierStop((Code), (Msg), (ULONG_PTR)(P1), (S1), (ULONG_PTR)(P2), (S2), (ULONG_PTR)(P3), (S3), \
                                   (ULONG_PTR)(P4), (S4));                                                             \
    }

    VOID RtlApplicationVerifierStop(ULONG_PTR Code, PCHAR Message, ULONG_PTR Param1, PCHAR Description1,
                                    ULONG_PTR Param2, PCHAR Description2, ULONG_PTR Param3, PCHAR Description3,
...
#define REG_OPTION_NON_VOLATILE       \
    (0x00000000L) // Key is preserved \
                  // when system is rebooted

#define REG_OPTION_VOLATILE               \
    (0x00000001L) // Key is not preserved \
                  // when system is rebooted

#define REG_OPTION_CREATE_LINK        \
    (0x00000002L) // Created key is a \
                  // symbolic link

#define REG_OPTION_BACKUP_RESTORE               \
    (0x00000004L) // open for backup or restore \
