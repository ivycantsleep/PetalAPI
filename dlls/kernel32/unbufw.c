...
VOID ParseSwitch(CHAR chSwitch, int *pArgc, char **pArgv[]);
...
int __cdecl main(int argc, char *argv[], char *envp)
{
    ...
    for (TestNumber = 0; TestNumber < TEST_REPEAT_COUNT; TestNumber++)
    {
        ...
        hFile = CreateFile(FileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                           fRaw ? OPEN_EXISTING : OPEN_ALWAYS, FileFlags, NULL);
        ...
        hPort = CreateIoCompletionPort(hFile, NULL, (DWORD)hFile, 0);
        if (!hPort)
        {
        ...
        if (TestNumber == 0 && fSequentialNew == FALSE)
        {
            ...
            b = GetQueuedCompletionStatus(hPort, &n2, &key, &ov2, (DWORD)-1);
            if (!b)
            {
        ...
        reissuewrite:
            ...
            if (!b && GetLastError() != ERROR_IO_PENDING)
            {
                ...
                if (GetLastError() == ERROR_INVALID_USER_BUFFER || GetLastError() == ERROR_NOT_ENOUGH_QUOTA ||
