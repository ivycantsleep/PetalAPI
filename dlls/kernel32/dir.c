...
BOOL APIENTRY CreateDirectoryA(LPCSTR lpPathName, LPSECURITY_ATTRIBUTES lpSecurityAttributes)
...
BOOL APIENTRY CreateDirectoryW(LPCWSTR lpPathName, LPSECURITY_ATTRIBUTES lpSecurityAttributes)
...

{
    ...
    if (FileName.Length > ((MAX_PATH - 12) << 1))
    {
        ...
        if (!(lpPathName[0] == '\\' && lpPathName[1] == '\\' && lpPathName[2] == '?' && lpPathName[3] == '\\'))
        {
    ...
    Status = NtCreateFile(&Handle, FILE_LIST_DIRECTORY | SYNCHRONIZE, &Obja, &IoStatusBlock, NULL,
                          FILE_ATTRIBUTE_NORMAL, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_CREATE,
                          FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT | FILE_OPEN_FOR_BACKUP_INTENT, NULL, 0L);
...
BOOL APIENTRY CreateDirectoryExA(LPCSTR lpTemplateDirectory, LPCSTR lpNewDirectory,
...

{
    ...
    b = CreateDirectoryExW((LPCWSTR)StaticUnicode->Buffer, (LPCWSTR)DynamicUnicode.Buffer, lpSecurityAttributes);
...
BOOL APIENTRY CreateDirectoryExW(LPCWSTR lpTemplateDirectory, LPCWSTR lpNewDirectory,
                                 LPSECURITY_ATTRIBUTES lpSecurityAttributes)
...

{
    ...
    TranslationStatus = RtlDosPathNameToNtPathName_U(lpTemplateDirectory, &PathName, NULL, &RelativeName);
    ...
    Status = NtOpenFile(&SourceFile, FILE_LIST_DIRECTORY | FILE_READ_EA | FILE_READ_ATTRIBUTES, &Obja, &IoStatusBlock,
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        if (Status == STATUS_INVALID_PARAMETER)
        {
            ...
            Status = NtOpenFile(&SourceFile, FILE_LIST_DIRECTORY | FILE_READ_EA | FILE_READ_ATTRIBUTES, &Obja,
    ...
    else
    {
        ...
        Status = NtQueryInformationFile(SourceFile, &IoStatusBlock, (PVOID)&BasicInfo, sizeof(BasicInfo),
        ...
        {
            Status = NtQueryInformationFile(SourceFile, &IoStatusBlock, (PVOID)&FileTagInformation,
    ...
    TranslationStatus = RtlDosPathNameToNtPathName_U(lpNewDirectory, &TargetName, NULL, &RelativeName);
    ...
    if (TargetName.Length > ((MAX_PATH - 12) << 1))
    {
        DWORD L;
        LPWSTR lp;
        if (!(lpNewDirectory[0] == '\\' && lpNewDirectory[1] == '\\' && lpNewDirectory[2] == '?' &&
              lpNewDirectory[3] == '\\'))
        {
    ...
    Status = NtQueryInformationFile(SourceFile, &IoStatusBlock, &EaInfo, sizeof(EaInfo), FileEaInformation);
    ...
    if (NT_SUCCESS(Status) && EaInfo.EaSize)
    {
        ...
        do
        {
            ...
            Status =
                NtQueryEaFile(SourceFile, &IoStatusBlock, EaBuffer, EaSize, FALSE, (PVOID)NULL, 0, (PULONG)NULL, TRUE);
    ...
    InitializeObjectAttributes(&Obja, &TargetName, OBJ_CASE_INSENSITIVE, RelativeName.ContainingDirectory, NULL);
    ...
    Status = NtCreateFile(&DestFile, DesiredAccess, &Obja, &IoStatusBlock, NULL, BasicInfo.FileAttributes,
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        if ((Status == STATUS_INVALID_PARAMETER) || (Status == STATUS_ACCESS_DENIED))
        {
            ...
            Status = NtCreateFile(
                &DestFile, FILE_LIST_DIRECTORY | FILE_WRITE_ATTRIBUTES | FILE_READ_ATTRIBUTES | SYNCHRONIZE, &Obja,
                &IoStatusBlock, NULL, BasicInfo.FileAttributes, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_CREATE,
                FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT | FILE_OPEN_FOR_BACKUP_INTENT, EaBuffer, EaSize);
        }
    }
    ...
    else
    {
        if (IsNameGrafting)
        {
            ...
            Status = NtFsControlFile(SourceFile, NULL, NULL, NULL, &IoStatusBlock, FSCTL_GET_REPARSE_POINT,
                                     NULL,                            //  Input buffer
            ...
            VolumeName.Buffer = (PWCHAR)((PCHAR)ReparseBufferHeader->MountPointReparseBuffer.PathBuffer +
            ...
            if (MOUNTMGR_IS_NT_VOLUME_NAME_WB(&VolumeName))
            {
                ...
                VolumeMountPoint =
                    RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TMP_TAG), MountPoint.Length + 2 * sizeof(WCHAR));
                if (!VolumeMountPoint)
                {
                ...
                else
                {
                    ...
                    RtlCopyMemory(VolumeMountPoint, MountPoint.Buffer, MountPoint.Length);
                    ...
                    b = SetVolumeMountPointW(VolumeMountPoint, VolumeName.Buffer);
            ...
            else
            {
                ...
                b = TRUE;
                Status =
                    NtFsControlFile(DestFile, NULL, NULL, NULL, &IoStatusBlock, FSCTL_SET_REPARSE_POINT, ReparseBuffer,
        ...
        do
        {
            ...
            Status = NtQueryInformationFile(SourceFile, &IoStatusBlock, (PVOID)StreamInfoBase, StreamInfoSize,
        ...
        if (NT_SUCCESS(Status) && IoStatusBlock.Information)
        {
            ...
            for (;;)
            {
                ...
                InitializeObjectAttributes(&Obja, &StreamName, 0, SourceFile, NULL);
                Status = NtCreateFile(&StreamHandle, GENERIC_READ | SYNCHRONIZE, &Obja, &IoStatusBlock, NULL, 0,
                ...
                if (NT_SUCCESS(Status))
                {
                    ...
                    Options = 0;
                    b = BaseCopyStream(NULL, StreamHandle, GENERIC_READ | SYNCHRONIZE, FileName, DestFile,
                                       &StreamInfo->StreamSize, &Options, &OutputStream, &CopySize, &CopyFileContext,
                                       (LPRESTART_STATE)NULL, (BOOL)FALSE, (DWORD)0, &DestFileFsAttributes);
...
BOOL APIENTRY RemoveDirectoryA(LPCSTR lpPathName)
...
BOOL APIENTRY RemoveDirectoryW(LPCWSTR lpPathName)
...

{
    ...
    Status = NtOpenFile(&Handle, DELETE | SYNCHRONIZE | FILE_READ_ATTRIBUTES, &Obja, &IoStatusBlock,
    ...
    else
    {
        ...
        if (NT_SUCCESS(Status) && (FileTagInformation.FileAttributes & FILE_ATTRIBUTE_REPARSE_POINT))
        {
            if (FileTagInformation.ReparseTag == IO_REPARSE_TAG_MOUNT_POINT)
            {
                ...
                b = DeviceIoControl(Handle, FSCTL_GET_REPARSE_POINT, NULL, 0, reparse, MAXIMUM_REPARSE_DATA_BUFFER_SIZE,
                                    &bytes, NULL);
                ...
                if (b)
                {
                    ...
                    mountName.Buffer = (PWCHAR)((PCHAR)reparse->MountPointReparseBuffer.PathBuffer +
                    ...
                    if (MOUNTMGR_IS_VOLUME_NAME(&mountName))
                    {
                        ...
                        volumeMountPoint =
                            RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TMP_TAG), mountName.Length + 2 * sizeof(WCHAR));
                        if (!volumeMountPoint)
                        {
                        ...
                        RtlCopyMemory(volumeMountPoint, mountName.Buffer, mountName.Length);
        ...
        if (NT_SUCCESS(Status) && (FileTagInformation.FileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) && !IsNameGrafting)
        {
            //
