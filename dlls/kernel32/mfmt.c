...

{
    ...
    return DeviceIoControl(hDisk, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, lpGeometry, sizeof(*lpGeometry),
...
GetSupportedGeometrys(HANDLE hDisk)
{
    ...
    b = DeviceIoControl(hDisk, IOCTL_DISK_GET_MEDIA_TYPES, NULL, 0, SupportedGeometry, sizeof(SupportedGeometry),
                        &ReturnedByteCount, NULL);
    if (b)
    {
...
VOID PrintGeometry(LPSTR lpDriveName, PDISK_GEOMETRY lpGeometry)
{
    LPSTR MediaType;
...
BOOL LowLevelFormat(HANDLE hDisk, PDISK_GEOMETRY lpGeometry)
{
    ...
    for (i = 0; i < lpGeometry->Cylinders.LowPart; i++)
    {
        ...
        b = DeviceIoControl(hDisk, IOCTL_DISK_FORMAT_TRACKS, &FormatParameters, sizeof(FormatParameters), lpBadTrack,
                            lpGeometry->TracksPerCylinder * sizeof(*lpBadTrack), &ReturnedByteCount, NULL);
...
_cdecl main(int argc, char *argv[], char *envp[])
{
    ...
    if (fShowGeometry)
    {
        ...
        hDrive = CreateFile(Drive, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    ...
    if (fDiskImage)
    {
        ...
        hDiskImage = CreateFile(DiskImage, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                                SourceIsDrive ? CREATE_ALWAYS : OPEN_EXISTING, 0, NULL);
        ...
        //
        {
            ...
            if (SourceIsDrive)
            {
                ...
                GeometrySize = Geometry.Cylinders.LowPart * Geometry.TracksPerCylinder * Geometry.SectorsPerTrack *
            ...
            else
            {
                ...
                for (i = 0; i < SupportedGeometryCount; i++)
                {
                    GeometrySize = SupportedGeometry[i].Cylinders.LowPart * SupportedGeometry[i].TracksPerCylinder *
                                   SupportedGeometry[i].SectorsPerTrack * SupportedGeometry[i].BytesPerSector;
