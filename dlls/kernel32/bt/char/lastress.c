...
DWORD Sizes[SIZES] = { 100, 10, 128, 256, 256, 100, 32, 64, 64, 100 };

VOID LaThread(PVOID h)
{
    int i, j;
    PVOID p;
...
VOID StartTest(DWORD SpinCount, DWORD ThreadCount)
{
    PHANDLE Threads, p;
    HANDLE hStartEvent;
    DWORD Id;
    ...
    for (i = 0, p = Threads; i < ThreadCount; i++, p++)
    {
        ...
        *p = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)LaThread, (PVOID)hStartEvent, 0, &Id);
