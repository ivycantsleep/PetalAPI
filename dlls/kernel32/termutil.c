...
#define TERMSRV_INIFILE_TIMES \
    L"\\Registry\\Machine\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Terminal Server\\Install\\IniFile Times"
...
BOOL IsTSAppCompatEnabled(VOID)
{
    ...
    if (UniString.Buffer && pKeyValInfo)
    {
        ...
        if (NT_SUCCESS(NtStatus))
        {
            ...
            RtlInitUnicodeString(&UniString, L"TSAppCompat");
            NtStatus = NtQueryValueKey(hKey, &UniString, KeyValuePartialInformation, pKeyValInfo, ulcbuf, &ul);
...
BOOL IsSystemLUID(VOID)
{
    ...
    if (CurrentLUID.LowPart == 0 && CurrentLUID.HighPart == 0)
    {
        ...
        NtQueryInformationToken(TokenHandle, TokenStatistics, &TokenInformation, sizeof(TokenInformation),
                                &ReturnLength);
        NtClose(TokenHandle);
...
void InitializeTermsrvFpns(void)
{
    ...
    if (IsTerminalServerCompatible() || (IsSystemLUID()) || (!IsTSAppCompatEnabled()))
    {
        return;
    }
...
WINBASEAPI
BOOL WINAPI SetTermsrvAppInstallMode(BOOL bState)
{
    ...
    if (dllHandle)
    {
        ...
        if (fPtr)
        {
            Status = RtlAllocateAndInitializeSid(&SidAuthority, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS,
                                                 0, 0, 0, 0, 0, 0, &pSid);
                    ...
                    if (FoundAdmin)
                    {
                        ...
                        if (NT_SUCCESS(Status))
                        {
                            ...
                            if (gpTermsrvUpdateAllUserMenu == NULL)
                            {
                                ...
                                if (dllHandle)
                                {
                                    gpTermsrvUpdateAllUserMenu = (PTERMSRVUPDATEALLUSERMENU)GetProcAddress(
                                        dllHandle, "TermsrvUpdateAllUserMenu");
                                }
                            }
...
/* External Functions */
NTSTATUS
BaseDllOpenIniFileOnDisk(PINIFILE_PARAMETERS a);
...
NTSTATUS
TermsrvIniSyncLoop(HANDLE SrcHandle, PINIFILE_PARAMETERS a, PBOOLEAN pfIniUpdated);
NTSTATUS
TermsrvGetSyncTime(PUNICODE_STRING pSysIniPath, PUNICODE_STRING pUserBasePath, PLARGE_INTEGER pLastSyncTime);

NTSTATUS
TermsrvPutSyncTime(PUNICODE_STRING pSysIniPath, PUNICODE_STRING pUserBasePath, PLARGE_INTEGER pLastSyncTime);
...
TermsrvGetSyncTime(PUNICODE_STRING pSysIniPath, PUNICODE_STRING pUserBasePath, PLARGE_INTEGER pLastSyncTime)
{
    ...
    UNICODE_STRING UniUpdateName = { 0, sizeof(wcUpdateFile), wcUpdateFile };
    ...
    Status = RtlAppendUnicodeStringToString(&UniUpdateName, pUserBasePath);
    if (NT_SUCCESS(Status))
    {
        Status = RtlAppendUnicodeToString(&UniUpdateName, L"\\inifile.upd");
    }
    ...
    Status = NtOpenFile(&hUpdate, FILE_GENERIC_READ, &ObjAUpd, &Iosb, FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    if (NT_SUCCESS(Status))
    {
        Status = NtQueryInformationFile(hUpdate, &Iosb, &StandardInfo, sizeof(StandardInfo), FileStandardInformation);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        Status = NtAllocateVirtualMemory(NtCurrentProcess(), &pBuff, 0, &ulBuffSize, MEM_RESERVE, PAGE_READWRITE);
    }
    ...
    if (NT_SUCCESS(Status))
    {
        Status = NtReadFile(hUpdate, NULL, NULL, NULL, &Iosb, pBuff, StandardInfo.EndOfFile.LowPart, NULL, NULL);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        while ((pwch < (PWCHAR)pBuffEnd) && ((lresult = _wcsicmp(pwch, pSysIniPath->Buffer)) < 0))
        {
    ...
    if (pBuff)
    {
        NtFreeVirtualMemory(NtCurrentProcess(), &pBuff, &ulBuffSize, MEM_RELEASE);
    }
...
TermsrvPutSyncTime(PUNICODE_STRING pSysIniPath, PUNICODE_STRING pUserBasePath, PLARGE_INTEGER pLastSyncTime)
{
    ...
    Iosb.Status = STATUS_SUCCESS;
    Status =
        NtCreateFile(&hUpdate, FILE_READ_DATA | FILE_WRITE_DATA | FILE_READ_ATTRIBUTES | SYNCHRONIZE, &ObjAUpd, &Iosb,
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        { // Ini file not in list
            ...
            if (NT_SUCCESS(Status))
            {
                ...
                {
                    RtlMoveMemory(pwch + (ulLength / sizeof(WCHAR)), pwch, pBuffEnd - (PCHAR)pwch);
                }
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        Status = NtSetInformationFile(hUpdate, &Iosb, &CurrentPos, sizeof(CurrentPos), FilePositionInformation);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        Status = NtSetInformationFile(hUpdate, &Iosb, &FileLength, sizeof(FileLength), FileEndOfFileInformation);
    }

    if (pBuff)
    {
        NtFreeVirtualMemory(NtCurrentProcess(), &pBuff, &ulRegionSize, MEM_RELEASE);
    }
...
BOOLEAN
TermsrvCheckIniSync(PUNICODE_STRING pSysIniPath, PUNICODE_STRING pUserBasePath)
{
    ...
    if (!NT_SUCCESS(Status) || ((BasicInfo.LastWriteTime.HighPart > LastSyncTime.HighPart) ||
...
BOOL TermsrvDoesFileExist(PUNICODE_STRING pFileName)
{
    NTSTATUS Status;
...
BOOL TermsrvSyncUserIniFile(PINIFILE_PARAMETERS a)
{
    ...
    UNICODE_STRING IniFilePath = { MAX_PATH, MAX_PATH + 1, wcIniPath };
    ...
    BOOLEAN OrigWrite, OrigMultiValue, OrigUnicode, OrigWriteOperation, fIniUpdated = FALSE;
...
//DbgPrint("TermsrvSyncUserIniFile: Error building Sys Ini Path!\n");
    ...
    pwcIniName = RtlAllocateHeap(RtlProcessHeap(), 0, (wcslen(pwch) + 1) * sizeof(WCHAR));
    if (pwcIniName == NULL)
    {
        return FALSE;
    }
    ...
    if (((ulCompatFlags & (TERMSRV_COMPAT_INISYNC | TERMSRV_COMPAT_WIN16)) ==
        ...
        TermsrvDoesFileExist(&UniSysPath) && TermsrvCheckIniSync(&UniSysPath, &UserBasePath))
    {
        ...
        Status = NtOpenFile(&SrcHandle, FILE_GENERIC_READ, &SrcObja, &SrcIosb, FILE_SHARE_READ | FILE_SHARE_WRITE,
        ...
        if (NT_SUCCESS(Status))
        {
            TermsrvPutSyncTime(&UniSysPath, &UserBasePath, &BasicInfo.LastWriteTime);
        }
    }
    ...
    *pfIniUpdated = FALSE;
    ...
    while (1)
    {
        ...
        if (*pStr == '[')
        {
            ...
            AnsiSection.Buffer = RtlAllocateHeap(RtlProcessHeap(), 0, AnsiSection.MaximumLength);
            if (!AnsiSection.Buffer)
            {
        ...
        else if (*pStr != ';')
        {
            ...
            if (pch)
            {
                a->VariableName.Length = a->VariableName.MaximumLength = (USHORT)(pch - pStr);
...
GetPerUserWindowsDirectory(PWCHAR TermSrvWindowsPath, ULONG len)
{
    ...
    if (NT_SUCCESS(Status = RtlQueryEnvironmentVariable_U(NULL, &BaseHomeDriveVariableName, &Path)))
    {
        ...
        if (NT_SUCCESS(Status = RtlQueryEnvironmentVariable_U(NULL, &BaseHomePathVariableName, &Path)))
        {
            Path.Buffer[Path.Length] = UNICODE_NULL;
