...
BOOL InsertResourceIntoLangList(PUPDATEDATA pUpd, PSDATA Type, PSDATA Name, PRESTYPE pType, PRESNAME pName, INT idLang,
                                INT fName, INT cb, PVOID lpData);
...
LONG PatchDebug(int inpfh, int outfh, PIMAGE_SECTION_HEADER pDebugOld, PIMAGE_SECTION_HEADER pDebugNew,
                PIMAGE_SECTION_HEADER pDebugDirOld, PIMAGE_SECTION_HEADER pDebugDirNew, NT_HEADER_TYPE *pOld,
                NT_HEADER_TYPE *pNew, ULONG ibMaxDbgOffsetOld, PULONG pPointerToRawData)

{
    ...
    pDbgSave = pDbg = (PIMAGE_DEBUG_DIRECTORY)RtlAllocateHeap(
        RtlProcessHeap(), MAKE_TAG(RES_TAG), pNew->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].Size);
    if (pDbg == NULL)
    ...
    pDbgLast =
        pDbg + (pNew->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].Size) / sizeof(IMAGE_DEBUG_DIRECTORY);
...
template <class NT_HEADER_TYPE>
LONG PatchRVAs(int inpfh, int outfh, PIMAGE_SECTION_HEADER po32, ULONG pagedelta, NT_HEADER_TYPE *pNew, ULONG OldSize)

{
    ...
    else
    {
        for (cimp = cmod = 0;; cmod++)
        {
            ...
            {
                if (rvaiat >= po32[i].VirtualAddress && rvaiat < po32[i].VirtualAddress + po32[i].SizeOfRawData)
                {
...
template <class NT_HEADER_TYPE>
LONG PEWriteResource(INT inpfh, INT outfh, ULONG cbOldexe, PUPDATEDATA pUpdate, NT_HEADER_TYPE *NtHeader)

{
    ...
    ULONG ibSave;
    ULONG adjust = 0;
    PIMAGE_SECTION_HEADER pObjtblOld, pObjtblNew, pObjDebug, pObjResourceOld, pObjResourceNew, pObjResourceOldX,
        pObjDebugDirOld, pObjDebugDirNew, pObjNew, pObjOld, pObjLast;
    PUCHAR p;
    ...
    DPrintf((DebugBuf, "Old section table: %#08lx bytes at %#08lx(mem)\n",
             Old.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER), pObjtblOld));
    MuMoveFilePos(inpfh, ibObjTab);
    MuRead(inpfh, (PUCHAR)pObjtblOld, Old.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER));
    ...
    cbRestab = sizeof(IMAGE_RESOURCE_DIRECTORY) + /* root dir (types) */
               ...
               cRes * sizeof(IMAGE_RESOURCE_DIRECTORY_ENTRY) + (cbName + cbType) + /* name/type strings */
               cb +                                                                /* padding */
    ...
    if (pObjResourceOld != NULL && cbResource > (ULONG)cb)
    {
        ...
        { /* have already merged .rsrc & .rsrc1, if possible */
    ...
    if (pObjDebug != NULL)
    {
        ...
        DPrintf(
            (DebugBuf, "Debug section: %#08lx bytes @%#08lx\n", pObjDebug->SizeOfRawData, pObjDebug->PointerToRawData));
    }
    ...
    cb = Old.OptionalHeader.SizeOfHeaders -
         (Old.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER) + sizeof(NT_HEADER_TYPE) + cbOldexe);
    if (adjust > (ULONG)cb)
    {
        int i;
    ...
    for (pObjOld = pObjtblOld, pObjNew = pObjtblNew; pObjOld < pObjLast; pObjOld++)
    {
        ...
        else if (pObjNew == pObjResourceNew)
        {
            ...
            else
            {
                *pObjNew = *pObjOld; /* copy obj table entry */
                ...
                { /* is smaller, but pad so will be valid */
                    adjust = 0;
        ...
        else if (nObjResourceX != -1 && pObjNew == pObjtblNew + nObjResourceX)
        {
            ...
            if (pObjResourceOldX == NULL)
            {
                ...
                pObjNew->VirtualAddress = pObjOld->VirtualAddress;
                pObjNew->Characteristics = IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA;
                adjust = ROUNDUP(cbResource, New.OptionalHeader.SectionAlignment) + pObjResourceNew->VirtualAddress -
                         pObjNew->VirtualAddress;
                DPrintf(
                    (DebugBuf, "Added .rsrc1. VirtualAddress %lu\t adjust: %lu\n", pObjNew->VirtualAddress, adjust));
            }
            else
            {
                    ...
                    pObjOld->VirtualAddress - pObjNew->VirtualAddress)
                {
                    ...
                    DPrintf((DebugBuf, "adjust: %ld\tsmall: New %lu\tOld %lu\n", adjust,
                             cbResource - (pObjResourceOldX->VirtualAddress - pObjResourceOld->VirtualAddress),
                             pObjOld->VirtualAddress - pObjNew->VirtualAddress));
                }
                else
                {
    ...
    New.OptionalHeader.SizeOfImage =
        ROUNDUP(pObjNew->VirtualAddress + pObjNew->SizeOfRawData, New.OptionalHeader.SectionAlignment);
    ...
    pResDirL = (PIMAGE_RESOURCE_DIRECTORY_ENTRY)(((PUCHAR)pResDirN) + cTypes * sizeof(IMAGE_RESOURCE_DIRECTORY) +
                                                 cNames * sizeof(IMAGE_RESOURCE_DIRECTORY_ENTRY));

    pResData = (PIMAGE_RESOURCE_DATA_ENTRY)(((PUCHAR)pResDirL) + cNames * sizeof(IMAGE_RESOURCE_DIRECTORY) +
    ...
    DPrintf((DebugBuf, "skipping section table: %#08lx bytes @%#08lx\n",
             New.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER), FilePos(outfh)));
    DPrintf((DebugBuf, "copying hdr data: %#08lx bytes @%#08lx ==> @%#08lx\n",
             Old.OptionalHeader.SizeOfHeaders - ibObjTabEnd, ibObjTabEnd,
             ibObjTabEnd + New.OptionalHeader.SizeOfHeaders - Old.OptionalHeader.SizeOfHeaders));

    MuMoveFilePos(outfh, ibNewObjTabEnd + New.OptionalHeader.SizeOfHeaders - Old.OptionalHeader.SizeOfHeaders);
    ...
    for (pObjOld = pObjtblOld, pObjNew = pObjtblNew; pObjOld < pObjLast; pObjNew++)
    {
        ...
        if (pObjNew == pObjResourceNew)
        {
            ...
            New.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE].VirtualAddress =
        ...
        {
            if (pObjNew < pObjResourceNew && pObjOld->PointerToRawData != 0 &&
    ...
    DPrintf((DebugBuf, "Writing new section table: %#08x bytes @%#08lx\n",
             New.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER), ibObjTab));
    ...
    cb = PatchDebug(inpfh, outfh, pObjDebug, pObjNew, pObjDebugDirOld, pObjDebugDirNew, &Old, &New, ibMaxDbgOffsetOld,
                    &adjust);
