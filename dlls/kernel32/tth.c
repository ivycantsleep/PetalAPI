...
VOID gettictst(int x)
{
    ...
    if (!x)
    {
        StartBenchMark("NtGetTickCount)", 100000, &PerfInfo);
        for (i = 0; i < 100000; i++)
        {
            j = GetTickCount();
        }
...
VOID latst()
{
    ...
    StartBenchMark("LocalAlloc/Free)", 200, &PerfInfo);
    for (i = 0; i < 200 / 5; i++)
    {
        h1 = LocalAlloc(0, 500);
        h2 = LocalAlloc(0, 600);
...
VOID WinWordGetDriveTypeTest()
{
    ...
    DiskName[2] = '\\';
    DiskName[3] = '\0';
    StartBenchMark("WinWord GetDriveTypeA (a-z)", 100, &PerfInfo);
    ...
    WDiskName[2] = (WCHAR)'\\';
    WDiskName[3] = (WCHAR)'\0';
    StartBenchMark("WinWord GetDriveTypeW (a-z)", 100, &PerfInfo);
...

{
    ...
    {
        RtlDosPathNameToNtPathName_U(L"foo", &FileName, NULL, &RelativeName);
...
void notifytst()
{
    ...
    fFile = FindFirstFile("c:\\*.*", &FindFileData);
    ...
    nHandle = FindFirstChangeNotification("C:\\", TRUE, FILE_NOTIFY_CHANGE_NAME);
...
void NewRip(int flag, LPSTR str)
{
...
void cptst()
{
    ...
    SetLastError(0);
    CreateProcess(NULL, buf, NULL, NULL, FALSE, 0, NULL, NULL, &StartupInfo, &ProcessInformation);
...
void badproctst()
{
    ...
    StartupInfo.cb = sizeof(StartupInfo);
    SetLastError(0);
    CreateProcess(NULL, buf, NULL, NULL, FALSE, 0, NULL, "*", &StartupInfo, &ProcessInformation);
    printf("GLE %d\n", GetLastError());
}
...
void fftst()
{
    ...
    fFile = FindFirstFile(buf, &FindFileData);
    ...
    while (b)
    {
        printf("0x%08x %08d %s\n", FindFileData.dwFileAttributes, FindFileData.nFileSizeLow, FindFileData.cFileName);
        b = FindNextFile(fFile, &FindFileData);
    }
    FindClose(fFile);
}
...
void spath()
{
    ...
    SearchPath("c:\\nt;c:\\xytty;c:\\nt\\system", "kernel32", ".dll", 512, cbuff, NULL);
    printf("%s\n", cbuff);
}
...
void muldivtst()
{
    ...
    for (Index = 0; Index < 50000; Index++)
    {
        ...
        if (answer != result)
            printf("MulDiv(%ld,%ld,%ld)=%ld %s\n", number, numerator, denom, result,
                   answer == result ? "SUCCESS" : "FAILED");
        ...
        result = MulDiv(number, numerator, denom);
        if (answer != result)
            printf("MulDiv(%ld,%ld,%ld)=0x%lx %s\n", number, numerator, denom, result,
                   answer == result ? "SUCCESS" : "FAILED");
    }
    FinishBenchMark(&PerfInfo);
}
...
void dname()
{
    ...
    for (i = 0; i < 26; i++)
    {
        *p = (WCHAR)'A' + i;
        ...
        InitializeObjectAttributes(&Obja, &LinkName, OBJ_CASE_INSENSITIVE, NULL, NULL);
        Status = NtOpenSymbolicLinkObject(&LinkHandle, SYMBOLIC_LINK_QUERY, &Obja);
        if (NT_SUCCESS(Status))
        {
            ...
            Status = NtQuerySymbolicLinkObject(LinkHandle, &DeviceName, NULL);
            NtClose(LinkHandle);
            if (NT_SUCCESS(Status))
            {
                printf("%wZ -> %wZ\n", &LinkName, &DeviceName);
...
VOID StartBounce(PVOID pThreadBlockInfo)
{
...
void lockuptst()
{
    ...
    hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)StartBounce, 0, 0, &id);
    EnterCriticalSection(&cs);
    Sleep(-1);
}
void getdisktst()
