...
INT APIENTRY GetExpandedNameA(LPSTR lpszSource, LPSTR lpszBuffer)
{
    ...
    if ((doshSource = (HFILE)MOpenFile(lpszSource, (LPOFSTRUCT)(&ofOpenBuf), OF_READ)) == -1)
        return (LZERROR_BADVALUE);

    // Get the compressed file header.
    if ((bHdr = GetHdr((FH *)&FHComp, doshSource, &cblInSize)) != TRUE && cblInSize >= (LONG)HEADER_LEN)
    {
        FCLOSE(doshSource);
        return (LZERROR_BADVALUE);
    }
...
//             successful.  One of the LZERROR_ codes if unsuccessful.
//
INT LZCreateFileW(LPWSTR lpFileName, DWORD fdwAccess, DWORD fdwShareMode, DWORD fdwCreate, LPWSTR lpCompressedName)
{
...
INT APIENTRY LZOpenFileA(LPSTR lpFileName, LPOFSTRUCT lpReOpenBuf, WORD wStyle)
{
    ...
    if ((dosh = OpenFile(pszCompName, lpReOpenBuf, wStyle)) == -1 && lpReOpenBuf->nErrCode == DEE_FILENOTFOUND)
    {
        // Let's try to open the file of the corresponding compressed name.
    ...
    while (nCount > 0 && lpLZ->lCurSeekPos < (LONG)lpLZ->cbulUncompSize)
    {
       ...
       */
        if (iCurReadPos >= 0)
        {
            ...
            for (cbCopied = 0; iCurReadPos < nNumExpBufBytes && nCount > 0; cbCopied++, nCount--)
        ...
        if (nCount > 0 && lpLZ->lCurSeekPos < (LONG)lpLZ->cbulUncompSize)
        {
          ...
          */
             ...
             */
                ...
                /* Continue decoding. */
                lNextExpEndOffset = lExpBufStart + (LONG)nNumExpBufBytes + lExpBufEnd - MAX_OVERRUN;
          ...
          */
            lNextDecodeTarget =
                MIN(lNextExpEndOffset, MIN(lpLZ->lCurSeekPos + (LONG)nCount, (LONG)lpLZ->cbulUncompSize - 1L));
            ...
            {
            case ALG_FIRST:
                f = LZDecode(lpLZ->dosh, NO_DOSH, lNextDecodeTarget, bRestartDecoding, TRUE, pLZI);
                break;
...
HFILE ConvertDosFHToWin32(INT DoshSource)
{
    ...
    if (DoshSource >= MAX_LZFILES || DoshSource < 0 || fhWin32File[DoshSource] == 0)
    {
