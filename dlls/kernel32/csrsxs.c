...
{
    KdPrintEx((DPFLTR_SXS_ID, DPFLTR_TRACE_LEVEL,
               "SXS: %s() %s {\n"
               "SXS:    FileType:%s\n"
               "SXS:    PathType:%s\n"
               "SXS:    Path:%wZ\n"
               ...
               (MessageStream->FileType == BASE_MSG_FILETYPE_NONE              ? "None"
                ...
                : MessageStream->FileType == BASE_MSG_FILETYPE_PRECOMPILED_XML ? "PrecompiledXml"
                                                                               : "Unknown"),
               ...
               (MessageStream->PathType == BASE_MSG_PATHTYPE_NONE       ? "None"
                ...
                : MessageStream->PathType == BASE_MSG_PATHTYPE_OVERRIDE ? "Override"
                                                                        : "Unknown"),
               ...
               (MessageStream->HandleType == BASE_MSG_HANDLETYPE_NONE             ? "None"
                ...
                : MessageStream->HandleType == BASE_MSG_HANDLETYPE_SECTION        ? "Section"
                                                                                  : "Unknown"),

               MessageStream->Handle, MessageStream->Offset, MessageStream->Size));
}
...
CsrBasepCreateActCtx(IN PBASE_SXS_CREATE_ACTIVATION_CONTEXT_MSG Message)
{
...
#if DBG
    KdPrintEx(
        (DPFLTR_SXS_ID, DPFLTR_TRACE_LEVEL,
         "SXS: %s() Message {\n"
         "SXS:   Flags:(%s | %s | %s)\n"
         ...
         Message->ProcessorArchitecture, Message->LangId, &Message->AssemblyDirectory,
    ...
    {
        const PUNICODE_STRING StringsToCapture[] = { &m.u.SxsCreateActivationContext.Manifest.Path,
    ...
    Status = CsrClientCallServer((PCSR_API_MSG)&m, CaptureBuffer,
                                 CSR_MAKE_API_NUMBER(BASESRV_SERVERDLL_INDEX, BasepSxsCreateActivationContext),
                                 sizeof(*Message));

#if defined(BUILD_WOW64)
    if ((m.u.SxsCreateActivationContext.ActivationContextData != NULL) && (ActivationContextDataOut32 != NULL))
    {
