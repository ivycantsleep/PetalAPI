...
CsrBasepRefreshIniFileMapping(PUNICODE_STRING BaseFileName)
{
    ...
    CaptureBuffer = CsrAllocateCaptureBuffer(1, BaseFileName->MaximumLength);
    if (CaptureBuffer == NULL)
    {
        return STATUS_NO_MEMORY;
    }

    CsrCaptureMessageString(CaptureBuffer, (PCHAR)BaseFileName->Buffer, BaseFileName->Length,
                            BaseFileName->MaximumLength, (PSTRING)&ap->IniFileName);
