...
CsrBasepDefineDosDevice(DWORD dwFlags, PUNICODE_STRING pDeviceName, PUNICODE_STRING pTargetPath)
{
    ...
    return NtWow64CsrBasepDefineDosDevice(dwFlags, pDeviceName, pTargetPath);
#else
    ...
    a->DeviceName.MaximumLength =
        (USHORT)CsrAllocateMessagePointer(p, pDeviceName->MaximumLength, (PVOID *)&a->DeviceName.Buffer);
    RtlUpcaseUnicodeString(&a->DeviceName, pDeviceName, FALSE);
    if (pTargetPath->Length != 0)
    {
        a->TargetPath.MaximumLength =
            (USHORT)CsrAllocateMessagePointer(p, pTargetPath->MaximumLength, (PVOID *)&a->TargetPath.Buffer);
