...
VOID WriteIoComplete(DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, LPOVERLAPPED lpOverlapped)
{
    PAIO paio;
...
VOID ReadIoComplete(DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, LPOVERLAPPED lpOverlapped)
{
    PAIO paio;
    ...
    IoOperationStatus =
        WriteFileEx(paio->WriteHandle, paio->Buffer, dwNumberOfBytesTransfered, &paio->Overlapped, WriteIoComplete);
...
VOID IoWorkerThread(PVOID Unused)
{
    ...
    for (;;)
    {
        ...
        CompletionStatus = WaitForMultipleObjectsEx(2, HandleVector, TRUE, 0xffffffff, TRUE);
        ...
        if (CompletionStatus != WAIT_IO_COMPLETION)
        {
            ...
            IoOperationStatus = ReadFileEx(paio->ReadHandle, paio->Buffer, paio->Overlapped.Internal, &paio->Overlapped,
                                           ReadIoComplete);
...
main(int argc, char *argv[], char *envp[])
{
    ...
    iFile = CreateFile(argv[1], GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
    ...
    oFile = CreateFile(argv[2], GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS,
