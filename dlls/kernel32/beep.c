...
BOOL APIENTRY Beep(DWORD dwFreq, DWORD dwDuration)
{
    ...
    if (IsTerminalServer())
    {
        ...
        if (!pWinStationBeepOpen)
        {
            ...
            if ((hwinsta = LoadLibraryW(L"WINSTA")) != NULL)
            {
                ...
                pWinStationBeepOpen = (PWINSTATIONBEEPOPEN)GetProcAddress(hwinsta, "_WinStationBeepOpen");
            }
        }
    ...
    else
    {
        ...
        Status = NtCreateFile(&hBeepDevice, FILE_READ_DATA | FILE_WRITE_DATA, &ObjectAttributes, &IoStatus, NULL, 0,
                              FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN_IF, 0, NULL, 0L);
    }
    ...
    if ((dwFreq != 0 || dwDuration != 0) && (dwFreq < (ULONG)0x25 || dwFreq > (ULONG)0x7FFF))
    {
    ...
    else
    {
        ...
        Status = NtDeviceIoControlFile(hBeepDevice, NULL, NULL, NULL, &IoStatus, IOCTL_BEEP_SET, &BeepParameters,
                                       sizeof(BeepParameters), NULL, 0);
    }
    ...
    if (IsTerminalServer() && IsActiveConsoleSession() && hTSBeepDevice)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            NtDeviceIoControlFile(hTSBeepDevice, NULL, NULL, NULL, &IoStatus, IOCTL_BEEP_SET, &BeepParameters,
                                  sizeof(BeepParameters), NULL, 0);
        }
