...
CsrBasepCreateProcess(PBASE_CREATEPROCESS_MSG a)
{
    ...
    CsrClientCallServer((PCSR_API_MSG)&m, CaptureBuffer,
                        CSR_MAKE_API_NUMBER(BASESRV_SERVERDLL_INDEX, BasepCreateProcess), sizeof(m.u.CreateProcess));
...
#if defined(BUILD_WOW6432)
    return NtWow64CsrBasepSetProcessShutdownParam(dwLevel, dwFlags);
#else
...
#if defined(BUILD_WOW6432)
    return NtWow64CsrBasepGetProcessShutdownParam(lpdwLevel, lpdwFlags);
#else
