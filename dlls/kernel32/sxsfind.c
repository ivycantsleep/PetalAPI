...
BOOL BasepFindActCtxSection_FillOutReturnData(IN DWORD dwWin32FlagsIn, OUT PACTCTX_SECTION_KEYED_DATA ReturnedData,
...
BOOL BasepFindActCtxSection_CheckAndConvertParameters(IN DWORD dwWin32Flags,
                                                      ...
                                                      OUT PULONG pulRtlFlags)
{
    ...
    if (((dwWin32Flags & ~(FIND_ACTCTX_SECTION_KEY_RETURN_HACTCTX | FIND_ACTCTX_SECTION_KEY_RETURN_FLAGS |
...
InvalidParameter:
    DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
               ...
               "   ReturnedData = %p\n"
               "      ->cbSize = %u\n",
               dwWin32Flags, pWin32ReturnedData, (pWin32ReturnedData != NULL) ? cbWin32Size : 0);
    SetLastError(ERROR_INVALID_PARAMETER);
    goto Exit;
}

BOOL BasepFindActCtxSectionString(DWORD dwFlags, const GUID *lpExtensionGuid, ULONG ulSectionId,
                                  PCUNICODE_STRING PUnicodeString, PACTCTX_SECTION_KEYED_DATA ReturnedData)
{
    BOOL fSuccess = FALSE;
    NTSTATUS Status;
    ...
    if (!BasepFindActCtxSection_CheckAndConvertParameters(dwFlags, ReturnedData, &ulRtlFindFlags))
    {
        goto Exit;
    }
    ...
    Status =
        RtlFindActivationContextSectionString(ulRtlFindFlags, lpExtensionGuid, ulSectionId, PUnicodeString, &TempData);
    ...
    if (!BasepFindActCtxSection_FillOutReturnData(dwFlags, ReturnedData, &TempData))
    {
        goto Exit;
    }
...
WINBASEAPI
BOOL FindActCtxSectionStringA(DWORD dwFlags, const GUID *lpExtensionGuid, ULONG ulSectionId, LPCSTR lpStringToFind,
...
BOOL FindActCtxSectionStringW(DWORD dwFlags, const GUID *lpExtensionGuid, ULONG ulSectionId, LPCWSTR lpStringToFind,
                              PACTCTX_SECTION_KEYED_DATA ReturnedData)
{
    BOOL fSuccess = FALSE;
    UNICODE_STRING UnicodeString;
                                  ...
                                  const GUID *lpGuidToFind, PACTCTX_SECTION_KEYED_DATA ReturnedData)
{
    ...
    Status = RtlFindActivationContextSectionGuid(ulRtlFindFlags, lpExtensionGuid, ulSectionId, lpGuidToFind, &TempData);
    if (!NT_SUCCESS(Status))
    {
        BaseSetLastNTError(Status);
        goto Exit;
