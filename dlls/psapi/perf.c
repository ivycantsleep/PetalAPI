...
--*/
{
    ...
    while (1)
    {
        ...
        Status = NtQuerySystemInformation(SystemProcessInformation, ProcInfo, BufferLength, &RetLen);
        if (NT_SUCCESS(Status))
        {
            break;
        }
...
--*/
{
    ...
    while (1)
    {
        ...
        Status = NtQuerySystemInformation(SystemPageFileInformation, PageFileInfo, BufferLength, &RetLen);
        if (NT_SUCCESS(Status))
        {
            break;
        }
    ...
    while (RetLen > sizeof(SYSTEM_PAGEFILE_INFORMATION))
    {
        ...
        tPageFileInfo = (PSYSTEM_PAGEFILE_INFORMATION)((PUCHAR)tPageFileInfo + tPageFileInfo->NextEntryOffset);
    }
    LocalFree(PageFileInfo);
    return TRUE;
}
...
BOOL CallBackConvertToAscii(LPVOID pContext, PENUM_PAGE_FILE_INFORMATION pPageFileInfo, LPCWSTR lpFilename)
{
    DWORD Len;
    LPSTR AsciiStr;
...
--*/
{
    ...
    RetVal = EnumPageFilesW(CallBackConvertToAscii, &Ctx);
    if (RetVal)
    {
        //
