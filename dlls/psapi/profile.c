...
VOID PsWriteProfileLine(IN HANDLE ProfileHandle, IN PSZ Line, IN int nbytes)
{
    IO_STATUS_BLOCK IoStatusBlock;

    NtWriteFile(ProfileHandle, NULL, NULL, NULL, &IoStatusBlock, Line, (ULONG)nbytes, NULL, NULL);
}
...
PsInitializeAndStartProfile(VOID)
{
    ...
    if (fKernel)
    {
        status = NtQuerySystemInformation(SystemModuleInformation, ModuleInfoBuffer, sizeof(ModuleInfoBuffer),
                                          &ReturnedLength);
        if (!NT_SUCCESS(status))
        {
        ...
        else
        {
            ...
            status = RtlAdjustPrivilege(SE_SYSTEM_PROFILE_PRIVILEGE,
                                        TRUE,  //Enable
            ...
            status = RtlAdjustPrivilege(SE_INCREASE_QUOTA_PRIVILEGE,
                                        TRUE,  //Enable
                                        FALSE, //not impersonating
    ...
    while (!Done)
    {
        if (Next != &Peb->Ldr->InMemoryOrderModuleList)
        {
            ...
            status = RtlUnicodeToOemN(ImageFileName, 256, &i, ImageName->Buffer, ImageName->Length);
            ImageFileName[i] = 0;
        ...
        else if (fKernel && (ModuleNumber < Modules->NumberOfModules))
        {
            ...
            RtlMultiByteToUnicodeN(ImageName->Buffer, cNameUCLength, &i,
                                   &Module->FullPathName[Module->OffsetToFileName], cNameMBLength);
        ...
        status = NtAllocateVirtualMemory(CurrentProcessHandle, (PVOID *)&Buffer, 0, &BufferSize,
        ...
        status = NtCreateProfile(
            &ProfileObject[NumberOfProfileObjects].Handle, CurrentProcessHandle,
            ProfileObject[NumberOfProfileObjects].CodeStart, ProfileObject[NumberOfProfileObjects].CodeLength,
            ProfileObject[NumberOfProfileObjects].BucketSize, ProfileObject[NumberOfProfileObjects].Buffer,
            ProfileObject[NumberOfProfileObjects].BufferSize, ProfileSource, (KAFFINITY)-1);

        if (status != STATUS_SUCCESS)
        {
            DbgPrint("create profile %wZ failed - status %lx\n", ProfileObject[NumberOfProfileObjects].ImageName,
                     status);
            return status;
        }
        ...
        if (UseSecondaryProfile)
        {
            ...
            status = NtCreateProfile(
                &ProfileObject[NumberOfProfileObjects].SecondaryHandle, CurrentProcessHandle,
                ProfileObject[NumberOfProfileObjects].CodeStart, ProfileObject[NumberOfProfileObjects].CodeLength,
                ProfileObject[NumberOfProfileObjects].BucketSize, ProfileObject[NumberOfProfileObjects].SecondaryBuffer,
                ProfileObject[NumberOfProfileObjects].BufferSize, SecondaryProfileSource, (KAFFINITY)-1);
...
unsigned long Percent(unsigned long arg1, unsigned long arg2, unsigned long *Low)
{
...
PsStopAndAnalyzeProfile(VOID)
{
    ...
    __try
    {
        ...
        ProfileHandle = CreateFile(OutputFile, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
        ...
        {
            n = sprintf(Line,
                        "Overflowed the maximum number of "
                        "modules: %d\n",
                        MAX_PROFILE_COUNT);
        ...
        for (i = 0; i < NumberOfProfileObjects; i++)
        {
            ...
            //
            BufferEnd = ProfileObject[i].Buffer + (ProfileObject[i].BufferSize / sizeof(ULONG));
            ...
            n = sprintf(Line, "%d,%wZ,Total%s\n", TotalCounts, ProfileObject[i].ImageName,
                    ...
                    if (*Counter)
                    {
                                    ...
                                    if (CountAtSymbol || SecondaryCountAtSymbol)
                                    {
                                        if (!UseSecondaryProfile)
                                        {
                                            n = sprintf(Line, "%d,%wZ,%s (%08lx)\n", CountAtSymbol,
                                                        ProfileObject[i].ImageName, LastSymbol->Name,
                                                        LastSymbol->Address);
                                        }
                                        else
                                        {
                                            if (SecondaryCountAtSymbol != 0)
                                            {
                                                ...
                                                n = sprintf(Line, "%d,%d,%2.2d.%3.3d,%wZ,%s (%08lx)\n", CountAtSymbol,
                                                            SecondaryCountAtSymbol, High, Low,
                                                            ProfileObject[i].ImageName, LastSymbol->Name,
                                                            LastSymbol->Address);
                                            }
                                            else
                                            {
                                                n = sprintf(Line, "%d,%d, -- ,%wZ,%s (%08lx)\n", CountAtSymbol,
                                                            SecondaryCountAtSymbol, ProfileObject[i].ImageName,
                                                            LastSymbol->Name, LastSymbol->Address);
                                            }
                                        }
                                        PsWriteProfileLine(ProfileHandle, Line, n);
                                        if (ShowAllHits)
                                            ...
                                            while (InitialCounter < Counter)
                                            {
                                                if (*InitialCounter)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            High = Percent(*InitialCounter, *SecondaryInitialCounter,
                                                                           &Low);
                                                            n = sprintf(Line, "\t%p:%d, %d, %2.2d.%3.3d\n", Va,
                                                                        *InitialCounter, *SecondaryInitialCounter, High,
                                                                        Low);
                                                        }
                                                        else
                                                        {
                        ...
                        else
                        {
                            if (CountAtSymbol || SecondaryCountAtSymbol)
                            {
                                if (!UseSecondaryProfile)
                                {
                                    n = sprintf(Line, "%d,%wZ,%s (%08lx)\n", CountAtSymbol, ProfileObject[i].ImageName,
                                                LastSymbol->Name, LastSymbol->Address);
                                }
                                else
                                {
                                    if (SecondaryCountAtSymbol != 0)
                                    {
                                        High = Percent(CountAtSymbol, SecondaryCountAtSymbol, &Low);
                                        n = sprintf(Line, "%d,%d,%2.2d.%3.3d,%wZ,%s (%08lx)\n", CountAtSymbol,
                                                    SecondaryCountAtSymbol, High, Low, ProfileObject[i].ImageName,
                                                    LastSymbol->Name, LastSymbol->Address);
                                    }
                                    else
                                    {
                                        ...
                                        if (*InitialCounter)
                                        {
                                            ...
                                            else
                                            {
                                                if (*SecondaryInitialCounter != 0)
                                                {
                                                    ...
                                                    n = sprintf(Line, "\t%p:%d, %d, %2.2d.%3.3d\n", Va, *InitialCounter,
                                                                *SecondaryInitialCounter, High, Low);
                                                }
                                                else
                                                {
                                                    n = sprintf(Line, "\t%p:%d, %d, --\n", Va, *InitialCounter,
                ...
                if (CountAtSymbol || SecondaryCountAtSymbol)
                {
                    ...
                    else
                    {
                        if (SecondaryCountAtSymbol != 0)
                        {
                            High = Percent(CountAtSymbol, SecondaryCountAtSymbol, &Low);
                            n = sprintf(Line, "%d,%d,%2.2d.%3.3d,%wZ,%s (%08lx)\n", CountAtSymbol,
                                        SecondaryCountAtSymbol, High, Low, ProfileObject[i].ImageName, LastSymbol->Name,
                                        LastSymbol->Address);
                        }
                        else
                        {
                            n = sprintf(Line, "%d,%d, -- ,%wZ,%s (%08lx)\n", CountAtSymbol, SecondaryCountAtSymbol,
                                        ProfileObject[i].ImageName, LastSymbol->Name, LastSymbol->Address);
                        }
                    }
                    PsWriteProfileLine(ProfileHandle, Line, n);
                    if (ShowAllHits)
...
BOOLEAN
DllMain(IN PVOID DllHandle, IN ULONG Reason, IN PCONTEXT Context OPTIONAL)
...
VOID PsParseCommandLine(VOID)
{
    ...
    //
    MappingHandle = OpenFileMapping(FILE_MAP_WRITE, FALSE, "ProfileStartupParameters");
    if (MappingHandle != NULL)
    {
        CommandLine = MapViewOfFile(MappingHandle, FILE_MAP_WRITE, 0, 0, 0);
        if (!CommandLine)
        {
    ...
    while (Argument != NULL)
    {
        if ((Argument[0] == '-') || (Argument[0] == '/'))
        {
            ...
            case 'F':
                ...
                //
                OutputFile = HeapAlloc(GetProcessHeap(), 0, lstrlen(&Argument[2]) + 1);
