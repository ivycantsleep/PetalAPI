...
Retry:
    ...
    Status = NtQuerySystemInformation(SystemProcessInformation, pvProcessInformation, cbProcessInformation, NULL);
    ...
    for (;;)
    {
        ...
        pProcessInformation = (PSYSTEM_PROCESS_INFORMATION)((BYTE *)pvProcessInformation + TotalOffset);
...

{
    ...
    Status = NtQueryInformationProcess(hProcess, ProcessVmCounters, &VmCounters, sizeof(VmCounters), NULL);
...
BOOL WINAPI GetWsChanges(HANDLE hProcess, PPSAPI_WS_WATCH_INFORMATION lpWatchInfo, DWORD cb)
{
    ...
    Status = NtQueryInformationProcess(hProcess, ProcessWorkingSetWatch, (PVOID *)lpWatchInfo, cb, NULL);
    if (NT_SUCCESS(Status))
    {
        return TRUE;
    }
...
GetProcessImageFileNameW(HANDLE hProcess, LPWSTR lpImageFileName, DWORD nSize)
{
    ...
    RtlCopyMemory(lpImageFileName, Buffer->Buffer, Buffer->Length);
...
GetProcessImageFileNameA(HANDLE hProcess, LPSTR lpImageFileName, DWORD nSize)
{
    ...
    ReturnLength = WideCharToMultiByte(CP_ACP, 0, Buffer->Buffer, Buffer->Length, lpImageFileName, nSize, NULL, NULL);
