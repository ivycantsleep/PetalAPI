...
typedef NTSTATUS(WINAPI ntPWI)(POWER_INFORMATION_LEVEL InformationLevel, PVOID InputBuffer, ULONG InputBufferLength,
...
__declspec(dllexport) NTSTATUS WINAPI CallNtPowerInformation(POWER_INFORMATION_LEVEL InformationLevel,
    ...
    if (InformationLevel == SystemExecutionState)
    {
        ...
        if (::GetLastError() == ERROR_NO_TOKEN && ::ImpersonateSelf(SecurityImpersonation))
        {
            ...
            LABEL_6:
                ...
                ReturnLength[0] = 28;
                if (!AdjustTokenPrivileges(TokenHandle, FALSE, &NewState, sizeof(_TOKEN_PRIVILEGES), &PreviousState,
                                           ReturnLength))
