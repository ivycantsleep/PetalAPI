...
#ifdef __cplusplus
template <size_t N> struct ArraySizeHelper
{
    char _[N];
};
...
typedef BOOL(WINAPI api_WaitOnAddress)(volatile VOID *Address, PVOID CompareAddress, SIZE_T AddressSize,
                                       DWORD dwMilliseconds);

typedef void(WINAPI api_WakeByAddressAll)(PVOID Address);


typedef void(WINAPI api_WakeByAddressSingle)(PVOID Address);

typedef LARGE_INTEGER *(WINAPI api_BaseFormatTimeOut)(LARGE_INTEGER *pLITimeout, DWORD dwMillis);
...
#define LIST_FOR_EACH_SAFE(cursor, cursor2, list)                                 \
    for ((cursor) = (list)->next, (cursor2) = (cursor)->next; (cursor) != (list); \
         (cursor) = (cursor2), (cursor2) = (cursor)->next)

/* iterate through the list using a list entry */
#define LIST_FOR_EACH_ENTRY(elem, list, type, field)                               \
    for ((elem) = LIST_ENTRY((list)->next, type, field); &(elem)->field != (list); \
         (elem) = LIST_ENTRY((elem)->field.next, type, field))

/* iterate through the list using a list entry, with safety against removal */
#define LIST_FOR_EACH_ENTRY_SAFE(cursor, cursor2, list, type, field)                                                  \
    for ((cursor) = LIST_ENTRY((list)->next, type, field), (cursor2) = LIST_ENTRY((cursor)->field.next, type, field); \
         &(cursor)->field != (list); (cursor) = (cursor2), (cursor2) = LIST_ENTRY((cursor)->field.next, type, field))
...
#define LIST_FOR_EACH_SAFE_REV(cursor, cursor2, list)                             \
    for ((cursor) = (list)->prev, (cursor2) = (cursor)->prev; (cursor) != (list); \
         (cursor) = (cursor2), (cursor2) = (cursor)->prev)

/* iterate through the list in reverse order using a list entry */
#define LIST_FOR_EACH_ENTRY_REV(elem, list, type, field)                           \
    for ((elem) = LIST_ENTRY((list)->prev, type, field); &(elem)->field != (list); \
         (elem) = LIST_ENTRY((elem)->field.prev, type, field))

/* iterate through the list in reverse order using a list entry, with safety against removal */
#define LIST_FOR_EACH_ENTRY_SAFE_REV(cursor, cursor2, list, type, field)                                              \
    for ((cursor) = LIST_ENTRY((list)->prev, type, field), (cursor2) = LIST_ENTRY((cursor)->field.prev, type, field); \
         &(cursor)->field != (list); (cursor) = (cursor2), (cursor2) = LIST_ENTRY((cursor)->field.prev, type, field))
...
#undef LIST_ENTRY
#define LIST_ENTRY(elem, type, field) ((type *)((char *)(elem) - offsetof(type, field)))
    ...
    EXPORT BOOL WINAPI _WaitOnAddress(volatile VOID *Address, PVOID CompareAddress, SIZE_T AddressSize,
                                      DWORD dwMilliseconds)
    {
        Sleep(0);
        SwitchToThread();
        ...
        if (AddressSize != sizeof(BYTE) && AddressSize != sizeof(WORD) && AddressSize != sizeof(DWORD) &&
            AddressSize != sizeof(__int64))
    ...
    EXPORT void WINAPI _WakeByAddressAll(PVOID Address)
    {
        Sleep(0);
        SwitchToThread();
        return;
    ...
    EXPORT void WINAPI _WakeByAddressSingle(PVOID Address)
    {
        Sleep(1);
        SwitchToThread();
        return;
