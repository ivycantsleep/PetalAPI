...
ULONG
QueryNumberOfPersonalGroupNames(HANDLE CurrentUser, PHANDLE GroupNamesKey, PHANDLE SettingsKey)
{
    NTSTATUS Status;
    ...
    InitializeObjectAttributes(&ObjectAttributes, &KeyName, OBJ_CASE_INSENSITIVE, CurrentUser, NULL);
    Status =
        NtOpenKey(GroupNamesKey,
    ...
    for (NumberOfPersonalGroups = 1;; NumberOfPersonalGroups++)
    {
        ...
        Status = NtQueryValueKey(*GroupNamesKey, &ValueName, KeyValuePartialInformation, (PVOID)&KeyValueInformation,
...
BOOL NewPersonalGroupName(HANDLE GroupNamesKey, PWSTR GroupName, ULONG GroupNumber)
{
    NTSTATUS Status;
    ...
    RtlInitUnicodeString(&ValueName, ValueNameBuffer);
    Status = NtSetValueKey(GroupNamesKey, &ValueName, 0, REG_SZ, GroupName, (wcslen(GroupName) + 1) * sizeof(WCHAR));
...
BOOL DoesExistGroup(HANDLE GroupsKey, PWSTR GroupName)
{
    NTSTATUS Status;
    UNICODE_STRING KeyName, ValueName;
    HANDLE Key;
    ...
    InitializeObjectAttributes(&ObjectAttributes, &KeyName, OBJ_CASE_INSENSITIVE, GroupsKey, NULL);
    ...
    Status = NtQueryValueKey(Key, &ValueName, KeyValueFullInformation, &KeyValueInformation,
                             sizeof(KeyValueInformation), &ResultLength);
    NtClose(Key);
    ...
    else if (KeyValueInformation.Type != REG_BINARY || KeyValueInformation.DataLength > 0xFFFF)
    {
        return FALSE;
    }
    else
...
PGROUP_DEF
LoadGroup(HANDLE GroupsKey, PWSTR GroupName)
{
    PGROUP_DEF Group;
    NTSTATUS Status;
    UNICODE_STRING KeyName, ValueName;
    ...
    if (KeyValueInformation.Type != REG_BINARY || KeyValueInformation.DataLength > 0xFFFF)
    {
        NtClose(Key);
        return NULL;
    }
    ...
    if (Base == NULL || !VirtualAlloc(Base, KeyValueInformation.DataLength, MEM_COMMIT, PAGE_READWRITE))
    {
        if (Base != NULL)
        {
    ...
    Status = NtQueryValueKey(Key, &ValueName, KeyValueFullInformation, &KeyValueInformation,
                             KeyValueInformation.DataLength, &ResultLength);
    NtClose(Key);
    if (!NT_SUCCESS(Status))
    {
        VirtualFree(Base, 0, MEM_RELEASE);
...
BOOL SaveGroup(HANDLE GroupsKey, PWSTR GroupName, PGROUP_DEF Group)
{
    NTSTATUS Status;
    UNICODE_STRING KeyName, ValueName;
    HANDLE Key;
    ...
    RtlInitUnicodeString(&ValueName, L"");
    Status = NtSetValueKey(Key, &ValueName, 0, REG_BINARY, Group, ValueLength);
...
BOOL DeleteGroup(HANDLE GroupsKey, PWSTR GroupName)
{
    NTSTATUS Status;
    ...
    Status = NtOpenKey(&Key,
                       STANDARD_RIGHTS_WRITE | DELETE | KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS | KEY_SET_VALUE |
                           KEY_CREATE_SUB_KEY,
                       &ObjectAttributes);
    if (!NT_SUCCESS(Status))
    {
...
BOOL UnloadGroup(PGROUP_DEF Group)
{
    return VirtualFree(Group, 0, MEM_RELEASE);
}


BOOL ExtendGroup(PGROUP_DEF Group, BOOL AppendToGroup, DWORD cb)
{
...
WORD AddDataToGroup(PGROUP_DEF Group, PBYTE Data, DWORD cb)
{
    WORD Offset;
...
BOOL AddTagToGroup(PGROUP_DEF Group, WORD wID, WORD wItem, WORD cb, PBYTE rgb)
{
    WORD Offset;
    PTAG_DEF Tag;
...
ULONG MonoChromePalette[] = { 0x00000000, 0x00ffffff };

BITMAPINFOHEADER DefaultQuestionIconBIH = { 0x00000028, 0x00000020, 0x00000040, 0x0001,     0x0004,    0x00000000,
                                            0x00000280, 0x00000000, 0x00000000, 0xFFFFFFFF, 0x00000000 };

ULONG DefaultQuestionIconBits[] = {
    0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x33000000, 0x00000030, 0x00000000, 0x00000000,
    0xbb080000, 0x00000083, 0x00000000, 0x00000000, 0xff0b0000, 0x000000b3, 0x00000000, 0x00000000, 0xff0b0000,
    0x000000b3, 0x00000000, 0x00000000, 0xbb000000, 0x00000080, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
    0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
    0x00000000, 0x33000000, 0x00000030, 0x00000000, 0x00000000, 0xbb080000, 0x00000083, 0x00000000, 0x00000000,
    0xbb0b0000, 0x000000b3, 0x00000000, 0x00000000, 0xff0b0000, 0x000000b3, 0x00000000, 0x00000000, 0xff0b0000,
    0x000030b8, 0x00000000, 0x00000000, 0xbf000000, 0x000083bb, 0x00000000, 0x00000000, 0xbf000000, 0x0033b8fb,
    0x00000000, 0x00000000, 0x0b000000, 0x33b8bbbf, 0x00000000, 0x00000000, 0x00000000, 0xb3bbfbbb, 0x00000030,
    0x00000000, 0x00000000, 0xbbbbbb00, 0x00000030, 0x00000000, 0x00000000, 0xbbbb0000, 0x00000083, 0x00000000,
    0x00000000, 0xbb0b0000, 0x000000b3, 0x00000000, 0x00003033, 0xbb0b0000, 0x000000b3, 0x08000000, 0x000083bb,
    0xbb0b0000, 0x000000b3, 0x0b000000, 0x0000b3bb, 0xbb0b0000, 0x000000b3, 0x0b000000, 0x0030b8ff, 0xbb3b0000,
    0x000000b3, 0x0b000000, 0x0083bbff, 0xbb8b0300, 0x000000b3, 0x00000000, 0x33b8fbbf, 0xbbbb3833, 0x00000080,
    0x00000000, 0xbbbbffbb, 0xb8bbbbbb, 0x00000030, 0x00000000, 0xbbffbf0b, 0x83bbbbbb, 0x00000000, 0x00000000,
    0xffbf0b00, 0x00b8bbff, 0x00000000, 0x00000000, 0xbb0b0000, 0x0000b8bb, 0x00000000, 0x00000000, 0x00000000,
    0x00000000, 0x00000000, 0xff7ffcff, 0xff3ff8ff, 0xff1ff0ff, 0xff1ff0ff, 0xff1ff0ff, 0xff3ff8ff, 0xff7ffcff,
    0xffffffff, 0xff7ffcff, 0xff3ff8ff, 0xff1ff0ff, 0xff1ff0ff, 0xff1ff0ff, 0xff0ff0ff, 0xff03f8ff, 0xff00f8ff,
    0x7f00fcff, 0x3f00feff, 0x3f00ffff, 0x1fc0ffff, 0x1ff01fff, 0x1ff00ffe, 0x1ff007fc, 0x1ff007fc, 0x1fe003fc,
    0x1f0000fc, 0x3f0000fe, 0x3f0000fe, 0x7f0000ff, 0xff0080ff, 0xff03e0ff, 0xff0ff8ff
};


BOOL ConvertIconBits(PCURSORSHAPE_16 pIconHeader);
...
BOOL ConvertIconBits(PCURSORSHAPE_16 pIconHeader)
{
    ...
    else if (pIconHeader->BitsPixel != 1)
    {
        return FALSE;
    }
    else if (pIconHeader->cx != pIconHeader->cy)
    {
        return FALSE;
    }
    else if (nScanLine != 32)
    {
        return FALSE;
    }
    else if (cbScanLine != 4)
    {
        return FALSE;
    }
...
VOID CopyIconBits(PBYTE Dst, PBYTE Src, UINT cbScanLine, UINT nScanLine);
...
WIN31_GROUP_ITEM_DESC GroupItemsToIgnore[] = { { "Main", "File Manager" }, { NULL, "Control Panel" },
                                               { NULL, "Print Manager" },  { NULL, "ClipBook Viewer" },
                                               { NULL, "MS-DOS Prompt" },  { NULL, "Windows Setup" },
                                               { NULL, "Read Me" },        { "Accessories", "Paintbrush" },
                                               { NULL, "Write" },          { NULL, "Terminal" },
                                               { NULL, "Notepad" },        { NULL, "Recorder" },
                                               { NULL, "Clock" },          { NULL, "Object Packager" },
                                               { NULL, "Media Player" },   { NULL, "Sound Recorder" },
                                               { "Network", NULL },        { NULL, NULL } };

PGROUP_DEF
CreateGroupFromGroup16(LPSTR GroupName, PGROUP_DEF16 Group16)
{
    ...
    {
        if (p1->GroupName && !_stricmp(p1->GroupName, GroupName))
        {
            if (p1->ItemName == NULL)
            {
                return (PGROUP_DEF)-1;
    ...
    if (!VirtualAlloc(Group, cb = FIELD_OFFSET(GROUP_DEF, rgiItems[0]), MEM_COMMIT, PAGE_READWRITE))
    {
    ...
    Group->pName = AddDataToGroup(Group, GroupName, 0);
    ...
    {
        lpGetSystemMetrics = (PGETSYSTEMMETRICS)GetProcAddress(LoadLibrary("user32.dll"), "GetSystemMetrics");
        if (lpGetSystemMetrics == NULL)
        {
    ...
    for (i = 0; i < Group16->cItems; i++)
    {
        ...
        else if (pIconHeader->BitsPixel != 1 && pIconHeader->BitsPixel != 4 && pIconHeader->BitsPixel != 8)
        {
            bUserDefaultIcon = TRUE;
        }
        ...
        if (!bUserDefaultIcon)
        {
            ...
            Item->cbIconRes = (WORD)(bmisize + colorsize + imagesize + masksize);
            ...
            if (Item->pIconRes != 0)
            {
                ...
                CopyIconBits(p + bmisize + colorsize, (PBYTE)PTR(Group16, Item16->pXORPlane),
                             (((pIconHeader->cx * pIconHeader->BitsPixel + 31) & ~31) / 8), pIconHeader->cy);

                CopyIconBits(p + bmisize + colorsize + imagesize, (PBYTE)PTR(Group16, Item16->pANDPlane),
                             (((pIconHeader->cx + 31) & ~31) / 8), pIconHeader->cy);
            }
        }
        else
        {
        ...
        if (Item->pIconRes != 0)
        {
            Item->pName = AddDataToGroup(Group, PTR(Group16, Item16->pName), 0);
            if (Item->pName != 0)
            {
                Item->pCommand = AddDataToGroup(Group, PTR(Group16, Item16->pCommand), 0);
                if (Item->pCommand != 0)
                {
                    Item->pIconPath = AddDataToGroup(Group, PTR(Group16, Item16->pIconPath), 0);
                    if (Item->pIconPath != 0)
                    {
    ...
    if (bItemConvertedOkay && Tag16->wID == ID_MAGIC && Tag16->wItem == ID_LASTTAG &&
        *(UNALIGNED DWORD *)&Tag16->rgb[0] == PMTAG_MAGIC)
    {
        while (Tag16->wID != ID_LASTTAG)
        {
            if (!AddTagToGroup(Group, Tag16->wID, Tag16->wItem, (WORD)(Tag16->cb - FIELD_OFFSET(TAG_DEF16, rgb[0])),
                               &Tag16->rgb[0]))
            {
                bItemConvertedOkay = FALSE;
                break;
        ...
        if (bItemConvertedOkay)
        {
            if (!AddTagToGroup(Group, ID_LASTTAG, 0xFFFF, 0, NULL))
            {
                bItemConvertedOkay = FALSE;
            }
        }
...
BOOL DumpGroup(PWSTR GroupName, PGROUP_DEF Group)
{
    ...
    DbgPrint("     rcNormal:       [%08x,%08x,%08x,%08x]\n", Group->rcNormal.left, Group->rcNormal.top,
             Group->rcNormal.right, Group->rcNormal.bottom);
    ...
    if (Tag->wID == ID_MAGIC && Tag->wItem == ID_LASTTAG && *(LPDWORD)&Tag->rgb == PMTAG_MAGIC)
    {
