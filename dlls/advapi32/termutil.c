...
BOOL IsTSAppCompatEnabled(BOOLEAN *RelaxedSecurityMode)
{
    ...
    if (UniString.Buffer && pKeyValInfo)
    {
        ...
        if (NT_SUCCESS(NtStatus))
        {
            ...
            RtlInitUnicodeString(&UniString, L"TSUserEnabled");
...
ULONG GetCompatFlags()
{
    ...
    if (UniString.Buffer && pKeyValInfo)
    {
        ...
        if (NT_SUCCESS(NtStatus))
        {
            ...
            RtlInitUnicodeString(&UniString, COMPAT_FLAGS);
            NtStatus = NtQueryValueKey(hKey, &UniString, KeyValuePartialInformation, pKeyValInfo, ulcbuf, &ul);
...
BOOLEAN AdvApi_InitializeTermsrvFpns(BOOLEAN *pIsInRelaxedSecurityMode, DWORD *pdwCompatFlags)
{
    ...
    *pdwCompatFlags = dwCompatFlags = GetCompatFlags();
    ...
    if (dllHandle)
    {
        ...
        gpfnTermsrvCreateRegEntry = (PTERMSRVCREATEREGENTRY)GetProcAddress(dllHandle, "TermsrvCreateRegEntry");

        gpfnTermsrvOpenRegEntry = (PTERMSRVOPENREGENTRY)GetProcAddress(dllHandle, "TermsrvOpenRegEntry");

        gpfnTermsrvSetValueKey = (PTERMSRVSETVALUEKEY)GetProcAddress(dllHandle, "TermsrvSetValueKey");

        gpfnTermsrvDeleteKey = (PTERMSRVDELETEKEY)GetProcAddress(dllHandle, "TermsrvDeleteKey");

        gpfnTermsrvDeleteValue = (PTERMSRVDELETEVALUE)GetProcAddress(dllHandle, "TermsrvDeleteValue");

        gpfnTermsrvRestoreKey = (PTERMSRVRESTOREKEY)GetProcAddress(dllHandle, "TermsrvRestoreKey");

        gpfnTermsrvSetKeySecurity = (PTERMSRVSETKEYSECURITY)GetProcAddress(dllHandle, "TermsrvSetKeySecurity");

        gpfnTermsrvOpenUserClasses = (PTERMSRVOPENUSERCLASSES)GetProcAddress(dllHandle, "TermsrvOpenUserClasses");

        gpfnTermsrvGetPreSetValue = (PTERMSRVGETPRESETVALUE)GetProcAddress(dllHandle, "TermsrvGetPreSetValue");
...
void GetRegistryExtensionFlags(DWORD dwCompatFlags)
/*++
...
--*/
{
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (pKeyValInfo)
        {
            ...
            Status = NtQueryValueKey(hKey, &ValueName, KeyValuePartialInformation, (PVOID)pKeyValInfo, ulSize, &ulSize);
