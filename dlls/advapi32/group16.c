...
LoadGroup16(PWSTR GroupFileName)
{
    ...
    File = CreateFileW(GroupFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
...
BOOL ExtendGroup16(PGROUP_DEF16 Group, BOOL AppendToGroup, DWORD cb)
{
...
WORD AddDataToGroup16(PGROUP_DEF16 Group, PBYTE Data, DWORD cb)
{
    WORD Offset;
...
BOOL AddTagToGroup16(PGROUP_DEF16 Group, WORD wID, WORD wItem, WORD cb, PBYTE rgb)
{
    WORD Offset;
    PTAG_DEF16 Tag;
...
BOOL DumpGroup16(PWSTR GroupFileName, PGROUP_DEF16 Group)
{
    ...
    DbgPrint("     pName:          [%04x] %s\n", Group->pName, (PSZ)PTR(Group, Group->pName));
    DbgPrint("     cxIcon:          %04x\n", Group->cxIcon);
    DbgPrint("     cyIcon:          %04x\n", Group->cyIcon);
    ...
    if (Tag->wID == ID_MAGIC && Tag->wItem == ID_LASTTAG && *(UNALIGNED DWORD *)&Tag->rgb == PMTAG_MAGIC)
    {
