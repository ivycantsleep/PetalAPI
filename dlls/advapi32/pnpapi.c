...
PWSTR
MultiByteToUnicode(IN PCSTR String, IN UINT Codepage);

RPC_STATUS
GuidToString(UUID *Uuid, LPTSTR StringGuid);
...

{
    ...
    try
    {
        ...
        //
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, pszRegIDConfigDB, 0, KEY_QUERY_VALUE, &hKey) != ERROR_SUCCESS)
        {
        ...
        //
        ulSize = sizeof(ULONG);
        if (RegQueryValueEx(hKey, pszRegCurrentConfig, NULL, NULL, (LPBYTE)&ulCurrentConfig, &ulSize) != ERROR_SUCCESS)
        {
            SetLastError(ERROR_REGISTRY_CORRUPT);
            Status = FALSE;
            goto Clean0;
        ...
        //
        _snwprintf(RegStr, MAX_PATH, L"%s\\%04u", pszRegKnownDockingStates, ulCurrentConfig);

        if (RegOpenKeyEx(hKey, RegStr, 0, KEY_QUERY_VALUE, &hCfgKey) != ERROR_SUCCESS)
        {
            SetLastError(ERROR_REGISTRY_CORRUPT);
            Status = FALSE;
            goto Clean0;
        ...
        //
        ulSize = sizeof(ULONG);
        if ((RegQueryValueEx(hCurrentDockInfoKey, pszRegDockingState, NULL, NULL, (LPBYTE)&lpHwProfileInfo->dwDockInfo,
                             &ulSize) != ERROR_SUCCESS) ||
            (!lpHwProfileInfo->dwDockInfo) ||
            ((lpHwProfileInfo->dwDockInfo & DOCKINFO_UNDOCKED) && (lpHwProfileInfo->dwDockInfo & DOCKINFO_DOCKED)))
        {
            ...
            //
            if ((RegQueryValueEx(hCfgKey, pszRegDockState, NULL, NULL, (LPBYTE)&lpHwProfileInfo->dwDockInfo, &ulSize) !=
                 ERROR_SUCCESS) ||
                (!lpHwProfileInfo->dwDockInfo))
            {
        ...
        if (RegQueryValueEx(hCfgKey, pszRegHwProfileGuid, NULL, NULL, (LPBYTE)&lpHwProfileInfo->szHwProfileGuid,
                            &ulSize) != ERROR_SUCCESS)
        {
        ...
        if (RegQueryValueEx(hCfgKey, pszRegFriendlyName, NULL, NULL, (LPBYTE)&lpHwProfileInfo->szHwProfileName,
                            &ulSize) != ERROR_SUCCESS)
        {
...

{
    ...
    try
    {
        ...
        pAnsiString = UnicodeToMultiByte(HwProfileInfoW.szHwProfileGuid, CP_ACP);
        if (!pAnsiString)
        {
            Status = FALSE;
            goto Clean0;
        ...
        pAnsiString = UnicodeToMultiByte(HwProfileInfoW.szHwProfileName, CP_ACP);
        if (!pAnsiString)
        {
            Status = FALSE;
            goto Clean0;
...

{
    ...
    CharsInUnicodeString =
        MultiByteToWideChar(Codepage, MB_PRECOMPOSED, String, BytesIn8BitString, UnicodeString, BytesIn8BitString);
