...
HANDLE
OpenCreateKey(IN HANDLE Root, IN PWSTR Path, IN BOOL WriteAccess);
...
BOOL InitializeWin31State(IN WIN31IO_EVENT EventType, OUT PWIN31IO_STATE State)
{
    ...
    cch = GetWindowsDirectoryW(State->szWindowsDirectory, sizeof(State->szWindowsDirectory) / sizeof(WCHAR));
    ...
    *State->FileNamePart++ = OBJ_NAME_PATH_SEPARATOR;
    ...
    {
        State->SoftwareRoot = OpenCreateKey(NULL, L"\\Registry\\Machine\\Software", FALSE);
    ...
    else
    {
        ...
        {
            if (GetTokenInformation(TokenHandle, TokenUser, &State->TokenInformation, sizeof(State->TokenInformation),
                                    &ReturnLength))
            {
    ...
    State->Win31IOKey = OpenCreateKey(EventType == Win31SystemStartEvent ? State->SoftwareRoot : State->UserRoot,
                                      L"Windows 3.1 Migration Status", TRUE);
...
VOID ReportWin31IOEvent(IN PWIN31IO_STATE State, IN WORD EventType, IN DWORD EventId, IN DWORD SizeOfRawData,
                        IN PVOID RawData, IN DWORD NumberOfStrings, ...)
{
    va_list arglist;
    ULONG i;
    ...
    {
        if (!ReportEventW(State->EventLog, EventType,
                          0, // event category
                          EventId, State->UserSid, (WORD)NumberOfStrings, SizeOfRawData, Strings, RawData))
        {
...
int Win31IOExceptionHandler(IN DWORD ExceptionCode, IN PEXCEPTION_POINTERS ExceptionInfo, IN OUT PWIN31IO_STATE State)
{
    ...
    ReportWin31IOEvent(State, EVENTLOG_ERROR_TYPE, WIN31IO_EVENT_EXCEPTION, sizeof(*(ExceptionInfo->ExceptionRecord)),
                       ExceptionInfo->ExceptionRecord, 0);
...
DWORD
WINAPI
QueryWindows31FilesMigration(IN WIN31IO_EVENT EventType)
{
    DWORD Flags;
    WIN31IO_STATE State;
    ...
    try
    {
        ...
        except(Win31IOExceptionHandler(GetExceptionCode(), GetExceptionInformation(), &State))
        {
...
BOOL WINAPI SynchronizeWindows31FilesAndWindowsNTRegistry(IN WIN31IO_EVENT EventType, IN DWORD Flags,
                                                          ...
                                                          IN PVOID CallbackParameter)
{
    ...
    try
    {
        try
        {
            ...
            except(VirtualBufferExceptionHandler(GetExceptionCode(), GetExceptionInformation(), &Buffer))
            {
...
BOOL SnapShotWin31IniFileKey(IN OUT PWIN31IO_STATE State, IN PWSTR TempFileName, IN PWSTR FileName,
                             IN PWSTR ApplicationName, IN PWSTR KeyName);

BOOL SnapShotWin31IniFileSection(IN OUT PWIN31IO_STATE State, IN PWSTR TempFileName, IN PWSTR FileName,
...
BOOL SnapShotWin31IniFilesToRegistry(IN OUT PWIN31IO_STATE State)
{
    ...
    if (State->UserRoot != NULL)
    {
        Result = QueryWin31IniFilesMappedToRegistry(WIN31_INIFILES_MAPPED_TO_USER, State->WorkBuffer,
                                                    State->cchWorkBuffer, &cchBufferUsed);
    }
    else
    {
        Result = QueryWin31IniFilesMappedToRegistry(WIN31_INIFILES_MAPPED_TO_SYSTEM, State->WorkBuffer,
                                                    State->cchWorkBuffer, &cchBufferUsed);
    }
            ...
            if (FileName)
            {
                ...
                if (TempIniFileName[0] != UNICODE_NULL)
                {
                    if (ApplicationName)
                    {
                        if (KeyName)
                        {
                            if (SnapShotWin31IniFileKey(State, TempIniFileName, FileName, ApplicationName, KeyName))
                            {
                                ReportWin31IOEvent(State, EVENTLOG_INFORMATION_TYPE, WIN31IO_EVENT_MIGRATE_INI_VARIABLE,
                                                   0, NULL, 3, KeyName, ApplicationName, FileName);
                            }
                            else
                            {
                                ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE,
                                                   WIN31IO_EVENT_MIGRATE_INI_VARIABLE_FAILED, 0, NULL, 3, KeyName,
                                                   ApplicationName, FileName);
                            }
                        }
                        else
                        {
                            if (SnapShotWin31IniFileSection(State, TempIniFileName, FileName, ApplicationName))
                            {
                                ReportWin31IOEvent(State, EVENTLOG_INFORMATION_TYPE, WIN31IO_EVENT_MIGRATE_INI_SECTION,
                                                   0, NULL, 2, ApplicationName, FileName);
                            }
                            else
                            {
                                ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE,
                                                   WIN31IO_EVENT_MIGRATE_INI_SECTION_FAILED, 0, NULL, 2,
                                                   ApplicationName, FileName);
                            }
                        }
                    }
                    ...
                    {
                        if (SnapShotWin31IniFileSections(State, TempIniFileName, FileName))
                        {
                            ReportWin31IOEvent(State, EVENTLOG_INFORMATION_TYPE, WIN31IO_EVENT_MIGRATE_INI_FILE, 0,
                                               NULL, 1, FileName);
                        }
                        else
                        {
                            ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE, WIN31IO_EVENT_MIGRATE_INI_FILE_FAILED, 0,
                                               NULL, 1, FileName);
                        }
                    }
                }
                             ...
                             IN PWSTR ApplicationName, IN PWSTR KeyName)
{
    ...
    cch =
        GetPrivateProfileStringW(ApplicationName, KeyName, NULL, State->WorkBuffer, State->cchWorkBuffer, TempFileName);
    if (cch != 0)
    {
        if (WritePrivateProfileStringW(ApplicationName, KeyName, State->WorkBuffer, FileName))
        {
            return TRUE;
        }
        else
        {
            KdPrint(("WIN31IO: Copy to %ws [%ws].%ws == %ws (failed - %u)\n", FileName, ApplicationName, KeyName,
                                 ...
                                 IN PWSTR ApplicationName)
{
    ...
    Result = TRUE;
    cch = GetPrivateProfileStringW(ApplicationName, NULL, NULL, State->WorkBuffer, State->cchWorkBuffer, TempFileName);
    if (cch != 0)
    {
        ...
        {
            Result &= SnapShotWin31IniFileKey(State, TempFileName, FileName, ApplicationName, KeyName);
...
BOOL SnapShotWin31IniFileSections(IN OUT PWIN31IO_STATE State, IN PWSTR TempFileName, IN PWSTR FileName)
{
    ...
    Result = TRUE;
    cch = GetPrivateProfileStringW(NULL, NULL, NULL, State->WorkBuffer, State->cchWorkBuffer, TempFileName);
    if (cch != 0)
    {
        ...
        {
            Result &= SnapShotWin31IniFileSection(State, TempFileName, FileName, ApplicationName);
...
BOOL ConvertWindows31GroupsToRegistry(IN OUT PWIN31IO_STATE State, IN HANDLE MigrationKey, IN HANDLE CommonGroupsKey,
                                      IN HANDLE PersonalGroupsKey, IN PWSTR IniFileName, IN PWSTR GroupNames,
...
BOOL SnapShotWin31GroupsToRegistry(IN OUT PWIN31IO_STATE State)
{
    ...
    {
        cchGroupNames =
            GetPrivateProfileStringW(L"Groups", NULL, L"", State->WorkBuffer, State->cchWorkBuffer, State->WindowsPath);
        if (cchGroupNames != 0)
        {
            Result = TRUE;
        }
    ...
    if (cchGroupNames)
    {
        PersonalGroupsKey = OpenCreateKey(State->UserRoot, L"UNICODE Program Groups", !State->QueryOnly);
        ...
        CommonGroupsKey = OpenCreateKey(NULL, L"\\Registry\\Machine\\Software\\Program Groups", FALSE);
        ...
        ConvertWindows31GroupsToRegistry(State, MigrationKey, CommonGroupsKey, PersonalGroupsKey, State->WindowsPath,
                                         GroupNames, nGroupNames);
                                      ...
                                      IN ULONG nGroupNames)
{
    ...
    NumberOfPersonalGroupNames = QueryNumberOfPersonalGroupNames(State->UserRoot, &GroupNamesKey, &SettingsKey);
    ...
    while (*GroupNames)
    {
        ...
        //
        if (GetPrivateProfileStringW(L"Groups", GroupNames, L"", State->WorkBuffer, State->cchWorkBuffer, IniFileName))
        {
            ...
            if (Group16 != NULL)
            {
                ...
                if (NT_SUCCESS(Status))
                {
                    ...
                    {
                        ReportWin31IOEvent(State, EVENTLOG_INFORMATION_TYPE, WIN31IO_EVENT_MIGRATE_GROUP_EXISTS, 0,
                                           NULL, 2, Group16PathName, Group32Name.Buffer);
                    }
                    else if (DoesExistGroup(CommonGroupsKey, Group32Name.Buffer))
                    {
                        ReportWin31IOEvent(State, EVENTLOG_INFORMATION_TYPE, WIN31IO_EVENT_MIGRATE_GROUP_EXISTS, 0,
                                           NULL, 2, Group16PathName, Group32Name.Buffer);
                    }
                    else
                    {
                        ...
                        if (Group32 != NULL)
                        {
                            ...
                            {
                                ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE, WIN31IO_EVENT_MIGRATE_GROUP_FAILED4, 0,
                                                   NULL, 1, Group32Name.Buffer);
                                Group32 = NULL;
                            }
                            else
                            {
                                // DumpGroup( Group32Name.Buffer, Group32 );
                                if (!SaveGroup(PersonalGroupsKey, Group32Name.Buffer, Group32))
                                {
                                    ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE, WIN31IO_EVENT_MIGRATE_GROUP_FAILED,
                                                       0, NULL, 1, Group32Name.Buffer);
                                }
                                else if (!NewPersonalGroupName(GroupNamesKey, Group32Name.Buffer,
                                ...
                                else
                                {
                                    ...
                                    ReportWin31IOEvent(State, EVENTLOG_INFORMATION_TYPE, WIN31IO_EVENT_MIGRATE_GROUP, 0,
                                                       NULL, 1, Group32Name.Buffer);
                                    NumberOfPersonalGroupNames += 1;
                        ...
                        else
                        {
                            ...
                            _snwprintf(ErrorCode, sizeof(ErrorCode) / sizeof(WCHAR), L"%u", GetLastError());
                            ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE, WIN31IO_EVENT_MIGRATE_GROUP_FAILED1, 0,
                                               NULL, 2, State->WorkBuffer, ErrorCode);
                        }
                    }
            ...
            else
            {
                ...
                _snwprintf(ErrorCode, sizeof(ErrorCode) / sizeof(WCHAR), L"%u", GetLastError());
                ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE, WIN31IO_EVENT_MIGRATE_GROUP_FAILED1, 0, NULL, 2,
                                   State->WorkBuffer, ErrorCode);
            }
        }
        else
        {
            ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE, WIN31IO_EVENT_MIGRATE_GROUP_FAILED2, 0, NULL, 2,
                               IniFileName, GroupNames);
        }
    ...
    if (OldNumberOfPersonalGroupNames != NumberOfPersonalGroupNames)
    {
        ...
        Status = NtSetValueKey(SettingsKey, &ValueName, 0, REG_DWORD, &ValueData, sizeof(ValueData));
#if DBG
        if (!NT_SUCCESS(Status))
        {
...
BOOL SnapShotWin31RegDatToRegistry(IN OUT PWIN31IO_STATE State)
{
    ...
    if (RegDat16 != NULL)
    {
        ...
        {
            ReportWin31IOEvent(State, EVENTLOG_INFORMATION_TYPE, WIN31IO_EVENT_MIGRATE_REGDAT, 0, NULL, 1,
                               State->WindowsPath);
        }
        else
        {
            ReportWin31IOEvent(State, EVENTLOG_WARNING_TYPE, WIN31IO_EVENT_MIGRATE_REGDAT_FAILED, 0, NULL, 1,
                               State->WindowsPath);
        }
...
BOOL WaitForEventLogToStart(VOID)
{
    ...
    if ((hService = OpenService(hScManager, SERVICE_TO_WAIT_FOR, SERVICE_QUERY_STATUS)) == (SC_HANDLE)NULL)
    {
...
OpenCreateKey(IN HANDLE Root, IN PWSTR Path, IN BOOL WriteAccess)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, &KeyName, OBJ_CASE_INSENSITIVE, Root, NULL);
    Status = NtOpenKey(&Key,
                       WriteAccess ? (STANDARD_RIGHTS_WRITE | KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS | KEY_SET_VALUE |
                                      KEY_CREATE_SUB_KEY)
                                   : GENERIC_READ,
                       &ObjectAttributes);
    if (NT_SUCCESS(Status))
