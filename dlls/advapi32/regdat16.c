...
REG16_PATH_SUBST Reg16PathSubstitutes[] = { { 10, "mciole.dll", "mciole16.dll" },
...
BOOL CreateRegistryClassesFromRegistry16(HANDLE SoftwareRoot, PREG_HEADER16 Registry)
{
    ...
    while (i != 0)
    {
        ...
        InitializeObjectAttributes(&ObjectAttributes, &KeyName, OBJ_CASE_INSENSITIVE,
                                   Level == 0 ? SoftwareRoot : State[Level - 1].KeyHandle, NULL);
        ...
        if (KeyNode->iValue != 0)
        {
            ...
            if (Dst != NULL)
            {
                ...
                *Dst = '\0';
                ...
                Status = NtSetValueKey(State[Level].KeyHandle, &ValueName, 0, REG_SZ, KeyValue.Buffer,
...
BOOL DumpRegistry16(PREG_HEADER16 Registry)
{
    ...
    while (i != 0)
    {
        ...
        DbgPrint("%.*s%.*s\n", Level * 4, Blanks, KeyNameString->cb, KeyName);
        if (KeyNode->iValue != 0)
        {
            KeyValueString = &NodeTable[KeyNode->iValue].str;
            KeyValue = StringTable + KeyValueString->irgb;
            DbgPrint("%.*s= %.*s\n", (Level + 1) * 4, Blanks, KeyValueString->cb, KeyValue);
        }
