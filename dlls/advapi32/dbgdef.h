...
#define __DBG_SS_TAGS                                             \
    {                                                             \
        DBG_SS_CRYPT32, "CRYPT32", DBG_SS_TRUSTCOMMON, "PKITRST", \
        DBG_SS_TRUST,   "WINTRST", DBG_SS_TRUSTPROV,   "SOFTPUB", \
        DBG_SS_CATALOG, "MSCAT32", DBG_SS_SIP,         "MSSIP32", \
        DBG_SS_SIGNING, "MSSGN32", DBG_SS_OFFSIGN,     "OFFSIGN", \
        DBG_SS_APP,     "CONAPPL", DBG_SS_CATDBSVC,    "CATDBSV", \
        NULL,           NULL                                      \
    }
...
#define TRACE_ERROR_EX(id, name)                                            \
    name## : DBG_TRACE_EX((id, "(" #name ":%s,%d)\n", __FILE__, __LINE__)); \
    goto ERROR_RETURN_LABEL;

#define SET_ERROR_EX(id, name, err)                                                                \
    name## : SetLastError((DWORD)(err));                                                           \
...
#define TRACE_HRESULT_EX(id, name)                                                                    \
    name## : DBG_TRACE_EX((id, "%s, %d\n         " #name ": hr = 0x%x\n", __FILE__, __LINE__, (hr))); \
    goto ERROR_RETURN_LABEL;

#define SET_DWRESULT_EX(id, name, err)                                                     \
    name## : dwResult = (DWORD)(err);                                                      \
    DBG_TRACE_EX((id, "%s, %d\n         " #name ": hr = " #err "\n", __FILE__, __LINE__)); \
    goto ERROR_RETURN_LABEL;
