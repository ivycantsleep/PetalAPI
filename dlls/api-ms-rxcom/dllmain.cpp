...
typedef HINSTANCE(WINAPI api_ShellExecute32W)(HWND hwnd, LPCWSTR lpOperation, LPCWSTR lpFile, LPCWSTR lpParameters,
                                              LPCWSTR lpDirectory, INT nShowCmd);

typedef HINSTANCE(WINAPI api_ShellExecute32A)(HWND hwnd, LPCSTR lpOperation, LPCSTR lpFile, LPCSTR lpParameters,
...
public:
    STDMETHODIMP QueryInterface(REFIID riid, void **ppv)
    {
        if (NULL == ppv)
            return E_POINTER;
        if ((IID_IUnknown == riid) || (__uuidof(IAgileObject) == riid))
        {
            AddRef();
    ...
    STDMETHODIMP_(HRESULT) IgnoreAppUriHandlers(boolean *value)
    {
        return 0;
    };
    STDMETHODIMP_(HRESULT) IgnoreAppUriHandlers(boolean value)
...
public:
    ...
    STDMETHODIMP_(HRESULT) TreatAsUntrusted(boolean *value)
    {
        *value = false;
        return 0;
    };
    ...
    STDMETHODIMP_(HRESULT) DisplayApplicationPicker(boolean *value)
    {
        return 0;
    }
    STDMETHODIMP_(HRESULT) DisplayApplicationPicker(boolean value)
    ...
    STDMETHODIMP_(HRESULT) UI(void **value)
    {
        return RPC_S_NO_INTERFACES;
    } //Windows.System.LauncherUIOptions**
    STDMETHODIMP_(HRESULT) PreferredApplicationPackageFamilyName(HSTRING *value)
    {
        return RPC_S_NO_INTERFACES;
    }
    STDMETHODIMP_(HRESULT) PreferredApplicationPackageFamilyName(HSTRING value)
    ...
    STDMETHODIMP_(HRESULT) PreferredApplicationDisplayName(HSTRING *value)
    {
        return RPC_S_NO_INTERFACES;
    }
    STDMETHODIMP_(HRESULT) PreferredApplicationDisplayName(HSTRING value)
    ...
    STDMETHODIMP_(HRESULT) FallbackUri(void *value)
    {
        return RPC_S_NO_INTERFACES;
    ...
    STDMETHODIMP_(HRESULT) ContentType(HSTRING *value)
    {
        return RPC_S_NO_INTERFACES;
    }
    STDMETHODIMP_(HRESULT) ContentType(HSTRING value)
...
public:
    ...
    STDMETHODIMP QueryInterface(REFIID riid, void **ppv)
    {
        if (NULL == ppv)
            return E_POINTER;
        if ((IID_IUnknown == riid) || (__uuidof(IUriRuntimeClassFactory) == riid) || (__uuidof(IInspectable) == riid))
        {
            AddRef();
            *ppv = reinterpret_cast<LPVOID>(this);
            return NOERROR;
...
extern "C++"
{
    ...
    public:
        ...
        virtual HRESULT STDMETHODCALLTYPE LaunchFileAsync(
            void *file,
            boolean **operation) = 0; //Windows.Storage.IStorageFile* // Windows.Foundation.IAsyncOperation<boolean>**
        virtual HRESULT STDMETHODCALLTYPE LaunchFileWithOptionsAsync(void *file, void *options,
                                                                     boolean **operation) = 0;
...
public:
    ...
    STDMETHODIMP_(HRESULT) LaunchFileAsync(void *file, boolean **operation)
    {
        return 8;
    };

    STDMETHODIMP_(HRESULT) LaunchFileWithOptionsAsync(void *file, void *options, boolean **operation)
    {
        return 9;
    };

    STDMETHODIMP_(HRESULT) LaunchUriAsync(void *uri, boolean **operation)
    {
        fbridge1.CreateUri((HSTRING)uri, (void **)operation);
        return 10;
    };

    STDMETHODIMP_(HRESULT) LaunchUriWithOptionsAsync(void *uri, void *options, boolean **operation)
    {
        return 100;
    };
};
...
extern "C"
{
    ...
    //#if defined(TRVIEW)
    EXPORT HRESULT WINAPI RoGetActivationFactory(HSTRING *activatableClassId, REFIID iid, void **factory)
    {
        //IActivationFactory
