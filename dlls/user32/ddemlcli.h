...
#define MONHSZ(pcii, hsz, type)                    \
    if ((pcii->MonitorFlags & MF_HSZ_INFO) && hsz) \
...
#define MONLINK(pcii, fEst, fNoD, aS, aT, aI, wFmt, fSvr, hConvS, hConvC) \
    ...
    MonitorLink(pcii, fEst, fNoD, aS, aT, aI, wFmt, fSvr, hConvS, hConvC)

#define MONCONV(pcoi, fConn)                                                                            \
    if (((pcoi)->pcii->MonitorFlags & MF_CONV) &&                                                       \
        ((((pcoi)->state & ST_ISLOCAL) && (pcoi)->state & ST_CLIENT) || !((pcoi)->state & ST_ISLOCAL))) \
...
#define CheckDDECritIn                                                \
    if (GetCurrentThreadId() != HandleToUlong(gcsDDEML.OwningThread)) \
    {                                                                 \
...
BOOL ValidateConnectParameters(HANDLE hInst, PCL_INSTANCE_INFO *ppcii, HSZ *phszService, HSZ hszTopic,
                               LATOM *plaNormalSvcName, PCONVCONTEXT *ppCC, HWND *phwndTarget, HCONVLIST hConvList);
PCL_CONV_INFO ConnectConv(PCL_INSTANCE_INFO pcii, LATOM laService, LATOM laTopic, HWND hwndTarget, HWND hwndSkip,
...
HDDEDATA InternalCreateDataHandle(PCL_INSTANCE_INFO pcii, LPBYTE pSrc, DWORD cb, DWORD cbOff, DWORD flags, WORD wStatus,
                                  WORD wFmt);
...
VOID MonitorLink(PCL_INSTANCE_INFO pcii, BOOL fEstablished, BOOL fNoData, LATOM laSvc, LATOM laTopic, GATOM gaItem,
