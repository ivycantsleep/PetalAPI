...
int SLCalcXOffsetSpecial(PED ped, HDC hdc, ICH ich)
{
     ...
     */
    pText = ECLock(ped);
    cch = ECCchInWidth(ped, hdc, (LPSTR)(pText + ichStart * ped->cbChar), ped->cch - ichStart,
    ...
    cx = ped->rcFmt.right - ped->rcFmt.left - SLCalcStringWidth(ped, hdc, ichStart, cch);
...
void SLSetCaretPosition(PED ped, HDC hdc)
{
    ...
    if (ped->pLpkEditCallout)
    {
        ...
        xPosition = min(xPosition, ped->rcFmt.right - 1 - ((ped->cxSysCharWidth > ped->aveCharWidth) ? 1 : 2));
    }
    else
    {
        xPosition = min(xPosition, ped->rcFmt.right - ((ped->cxSysCharWidth > ped->aveCharWidth) ? 1 : 2));
    }
...
int SLIchToLeftXPos(PED ped, HDC hdc, ICH ich)
{
    ...
    if (ped->fAnsi)
    {
        if (cchDiff >= 0)
        {
            ...
            GetTextExtentPointA(hdc, (LPSTR)(pText + ped->ichScreenStart), cchDiff, &size);
            textExtent = size.cx;
    ...
    { //!fAnsi
        if (cchDiff >= 0)
        {
            ...
            GetTextExtentPointW(hdc, (LPWSTR)(pText + ped->ichScreenStart * sizeof(WCHAR)), cchDiff, &size);
            textExtent = size.cx;
...
void SLChangeSelection(PED ped, HDC hdc, ICH ichNewMinSel, ICH ichNewMaxSel)
{
    ...
    if (_IsWindowVisible(ped->pwnd) && (ped->fFocus || ped->fNoHideSel))
    {
        ...
        if (ped->pLpkEditCallout)
        {
            ...
            pText = ECLock(ped);
            ped->pLpkEditCallout->EditDrawText(ped, hdc, pText, ped->cch, ped->ichMinSel, ped->ichMaxSel,
                                               ped->rcFmt.top);
            ECUnlock(ped);
        }
        else
        {
            ...
            if (ECCalcChangeSelection(ped, ichOldMinSel, ichOldMaxSel, (LPBLOCK)&Blk[0], (LPBLOCK)&Blk[1]))
            {
                ...
                for (i = 0; i < 2; i++)
                {
                    ...
                    {
                        SLGetClipRect(ped, hdc, Blk[i].StPos, Blk[i].EndPos - Blk[i].StPos, (LPRECT)&rc);
                        SLDrawLine(ped, hdc, rc.left, rc.right, Blk[i].StPos, Blk[i].EndPos - Blk[i].StPos,
                                   ((Blk[i].StPos >= ped->ichMinSel) && (Blk[i].StPos < ped->ichMaxSel)));
                    }
                }
            }
        }
...
void SLDrawLine(PED ped, HDC hdc, int xClipStPos, int xClipEndPos, ICH ichStart, int iCount, BOOL fSelStatus)
{
    ...
    if (ped->fAnsi && ped->fDBCS)
    {
        ...
        if (ichStart > 0)
        {
            ...
            while (ichNewStart && (ichStart - ichNewStart < ped->wMaxNegCcharPos))
            {
        ...
        if (ichStart + iCount < ped->cch)
        {
            ...
            while ((iTCount < (int)ped->wMaxNegAcharPos) && (ichStart + iCount + iTCount < ped->cch))
            {
    ...
    if (ped->charPasswordChar)
    {
        ...
        {
            if (ped->fAnsi)
                ExtTextOutA(hdc, rc.left, rc.top, ETO_CLIPPED, &rcClip, (LPSTR)&ped->charPasswordChar, 1, NULL);
            else
                ExtTextOutW(hdc, rc.left, rc.top, ETO_CLIPPED, &rcClip, (LPWSTR)&ped->charPasswordChar, 1, NULL);
    ...
    {
        if (ped->fAnsi)
            ExtTextOutA(hdc, rc.left, rc.top, ETO_CLIPPED, &rcClip, pText + ichStart, iCount, NULL);
        else
            ExtTextOutW(hdc, rc.left, rc.top, ETO_CLIPPED, &rcClip, ((LPWSTR)pText) + ichStart, iCount, NULL);
    }
...
void SLDrawText(PED ped, HDC hdc, ICH ichStart)
{
     ...
     */
    cchToDraw = ECCchInWidth(ped, hdc, (LPSTR)(pText + ped->ichScreenStart * ped->cbChar),
    ...
    while (ichStart < ichEnd)
    {
        ...
        else
        {
            if (fDrawLeftMargin)
            {
                ...
                iCount = SLGetBlkEnd(ped, ichStart, ichEnd, (BOOL *)&fSelStatus);
        }
    ...
    if (fDrawEndOfLineStrip && (rc.left < (rc.right = (ped->rcFmt.right + ped->wRightMargin))))
    {
        SLDrawLine(ped, hdc, rc.left, rc.right, ichStart, 0, (ichEnd < ped->ichMaxSel) ? TRUE : FALSE);
    }
...
BOOL SLScrollText(PED ped, HDC hdc)
{
    ...
    if (ped->ichCaret <= ped->ichScreenStart)
    {
        ...
        scrollAmount = ECCchInWidth(ped, hdc, (LPSTR)pTextScreenStart, ped->ichCaret,
                                    (ped->rcFmt.right - ped->rcFmt.left) / 4, FALSE);
    ...
    else if (ped->ichCaret != ped->ichScreenStart)
    {
        ...
        cch = ECCchInWidth(ped, hdc, (LPSTR)pTextScreenStart, ped->ichCaret - ped->ichScreenStart,
        ...
        if (cch < ped->ichCaret - ped->ichScreenStart)
        {
            ...
            cch = ECCchInWidth(ped, hdc, (LPSTR)pTextScreenStart, ped->cch - ped->ichScreenStart,
                               ped->rcFmt.right - ped->rcFmt.left, FALSE);
        ...
        else if (ped->format != ES_LEFT)
        {
            ...
            */
            if (ped->ichScreenStart == ped->cch - cch)
            {
                ...
                pTextScreenStart -= ped->ichScreenStart * ped->cbChar;
                cch = ECCchInWidth(ped, hdc, (LPSTR)pTextScreenStart, ped->cch, ped->rcFmt.right - ped->rcFmt.left,
                                   FALSE);
    ...
    if (ped->fAnsi && ped->fDBCS)
    {
        newScreenStartX = (fAdjustNext ? ECAdjustIchNext : ECAdjustIch)(ped, ECLock(ped), newScreenStartX);
        ECUnlock(ped);
    }
...
ICH SLInsertText(PED ped, LPSTR lpText, ICH cchInsert)
{
    ...
    else
    {
        if (!ped->fAutoHScroll)
        {
            ...
            cchT = ECCchInWidth(ped, hdc, lpText, cchInsert, ped->rcFmt.right - ped->rcFmt.left - textWidth, TRUE);
...
void SLChar(PED ped, DWORD keyValue)
{
    ...
    default:
        ...
        { // 1E,1F are unicode block and segment separators
            ...
            {
                if ((DBCSkey = DbcsCombine(ped->hwnd, keyPress)) != 0 && SLInsertText(ped, (LPSTR)&DBCSkey, 2) == 2)
                {
                    InsertTextLen = 2;
                    updateText = TRUE;
                }
...
ICH SLMouseToIch(PED ped, HDC hdc, LPPOINT mousePt)
{
    ...
    if (width > ped->rcFmt.right)
    {
         ...
         */
        cch = ECCchInWidth(ped, hdc, (LPSTR)(pText + ped->ichScreenStart * ped->cbChar), ped->cch - ped->ichScreenStart,
                           ped->rcFmt.right - ped->rcFmt.left, TRUE) +
              ped->ichScreenStart;
     ...
     */
    if (ped->charPasswordChar)
        return min((DWORD)((width - ped->rcFmt.left) / ped->cPasswordCharWidth), ped->cch);
    ...
    pGetTextExtentPoint =
        ped->fAnsi ? (FnGetTextExtentPoint)GetTextExtentPointA : (FnGetTextExtentPoint)GetTextExtentPointW;
...
void SLMouseMotion(PED ped, UINT message, UINT virtKeyDown, LPPOINT mousePt)
{
    ...
    case WM_LBUTTONDBLCLK:
        ...
        if (ped->fAnsi && ped->fDBCS)
        {
            ...
            ECWord(ped, mouseIch, (ECIsDBCSLeadByte(ped, *pText) && mouseIch < ped->cch) ? FALSE : TRUE, &selectionl,
                   &selectionh);
            ECUnlock(ped);
        }
        else
...
LRESULT SLEditWndProc(HWND hwnd, PED ped, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_GETDLGCODE:
    {
        ...
        if (lParam)
        {
            ...
            {
            case WM_SYSCHAR:
                if ((HIWORD(((LPMSG)lParam)->lParam) & SYS_ALTERNATE) && ((WORD)wParam == VK_BACK))
                {
            ...
            case WM_KEYDOWN:
                if (((((WORD)wParam == VK_RETURN) || ((WORD)wParam == VK_ESCAPE)) && (ped->listboxHwnd) &&
