...
BOOL IsMetaFile(HDC hdc)
{
    ...
    return (dwType == OBJ_METAFILE || dwType == OBJ_METADC || dwType == OBJ_ENHMETAFILE || dwType == OBJ_ENHMETADC);
}
...
FUNCLOG6(LOG_GENERAL, int, DUMMYCALLINGTYPE, DrawTextExA, HDC, hdc, LPSTR, lpchText, int, cchText, LPRECT, lprc, UINT,
         format, LPDRAWTEXTPARAMS, lpdtp)
...
FUNCLOG5(LOG_GENERAL, int, DUMMYCALLINGTYPE, DrawTextW, HDC, hdc, LPCWSTR, lpchText, int, cchText, LPRECT, lprc, UINT,
         format)
...
FUNCLOG5(LOG_GENERAL, int, DUMMYCALLINGTYPE, DrawTextA, HDC, hdc, LPCSTR, lpchText, int, cchText, LPRECT, lprc, UINT,
         format)
                ...
                int iTabOrigin, BOOL fDrawTheText, int iCharset)
{
    ...
    return (*fpLpkTabbedTextOut)(hdc, x, y, lpstring, nCount, nTabPositions, lpTabPositions, iTabOrigin, fDrawTheText,
                          ...
                          int cyCharHeight, int iCharset)
{
    ...
    while (TRUE)
    {
        ...
        if (fDrawTheText)
        {
            ...
            ExtTextOutW(hdc, x, y, uOpaque, &rc, (LPWSTR)lpstring, cch, NULL);
            rc.left = rc.right;
        }
...
LONG TabbedTextOutW(HDC hdc, int x, int y, LPCWSTR lpstring, int cchChars, int nTabPositions,
                    CONST INT *lpintTabStopPositions, int iTabOrigin)
{
    return TabTextOut(hdc, x, y, lpstring, cchChars, nTabPositions, lpintTabStopPositions, iTabOrigin, TRUE, -1);
}
...
LONG TabbedTextOutA(HDC hdc, int x, int y, LPCSTR pString, int chCount, int nTabPositions,
                    CONST INT *pnTabStopPositions, int nTabOrigin)
{
    LPWSTR lpwstr;
    BOOL bRet;
    WORD wCodePage = (WORD)GdiGetCodePage(hdc);
    ...
    bRet = TabTextOut(hdc, x, y, lpwstr, iUniString, nTabPositions, pnTabStopPositions, nTabOrigin, TRUE,
                      GetTextCharset(hdc));
...
{
    return TabTextOut(hdc, 0, 0, pString, chCount, nTabPositions, pnTabStopPositions, 0, FALSE, -1);
}
