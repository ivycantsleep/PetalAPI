...
HRESULT mndtDragOver(LPDROPTARGET pdt, DWORD grfKeyState, POINTL ptl, LPDWORD pdwEffect)
{
    ...
    else
    {
        ...
        if (PMNIDT(pdt)->pidt != NULL)
        {
            ...
            return PMNIDT(pdt)->pidt->lpVtbl->DragEnter(PMNIDT(pdt)->pidt, PMNIDT(pdt)->pido, grfKeyState, ptl,
                                                        pdwEffect);
        }
    } /* if (!(mndoi.dwFlags & MNGOF_CROSSBOUNDARY)) */
...
*
\**************************************************************************/
IDropTargetVtbl idtVtbl = { mndtQueryInterface, mndtAddRef,    mndtRelease, mndtDragEnter,
                            mndtDragOver,       mndtDragLeave, mndtDrop };
/**************************************************************************\
...
NTSTATUS LoadOLEOnce(void)
{
     ...
     */
    GETPROCINFO gpi[] = { { &((FARPROC)pfnOLEOleInitialize), (LPCSTR) "OleInitialize" },
    ...
    while (pgpi->ppfn != NULL)
    {
        *(pgpi->ppfn) = GetProcAddress(ghinstOLE, pgpi->lpsz);
        if (*(pgpi->ppfn) == NULL)
        {
            RIPMSG2(RIP_WARNING, "LoadOLEOnce: GetProcAddress failed: '%s': %#lx", pgpi->lpsz, GetLastError());
            break;
        }
        pgpi++;
    }
