...
BOOL AddLink(PCONV_INFO pcoi, GATOM gaItem, WORD wFmt, WORD wType)
{
    ...
    {
        aLinkNew = (PADVISE_LINK)DDEMLReAlloc(pcoi->aLinks, sizeof(ADVISE_LINK) * (pcoi->cLinks + 1));
    }
    if (aLinkNew == NULL)
    {
    ...
    if (!(pcoi->state & ST_CLIENT))
    {
        ...
        for (aLinkNew->pLinkCount = pcii->pLinkCount; aLinkNew->pLinkCount;
             aLinkNew->pLinkCount = aLinkNew->pLinkCount->next)
        {
            if (aLinkNew->pLinkCount->laTopic == pcoi->laTopic && aLinkNew->pLinkCount->gaItem == gaItem &&
...
{
    if (!OpenThreadToken(GetCurrentThread(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, TRUE, pTokenHandle))
    {
...
BOOL GetUserSid(PTOKEN_USER *ppTokenUser)
{
    ...
    bRet = GetTokenInformation(TokenHandle, TokenUser, pTokenUser, cbTokenUser, &cbNeeded);
