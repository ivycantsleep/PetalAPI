...
BOOL WINAPI SetSysColors(int cElements, CONST INT *lpaElements, CONST COLORREF *lpaRgbValues)
{
    ...
    return NtUserSetSysColors(cElements, lpaElements, lpaRgbValues,
...
BOOL UpdatePerUserSystemParameters(HANDLE hToken, DWORD dwFlags)
{
...
BOOL InitTask(UINT wVersion, DWORD dwAppCompatFlags, DWORD dwUserWOWCompatFlags, LPCSTR pszModName,
              LPCSTR pszBaseFileName, DWORD hTaskWow, DWORD dwHotkey, DWORD idTask, DWORD dwX, DWORD dwY, DWORD dwXSize,
              DWORD dwYSize)
{
    ...
    Status = NtUserInitTask(wVersion, dwAppCompatFlags, dwUserWOWCompatFlags, strModName.pstr, strBaseFileName.pstr,
                            hTaskWow, dwHotkey, idTask, dwX, dwY, dwXSize, dwYSize);
...
HWINEVENTHOOK
WINAPI
SetWinEventHook(DWORD eventMin, DWORD eventMax,
                ...
                DWORD dwFlags)
{
    ...
    retval = (ULONG_PTR)NtUserSetWinEventHook(eventMin, eventMax, hmodWinEventProc, pstr, lpfnWinEventProc, idProcess,
                                              idThread, dwFlags);
...
WINUSERAPI
VOID WINAPI NotifyWinEvent(DWORD dwEvent, HWND hwnd, LONG idObject, LONG idChild)
{
    BEGINCALLVOID()
...
BOOL RegisterMessagePumpHook(INITMESSAGEPUMPHOOK pfnInitMPH)
{
    BOOL fInit = FALSE;
    ...
    if (mii.fMask & MIIM_BITMAP)
    {
        if (((HBITMAP)LOWORD(HandleToUlong(mii.hbmpItem)) < HBMMENU_MAX) && IS_PTR(mii.hbmpItem))
        {
            /*
                 ...
                 */
            RIPMSG1(RIP_WARNING, "Invalid HIWORD data (0x%04X) for HBMMENU_* bitmap.",
                    HIWORD(HandleToUlong(mii.hbmpItem)));
...
\***************************************************************************/
FUNCLOG4(LOG_GENERAL, BOOL, DUMMYCALLINGTYPE, DrawCaption, HWND, hwnd, HDC, hdc, CONST RECT *, lprc, UINT, flags)
...
LONG_PTR _SetWindowLongPtr(HWND hwnd, int nIndex, LONG_PTR dwNewLong, BOOL bAnsi)
{
    ...
    if (nIndex == GWL_STYLE && GETFNID(pwnd) == FNID_LISTBOX && ((PLBWND)pwnd)->pLBIV != NULL &&
    ...
    default:
        if (pmsg->message & RESERVED_MSG_BITS)
        {
            RIPERR1(ERROR_INVALID_PARAMETER, RIP_WARNING,
                    "Invalid parameter \"pmsg->message\" (%ld) to TranslateMessageEx", pmsg->message);
        }
        MSGERROR();
    }
...
int ToUnicodeEx(UINT wVirtKey, UINT wScanCode, CONST BYTE *pKeyState, LPWSTR pwszBuff, int cchBuff, UINT wFlags,
                HKL hkl)
{
    BEGINCALL()
...
DWORD DbgWin32HeapStat(PDBGHEAPSTAT phs, DWORD dwLen, DWORD dwFlags)
{
    if ((dwFlags | WHF_VALID) != WHF_VALID)
    {
        RIPMSG1(RIP_WARNING, "Invalid flags for DbgWin32HeapFail %x", dwFlags);
...
FUNCLOG4(LOG_GENERAL, BOOL, DUMMYCALLINGTYPE, SetWindowStationUser, HWINSTA, hwinsta, PLUID, pluidUser, PSID, psidUser,
         DWORD, cbsidUser)
                     ...
                     int nHeight, HWND hwndParent, HMENU hmenu, HANDLE hModule, LPVOID pParam, DWORD dwFlags)
{
    ...
    }
    {
        ...
        retval = (ULONG_PTR)VerNtUserCreateWindowEx(dwExStyle, pstrClassName, pstrWindowName, dwStyle, x, y, nWidth,
                                                    nHeight, hwndParent, hmenu, hModule, pParam, dwExpWinVerAndFlags);

        // If this is an MDI child, we need to do some more to complete the
        // process of creating an MDI child.
        if (retval && fMDIchild)
        {
            MDICompleteChildCreation((HWND)retval, hSysMenu, ((dwStyle & WS_VISIBLE) != 0L),
                                     (BOOL)((dwStyle & WS_DISABLED) != 0L));
        }
...
HKL _LoadKeyboardLayoutEx(HANDLE hFile, UINT offTable, PKBDTABLE_MULTI_INTERNAL pKbdTableMulti, HKL hkl, LPCTSTR pwszKL,
                          UINT KbdInputLocale, UINT Flags)
{
    ...
    retval =
        (ULONG_PTR)NtUserLoadKeyboardLayoutEx(hFile, offTable, pKbdTableMulti, hkl, strKL.pstr, KbdInputLocale, Flags);
...
FUNCLOGVOID5(LOG_GENERAL, DUMMYCALLINGTYPE, mouse_event, DWORD, dwFlags, DWORD, dx, DWORD, dy, DWORD, dwData, ULONG_PTR,
             dwExtraInfo)
...
MESSAGECALL(fnCOPYGLOBALDATA)
{
    ...
    retval = NtUserMessageCall(hwnd, msg, wParam, (LPARAM)pData, xParam, xpfnProc, bAnsi);
...
MESSAGECALL(fnINDEVICECHANGE)
{
    ...
    case DBT_DEVTYP_HANDLE:
        ...
        bAnsi = FALSE;
        if ((wParam != DBT_CUSTOMEVENT) || (pHandleA->dbch_nameoffset < 0))
            break;
        ...
        if (pHandleW->dbch_nameoffset & 1)
            pHandleW->dbch_nameoffset++;

        lpStr = (LPWSTR)(pHandleW->dbch_data + pHandleW->dbch_nameoffset);
        if (iStr)
        {
            MBToWCS(pHandleA->dbch_data + pHandleA->dbch_nameoffset, -1, &lpStr, iStr, FALSE);
        }
        lpStr[iStr] = 0;
...
shipit:
    ...
    if (pPortW)
        UserLocalFree(pPortW);
    if (pInterfaceW)
        UserLocalFree(pInterfaceW);
    if (pHandleW)
        UserLocalFree(pHandleW);
...
FUNCLOG4(LOG_GENERAL, DWORD, WINAPI, ImmReconversionWorker, LPRECONVERTSTRING, lpRecTo, LPRECONVERTSTRING, lpRecFrom,
         BOOL, bToAnsi, DWORD, dwCodePage)
DWORD WINAPI ImmReconversionWorker(LPRECONVERTSTRING lpRecTo, LPRECONVERTSTRING lpRecFrom, BOOL bToAnsi,
                                   DWORD dwCodePage)
{
    INT i;
    DWORD dwSize = 0;
    ...
    if (bToAnsi)
    {
        ...
        i = WideCharToMultiByte(dwCodePage, (DWORD)0,
                                (LPWSTR)((LPSTR)lpRecFrom + lpRecFrom->dwStrOffset), // src
                                (INT)lpRecFrom->dwStrLen,
                                (LPSTR)lpRecTo + lpRecTo->dwStrOffset, // dest
                                (INT)lpRecFrom->dwStrLen * DBCS_CHARSIZE, (LPSTR)NULL, (LPBOOL)NULL);
        lpRecTo->dwCompStrOffset =
            CalcCharacterPositionWtoA(lpRecFrom->dwCompStrOffset / sizeof(WCHAR),
                                      (LPWSTR)((LPBYTE)lpRecFrom + lpRecFrom->dwStrOffset), dwCodePage) *
            sizeof(CHAR);

        lpRecTo->dwCompStrLen =
            (CalcCharacterPositionWtoA(lpRecFrom->dwCompStrOffset / sizeof(WCHAR) + lpRecFrom->dwCompStrLen,
                                       (LPWSTR)((LPBYTE)lpRecFrom + lpRecFrom->dwStrOffset), dwCodePage) *
             sizeof(CHAR)) -
            lpRecTo->dwCompStrOffset;
        ...
        lpRecTo->dwTargetStrLen =
            (CalcCharacterPositionWtoA(lpRecFrom->dwTargetStrOffset / sizeof(WCHAR) + lpRecFrom->dwTargetStrLen,
                                       (LPWSTR)((LPBYTE)lpRecFrom + lpRecFrom->dwStrOffset), dwCodePage) *
             sizeof(CHAR)) -
            lpRecTo->dwTargetStrOffset;
    ...
    else
    {
        ...
        lpRecTo->dwCompStrOffset = CalcCharacterPositionAtoW(lpRecFrom->dwCompStrOffset,
                                                             (LPSTR)lpRecFrom + lpRecFrom->dwStrOffset, dwCodePage) *
                                   sizeof(WCHAR);

        lpRecTo->dwCompStrLen = ((CalcCharacterPositionAtoW(lpRecFrom->dwCompStrOffset + lpRecFrom->dwCompStrLen,
                                                            (LPSTR)lpRecFrom + lpRecFrom->dwStrOffset, dwCodePage) *
                                  sizeof(WCHAR)) -
                                 lpRecTo->dwCompStrOffset) /
                                sizeof(WCHAR);

        lpRecTo->dwTargetStrOffset = CalcCharacterPositionAtoW(lpRecFrom->dwTargetStrOffset,
                                                               (LPSTR)lpRecFrom + lpRecFrom->dwStrOffset, dwCodePage) *
                                     sizeof(WCHAR);

        lpRecTo->dwTargetStrLen = ((CalcCharacterPositionAtoW(lpRecFrom->dwTargetStrOffset + lpRecFrom->dwTargetStrLen,
                                                              (LPSTR)lpRecFrom + lpRecFrom->dwStrOffset, dwCodePage) *
                                    sizeof(WCHAR)) -
                                   lpRecTo->dwTargetStrOffset) /
                                  sizeof(WCHAR);
    ...
    if (bAnsi)
    {
        ...
        case IMR_DOCUMENTFEED:
            ...
            if (retval)
            {
                ...
                {
                    RIPMSG2(RIP_WARNING, "WM_IME_REQUEST(%x): return value from application %d is invalid.", wParam,
                            retval);
                    retval = 0;
                }
                else if (lParam)
    ...
    if (bAnsi != ((TestWF(pwnd, WFANSIPROC)) ? TRUE : FALSE))
    {
        ...
        if (cchTextLength)
        {
            ...
            if (pvString)
            {
                ...
                retval = (DWORD)NtUserMessageCall(hwnd, WM_GETTEXT, cchTextLength, (LPARAM)pvString, xParam, xpfnProc,
                                                  bAnsi);
...
MESSAGECALL(fnEMSETSEL)
{
    ...
    if (bAnsi != ((TestWF(pwnd, WFANSIPROC)) ? TRUE : FALSE))
    {
        ...
        else
        {
            ...
            cchTextLength =
                (DWORD)NtUserMessageCall(hwnd, WM_GETTEXTLENGTH, (WPARAM)0, (LPARAM)0, xParam, xpfnProc, bAnsi);
            ...
            if (cchTextLength)
            {
                ...
                if (pvString)
                {
                    ...
                    retval = (DWORD)NtUserMessageCall(hwnd, WM_GETTEXT, cchTextLength, (LPARAM)pvString, xParam,
                                                      xpfnProc, bAnsi);
...
LONG BroadcastSystemMessageWorker(DWORD dwFlags, LPDWORD lpdwRecipients, UINT message, WPARAM wParam, LPARAM lParam,
                                  PBSMINFO pBSMInfo, BOOL fAnsi)
{
    ...
    if (dwFlags & BSF_ASYNC)
    {
        ...
        {
            RIPERR0(ERROR_MESSAGE_SYNC_ONLY, RIP_WARNING,
                    "BroadcastSystemMessage: Can't post messages with pointers\n");
            dwFlags &= ~BSF_ASYNC; // Strip the BSF_ASYNC flags.
        }
    }
    ...
    if (dwRecipients & BSM_APPLICATIONS)
    {
        ...
        lret = (LONG)CsSendMessage(GetDesktopWindow(), message, wParam, lParam, (ULONG_PTR)&bsmParams,
                                   FNID_SENDMESSAGEBSM, fAnsi);
        //
...
HDEVNOTIFY
RegisterDeviceNotificationWorker(IN HANDLE hRecipient, IN LPVOID NotificationFilter, IN DWORD Flags, IN BOOL IsAnsi)
{
    HINSTANCE hLib = NULL;
    ...
    extern CONFIGRET CMP_RegisterNotification(IN HANDLE hRecipient, IN LPBYTE NotificationFilter, IN DWORD Flags,
                                              OUT PVOID * Context);
