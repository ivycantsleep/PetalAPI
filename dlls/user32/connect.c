...
BOOL ValidateConvList(HCONVLIST hConvList)
{
    PCONVLIST pcl;
    PCL_CONV_INFO pci;
    PXACT_INFO pxi;
    ...
    pcl = (PCONVLIST)ValidateCHandle((HANDLE)hConvList, HTYPE_CONVERSATION_LIST, HINST_ANY);
    for (i = 0; i < pcl->chwnd; i++)
    {
        ...
        */
        ...
        while (pci != NULL)
        {
             ...
             */
            if (pci->hConvList != hConvList && TypeFromHandle(pci->ci.hConv) != HTYPE_ZOMBIE_CONVERSATION)
            {
                DebugBreak();
            }
            /*
...
{
    ApplyFunctionToObjects(HTYPE_CONVERSATION_LIST, HINST_ANY, (PFNHANDLEAPPLY)ValidateConvList);
}
...
CONVCONTEXT DefConvContext = { sizeof(CONVCONTEXT),
                               ...
                               0L,
                               0L,
                               { sizeof(SECURITY_QUALITY_OF_SERVICE), SecurityImpersonation, SECURITY_STATIC_TRACKING,
                                 TRUE } };
...
FUNCLOG4(LOG_GENERAL, HCONV, DUMMYCALLINGTYPE, DdeConnect, DWORD, idInst, HSZ, hszService, HSZ, hszTopic, PCONVCONTEXT,
         pCC)
HCONV DdeConnect(DWORD idInst, HSZ hszService, HSZ hszTopic, PCONVCONTEXT pCC)
{
    ...
    if (!ValidateConnectParameters((HANDLE)LongToHandle(idInst), &pcii, &hszService, hszTopic, &aNormalSvcName, &pCC,
                                   &hwndTarget, 0))
    {
        goto Exit;
    }
    pci =
        ConnectConv(pcii, LATOM_FROM_HSZ(hszService), LATOM_FROM_HSZ(hszTopic), hwndTarget,
                    (pcii->afCmd & CBF_FAIL_SELFCONNECTIONS) ? pcii->hwndMother : 0, pCC, 0, CLST_SINGLE_INITIALIZING);
    if (pci == NULL)
    {
...
FUNCLOG5(LOG_GENERAL, HCONVLIST, DUMMYCALLINGTYPE, DdeConnectList, DWORD, idInst, HSZ, hszService, HSZ, hszTopic,
...
HCONVLIST DdeConnectList(DWORD idInst, HSZ hszService, HSZ hszTopic, HCONVLIST hConvList, PCONVCONTEXT pCC)
{
    ...
    pcoi = (PCONV_INFO)ConnectConv(pcii, LATOM_FROM_HSZ(hszService), LATOM_FROM_HSZ(hszTopic), hwndTarget,
                                   (pcii->afCmd & (CBF_FAIL_SELFCONNECTIONS | CBF_FAIL_CONNECTIONS)) ? pcii->hwndMother
                                                                                                     : 0,
                                   pCC, hConvListOld, CLST_MULT_INITIALIZING);
    ...
    if (hConvListOld == 0)
    {
        ...
        hConvList = (HCONVLIST)CreateHandle((ULONG_PTR)pcl, HTYPE_CONVERSATION_LIST, InstFromHandle(pcii->hInstClient));
        if (hConvList == 0)
        {
            DDEMLFree(pcl);
            SetLastDDEMLError(pcii, DMLERR_MEMORY_ERROR);
    ...
    if (hConvListOld)
    {
        ...
        for (i = 0; i < pcl->chwnd; i++)
        {
            ...
            for (pcoiExisting = (PCONV_INFO)GetWindowLongPtr(pcl->ahwnd[i], GWLP_PCI);
                 pcoi != NULL && pcoiExisting != NULL; pcoiExisting = pcoiExisting->next)
            {
                ...
                for (pcoiNew = pcoi; pcoiNew != NULL; pcoiNew = pcoiNext)
                {
                    ...
                    if (((PCL_CONV_INFO)pcoiExisting)->hwndReconnect == ((PCL_CONV_INFO)pcoiNew)->hwndReconnect &&
                        pcoiExisting->laTopic == pcoiNew->laTopic && pcoiExisting->laService == pcoiNew->laService)
                    {
                        /*
...
HCONV DdeReconnect(HCONV hConv)
{
    ...
    pciNew =
        ConnectConv(pcii, pci->ci.laService, pci->ci.laTopic, pci->hwndReconnect, 0, &cc, 0, CLST_SINGLE_INITIALIZING);
    if (pciNew == NULL)
    {
        SetLastDDEMLError(pcii, DMLERR_NO_CONV_ESTABLISHED);
        goto Exit;
                               ...
                               HCONVLIST hConvList)
{
    ...
    *ppcii = ValidateInstance(hInst);
    ...
    {
        *phwndTarget = ParseInstSpecificAtom(LATOM_FROM_HSZ(*phszService), plaNormalSvcName);
    ...
    if (hConvList != 0 &&
        !ValidateCHandle((HANDLE)hConvList, HTYPE_CONVERSATION_LIST, (DWORD)InstFromHandle((*ppcii)->hInstClient)))
    {
        return (FALSE);
    }
    return (TRUE);
...
* 11-1-91 sanfords Created.
\***************************************************************************/
PCL_CONV_INFO ConnectConv(PCL_INSTANCE_INFO pcii, LATOM laService, LATOM laTopic,
                          ...
                          PCONVCONTEXT pCC, HCONVLIST hConvList, DWORD clst)
{
    ...
    {
        ie.hwndClient = CreateWindowW((LPWSTR)(gpsi->atomSysClass[ICLS_DDEMLCLIENTW]), L"", WS_CHILD, 0, 0, 0, 0,
                                      pcii->hwndMother, (HMENU)0, (HANDLE)0, (LPVOID)NULL);
    }
    else
    {
        ie.hwndClient = CreateWindowA((LPSTR)(gpsi->atomSysClass[ICLS_DDEMLCLIENTA]), "", WS_CHILD, 0, 0, 0, 0,
                                      pcii->hwndMother, (HMENU)0, (HANDLE)0, (LPVOID)NULL);
    }
    ...
    if (hwndTarget)
    {
        SendMessage(hwndTarget, WM_DDE_INITIATE, (WPARAM)ie.hwndClient, ie.lParam);
    }
    else
    {
        /*
        ...
        if (hwndAgent)
        {
            SendMessage(hwndAgent, WM_DDE_INITIATE, (WPARAM)ie.hwndClient, ie.lParam);
        }
...
BOOL InitiateEnumerationProc(HWND hwndTarget, PINIT_ENUM pie)
{
    ...
    if (pie->hConvList && pie->laTopic && pie->laServiceRequested)
    {
         ...
         */
        pcl = (PCONVLIST)ValidateCHandle((HANDLE)pie->hConvList, HTYPE_CONVERSATION_LIST, HINST_ANY);
        if (pcl != NULL)
        {
            for (i = 0; i < pcl->chwnd; i++)
            {
                for (pcoiExisting = (PCONV_INFO)GetWindowLongPtr(pcl->ahwnd[i], GWLP_PCI); pcoiExisting != NULL;
                     pcoiExisting = pcoiExisting->next)
                {
                    if (pcoiExisting->state & ST_CONNECTED &&
                        ((PCL_CONV_INFO)pcoiExisting)->hwndReconnect == hwndTarget &&
                        pcoiExisting->laTopic == pie->laTopic && pcoiExisting->laService == pie->laServiceRequested)
                    {
                        LeaveDDECrit;
                        return (TRUE);
                    }
...
HCONV DdeQueryNextServer(HCONVLIST hConvList, HCONV hConvPrev)
{
    ...
    do
    {
        ...
        pci = (PCL_CONV_INFO)ValidateCHandle((HANDLE)hConvPrev, HTYPE_CLIENT_CONVERSATION, InstFromHandle(hConvList));
        if (pci == NULL)
        {
            pci =
                (PCL_CONV_INFO)ValidateCHandle((HANDLE)hConvPrev, HTYPE_ZOMBIE_CONVERSATION, InstFromHandle(hConvList));
            if (pci == NULL)
            {
                SetLastDDEMLError(pcii, DMLERR_INVALIDPARAMETER);
                break;
...
BOOL DdeDisconnect(HCONV hConv)
{
    ...
    pcoi = (PCONV_INFO)ValidateCHandle((HANDLE)hConv, HTYPE_CLIENT_CONVERSATION, HINST_ANY);
    if (pcoi == NULL)
    {
...
VOID ShutdownConversation(PCONV_INFO pcoi, BOOL fMakeCallback)
{
    ...
    if (pcoi->state & ST_CONNECTED)
    {
        ...
        {
            DoCallback(pcoi->pcii, (WORD)XTYP_DISCONNECT, 0, pcoi->hConv, 0, 0, 0, 0,
                       (pcoi->state & ST_ISSELF) ? 1L : 0L);
        }
        MONCONV(pcoi, FALSE);
    }
        ...
        { // SERVER
            ...
            for (i = 0; i < pcoi->pcii->cServerLookupAlloc; i++)
            {
                if (pcoi->pcii->aServerLookup[i].hwndServer == pcoi->hwndConv)
                {
                    ...
                    {
                        ORPATH(0x400);
                        pcoi->pcii->aServerLookup[i] = pcoi->pcii->aServerLookup[pcoi->pcii->cServerLookupAlloc];
                    }
                    else
                    {
...
VOID FreeConversationResources(PCONV_INFO pcoi)
{
    ...
    if (pcoi->state & ST_INTRA_PROCESS && !(pcoi->state & ST_TERMINATE_RECEIVED))
    {
        ...
        pcoi->hConv = (HCONV)CreateHandle((ULONG_PTR)pcoi, HTYPE_ZOMBIE_CONVERSATION, InstFromHandle(pcoi->hConv));
    ...
    while (pcoi->cLinks)
    {
        if (pcoi->state & ST_CLIENT)
        {
            MONLINK(pcoi->pcii, FALSE, paLink->wType & XTYPF_NODATA, pcoi->laService, pcoi->laTopic,
                    LocalToGlobalAtom(paLink->laItem), paLink->wFmt, FALSE, (HCONV)pcoi->hwndPartner,
                    (HCONV)pcoi->hwndConv);
        }
        else
        {
            MONLINK(pcoi->pcii, FALSE, paLink->wType & XTYPF_NODATA, pcoi->laService, pcoi->laTopic,
                    LocalToGlobalAtom(paLink->laItem), paLink->wFmt, TRUE, (HCONV)pcoi->hwndConv,
                    (HCONV)pcoi->hwndPartner);
        }
        if (!(pcoi->state & ST_CLIENT))
        {
...
BOOL WaitForZombieTerminate(HANDLE hData)
{
    ...
    fTerminated = FALSE;
    while ((pcoi = (PCONV_INFO)ValidateCHandle(hData, HTYPE_ZOMBIE_CONVERSATION, InstFromHandle(hData))) != NULL &&
