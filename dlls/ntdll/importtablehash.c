...
NTSTATUS
RtlComputeImportTableHash(IN HANDLE hFile, IN PCHAR Hash, IN ULONG ImportTableHashRevision)
/*++
...
--*/
{
    ...
    Status = NtCreateSection(&hMap, DesiredAccess, NULL, NULL, flProtect, AllocationAttributes, hFile);
    ...
    Status = NtMapViewOfSection(hMap, NtCurrentProcess(), &FileMapping, 0L, 0L, &SectionOffset, &ViewSize, ViewShare,
                                0L, PAGE_READONLY);
    ...
    ImportDescriptor = (PIMAGE_IMPORT_DESCRIPTOR)RtlImageDirectoryEntryToData(
        FileMapping, FALSE, IMAGE_DIRECTORY_ENTRY_IMPORT, &ImportDescriptorSize);
    ...
    while ((ImportDescriptor->Name != 0) && (ImportDescriptor->FirstThunk != 0))
    {
        ...
        PSZ ImportName =
            (PSZ)RtlAddressInSectionTable(RtlImageNtHeader(FileMapping), FileMapping, ImportDescriptor->Name);
        ...
        ListEntry = (PIMPORTTABLEP_SORTED_LIST_ENTRY)RtlAllocateHeap(RtlProcessHeap(), 0,
                                                                     sizeof(IMPORTTABLEP_SORTED_LIST_ENTRY));
        ...
        OriginalFirstThunk = (PIMAGE_THUNK_DATA)RtlAddressInSectionTable(RtlImageNtHeader(FileMapping), FileMapping,
        ...
        while (OriginalFirstThunk->u1.Ordinal)
        {
            ...
            if (!IMAGE_SNAP_BY_ORDINAL(OriginalFirstThunk->u1.Ordinal))
            {
                ...
                AddressOfData = (PIMAGE_IMPORT_BY_NAME)RtlAddressInSectionTable(
                    RtlImageNtHeader(FileMapping), FileMapping, (ULONG)OriginalFirstThunk->u1.AddressOfData);
                ...
                FunctionListEntry = (PIMPORTTABLEP_SORTED_FUNCTION_LIST_ENTRY)RtlAllocateHeap(
                    RtlProcessHeap(), 0, sizeof(IMPORTTABLEP_SORTED_FUNCTION_LIST_ENTRY));
...
VOID ImportTablepInsertFunctionSorted(IN PIMPORTTABLEP_SORTED_FUNCTION_LIST_ENTRY pFunctionName,
...
VOID ImportTablepInsertModuleSorted(IN PIMPORTTABLEP_SORTED_LIST_ENTRY pImportName,
...
NTSTATUS
ImportTablepHashCanonicalLists(IN PIMPORTTABLEP_SORTED_LIST_ENTRY ImportedNameList, OUT PBYTE Hash)
...

{
    ...
    typedef BOOL(WINAPI * CryptAcquireContextW)(HCRYPTPROV * phProv, LPCWSTR szContainer, LPCWSTR szProvider,
                                                DWORD dwProvType, DWORD dwFlags);

    typedef BOOL(WINAPI * CryptCreateHash)(HCRYPTPROV hProv, ALG_ID Algid, HCRYPTKEY hKey, DWORD dwFlags,
                                           HCRYPTHASH * phHash);

    typedef BOOL(WINAPI * CryptHashData)(HCRYPTHASH hHash, CONST BYTE * pbData, DWORD dwDataLen, DWORD dwFlags);

    typedef BOOL(WINAPI * CryptGetHashParam)(HCRYPTHASH hHash, DWORD dwParam, BYTE * pbData, DWORD * pdwDataLen,
                                             DWORD dwFlags);

    const static UNICODE_STRING ModuleName = RTL_CONSTANT_STRING(L"ADVAPI32.DLL");

    const static ANSI_STRING ProcedureNameCryptAcquireContext = RTL_CONSTANT_STRING("CryptAcquireContextW");

    const static ANSI_STRING ProcedureNameCryptCreateHash = RTL_CONSTANT_STRING("CryptCreateHash");

    const static ANSI_STRING ProcedureNameCryptHashData = RTL_CONSTANT_STRING("CryptHashData");

    const static ANSI_STRING ProcedureNameCryptGetHashParam = RTL_CONSTANT_STRING("CryptGetHashParam");
    ...
    if (AdvApi32ModuleHandle == LongToHandle(-1))
    {
        ...
        Status = LdrGetProcedureAddress(TempModuleHandle, (PANSI_STRING)&ProcedureNameCryptAcquireContext, 0,
        ...
        Status = LdrGetProcedureAddress(TempModuleHandle, (PANSI_STRING)&ProcedureNameCryptCreateHash, 0,
        ...
        Status = LdrGetProcedureAddress(TempModuleHandle, (PANSI_STRING)&ProcedureNameCryptHashData, 0,
        ...
        Status = LdrGetProcedureAddress(TempModuleHandle, (PANSI_STRING)&ProcedureNameCryptGetHashParam, 0,
    ...
    if (!lpfnCryptAcquireContextW(&hProvVerify, NULL, MS_DEF_PROV_W, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT | CRYPT_SILENT))
    {
    ...
    if (!lpfnCryptCreateHash(hProvVerify, CALG_MD5, 0, 0, &hHash))
    {
    ...
    while (pTemp != NULL)
    {
        ...
        if (!lpfnCryptHashData(hHash, (PBYTE)(pTemp->String), strlen(pTemp->String), 0))
        {
        ...
        while (pTemp2 != NULL)
        {
            ...
            if (!lpfnCryptHashData(hHash, (PBYTE)(pTemp2->String), strlen(pTemp2->String), 0))
            {
    ...
    if (!lpfnCryptGetHashParam(hHash, HP_HASHVAL, Hash, &dwHashedDataLength, 0))
    {
...
VOID ImportTablepFreeModuleSorted(IN PIMPORTTABLEP_SORTED_LIST_ENTRY pImportNameList)
/*++
