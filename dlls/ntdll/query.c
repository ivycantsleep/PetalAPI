...
NTSTATUS
NTAPI
RtlpQueryProcessDebugInformationRemote(IN OUT PRTL_DEBUG_INFORMATION Buffer)
{
    NTSTATUS Status;
    ULONG i;
    ULONG_PTR Delta;
...
NTSTATUS
RtlpChangeQueryDebugBufferTarget(IN PRTL_DEBUG_INFORMATION Buffer, IN HANDLE TargetProcessId,
                                 OUT PHANDLE ReturnedTargetProcessHandle)
{
    ...
    {
        Status =
            NtDuplicateObject(OldTargetProcess, *pOldHandle, NewTargetProcess, &NewHandle, 0, 0, DuplicateHandleFlags);
        if (!NT_SUCCESS(Status))
        {
            if (OldTargetProcess != NtCurrentProcess())
            {
    ...
    if (NewTargetProcess != NULL)
    {
        Status = NtMapViewOfSection(Buffer->SectionHandleClient, NewTargetProcess, &Buffer->ViewBaseTarget, 0, 0, NULL,
                                    &Buffer->ViewSize, ViewUnmap, 0, PAGE_READWRITE);
        if (Status == STATUS_CONFLICTING_ADDRESSES)
        {
            Buffer->ViewBaseTarget = NULL;
            Status = NtMapViewOfSection(Buffer->SectionHandleClient, NewTargetProcess, &Buffer->ViewBaseTarget, 0, 0,
                                        NULL, &Buffer->ViewSize, ViewUnmap, 0, PAGE_READWRITE);
        }
...
PVOID
RtlpCommitQueryDebugInfo(IN PRTL_DEBUG_INFORMATION Buffer, IN ULONG Size)
{
    NTSTATUS Status;
    PVOID Result;
    PVOID CommitBase;
...
RtlCreateQueryDebugBuffer(IN ULONG MaximumCommit OPTIONAL, IN BOOLEAN UseEventPair)
{
    ...
    { // Sundown Note: ULONG zero-extended.
    ...
    Status =
        NtMapViewOfSection(Section, NtCurrentProcess(), &Buffer, 0, 0, NULL, &ViewSize, ViewUnmap, 0, PAGE_READWRITE);
    if (!NT_SUCCESS(Status))
    {
        NtClose(Section);
        return NULL;
...
NTSTATUS
NTAPI
RtlDestroyQueryDebugBuffer(IN PRTL_DEBUG_INFORMATION Buffer)
{
    NTSTATUS Status;
    ...
    if (Buffer->TargetThreadHandle != NULL)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            Status = NtQueryInformationThread(Buffer->TargetThreadHandle, ThreadBasicInformation, &BasicInformation,
...
NTSTATUS
NTAPI
RtlQueryProcessDebugInformation(IN HANDLE UniqueProcessId, IN ULONG Flags, IN OUT PRTL_DEBUG_INFORMATION Buffer)
{
    ...
    //
    if ((NtCurrentTeb()->ClientId.UniqueProcess != UniqueProcessId) && (Flags & RTL_QUERY_PROCESS_NONINVASIVE) &&
        (Flags & (RTL_QUERY_PROCESS_MODULES | RTL_QUERY_PROCESS_MODULES32)) &&
        !(Flags & ~(RTL_QUERY_PROCESS_MODULES | RTL_QUERY_PROCESS_MODULES32 | RTL_QUERY_PROCESS_NONINVASIVE)))
    {
        OBJECT_ATTRIBUTES ObjectAttributes;
        ...
        if (!NT_SUCCESS(NtOpenProcess(&hNiProcess, PROCESS_ALL_ACCESS, &ObjectAttributes, &NiProcessId)))
        {
            hNiProcess = NULL;
        }
    }
    ...
    if ((NtCurrentTeb()->ClientId.UniqueProcess != UniqueProcessId) && !hNiProcess)
    {
        ...
        else
        {
            ...
            Status = RtlCreateUserThread(ProcessHandle, NULL, TRUE, 0, 0, 0, RtlpQueryProcessDebugInformationRemote,
                                         Buffer->ViewBaseTarget, &ThreadHandle, NULL);
            if (NT_SUCCESS(Status))
            {
                ...
                if (NT_SUCCESS(Status))
                {
                    Status = NtQueryInformationThread(ThreadHandle, ThreadBasicInformation, &BasicInformation,
                                                      sizeof(BasicInformation), NULL);
                    if (NT_SUCCESS(Status))
                    {
                        Status = BasicInformation.ExitStatus;
...
NTSTATUS
LdrQueryProcessModuleInformationEx(IN HANDLE hProcess OPTIONAL, IN ULONG_PTR Flags OPTIONAL,
...
NTSTATUS
NTAPI
RtlQueryProcessModuleInformation(IN HANDLE hProcess OPTIONAL, IN ULONG Flags, IN OUT PRTL_DEBUG_INFORMATION Buffer)
{
    NTSTATUS Status;
    ...
    if (Status == STATUS_INFO_LENGTH_MISMATCH)
    {
        ...
        if (Modules != NULL)
        {
            Status = LdrQueryProcessModuleInformationEx(hProcess, LdrFlags, Modules, RequiredLength, &RequiredLength);
            if (NT_SUCCESS(Status))
            {
...
NTSTATUS
RtlpQueryProcessEnumHeapsRoutine(PVOID HeapHandle, PVOID Parameter)
{
...
NTSTATUS
NTAPI
RtlQueryProcessHeapInformation(IN OUT PRTL_DEBUG_INFORMATION Buffer)
{
    NTSTATUS Status;
    PHEAP Heap;
                ...
                if (Buffer->SpecificHeap == NULL || Buffer->SpecificHeap == Heap)
                {
                    ...
                    try
                    {
                        for (SegmentIndex = 0; SegmentIndex < HEAP_MAXIMUM_SEGMENTS; SegmentIndex++)
                        {
                            ...
                            Entries->u.s2.CommittedSize =
                                (Segment->NumberOfPages - Segment->NumberOfUnCommittedPages) * PAGE_SIZE;
                            ...
                            while (CurrentBlock < Segment->LastValidEntry)
                            {
                                ...
                                if (CurrentBlock->Flags & HEAP_ENTRY_LAST_ENTRY)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        CurrentBlock =
                                            (PHEAP_ENTRY)((PCHAR)UnCommittedRange->Address + UnCommittedRange->Size);
...
NTSTATUS
NTAPI
RtlQueryProcessLockInformation(IN OUT PRTL_DEBUG_INFORMATION Buffer)
{
    NTSTATUS Status;
    PLIST_ENTRY Head, Next;
