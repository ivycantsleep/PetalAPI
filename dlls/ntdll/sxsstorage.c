...
VOID NTAPI RtlpAssemblyStorageMapResolutionDefaultCallback(ULONG Reason,
                                                           ...
                                                           PVOID Context)
{
    ...
    {
        static const WCHAR NameStringBuffer[] =
            L"\\Registry\\Machine\\Software\\Microsoft\\Windows\\CurrentVersion\\SideBySide\\AssemblyStorageRoots";
        static const UNICODE_STRING NameString = { sizeof(NameStringBuffer) - sizeof(WCHAR), sizeof(NameStringBuffer),
                                                   (PWSTR)NameStringBuffer };

        OBJECT_ATTRIBUTES Obja;
        HANDLE KeyHandle = NULL;

        InitializeObjectAttributes(&Obja, (PUNICODE_STRING)&NameString, OBJ_CASE_INSENSITIVE, NULL, NULL);
        ...
        if (!NT_SUCCESS(Status))
        {
            ...
            if ((Status != STATUS_OBJECT_NAME_NOT_FOUND) && (Status != STATUS_TOO_LATE))
            {
                DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: Unable to open registry key %wZ Status = 0x%08lx\n",
                           &NameString, Status);
    ...
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_TRACE_LEVEL, "SXS: Getting assembly storage root #%Iu\n",
        ...
        else if (Data->GetRoot.RootIndex == 1)
        {
            ...
            static const UNICODE_STRING WinSxS = { sizeof(WinSxSBuffer) - sizeof(WCHAR), sizeof(WinSxSBuffer),
                                                   (PWSTR)WinSxSBuffer };
        ...
        else if (Data->GetRoot.RootIndex <= MAXULONG)
        {
            ...
            ULONG SubKeyIndex =
                (ULONG)(Data->GetRoot.RootIndex -
                        2); // minus two because we use index 0 for privatized assembly probing and 1 for %SystemRoot%\winsxs
            ...
            Status =
                NtEnumerateKey(KeyHandle, SubKeyIndex, KeyBasicInformation, &KeyData, sizeof(KeyData), &ResultLength);
            if (!NT_SUCCESS(Status))
            {
            ...
            Status = RtlpGetAssemblyStorageMapRootLocation(KeyHandle, &SubKeyName, &Data->GetRoot.Root);
            if (!NT_SUCCESS(Status))
            {
                DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
...
NTSTATUS
RtlpGetAssemblyStorageMapRootLocation(HANDLE KeyHandle, PCUNICODE_STRING SubKeyName, PUNICODE_STRING Root)
{
    ...
    static const UNICODE_STRING ValueName = { sizeof(ValueNameBuffer) - sizeof(WCHAR), sizeof(ValueNameBuffer),
                                              (PWSTR)ValueNameBuffer };

    ASSERT(KeyHandle != NULL);
    ASSERT(SubKeyName != NULL);
    ASSERT(Root != NULL);

    if ((KeyHandle == NULL) || (SubKeyName == NULL) || (Root == NULL))
    {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }

    InitializeObjectAttributes(&Obja, (PUNICODE_STRING)&SubKeyName, OBJ_CASE_INSENSITIVE, KeyHandle, NULL);

    Status = NtOpenKey(&SubKeyHandle, KEY_QUERY_VALUE, &Obja);
    if (!NT_SUCCESS(Status))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: Unable to open storage root subkey %wZ; Status = 0x%08lx\n",
                   &SubKeyName, Status);

        goto Exit;
    }

    Status = NtQueryValueKey(SubKeyHandle, (PUNICODE_STRING)&ValueName, KeyValuePartialInformation, &ValueData,
                             sizeof(ValueData), &ResultLength);
    if (!NT_SUCCESS(Status))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
    ...
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: Assembly storage root location value type is not REG_SZ\n");
    ...
    if ((ValueData.kvpi.DataLength % 2) != 0)
    {
        ...
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: Assembly storage root location value has non-even size\n");
        Status = STATUS_OBJECT_PATH_NOT_FOUND;
        goto Exit;
    }
    ...
    RtlCopyMemory(Root->Buffer, ValueData.kvpi.Data, ValueData.kvpi.DataLength);
