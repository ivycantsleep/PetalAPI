...
INVERTED_FUNCTION_TABLE LdrpInvertedFunctionTable = { 0, MAXIMUM_INVERTED_FUNCTION_TABLE_SIZE, FALSE };
#endif
...
extern const UNICODE_STRING SlashSystem32SlashMscoreeDllString = {
    ...
    sizeof(SlashSystem32SlashMscoreeDllWCharArray), (PWSTR)SlashSystem32SlashMscoreeDllWCharArray
};
...
NTSTATUS
LdrpOpenImageFileOptionsKey(IN PUNICODE_STRING ImagePathName, OUT PHANDLE KeyHandle);

VOID LdrpInitializeApplicationVerifierPackage(PUNICODE_STRING UnicodeImageName, PPEB Peb, BOOLEAN EnabledSystemWide,
                                              BOOLEAN OptionsKeyPresent);

BOOLEAN
LdrpInitializeExecutionOptions(PUNICODE_STRING UnicodeImageName, PPEB Peb);

NTSTATUS
LdrpQueryImageFileKeyOption(IN HANDLE KeyHandle, IN PWSTR OptionName, IN ULONG Type, OUT PVOID Buffer,
                            IN ULONG BufferSize, OUT PULONG ResultSize OPTIONAL);

NTSTATUS
LdrpTouchThreadStack(SIZE_T EnforcedStackCommit);
...
VOID LdrQueryApplicationCompatibilityGoo(IN PUNICODE_STRING UnicodeImageName, IN BOOLEAN ImageFileOptionsPresent);

NTSTATUS
LdrFindAppCompatVariableInfo(IN ULONG dwTypeSeeking, OUT PAPP_VARIABLE_INFO *AppVariableInfo);

NTSTATUS
LdrpSearchResourceSection_U(IN PVOID DllHandle, IN PULONG_PTR ResourceIdPath, IN ULONG ResourceIdPathLength,
                            IN ULONG Flags, OUT PVOID *ResourceDirectoryOrData);

NTSTATUS
LdrpAccessResourceData(IN PVOID DllHandle, IN PIMAGE_RESOURCE_DATA_ENTRY ResourceDataEntry, OUT PVOID *Address OPTIONAL,
                       OUT PULONG Size OPTIONAL);

PVOID
NtdllpAllocateStringRoutine(SIZE_T NumberOfBytes)
{
...
RTL_CRITICAL_SECTION LdrpLoaderLock = { &LoaderLockDebug, -1 };
...
VOID LdrpInitializationFailure(IN NTSTATUS FailureCode)
{
    ...
    ErrorStatus = NtRaiseHardError(STATUS_APP_INIT_FAILURE, 1, 0, &ErrorParameter, OptionOk, &ErrorResponse);
}

INT LdrpInitializeProcessWrapperFilter(const struct _EXCEPTION_POINTERS *ExceptionPointers)
/*++
...

{
    ...
    if (!Peb->Ldr)
    {
        ...
        ImageFileOptionsPresent = LdrpInitializeExecutionOptions(&UnicodeImageName, Peb);
        ...
        if (NtHeader && (NtHeader->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC))
        {
            ...
            st = NtQueryInformationProcess(NtCurrentProcess(), ProcessWow64Information, &Wow64Info, sizeof(Wow64Info),
                                           NULL);
            if (!NT_SUCCESS(st))
            {
...
#else
        Cor20Header = RtlImageDirectoryEntryToData(Peb->ImageBaseAddress, TRUE, IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR,
                                                   &Cor20HeaderSize);
        if (Cor20Header)
        {
            UseCOR = TRUE;
    ...
    if (Teb->DeallocationStack == NULL)
    {
        ...
        st = NtQueryVirtualMemory(NtCurrentProcess(), Teb->NtTib.StackLimit, MemoryBasicInformation, (PVOID)&MemInfo,
                                  sizeof(MemInfo), NULL);
        if (!NT_SUCCESS(st))
        {
            DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR,
    ...
    __try
    {
        ...
        if (!Peb->Ldr)
        {
            ...
            __try
            {
                ...
                InitStatus =
                    LdrpInitializeProcess(Context, SystemArgument1, &UnicodeImageName, UseCOR, ImageFileOptionsPresent);
            ...
            if (LdrpDisplayLoadTime)
            {
                ...
                DbgPrint("Compares %d Bypasses %d Normal Snaps %d\nSecOpens %d SecCreates %d Maps %d Relocates %d\n",
                         LdrpCompareCount, LdrpSnapBypass, LdrpNormalSnap, LdrpSectionOpens, LdrpSectionCreates,
                         LdrpSectionMaps, LdrpSectionRelocates);
            }
#endif // DBG
        }
        else
...
void LdrpGetShimEngineInterface(void)
{
    STRING strProcName;
...
BOOL LdrInitShimEngineDynamic(PVOID pShimEngineModule)
{
...
void LdrpLoadShimEngine(WCHAR *pwszShimEngine, PUNICODE_STRING pstrExeFullPath, PVOID pAppCompatExeData)
{
...
void LdrpUnloadShimEngine(void)
{
...

{
    ...
    if (!NtHeader)
    {
        DbgPrintEx(
            ...
            __FUNCTION__, Peb->ImageBaseAddress);
...
#if defined(BUILD_WOW6432)
    {
        ...
        if (NtHeader32->FileHeader.Machine == IMAGE_FILE_MACHINE_I386 &&
            ...
            !NT_SUCCESS(st = LdrpWx86FormatVirtualImage(NULL, NtHeader32, Peb->ImageBaseAddress)))
        {
            DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR,
                       ...
                       __FUNCTION__, NtHeader32->FileHeader.Machine, IMAGE_FILE_MACHINE_I386,
                       NtHeader32->OptionalHeader.SectionAlignment, NATIVE_PAGE_SIZE, st);
    ...
    RtlInitNlsTables(Peb->AnsiCodePageData, Peb->OemCodePageData, Peb->UnicodeCaseTableData, &InitTableInfo);
...
#endif
    {
        ...
        ImageConfigData =
            RtlImageDirectoryEntryToData(Peb->ImageBaseAddress, TRUE, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &i);
    }
    ...
    if (ImageConfigData != NULL && i == sizeof(*ImageConfigData))
    {
        ...
        if (ImageConfigData->CriticalSectionDefaultTimeout != 0)
        {
            ...
            //
            RtlpTimeout.QuadPart = Int32x32To64((LONG)ImageConfigData->CriticalSectionDefaultTimeout, -10000);
        }
    ...
    {
        st = NtSetInformationProcess(NtCurrentProcess(), ProcessAffinityMask, &Peb->ImageProcessAffinityMask,
    ...
    if (ShowSnaps)
    {
        DbgPrint("LDR: PID: 0x%x started - '%wZ'\n", NtCurrentTeb()->ClientId.UniqueProcess, &CommandLine);
    }
    ...
    RtlInitializeBitMap(&TlsBitMap, &Peb->TlsBitmapBits[0], RTL_BITS_OF(Peb->TlsBitmapBits));

    RtlInitializeBitMap(&TlsExpansionBitMap, &Peb->TlsExpansionBitmapBits[0], RTL_BITS_OF(Peb->TlsExpansionBitmapBits));
    ...
    if ((Peb->NtGlobalFlag & FLG_USER_STACK_TRACE_DB) || LdrpShouldCreateStackTraceDb)
    {

        PVOID BaseAddress = NULL;
        SIZE_T ReserveSize = 8 * RTL_MEG;

        st = LdrQueryImageFileExecutionOptions(UnicodeImageName, L"StackTraceDatabaseSizeInMb", REG_DWORD, &ReserveSize,
                                               sizeof(ReserveSize), NULL);
        ...
        st = NtAllocateVirtualMemory(NtCurrentProcess(), (PVOID *)&BaseAddress, 0, &ReserveSize, MEM_RESERVE,
                                     PAGE_READWRITE);
        if (NT_SUCCESS(st))
        {
            st = RtlInitializeStackTraceDataBase(BaseAddress, 0, ReserveSize);
            if (!NT_SUCCESS(st))
            {
                NtFreeVirtualMemory(NtCurrentProcess(), (PVOID *)&BaseAddress, &ReserveSize, MEM_RELEASE);
            }
            else
            {
...
#if defined(_WIN64)
    if ((UseWOW64) || (NtHeader->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC))
    {
        ...
        //
        Peb->ProcessHeap = RtlCreateHeap(ProcessHeapFlags, NULL, 0, 0, NULL, &HeapParameters);
    }
    else
#endif
    {

        if (NtHeader->OptionalHeader.MajorSubsystemVersion <= 3 && NtHeader->OptionalHeader.MinorSubsystemVersion < 51)
        {
            ProcessHeapFlags |= HEAP_CREATE_ALIGN_16;
        }

        Peb->ProcessHeap = RtlCreateHeap(ProcessHeapFlags, NULL, NtHeader->OptionalHeader.SizeOfHeapReserve,
    ...
    if (Peb->ProcessHeap == NULL)
    {
        DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR, "LDR: %s - unable to create process heap\n", __FUNCTION__);
    ...
    NtdllBaseTag = RtlCreateTagHeap(Peb->ProcessHeap, 0, L"NTDLL!",
    ...
    if (ImageFileOptionsPresent)
    {
        ...
        st = LdrQueryImageFileExecutionOptions(UnicodeImageName, L"DebugProcessHeapOnly", REG_DWORD,
                                               &DebugProcessHeapOnly, sizeof(DebugProcessHeapOnly), NULL);
        if (NT_SUCCESS(st))
        {
    ...
    InitializeObjectAttributes(&Obja, &Unicode, OBJ_CASE_INSENSITIVE, NULL, NULL);
    st = NtOpenDirectoryObject(&LdrpKnownDllObjectDirectory, DIRECTORY_QUERY | DIRECTORY_TRAVERSE, &Obja);
    if (!NT_SUCCESS(st))
    {
    ...
    else
    {
        ...
        if (NT_SUCCESS(st))
        {
            ...
            st = NtQuerySymbolicLinkObject(LinkHandle, &LdrpKnownDllPath, NULL);
            NtClose(LinkHandle);
            if (!NT_SUCCESS(st))
            {
                DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR,
...
#define DRIVE_ROOT_DIRECTORY_LENGTH 3 /* (sizeof("X:\\") - 1) */
        if (CurDir.Buffer == NULL || CurDir.Buffer[0] == UNICODE_NULL || CurDir.Length == 0)
        {
            ...
            RtlMoveMemory(CurDir.Buffer, USER_SHARED_DATA->NtSystemRoot, DRIVE_ROOT_DIRECTORY_LENGTH * sizeof(WCHAR));
    ...
    if (!Peb->Ldr)
    {
        DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR, "LDR: %s - failed to allocate PEB_LDR_DATA\n", __FUNCTION__);
    ...
    if (pp != NULL)
    {
        ...
        LdrDataTableEntry->BaseDllName.Buffer =
            (PWSTR)(((ULONG_PTR)LdrDataTableEntry->FullDllName.Buffer) +
    ...
    InsertHeadList(&Peb->Ldr->InInitializationOrderModuleList, &LdrDataTableEntry->InInitializationOrderLinks);
    ...
    else
    {
        ...
        ImagePathNameBuffer =
            (PWCHAR)RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TEMP_TAG), ImagePathName.MaximumLength);
    ...
    if (UseWOW64)
    {
        ...
        // PspInitSystemDll().
        //
        st = LdrGetProcedureAddress(Wow64Handle, &Wow64LdrpInitializeProcName, 0, (PVOID *)&Wow64LdrpInitialize);
        if (!NT_SUCCESS(st))
        {
            if (ShowSnaps)
            {
        ...
        st = LdrGetProcedureAddress(Wow64Handle, &Wow64PrepareForExceptionProcName, 0,
                                    (PVOID *)&Wow64PrepareForException);
        if (!NT_SUCCESS(st))
        {
            if (ShowSnaps)
        ...
        st = LdrGetProcedureAddress(Wow64Handle, &Wow64ApcRoutineProcName, 0, (PVOID *)&Wow64ApcRoutine);
        if (!NT_SUCCESS(st))
        {
            if (ShowSnaps)
            {
    ...
    if (UseCOR)
    {
        ...
        st = LdrpCorValidateImage(&Peb->ImageBaseAddress, LdrpImageEntry->FullDllName.Buffer);
        if (!NT_SUCCESS(st))
        {
            return st;
        }
        ...
        (NtHeader->OptionalHeader.Subsystem == IMAGE_SUBSYSTEM_WINDOWS_CUI))
    {
        ...
        st = LdrGetProcedureAddress(Kernel32Handle, &Kernel32ProcessInitPostImportFunctionName, 0,
                                    (PVOID *)&Kernel32ProcessInitPostImportFunction);
        if (!NT_SUCCESS(st))
        {
            if (ShowSnaps)
            {
                DbgPrint("LDR: Failed to find post-import process init function in kernel32; ntstatus 0x%08lx\n", st);
            }
    ...
    if ((PVOID)NtHeader->OptionalHeader.ImageBase != Peb->ImageBaseAddress)
    {
        ...
        status = LdrRelocateImage(ViewBase, "LDR", STATUS_SUCCESS, STATUS_CONFLICTING_ADDRESSES,
                                  STATUS_INVALID_IMAGE_FORMAT);

        if (!NT_SUCCESS(status))
        {
            DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR, "LDR: %s - call to LdrRelocateImage failed with status %x\n",
                       __FUNCTION__, status);
    ...
    {
        DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR, "LDR: %s - failed to initialize TLS slots; status %x\n",
                   __FUNCTION__, st);
    ...
    if (!NT_SUCCESS(st))
    {
        DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR, "LDR: %s - Failed running initialization routines; status %x\n",
                   __FUNCTION__, st);
...

{
    ...
    try
    {
        ...
        else
        {
            ...
            {
                LdrDataTableEntry =
                    (PLDR_DATA_TABLE_ENTRY)(CONTAINING_RECORD(Next, LDR_DATA_TABLE_ENTRY, InInitializationOrderLinks));
                ...
                if (Peb->ImageBaseAddress != LdrDataTableEntry->DllBase)
                {
                    ...
                    if (InitRoutine && (LdrDataTableEntry->Flags & LDRP_PROCESS_ATTACH_CALLED))
                    {
                        ...
                        LdrpCallInitRoutine(InitRoutine, LdrDataTableEntry->DllBase, DLL_PROCESS_DETACH, (PVOID)1);
                ...
                if (!(LdrDataTableEntry->Flags & LDRP_DONT_CALL_FOR_THREADS))
                {
                    ...
                    if (InitRoutine && (LdrDataTableEntry->Flags & LDRP_PROCESS_ATTACH_CALLED))
                    {
                        if (LdrDataTableEntry->Flags & LDRP_IMAGE_DLL)
                        {
                            ...
                            LdrpCallInitRoutine(InitRoutine, LdrDataTableEntry->DllBase, DLL_THREAD_DETACH, NULL);
                            LDRP_DEACTIVATE_ACTIVATION_CONTEXT();
                        }
                    }
                }
            ...
            if (!(LdrDataTableEntry->Flags & LDRP_DONT_CALL_FOR_THREADS))
            {
                ...
                if (InitRoutine && (LdrDataTableEntry->Flags & LDRP_PROCESS_ATTACH_CALLED))
                {
                    if (LdrDataTableEntry->Flags & LDRP_IMAGE_DLL)
                    {
                        ...
                        if (!LdrpShutdownInProgress)
                        {
                            LdrpCallInitRoutine(InitRoutine, LdrDataTableEntry->DllBase, DLL_THREAD_ATTACH, NULL);
                        }
                        LDRP_DEACTIVATE_ACTIVATION_CONTEXT();
                    }
                }
                            ...
                            IN ULONG BufferSize, OUT PULONG ResultSize OPTIONAL)
{
    ...
    Status = NtQueryValueKey(KeyHandle, &UnicodeString, KeyValuePartialInformation, KeyValueInformation,
    ...
    if (Status == STATUS_BUFFER_OVERFLOW)
    {
        ...
        if (RtlProcessHeap())
        {
            ...
            AllocLength = sizeof(*KeyValueInformation) + KeyValueInformation->DataLength;
            KeyValueInformation =
                (PKEY_VALUE_PARTIAL_INFORMATION)RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TEMP_TAG), AllocLength);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        else if (KeyValueInformation->Type == REG_DWORD)
        {
            ...
            {
                if ((Buffer) && (BufferSize == sizeof(ULONG)) && (KeyValueInformation->DataLength == BufferSize))
                {
        ...
        else
        {
            if (Type == REG_DWORD)
            {
                ...
                else
                {
                    ...
                    UnicodeString.Length = (USHORT)(KeyValueInformation->DataLength - sizeof(UNICODE_NULL));
...
NTSTATUS
LdrQueryImageFileExecutionOptions(IN PUNICODE_STRING ImagePathName, IN PWSTR OptionName, IN ULONG Type,
                                  OUT PVOID Buffer, IN ULONG BufferSize, OUT PULONG ResultSize OPTIONAL)
{
    NTSTATUS Status;
    HANDLE KeyHandle;

    Status = LdrpOpenImageFileOptionsKey(ImagePathName, &KeyHandle);

    if (NT_SUCCESS(Status))
    {

        Status = LdrpQueryImageFileKeyOption(KeyHandle, OptionName, Type, Buffer, BufferSize, ResultSize);
...
LdrpInitializeTls(VOID)
{
    ...
    while (Next != Head)
    {
        ...
        TlsImage = (PIMAGE_TLS_DIRECTORY)RtlImageDirectoryEntryToData(Entry->DllBase, TRUE, IMAGE_DIRECTORY_ENTRY_TLS,
                                                                      &TlsSize);
    ...
    if (LdrpNumberOfTlsEntries)
    {
        ...
        while (Next != Head)
        {
            ...
            TlsVector[TlsEntry->Tls.Characteristics] =
                RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TLS_TAG),
            ...
            if (ShowSnaps)
            {
                DbgPrint("LDR: TlsVector %x Index %d = %x copied from %x to %x\n", TlsVector,
                         TlsEntry->Tls.Characteristics, &TlsVector[TlsEntry->Tls.Characteristics],
                         TlsEntry->Tls.StartAddressOfRawData, TlsVector[TlsEntry->Tls.Characteristics]);
            }

            RtlCopyMemory(TlsVector[TlsEntry->Tls.Characteristics], (PVOID)TlsEntry->Tls.StartAddressOfRawData,
                          TlsEntry->Tls.EndAddressOfRawData - TlsEntry->Tls.StartAddressOfRawData);
...
VOID LdrpFreeTls(VOID)
{
    ...
    if (TlsVector)
    {
        ...
        while (Next != Head)
        {
            ...
            if (TlsVector[TlsEntry->Tls.Characteristics])
            {
                RtlFreeHeap(RtlProcessHeap(), 0, TlsVector[TlsEntry->Tls.Characteristics]);
            }
        }

        RtlFreeHeap(RtlProcessHeap(), 0, TlsVector);
    }
}
...
VOID LdrpCallTlsInitializers(PVOID DllBase, ULONG Reason)
{
    ...
    TlsImage = (PIMAGE_TLS_DIRECTORY)RtlImageDirectoryEntryToData(DllBase, TRUE, IMAGE_DIRECTORY_ENTRY_TLS, &TlsSize);
        ...
        if (TlsImage)
        {
            ...
            if (CallBackArray)
            {
                ...
                while (*CallBackArray)
                {
                    ...
                    LdrpCallInitRoutine((PDLL_INIT_ROUTINE)InitRoutine, DllBase, Reason, 0);
                }
            }
        }
    }
    ...
    {
        DbgPrintEx(DPFLTR_LDR_ID, LDR_ERROR_DPFLTR, "LDR: %s - caught exception %08lx calling TLS callbacks\n",
...

{
    ...
    if (ImageFileOptionsPresent)
    {
        ...
        st = LdrQueryImageFileExecutionOptions(UnicodeImageName, L"ApplicationGoo", REG_BINARY, &LocalAppCompatGoo,
                                               sizeof(APP_COMPAT_GOO), &ResultSize);
        ...
        if (st == STATUS_BUFFER_OVERFLOW)
        {
            ...
            //
            AppCompatGoo = RtlAllocateHeap(Peb->ProcessHeap, HEAP_ZERO_MEMORY, ResultSize);
            ...
            //
            st = LdrQueryImageFileExecutionOptions(UnicodeImageName, L"ApplicationGoo", REG_BINARY, AppCompatGoo,
            ...
            try
            {
                st = LdrpSearchResourceSection_U(Peb->ImageBaseAddress, IdPath, 3, 0, &DataEntry);
            }
            except(LdrpGenericExceptionFilter(GetExceptionInformation(), __FUNCTION__))
            {
            ...
            if (NT_SUCCESS(st))
            {
                ...
                try
                {
                    st = LdrpAccessResourceData(Peb->ImageBaseAddress, DataEntry, &Resource, &ResourceSize);
                }
                except(LdrpGenericExceptionFilter(GetExceptionInformation(), __FUNCTION__))
                {
                    st = STATUS_UNSUCCESSFUL;
            ...
            TotalGooLength = AppCompatGoo->dwTotalGooSize - sizeof(AppCompatGoo->dwTotalGooSize);
            while (TotalGooLength)
            {
                ...
                try
                {
                    ...
                    if (bImageContainsVersionResourceInfo)
                    {
                        ...
                        OutputCompareLength = (ULONG)RtlCompareMemory(ResourceInfo, Resource, InputCompareLength);
                    }
                ...
                if ((!NT_SUCCESS(st)) || (InputCompareLength != OutputCompareLength))
                {
                ...
                Peb->AppCompatInfo = RtlAllocateHeap(Peb->ProcessHeap, HEAP_ZERO_MEMORY, AppCompatLength);
                ...
                //
                RtlMoveMemory(&AppCompatInfo->CompatibilityFlags,
                ...
                if (AppCompatInfo->CompatibilityFlags.QuadPart & KACF_VERSIONLIE)
                {
                    ...
                    Peb->CSDVersion.Buffer =
                        RtlAllocateHeap(Peb->ProcessHeap, HEAP_ZERO_MEMORY, Peb->CSDVersion.MaximumLength);
    ...
    if (FALSE == fNewCSDVersionBuffer)
    {
        ...
        st = RtlQueryEnvironmentVariable_U(NULL, &EnvName, &EnvValue);
        ...
        if (STATUS_SUCCESS == st)
        {
            ...
            if (len)
            {
                NewCSDString = RtlAllocateHeap(Peb->ProcessHeap, HEAP_ZERO_MEMORY, (len + 1) * sizeof(WCHAR));
                                              ...
                                              BOOLEAN OptionsKeyPresent)
{
    ...
    if ((Peb->NtGlobalFlag & FLG_APPLICATION_VERIFIER))
    {
        ...
        AVrfInitializeVerifier(EnabledSystemWide, UnicodeImageName, 0);
    }
    ...
    if ((Peb->NtGlobalFlag & FLG_HEAP_PAGE_ALLOCS))
    {
        ...
        Peb->NtGlobalFlag &= ~(FLG_HEAP_ENABLE_TAGGING | FLG_HEAP_ENABLE_TAG_BY_DLL | FLG_HEAP_ENABLE_TAIL_CHECK |
                               FLG_HEAP_ENABLE_FREE_CHECK | FLG_HEAP_VALIDATE_PARAMETERS | FLG_HEAP_VALIDATE_ALL |
        ...
        if (OptionsKeyPresent)
        {
            ...
            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapFlags", REG_DWORD, &RtlpDphGlobalFlags,
                                              sizeof(RtlpDphGlobalFlags), NULL);
        }
        ...
        if (OptionsKeyPresent)
        {
            ...
            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapSizeRangeStart", REG_DWORD,
                                              &RtlpDphSizeRangeStart, sizeof(RtlpDphSizeRangeStart), NULL);

            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapSizeRangeEnd", REG_DWORD,
                                              &RtlpDphSizeRangeEnd, sizeof(RtlpDphSizeRangeEnd), NULL);

            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapRandomProbability", REG_DWORD,
                                              &RtlpDphRandomProbability, sizeof(RtlpDphRandomProbability), NULL);

            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapFaultProbability", REG_DWORD,
                                              &RtlpDphFaultProbability, sizeof(RtlpDphFaultProbability), NULL);

            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapFaultTimeOut", REG_DWORD,
                                              &RtlpDphFaultTimeOut, sizeof(RtlpDphFaultTimeOut), NULL);
            ...
            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapDllRangeStart", REG_DWORD,
                                              &RtlpDphDllRangeStart, sizeof(RtlpDphDllRangeStart), NULL);

            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapDllRangeEnd", REG_DWORD, &RtlpDphDllRangeEnd,
                                              sizeof(RtlpDphDllRangeEnd), NULL);

            LdrQueryImageFileExecutionOptions(UnicodeImageName, L"PageHeapTargetDlls", REG_SZ, &RtlpDphTargetDlls, 512,
                                              NULL);
        }
...
--*/
{
    ...
    try
    {
        ...
        Status = NtQueryVirtualMemory(NtCurrentProcess(), NtCurrentTeb()->NtTib.StackLimit, MemoryBasicInformation,
                                      &MemoryInformation, sizeof MemoryInformation, &ReturnLength);
...
--*/
{
    ...
    st = LdrpOpenImageFileOptionsKey(UnicodeImageName, &KeyHandle);
    ...
    if (NT_SUCCESS(st))
    {
        ...
        LdrpQueryImageFileKeyOption(KeyHandle, L"DisableHeapLookaside", REG_DWORD, &RtlpDisableHeapLookaside,
                                    sizeof(RtlpDisableHeapLookaside), NULL);

        //
        // Verification options during process shutdown (heap leaks, etc.).
        //

        LdrpQueryImageFileKeyOption(KeyHandle, L"ShutdownFlags", REG_DWORD, &RtlpShutdownProcessFlags,
                                    sizeof(RtlpShutdownProcessFlags), NULL);
...

        {
            ...
            LdrpQueryImageFileKeyOption(KeyHandle, L"MinimumStackCommitInBytes", REG_DWORD, &MinimumStackCommitInBytes,
                                        sizeof(MinimumStackCommitInBytes), NULL);
...

        {
            ...
            LdrpQueryImageFileKeyOption(KeyHandle, L"ExecuteOptions", REG_DWORD, &(ExecuteOptions),
                                        sizeof(ExecuteOptions), NULL);
...

        {
            ...
            st = LdrpQueryImageFileKeyOption(KeyHandle, L"GlobalFlag", REG_DWORD, &ProcessFlags,
                                             sizeof(Peb->NtGlobalFlag), NULL);
            ...
            if ((Peb->NtGlobalFlag & (FLG_APPLICATION_VERIFIER | FLG_HEAP_PAGE_ALLOCS)))
            {
                ...
                LdrpInitializeApplicationVerifierPackage(UnicodeImageName, Peb, EnabledSystemWide, TRUE);
            }
        }
    ...
    else
    {
        ...
        if ((Peb->NtGlobalFlag & (FLG_APPLICATION_VERIFIER | FLG_HEAP_PAGE_ALLOCS)))
        {
            ...
            LdrpInitializeApplicationVerifierPackage(UnicodeImageName, Peb, TRUE, FALSE);
        }
        else
        {
            ...
            if (Peb->BeingDebugged)
            {
                ...
                st = RtlQueryEnvironmentVariable_U(Peb->ProcessParameters->Environment, &DebugVarName, &DebugVarValue);
...
--*/
{
    ...
    Status = NtProtectVirtualMemory(NtCurrentProcess(), (PVOID)&Address, &Size, StackProtect, &OldProtect);
    ...
    Status = NtQueryVirtualMemory(NtCurrentProcess(), (PVOID)Address, MemoryBasicInformation, &MemoryInformation,
                                  sizeof MemoryInformation, &Length);
    ...
    if (MemoryInformation.Protect != (StackProtect | PAGE_GUARD))
    {
        ...
        Status =
            NtProtectVirtualMemory(NtCurrentProcess(), (PVOID)&Address, &Size, StackProtect | PAGE_GUARD, &OldProtect);
...
VOID RtlGetNtVersionNumbers(ULONG *pNtMajorVersion, ULONG *pNtMinorVersion, ULONG *pNtBuildNumber)
/*++
