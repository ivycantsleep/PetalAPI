...
#define DPFLTR_LEVEL_STATUS(x) \
    ((NT_SUCCESS(x) || (x) == STATUS_OBJECT_NAME_NOT_FOUND) ? DPFLTR_TRACE_LEVEL : DPFLTR_ERROR_LEVEL)

ULONG
RtlpComputeBackupIndex(IN PCURDIR CurDir)
{
    ULONG BackupIndex;
...
NTSTATUS
RtlSetCurrentDirectory_U(PUNICODE_STRING PathName)
...

{
    ...
    try
    {
        try
        {
            ...
            DosDirLength = RtlGetFullPathName_Ustr(PathName, DosDirLength, DosDir.Buffer, NULL, NULL, &InputPathType);
            if (!DosDirLength)
            {
            ...
            //
            TranslationStatus = RtlDosPathNameToNtPathName_U(DosDir.Buffer, &NtFileName, NULL, NULL);
            ...
            InitializeObjectAttributes(&Obja, &NtFileName, OBJ_CASE_INSENSITIVE | OBJ_INHERIT, NULL, NULL);
            ...
            if (((ULONG_PTR)CurDir->Handle & OBJ_HANDLE_TAGBITS) == RTL_USER_PROC_CURDIR_INHERIT)
            {
                ...
                Status = NtQueryVolumeInformationFile(NewDirectoryHandle, &IoStatusBlock, &DeviceInfo,
            ...
            else
            {
                ...
                Status =
                    NtOpenFile(&NewDirectoryHandle, FILE_TRAVERSE | SYNCHRONIZE, &Obja, &IoStatusBlock,
...
RTL_PATH_TYPE
NTAPI
RtlDetermineDosPathNameType_Ustr(IN PCUNICODE_STRING String)
...
NTSTATUS
NTAPI
RtlpDetermineDosPathNameType4(IN ULONG InFlags, IN PCUNICODE_STRING DosPath, OUT RTL_PATH_TYPE *OutType,
                              OUT ULONG *OutFlags)
{
    ...
    if (!RTL_SOFT_VERIFY(DosPath != NULL) || !RTL_SOFT_VERIFY(OutType != NULL) || !RTL_SOFT_VERIFY(OutFlags != NULL) ||
        !RTL_SOFT_VERIFY((InFlags & ~(RTL_DETERMINE_DOS_PATH_NAME_TYPE_IN_FLAG_OLD |
                                      RTL_DETERMINE_DOS_PATH_NAME_TYPE_IN_FLAG_STRICT_WIN32NT)) == 0))
    {
    ...
    *OutType = PathType;
    ...
    else if (RtlEqualUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpWin32NtRoot),
                                   RTL_CONST_CAST(PUNICODE_STRING)(DosPath), TRUE))
    {
        IncompleteRoot = TRUE;
        Win32Nt = TRUE;
    }
    else if (RtlEqualUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpWin32NtRootSlash),
                                   RTL_CONST_CAST(PUNICODE_STRING)(DosPath), TRUE))
    {
        IncompleteRoot = TRUE;
        Win32Nt = TRUE;
    }
    else if (RtlPrefixUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpWin32NtRootSlash),
                                    RTL_CONST_CAST(PUNICODE_STRING)(DosPath), TRUE))
    {
        Win32Nt = TRUE;
    }
    ...
    if (Win32Nt)
    {
        if (RtlEqualUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpWin32NtUncRoot),
                                  RTL_CONST_CAST(PUNICODE_STRING)(DosPath), TRUE))
        {
            IncompleteRoot = TRUE;
            Win32NtUncAbsolute = TRUE;
        }
        else if (RtlEqualUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpWin32NtUncRootSlash),
                                       RTL_CONST_CAST(PUNICODE_STRING)(DosPath), TRUE))
        {
            IncompleteRoot = TRUE;
            Win32NtUncAbsolute = TRUE;
        }
        else if (RtlPrefixUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpWin32NtUncRootSlash),
                                        RTL_CONST_CAST(PUNICODE_STRING)(DosPath), TRUE))
        {
            Win32NtUncAbsolute = TRUE;
        }
...
ULONG
RtlIsDosDeviceName_Ustr(IN PUNICODE_STRING DosFileName)
...
ULONG
RtlIsDosDeviceName_U(IN PWSTR DosFileName)
{
    UNICODE_STRING UnicodeString;
...
NTSTATUS
RtlpCheckDeviceName(PUNICODE_STRING DevName, ULONG DeviceNameOffset, BOOLEAN *NameInvalid)
{
    PWSTR DevPath;
    NTSTATUS Status = STATUS_SUCCESS;
...

{
    ...
    FsCtlStatus =
        NtFsControlFile(CurDir->Handle, NULL, NULL, NULL, &IoStatusBlock, FSCTL_IS_VOLUME_MOUNTED, NULL, 0, NULL, 0);
...
ULONG
RtlGetFullPathName_Ustr(PUNICODE_STRING FileName, ULONG nBufferLength, PWSTR lpBuffer, PWSTR *lpFilePart OPTIONAL,
                        PBOOLEAN NameInvalid, RTL_PATH_TYPE *InputPathType)
...

{
    ...
    *InputPathType = RtlPathTypeUnknown;
    ...
    if (DeviceNameLength)
    {
        ...
        {
            RtlMoveMemory(lpBuffer, RtlpSlashSlashDot.Buffer, RtlpSlashSlashDot.Length);
            RtlMoveMemory((PVOID)((PUCHAR)lpBuffer + RtlpSlashSlashDot.Length), (PSZ)lpFileName + DeviceNameOffset,
                          DeviceNameLength);
    ...
    try
    {
        ...
        case RtlPathTypeDriveRelative:
            ...
            else
            {
                ...
                Prefix = FullPath;
                Status = RtlQueryEnvironmentVariable_U(NULL, &EnvVarName, &Prefix);
                if (!NT_SUCCESS(Status))
                {
...
NTSTATUS
RtlGetFullPathName_UstrEx(PUNICODE_STRING FileName, PUNICODE_STRING StaticString, PUNICODE_STRING DynamicString,
                          PUNICODE_STRING *StringUsed, SIZE_T *FilePartPrefixCch OPTIONAL, PBOOLEAN NameInvalid,
                          RTL_PATH_TYPE *InputPathType, SIZE_T *BytesRequired OPTIONAL)
...
--*/
{
    ...
    Length = RtlGetFullPathName_Ustr(FileName, StaticBufferSize, StaticString->Buffer, &FilePart, NameInvalid,
                                     InputPathType);
    if (Length == 0)
    {
#if DBG
    ...
    else
    {
        ...
        __try
        {
            ...
            else
            {
                ...
                Length = RtlGetFullPathName_Ustr(FileName, TempDynamicString.MaximumLength - sizeof(WCHAR),
                                                 TempDynamicString.Buffer, &FilePart, NameInvalid, InputPathType);
                if (Length == 0)
                {
#if DBG
                    DbgPrint("%s line %d: RtlGetFullPathName_Ustr() returned 0\n", __FUNCTION__, __LINE__);
...
ULONG
RtlGetFullPathName_U(PCWSTR lpFileName, ULONG nBufferLength, PWSTR lpBuffer, PWSTR *lpFilePart OPTIONAL)
...
NTSTATUS
RtlpWin32NTNameToNtPathName_U(IN PUNICODE_STRING DosFileName, OUT PUNICODE_STRING NtFileName,
                              OUT PWSTR *FilePart OPTIONAL, OUT PRTL_RELATIVE_NAME RelativeName OPTIONAL)
{
    ...
    FullNtPathName = RtlAllocateHeap(RtlProcessHeap(), 0,
    ...
    RtlMoveMemory((PUCHAR)FullNtPathName + RtlpDosDevicesPrefix.Length, DosFileName->Buffer + 4,
...
BOOLEAN
RtlDosPathNameToNtPathName_Ustr(IN PCUNICODE_STRING DosFileNameString, OUT PUNICODE_STRING NtFileName,
                                OUT PWSTR *FilePart OPTIONAL, OUT PRTL_RELATIVE_NAME RelativeName OPTIONAL)
/*++
...

{
    ...
    //
    if (UnicodeString.Length > 8 && UnicodeString.Buffer[0] == '\\' && UnicodeString.Buffer[1] == '\\' &&
        UnicodeString.Buffer[2] == '?' && UnicodeString.Buffer[3] == '\\')
    {
    ...
    __try
    {
        __try
        {
            ...
            FullDosPathNameLength = RtlGetFullPathName_Ustr(&UnicodeString, DosPathLength, FullDosPathName, FilePart,
                                                            &NameInvalid, &InputDosPathType);
            ...
            RtlMoveMemory((PUCHAR)FullNtPathName + Prefix.Length, FullDosPathName + DosPathNameOffset,
                ...
                if (InputDosPathType == RtlPathTypeRelative)
                {
                    ...
                    if (CurDir->Handle)
                    {
                        ...
                        if (CurDir->DosPath.Length <= FullDosPathString.Length)
                        {
                            ...
                            if (RtlEqualUnicodeString((PUNICODE_STRING)&CurDir->DosPath, &FullDosPathString, TRUE))
                            {
                                ...
                                RelativeName->RelativeName.Buffer =
                                    ((PUCHAR)FullNtPathName + Prefix.Length - (DosPathNameOffset << 1) +
                                     (CurDir->DosPath.Length));
...
BOOLEAN
RtlDosPathNameToNtPathName_U(IN PCWSTR DosFileName, OUT PUNICODE_STRING NtFileName, OUT PWSTR *FilePart OPTIONAL,
...
NTSTATUS
RtlDoesFileExist3(OUT PHANDLE FileHandle, IN PCUNICODE_STRING FileNameString, IN ACCESS_MASK DesiredAccess,
...
--*/
{
    ...
    DosPathToNtPathReturnValue = RtlDosPathNameToNtPathName_Ustr(FileNameString, &NtFileName, NULL, &RelativeName);
    ...
    InitializeObjectAttributes(&Obja, &NtFileName, OBJ_CASE_INSENSITIVE, RelativeName.ContainingDirectory, NULL);

    //
    // do we worry about synchronize / pending?
    //
    Status =
        NtOpenFile(FileHandle, DesiredAccess | FILE_READ_ATTRIBUTES, &Obja, &IoStatusBlock, ShareAccess, OpenOptions);
    if (!NT_SUCCESS(Status))
        goto Exit;
...
BOOLEAN
RtlDoesFileExists_UstrEx(IN PCUNICODE_STRING FileNameString, IN BOOLEAN TreatDeniedOrSharingAsHit)
/*++
...

{
    ...
    ReturnValue = RtlDosPathNameToNtPathName_Ustr(FileNameString, &NtFileName, NULL, &RelativeName);
...
BOOLEAN
RtlDoesFileExists_UEx(IN PCWSTR FileName, IN BOOLEAN TreatDeniedOrSharingAsHit)
{
...
BOOLEAN
RtlDoesFileExists_UStr(IN PCUNICODE_STRING FileName)
/*
...
ULONG
RtlDosSearchPath_U(IN PCWSTR lpPath, IN PCWSTR lpFileName, IN PCWSTR lpExtension OPTIONAL, IN ULONG nBufferLength,
                   OUT PWSTR lpBuffer, OUT PWSTR *lpFilePart)
...

{
    ...
    if (RtlDetermineDosPathNameType_U(lpFileName) != RtlPathTypeRelative)
    {
        ...
        {
            PathLength = RtlGetFullPathName_U(lpFileName, nBufferLength, lpBuffer, lpFilePart);
            return PathLength;
        }
        else
        {
    ...
    ComputedFileName =
        RtlAllocateHeap(RtlProcessHeap(), 0, PathLength + FileLength + ExtensionLength + 3 * sizeof(UNICODE_NULL));
    ...
    do
    {
        ...
        if (Cursor != ComputedFileName && Cursor[-1] != L'\\')
        {
            *Cursor++ = L'\\';
        }
        ...
        {
            PathLength = RtlGetFullPathName_U(ComputedFileName, nBufferLength, lpBuffer, lpFilePart);
...
NTSTATUS
RtlDosSearchPath_Ustr(IN ULONG Flags, IN PCUNICODE_STRING Path, IN PCUNICODE_STRING FileName,
                      IN PCUNICODE_STRING DefaultExtension OPTIONAL, OUT PUNICODE_STRING StaticString OPTIONAL,
                      OUT PUNICODE_STRING DynamicString OPTIONAL, OUT PCUNICODE_STRING *FullFileNameOut OPTIONAL,
...

{
    ...
    if ((Flags & RTL_DOS_SEARCH_PATH_FLAG_DISALLOW_DOT_RELATIVE_PATH_SEARCH) && (PathType == RtlPathTypeRelative))
    {
        if (FileName->Length >= (2 * sizeof(WCHAR)))
        {
            if (FileName->Buffer[0] == L'.')
            {
                ...
                else if ((FileName->Buffer[1] == L'.') && (FileName->Length >= (3 * sizeof(WCHAR))) &&
    ...
    if (PathType != RtlPathTypeRelative)
    {
        ...
        {
            Status = RtlGetFullPathName_UstrEx(RTL_CONST_CAST(PUNICODE_STRING)(FileName), StaticString, DynamicString,
                                               (PUNICODE_STRING *)FullFileNameOut, FilePartPrefixCch, NULL, &PathType,
                                               BytesRequired);
            if (!NT_SUCCESS(Status))
            {
#if DBG
...
//                DbgPrint("%s: Failing because RtlDoesFileExists_UstrEx() on %wZ says it does not exist and there is no default extension to apply\n", __FUNCTION__, FileName);
            ...
            RtlCopyMemory(CandidateString.Buffer + (FileName->Length / sizeof(WCHAR)), DefaultExtension->Buffer,
            ...
            Status = RtlGetFullPathName_UstrEx(&CandidateString, StaticString, DynamicString,
                                               (PUNICODE_STRING *)FullFileNameOut, FilePartPrefixCch, NULL, &PathType,
                                               BytesRequired);
            if (!NT_SUCCESS(Status))
            {
    ...
    if (Flags & RTL_DOS_SEARCH_PATH_FLAG_APPLY_ISOLATION_REDIRECTION)
    {
        ...
        Status = RtlDosApplyFileIsolationRedirection_Ustr(RTL_DOS_APPLY_FILE_REDIRECTION_USTR_FLAG_RESPECT_DOT_LOCAL,
                                                          FileName, DefaultExtension, StaticString, DynamicString,
                                                          &FullPathStringFound, NULL, FilePartPrefixCch, BytesRequired);
        if (NT_SUCCESS(Status))
        {
            if (FullFileNameOut != NULL)
            {
    ...
    BiggestPossibleFileName = MaximumPathSegmentLength + FileName->Length + DefaultExtensionLength +
    ...
    while (Cursor != EndMarker)
    {
...
#if DBG
                DbgPrint(
                    "%s: internal error #2; SegmentSize = %u, FileName->Length = %u, DefaultExtensionLength = %u\n",
                    __FUNCTION__, SegmentSize, FileName->Length, DefaultExtensionLength);
#endif // DBG
                Status = STATUS_INTERNAL_ERROR;
                goto Exit;
            }
        ...
        RtlCopyMemory(CandidateString.Buffer, Cursor, BytesToCopy);
        ...
        RtlCopyMemory(BufferToFillCursor, FileName->Buffer, FileName->Length);
        BufferToFillCursor += (FileName->Length / sizeof(WCHAR));

        if (DefaultExtension != NULL)
        {
            RtlCopyMemory(BufferToFillCursor, DefaultExtension->Buffer, DefaultExtension->Length);
        ...
        *BufferToFillCursor = UNICODE_NULL;
        ...
        if (RtlDoesFileExists_UEx(CandidateString.Buffer, FALSE))
        {
            ...
            else
            {
                ...
                if ((Status != STATUS_NO_SUCH_FILE) && (Status != STATUS_BUFFER_TOO_SMALL))
                {
                    DbgPrint("%s: Failing because we thought we found %wZ on the search path, but "
                             "RtlGetFullPathName_UstrEx() returned %08lx\n",
                             __FUNCTION__, FileName, Status);
                }
#endif // DBG
            }
...
Exit:
    if ((CandidateString.Buffer != NULL) && (CandidateString.Buffer != StaticCandidateBuffer))
    {
...

{
    ...
    Status = RtlQueryEnvironmentVariable_U(NULL, &EnvVarName, &EnvValue);
    if (!NT_SUCCESS(Status))
    {
    ...
    InitializeObjectAttributes(&Obja, &EnvValue, OBJ_CASE_INSENSITIVE, NULL, NULL);
    ...
    Status = NtOpenFile(&DirHandle, SYNCHRONIZE, &Obja, &IoStatusBlock, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
NTSTATUS
RtlDosApplyFileIsolationRedirection_Ustr(ULONG Flags, PCUNICODE_STRING FileNameIn, PCUNICODE_STRING DefaultExtension,
                                         PUNICODE_STRING PreAllocatedString, PUNICODE_STRING DynamicallyAllocatedString,
                                         PUNICODE_STRING *FullPath, ULONG *OutFlags, SIZE_T *FilePartPrefixCch,
                                         SIZE_T *BytesRequired)
{
...
#if DBG_SXS
    DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_TRACE_LEVEL, "SXS: Entered %s\n", __FUNCTION__);
#endif // DBG_SXS
    ...
    if (((Flags & ~(RTL_DOS_APPLY_FILE_REDIRECTION_USTR_FLAG_RESPECT_DOT_LOCAL)) != 0) || (FileNameIn == NULL) ||
        ((PreAllocatedString == NULL) && (DynamicallyAllocatedString == NULL) && (FilePartPrefixCch != NULL)) ||
        ((PreAllocatedString != NULL) && (DynamicallyAllocatedString != NULL) && (FullPath == NULL)))
    {
        ...
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   FullPath = %p\n"
                   "   FilePartPrefixCch = %p\n",
                   __FUNCTION__, FileNameIn, PreAllocatedString, DynamicallyAllocatedString, FullPath,
                   FilePartPrefixCch);
#endif // DBG
    ...
    if (PathType != RtlPathTypeRelative)
    {
        ...
        if ((PathType == RtlPathTypeDriveAbsolute) || (PathType == RtlPathTypeUncAbsolute))
        {
            ...
            if ((pPeb != NULL) && (pPeb->ProcessParameters != NULL) &&
                (pPeb->ProcessParameters->Flags & RTL_USER_PROC_DLL_REDIRECTION_LOCAL)) // there is .local
            {
                ...
                t = FileNameIn->Buffer + FileNameIn->Length / sizeof(WCHAR) -
                    1; // point to last char of this full-qualified filename
                t1 = NULL;
...
DoAllWork:
    ...
    if ((DefaultExtension != NULL) && (DefaultExtension->Length != 0))
    {
        ...
        if (DefaultExtension != NULL)
        {
            ...
            RtlCopyMemory(FileNameWithExtension.Buffer, FileNameIn->Buffer, FileNameIn->Length);
            RtlCopyMemory(FileNameWithExtension.Buffer + (FileNameIn->Length / sizeof(WCHAR)), DefaultExtension->Buffer,
                          DefaultExtension->Length);
            FileNameWithExtension.Buffer[(FileNameIn->Length + DefaultExtension->Length) / sizeof(WCHAR)] =
                UNICODE_NULL;
    ...
    Status = RtlFindActivationContextSectionString(FIND_ACTIVATION_CONTEXT_SECTION_KEY_RETURN_ACTIVATION_CONTEXT |
                                                       FIND_ACTIVATION_CONTEXT_SECTION_KEY_RETURN_FLAGS,
                                                   NULL, ACTIVATION_CONTEXT_SECTION_DLL_REDIRECTION, FileName, &askd);
    if (!NT_SUCCESS(Status))
    {
        ...
        Status = RtlGetAssemblyStorageRoot(
            ((askd.Flags & ACTIVATION_CONTEXT_SECTION_KEYED_DATA_FLAG_FOUND_IN_PROCESS_DEFAULT)
                ...
                ((askd.Flags & ACTIVATION_CONTEXT_SECTION_KEYED_DATA_FLAG_FOUND_IN_SYSTEM_DEFAULT)
                     ...
                     : 0),
            ActivationContext, askd.AssemblyRosterIndex, &AssemblyStorageRoot,
            &RtlpAssemblyStorageMapResolutionDefaultCallback, (PVOID)&InnerStatus);
        if (NT_ERROR(Status))
        {
    ...
    if ((PreAllocatedString == NULL) || (PreAllocatedString->MaximumLength < (RealTotalPathLength + sizeof(WCHAR))))
    {
    ...
    if ((((ULONG)DllRedirData->PathSegmentOffset) > askd.SectionTotalLength) ||
        (((ULONG)(DllRedirData->PathSegmentOffset +
                  (DllRedirData->PathSegmentCount * sizeof(ACTIVATION_CONTEXT_DATA_DLL_REDIRECTION_PATH_SEGMENT)))) >
         askd.SectionTotalLength))
    {
#if DBG
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: %s - Path segment array extends beyond section limits\n",
                   __FUNCTION__);
#endif // DBG
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    ...
    PathSegmentArray = (PCACTIVATION_CONTEXT_DATA_DLL_REDIRECTION_PATH_SEGMENT)(((ULONG_PTR)askd.SectionBase) +
                                                                                DllRedirData->PathSegmentOffset);
    ...
    for (i = 0; i < DllRedirData->PathSegmentCount; i++)
    {
        ...
        {
#if DBG
            DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: %s - path segment %lu overflowed buffer\n",
                       __FUNCTION__, i);
#endif // DBG
            Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
            goto Exit;
...
#if DBG
            DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                       "SXS: %s - path segment %lu at %p (offset: %ld, length: %lu, bounds: %lu) buffer is outside "
                       "section bounds\n",
                       __FUNCTION__, i, &PathSegmentArray[i].Offset, PathSegmentArray[i].Offset,
                       PathSegmentArray[i].Length, askd.SectionTotalLength);
#endif // DBG
            Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
            goto Exit;
        }

        RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)askd.SectionBase) + PathSegmentArray[i].Offset),
    ...
    if (!(DllRedirData->Flags & ACTIVATION_CONTEXT_DATA_DLL_REDIRECTION_PATH_INCLUDES_BASE_NAME))
    {
        ...
        {
#if DBG
            DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: %s - appending base file name overflowed buffer\n",
                       __FUNCTION__);
#endif // DBG
            Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
            goto Exit;
        ...
        RtlCopyMemory(Cursor, FileName->Buffer, FileName->Length);
    ...
    *Cursor = L'\0';
    ...
    if (DllRedirData->Flags & ACTIVATION_CONTEXT_DATA_DLL_REDIRECTION_PATH_EXPAND)
    {
        ...
        if (NT_ERROR(Status))
        {
            ...
            if ((PreAllocatedString != NULL) &&
                (PreAllocatedString->MaximumLength >= ExpandedTemporaryString.MaximumLength))
            {
                // It does.  Just copy the bits and let's go.
                RtlCopyMemory(PreAllocatedString->Buffer, ExpandedTemporaryString.Buffer,
                              ExpandedTemporaryString
                                  .MaximumLength); // MaximumLength because it includes the trailing null character
        ...
        else
        {
            ...
            if ((PreAllocatedString != NULL) &&
                ((ExpandedTemporaryString.Length + sizeof(WCHAR)) < PreAllocatedString->MaximumLength))
            {
                // It fits!
                RtlCopyMemory(PreAllocatedString->Buffer, ExpandedTemporaryBuffer,
            ...
            else
            {
                ...
                RtlCopyMemory(ExpandedTemporaryString.Buffer, ExpandedTemporaryBuffer,
...
NTSTATUS
NTAPI
RtlpGetLengthWithoutLastPathElement(IN ULONG Flags, IN ULONG PathType, IN PCUNICODE_STRING Path, OUT ULONG *LengthOut)
/*++
...
--*/
{
    ...
    ULONG AllowedDosPathTypeBits = (1UL << RtlPathTypeRooted) | (1UL << RtlPathTypeUncAbsolute) |
                                   (1UL << RtlPathTypeDriveAbsolute) | (1UL << RtlPathTypeLocalDevice) // "\\?\"
...
#define LOCAL_IS_PATH_SEPERATOR(ch_) ((ch_) == PathSeperators[0] || (ch_) == PathSeperators[1])
    ...
    if (!RTL_SOFT_VERIFY(Path != NULL) || !RTL_SOFT_VERIFY(Flags == 0) || !RTL_SOFT_VERIFY(LengthOut != NULL))
    {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    if (!RTL_SOFT_VERIFY(
            NT_SUCCESS(Status = RtlpDetermineDosPathNameType4(RTL_DETERMINE_DOS_PATH_NAME_TYPE_IN_FLAG_STRICT_WIN32NT,
                                                              Path, &DosPathType, &DosPathFlags))))
    {
        goto Exit;
    }
    ...
    if (!RTL_SOFT_VERIFY((PathType & RTLP_DETERMINE_DOS_PATH_NAME_TYPE_OUT_FLAG_INVALID) == 0))
    {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
...
NTSTATUS
NTAPI
RtlGetLengthWithoutLastNtPathElement(IN ULONG Flags, IN PCUNICODE_STRING Path, OUT ULONG *LengthOut)
/*++
...
NTSTATUS
NTAPI
RtlGetLengthWithoutLastFullDosOrNtPathElement(IN ULONG Flags, IN PCUNICODE_STRING Path, OUT ULONG *LengthOut)
/*++
...
{
    NTSTATUS Status =
        RtlpGetLengthWithoutLastPathElement(Flags, RTLP_LAST_PATH_ELEMENT_PATH_TYPE_FULL_DOS_OR_NT, Path, LengthOut);
    return Status;
}

CONST CHAR *RtlpDbgBadDosRootPathTypeToString(IN ULONG Flags, IN ULONG RootType)
/*++
...
--*/
{
    ...
    switch (RootType)
    {
    case RTLP_GOOD_DOS_ROOT_PATH:
        s = "good";
        break;
    case RTLP_BAD_DOS_ROOT_PATH_WIN32NT_PREFIX:
        s = "\\\\?\\";
        break;
    case RTLP_BAD_DOS_ROOT_PATH_WIN32NT_UNC_PREFIX:
        s = "\\\\?\\unc";
        break;
    case RTLP_BAD_DOS_ROOT_PATH_NT_PATH:
        s = "\\??\\";
        break;
    case RTLP_BAD_DOS_ROOT_PATH_MACHINE_NO_SHARE:
        s = "\\\\machine or \\\\?\\unc\\machine";
        break;
    default:
        s = "unknown";
...
NTSTATUS
RtlpCheckForBadDosRootPath(IN ULONG Flags, IN PCUNICODE_STRING Path, OUT ULONG *RootType)
/*++
...
--*/
{
    ...
    if (!RTL_SOFT_VERIFY(Path != NULL) || !RTL_SOFT_VERIFY(RootType != NULL) || !RTL_SOFT_VERIFY(Flags == 0))
    {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    if (RtlPrefixUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpDosDevicesPrefix),
                               RTL_CONST_CAST(PUNICODE_STRING)(Path), TRUE))
    {
    ...
    Unc = (Unc1 = RtlPrefixUnicodeString(RTL_CONST_CAST(PUNICODE_STRING)(&RtlpWin32NtUncRootSlash),
                                         RTL_CONST_CAST(PUNICODE_STRING)(Path), TRUE)) ||
...
NTSTATUS
NTAPI
RtlpBadDosRootPathToEmptyString(IN ULONG Flags, IN OUT PUNICODE_STRING Path)
/*++
...
NTSTATUS
NTAPI
RtlGetLengthWithoutLastFullDosPathElement(IN ULONG Flags, IN PCUNICODE_STRING Path, OUT ULONG *LengthOut)
/*++
...
--*/
{
    ...
    *LengthOut = RTL_STRING_GET_LENGTH_CHARS(&CheckRootString);
    ...
    Status = STATUS_SUCCESS;
Exit:
    KdPrintEx((DPFLTR_SXS_ID, DPFLTR_LEVEL_STATUS(Status), "%s(%d):%s(%wZ): 0x%08lx\n", __FILE__, __LINE__,
...
NTSTATUS
NTAPI
RtlAppendPathElement(IN ULONG Flags, IN OUT PRTL_UNICODE_STRING_BUFFER Path, PCUNICODE_STRING ConstElement)
/*++
...
#define LOCAL_IS_PATH_SEPERATOR(ch_) ((ch_) == PathSeperators[0] || (ch_) == PathSeperators[1])
    ...
    if (!RTL_SOFT_VERIFY((Flags & ~(RTL_APPEND_PATH_ELEMENT_ONLY_BACKSLASH_IS_SEPERATOR)) == 0) ||
        !RTL_SOFT_VERIFY(Path != NULL) || !RTL_SOFT_VERIFY(ConstElement != NULL))
    {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    Status = STATUS_SUCCESS;
Exit:
    KdPrintEx((DPFLTR_SXS_ID, DPFLTR_LEVEL_STATUS(Status), "%s(%d):%s(%wZ, %wZ): 0x%08lx\n", __FILE__, __LINE__,
               __FUNCTION__, Path ? &Path->String : NULL, ConstElement, Status));
    return Status;
#undef LOCAL_IS_PATH_SEPERATOR
}
...
NTSTATUS
NTAPI
RtlGetLengthWithoutTrailingPathSeperators(IN ULONG Flags, IN PCUNICODE_STRING Path, OUT ULONG *LengthOut)
/*++
...
--*/
{
    ...
    if (!RTL_SOFT_VERIFY(Path != NULL) || !RTL_SOFT_VERIFY(LengthOut != NULL) || !RTL_SOFT_VERIFY(Flags == 0))
    {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
...
NTSTATUS
NTAPI
RtlpApplyLengthFunction(IN ULONG Flags, IN SIZE_T SizeOfStruct, IN OUT PVOID UnicodeStringOrUnicodeStringBuffer,
...
Exit:
    KdPrintEx((DPFLTR_SXS_ID, DPFLTR_LEVEL_STATUS(Status), "%s(%d):%s(%wZ): 0x%08lx\n", __FILE__, __LINE__,
               __FUNCTION__, UnicodeString, Status));
    return Status;
}

NTSTATUS
NTAPI
RtlNtPathNameToDosPathName(IN ULONG Flags, IN OUT PRTL_UNICODE_STRING_BUFFER Path, OUT ULONG *Disposition OPTIONAL,
                           IN OUT PWSTR *FilePart OPTIONAL)
{
    ...
    if (!RTL_SOFT_VERIFY(Path != NULL) || !RTL_SOFT_VERIFY(Flags == 0))
    {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    Cch = RTL_STRING_GET_LENGTH_CHARS(&Path->String) + RTL_STRING_GET_LENGTH_CHARS(DosPrefix) -
          RTL_STRING_GET_LENGTH_CHARS(NtPrefix);

    Status = RtlEnsureUnicodeStringBufferSizeChars(Path, Cch);
    if (!NT_SUCCESS(Status))
    {
        goto Exit;
    }
    ...
    //
    RtlMoveMemory(Path->String.Buffer + RTL_STRING_GET_LENGTH_CHARS(DosPrefix),
                  Path->String.Buffer + RTL_STRING_GET_LENGTH_CHARS(NtPrefix), Path->String.Length - NtPrefix->Length);
    RtlMoveMemory(Path->String.Buffer, DosPrefix->Buffer, DosPrefix->Length);
    ...
    {
        // review/test..
        *FilePart = Path->String.Buffer +
                    (NtFilePartOffset - RTL_STRING_GET_LENGTH_CHARS(NtPrefix) + RTL_STRING_GET_LENGTH_CHARS(DosPrefix));
    }
    Status = STATUS_SUCCESS;
Exit:
    KdPrintEx((DPFLTR_SXS_ID, DPFLTR_LEVEL_STATUS(Status), "%s(%d):%s(%wZ): 0x%08lx\n", __FILE__, __LINE__,
