...
VOID RtlpLocateActivationContextSectionForQuery(OUT PULONG Disposition, OUT NTSTATUS *Status, PVOID Buffer,
                                                SIZE_T InLength, PSIZE_T OutLength OPTIONAL, SIZE_T MinimumLength,
                                                IN PCACTIVATION_CONTEXT_DATA ActivationContextData,
                                                IN const GUID *ExtensionGuid OPTIONAL, IN ULONG Id,
                                                OUT VOID CONST **SectionData, OUT ULONG *SectionLength)
{
...
#define RTLP_LOCATE_ACTIVATION_CONTEXT_SECTION_FOR_QUERY_RETURN (2)
    ...
    {
        *Status =
            RtlpLocateActivationContextSection(ActivationContextData, ExtensionGuid, Id, SectionData, SectionLength);
...
NTSTATUS
RtlpCrackActivationContextStringSectionHeader(IN PCVOID SectionBase, IN SIZE_T SectionLength,
                                              ...
                                              OUT ULONG *HashAlgorithm OPTIONAL, OUT PCVOID *SearchStructure OPTIONAL,
                                              OUT ULONG *UserDataLength OPTIONAL, OUT PCVOID *UserData OPTIONAL)
{
    ...
    if (SectionLength < (RTL_SIZEOF_THROUGH_FIELD(ACTIVATION_CONTEXT_STRING_SECTION_HEADER, HeaderSize)))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   "SXS: %s() passed string section at %p only %lu bytes long; that's not even enough for the 4-byte "
                   "magic and 4-byte header length!\n",
                   __FUNCTION__, SectionBase, SectionLength);
    ...
    if (Header->Magic != ACTIVATION_CONTEXT_STRING_SECTION_MAGIC)
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   Expected %lu; got %lu\n",
                   __FUNCTION__, ACTIVATION_CONTEXT_STRING_SECTION_MAGIC, Header->Magic);
    ...
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   "SXS: %s() passed string section at %p claims %lu byte header size; that doesn't even include the "
                   "HeaderSize member!\n",
                   __FUNCTION__, Header, Header->HeaderSize);
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }
    ...
    if ((Header->ElementListOffset != 0) && (Header->ElementListOffset < Header->HeaderSize))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   __FUNCTION__, Header, Header->HeaderSize, Header->ElementListOffset);
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }

    if ((Header->SearchStructureOffset != 0) && (Header->SearchStructureOffset < Header->HeaderSize))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   __FUNCTION__, Header, Header->HeaderSize, Header->SearchStructureOffset);
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }

    if ((Header->UserDataOffset != 0) && (Header->UserDataOffset < Header->HeaderSize))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   Header Size: %lu\n"
                   "   User Data Offset: %lu\n",
                   __FUNCTION__, Header, Header->HeaderSize, Header->UserDataOffset);
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }
    ...
    if (Header->UserDataSize < sizeof(ACTIVATION_CONTEXT_DATA_ASSEMBLY_GLOBAL_INFORMATION))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   __FUNCTION__, Header, Header->UserDataSize,
    ...
    if ((Header->UserDataOffset + Header->UserDataSize) > SectionLength)
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   UserDataOffset: %lu\n"
                   "   Section size: %lu\n",
                   __FUNCTION__, Header, Header->UserDataSize, Header->UserDataOffset, SectionLength);
...
// This is never used.
{
    ...
    if (((InLength != 0) && (OutBuffer == NULL)) || ((OutBuffer == NULL) && (OutLength == NULL)))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   OutBuffer = %p\n"
                   "   OutLength = %p\n",
                   __FUNCTION__, InLength, OutBuffer, OutLength);
    ...
    Status = RtlpLocateActivationContextSection(ActivationContextData, NULL,
                                                ACTIVATION_CONTEXT_SECTION_ASSEMBLY_INFORMATION, &pvTemp, &ulTemp);
    if (!NT_SUCCESS(Status))
        goto Exit;
    ...
    if (ulTemp < sizeof(ACTIVATION_CONTEXT_STRING_SECTION_HEADER))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   __FUNCTION__, sizeof(ACTIVATION_CONTEXT_STRING_SECTION_HEADER), ulTemp);
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }
    ...
    if (Header->UserDataOffset < sizeof(ACTIVATION_CONTEXT_STRING_SECTION_HEADER))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   Section header: %p\n"
                   "   User Data Offset: %lu\n",
                   __FUNCTION__, Header, Header->UserDataOffset);
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }
    ...
    if ((Header->UserDataOffset + Header->UserDataSize) > ulTemp)
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   UserDataOffset: %lu\n"
                   "   Section size: %lu\n",
                   __FUNCTION__, Header, Header->UserDataSize, Header->UserDataOffset, ulTemp);
    ...
    if (GlobalInfo->Size < sizeof(ACTIVATION_CONTEXT_DATA_ASSEMBLY_GLOBAL_INFORMATION))
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   "   Global Info Size: %lu\n"
                   "   Structure size: %lu\n",
                   __FUNCTION__, Header, GlobalInfo, GlobalInfo->Size,
                   sizeof(ACTIVATION_CONTEXT_DATA_ASSEMBLY_GLOBAL_INFORMATION));
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }
    ...
    if (GlobalInfo->ApplicationDirectoryOffset != 0)
    {
        if (GlobalInfo->ApplicationDirectoryOffset < sizeof(ACTIVATION_CONTEXT_DATA_ASSEMBLY_GLOBAL_INFORMATION))
        {
            DbgPrintEx(
                ...
                __FUNCTION__, Header, GlobalInfo, GlobalInfo->ApplicationDirectoryOffset);
            Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
            goto Exit;
        }
        ...
        {
            DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                       "SXS: %s() found assembly information section global data with app dir extending beyond end of "
                       "global data\n"
                       "   Section header: %p\n"
                       "   GlobalInfo: %p\n"
                       "   ApplicationDirectoryOffset: %lu\n"
                       ...
                       "   GlobalInfo size: %lu\n",
                       __FUNCTION__, Header, GlobalInfo, GlobalInfo->ApplicationDirectoryOffset,
                       GlobalInfo->ApplicationDirectoryLength, GlobalInfo->Size);
            Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
            goto Exit;
        }
        ...
        RtlCopyMemory(OutBuffer, (PVOID)(((ULONG_PTR)GlobalInfo) + GlobalInfo->ApplicationDirectoryOffset),
...
#define RTLP_QUERY_INFORMATION_ACTIVATION_CONTEXT_BASIC_INFORMATION_FLAG_NO_ADDREF (0x00000001)

NTSTATUS
RtlpQueryInformationActivationContextBasicInformation(IN ULONG Flags, IN PCACTIVATION_CONTEXT ConstActivationContext,
                                                      IN PCACTIVATION_CONTEXT_DATA ActivationContextData,
                                                      IN ULONG SubInstanceIndex, OUT PVOID Buffer, IN SIZE_T InLength,
                                                      OUT PSIZE_T OutLength OPTIONAL)
{
    ...
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: %s() received invalid non-zero sub-instance index %lu\n",
                   __FUNCTION__, SubInstanceIndex);
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
...
NTSTATUS
RtlpQueryInformationActivationContextDetailedInformation(PCACTIVATION_CONTEXT_DATA ActivationContextData,
                                                         ULONG SubInstanceIndex, OUT PVOID Buffer, IN SIZE_T InLength,
                                                         OUT PSIZE_T OutLength OPTIONAL)
{
    NTSTATUS Status = STATUS_INTERNAL_ERROR;
    ...
    // We need to find the assembly metadata section...
    RtlpLocateActivationContextSectionForQuery(&RtlpLocateActivationContextSectionForQueryDisposition, &Status, Buffer,
                                               InLength, OutLength, sizeof(*Info), ActivationContextData, NULL,
                                               ACTIVATION_CONTEXT_SECTION_ASSEMBLY_INFORMATION, &StringSectionHeader,
                                               &StringSectionSize);
    ...
    Status =
        RtlpCrackActivationContextStringSectionHeader(StringSectionHeader, StringSectionSize, NULL, &DataFormatVersion,
                                                      NULL, NULL, NULL, NULL, NULL, &UserDataSize, &UserData);
    if (!NT_SUCCESS(Status))
        goto Exit;
    ...
    AssemblyRosterEntryList = (PCACTIVATION_CONTEXT_DATA_ASSEMBLY_ROSTER_ENTRY)(((ULONG_PTR)ActivationContextData) +
    ...
    if (i == EntryCount)
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
                   ...
                   __FUNCTION__, ActivationContextData);
        Status = STATUS_SXS_INVALID_ACTCTXDATA_FORMAT;
        goto Exit;
    }

    RootAssemblyInformation =
        (PCACTIVATION_CONTEXT_DATA_ASSEMBLY_INFORMATION)(((ULONG_PTR)ActivationContextData) +
                                                         AssemblyRosterEntryList[i].AssemblyInformationOffset);
    ...
    if (RootAssemblyInformation->ManifestPathLength != 0)
    {
        RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)StringSectionHeader) + RootAssemblyInformation->ManifestPathOffset),
    ...
    if (RootAssemblyInformation->PolicyPathLength != 0)
    {
        RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)StringSectionHeader) + RootAssemblyInformation->PolicyPathOffset),
...
NTSTATUS
RtlpQueryAssemblyInformationActivationContextDetailedInformation(PCACTIVATION_CONTEXT_DATA ActivationContextData,
                                                                 ULONG SubInstanceIndex, // 0-based index of assembly
                                                                 OUT PVOID Buffer, IN SIZE_T InLength,
                                                                 OUT PSIZE_T OutLength OPTIONAL)
{
    NTSTATUS Status = STATUS_INTERNAL_ERROR;
    SIZE_T BytesNeeded = 0;
    ...
    if (SubInstanceIndex > AssemblyRosterHeader->EntryCount) // AssemblyRosterHeader->EntryCount is 1-based,
    {
        DbgPrintEx(
            ...
            __FUNCTION__, SubInstanceIndex, AssemblyRosterHeader->EntryCount);
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }

    AssemlbyDataInfo = (PACTIVATION_CONTEXT_DATA_ASSEMBLY_INFORMATION)((ULONG_PTR)ActivationContextData +
                                                                       AssemblyRosterEntryList[SubInstanceIndex]
                                                                           .AssemblyInformationOffset);

    // We need to find the assembly metadata section...
    RtlpLocateActivationContextSectionForQuery(
        &RtlpLocateActivationContextSectionForQueryDisposition, &Status, Buffer, InLength, OutLength,
        sizeof(ACTIVATION_CONTEXT_ASSEMBLY_DETAILED_INFORMATION), ActivationContextData, NULL,
        ACTIVATION_CONTEXT_SECTION_ASSEMBLY_INFORMATION, &StringSectionHeader, &StringSectionSize);
    switch (RtlpLocateActivationContextSectionForQueryDisposition)
    {
    case RTLP_LOCATE_ACTIVATION_CONTEXT_SECTION_FOR_QUERY_RETURN:
        goto Exit;
    ...
    if (AssemlbyDataInfo->ManifestPathLength != 0)
    {
        RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)StringSectionHeader) + AssemlbyDataInfo->ManifestPathOffset),
    ...
    if (AssemlbyDataInfo->PolicyPathLength != 0)
    {
        RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)StringSectionHeader) + AssemlbyDataInfo->PolicyPathOffset),
    ...
    if (AssemlbyDataInfo->AssemblyDirectoryNameLength != 0)
    {
        RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)StringSectionHeader) + AssemlbyDataInfo->AssemblyDirectoryNameOffset),
...
RtlpQueryFilesInAssemblyInformationActivationContextDetailedInformation(
    PCACTIVATION_CONTEXT_DATA ActivationContextData, PCACTIVATION_CONTEXT_QUERY_INDEX SubInstanceIndex,
    OUT PVOID Buffer, IN SIZE_T InLength, OUT PSIZE_T OutLength OPTIONAL)
{
    NTSTATUS Status = STATUS_INTERNAL_ERROR;
    SIZE_T BytesNeeded = 0;
    ...
    if (SubInstanceIndex->ulAssemblyIndex >=
        AssemblyRosterHeader->EntryCount - 1) // AssemblyRosterHeader->EntryCount is 1-based,
    {
        DbgPrintEx(
            DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL,
            "SXS: %s() received invalid sub-instance index %lu out of %lu Assemblies in the Acitvation Context\n",
            __FUNCTION__, SubInstanceIndex->ulAssemblyIndex, AssemblyRosterHeader->EntryCount);
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    RtlpLocateActivationContextSectionForQuery(
        &RtlpLocateActivationContextSectionForQueryDisposition, &Status, Buffer, InLength, OutLength,
        sizeof(ACTIVATION_CONTEXT_ASSEMBLY_DETAILED_INFORMATION), ActivationContextData, NULL,
        ACTIVATION_CONTEXT_SECTION_DLL_REDIRECTION, &StringSectionHeader, &StringSectionSize);
    switch (RtlpLocateActivationContextSectionForQueryDisposition)
    {
    case RTLP_LOCATE_ACTIVATION_CONTEXT_SECTION_FOR_QUERY_RETURN:
        goto Exit;
    ...
    {
        DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: %s() received invalid file index (%d) in Assembly (%d)\n",
                   __FUNCTION__, SubInstanceIndex->ulFileIndexInAssembly, SubInstanceIndex->ulAssemblyIndex,
    ...
    if (StringSectionHeader->ElementListOffset != 0)
        ElementList = (PCACTIVATION_CONTEXT_STRING_SECTION_ENTRY)(((ULONG_PTR)StringSectionHeader) +
                                                                  StringSectionHeader->ElementListOffset);
    else
    {
        ...
        if (ElementList[i].AssemblyRosterIndex == SubInstanceIndex->ulAssemblyIndex + 1)
        {
            ...
            if (CounterForFilesFoundInSpecifiedAssembly == SubInstanceIndex->ulFileIndexInAssembly)
            {
                if (ElementList[i].Offset != 0)
                {
                    ...
                    EntryData = (PCACTIVATION_CONTEXT_DATA_DLL_REDIRECTION)(((ULONG_PTR)StringSectionHeader) +
    ...
    if (ElementList[i].KeyLength != 0)
    {
        RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)StringSectionHeader) + ElementList[i].KeyOffset),
    ...
    if (EntryData->TotalPathLength != 0)
    {
        if (EntryData->PathSegmentOffset != 0)
            PathSegments = (PCACTIVATION_CONTEXT_DATA_DLL_REDIRECTION_PATH_SEGMENT)(StringSectionHeader +
                                                                                    EntryData->PathSegmentOffset);
        ...
        if (PathSegments != NULL)
        {
            ...
            for (i = 0; i < EntryData->PathSegmentCount; i++)
            {
                ...
                {
                    RtlCopyMemory(Cursor, (PVOID)(((ULONG_PTR)StringSectionHeader) + PathSegments[i].Offset),
...
NTSTATUS
NTAPI
RtlQueryInformationActivationContext(IN ULONG Flags, IN PCACTIVATION_CONTEXT ActivationContext,
                                     IN PVOID SubInstanceIndex, IN ACTIVATION_CONTEXT_INFO_CLASS InfoClass,
                                     OUT PVOID Buffer, IN SIZE_T InLength, OUT PSIZE_T OutLength OPTIONAL)
{
    NTSTATUS Status = STATUS_INTERNAL_ERROR;
    ...
    __try
    {
        ...
        if ((Flags & ~(RTL_QUERY_INFORMATION_ACTIVATION_CONTEXT_FLAG_USE_ACTIVE_ACTIVATION_CONTEXT |
                       ...
                       RTL_QUERY_INFORMATION_ACTIVATION_CONTEXT_FLAG_NO_ADDREF)) != 0)
        {
        ...
        switch (Flags & (RTL_QUERY_INFORMATION_ACTIVATION_CONTEXT_FLAG_USE_ACTIVE_ACTIVATION_CONTEXT |
                         RTL_QUERY_INFORMATION_ACTIVATION_CONTEXT_FLAG_ACTIVATION_CONTEXT_IS_MODULE |
                         RTL_QUERY_INFORMATION_ACTIVATION_CONTEXT_FLAG_ACTIVATION_CONTEXT_IS_ADDRESS))
        {
        ...
        case RTL_QUERY_INFORMATION_ACTIVATION_CONTEXT_FLAG_ACTIVATION_CONTEXT_IS_MODULE:
        {
            ...
            {
                DbgPrintEx(DPFLTR_SXS_ID, DPFLTR_ERROR_LEVEL, "SXS: %s() - Caller passed invalid hmodule (%p)\n",
                           __FUNCTION__, ActivationContext);
                Status = STATUS_DLL_NOT_FOUND; // REVIEW
                goto Exit;
            }
        ...
        Status = RtlpGetActivationContextData(RTLP_GET_ACTIVATION_CONTEXT_DATA_MAP_NULL_TO_EMPTY, ActivationContext,
                                              NULL, &ActivationContextData);
        if (!NT_SUCCESS(Status))
            goto Exit;
        ...
        case ActivationContextBasicInformation:
        {
            ...
            Status = RtlpQueryInformationActivationContextBasicInformation(
                BasicInfoFlags, ActivationContext, ActivationContextData, 0, Buffer, InLength, OutLength);
            if (!NT_SUCCESS(Status))
                goto Exit;
        }
        break;
        ...
        case ActivationContextDetailedInformation:
            Status = RtlpQueryInformationActivationContextDetailedInformation(ActivationContextData, 0, Buffer,
                                                                              InLength, OutLength);
            if (!NT_SUCCESS(Status))
                goto Exit;
            break;
        case AssemblyDetailedInformationInActivationContxt:
            ...
            Status = RtlpQueryAssemblyInformationActivationContextDetailedInformation(
                ActivationContextData, *((ULONG *)SubInstanceIndex), Buffer, InLength, OutLength);
            if (!NT_SUCCESS(Status))
                goto Exit;
            break;
        case FileInformationInAssemblyOfAssemblyInActivationContxt:
            Status = RtlpQueryFilesInAssemblyInformationActivationContextDetailedInformation(
                ActivationContextData, ((ACTIVATION_CONTEXT_QUERY_INDEX *)SubInstanceIndex), Buffer, InLength,
                OutLength);
            if (!NT_SUCCESS(Status))
                goto Exit;
            break;
...
NTSTATUS
NTAPI
RtlQueryInformationActiveActivationContext(IN ACTIVATION_CONTEXT_INFO_CLASS InfoClass, OUT PVOID OutBuffer,
                                           IN SIZE_T InLength, OUT PSIZE_T OutLength OPTIONAL)
{
    return RtlQueryInformationActivationContext(
        RTL_QUERY_INFORMATION_ACTIVATION_CONTEXT_FLAG_USE_ACTIVE_ACTIVATION_CONTEXT, NULL, 0, InfoClass, OutBuffer,
        InLength, OutLength);
}
