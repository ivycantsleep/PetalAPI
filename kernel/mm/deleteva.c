...
VOID MiInitUseCounts(VOID)
{
    MiTrackUse = ExAllocatePoolWithTag(NonPagedPool, MiTrackUseSize * sizeof(MI_TRACK_USE), 'lqrI');
    ASSERT(MiTrackUse != NULL);
}


VOID MiCheckUseCounts(PVOID TempHandle, PVOID Va, ULONG Id)
...

{
    ...
    restart:
        ...
        while (MiDoesPxeExistAndMakeValid(PointerPxe, CurrentProcess, TRUE, &Waited) == FALSE)
        {
        ...
        while (MiDoesPpeExistAndMakeValid(PointerPpe, CurrentProcess, TRUE, &Waited) == FALSE)
        {
        ...
        ASSERT((MI_GET_USED_PTES_FROM_HANDLE(TempHandle) != 0) || (PointerPde->u.Long == 0));
#endif
        ...
        ASSERT((MI_GET_USED_PTES_FROM_HANDLE(TempHandle) != 0) || (PointerPte->u.Long == 0));
#endif
    ...
    if ((Vad == (PMMVAD)NULL) || (Vad->u.VadFlags.PrivateMemory) || (Vad->FirstPrototypePte == (PMMPTE)NULL))
    {
    ...
    do
    {
        ...
        if (PointerPte->u.Long != 0)
        {
            ...
            if (IS_PTE_NOT_DEMAND_ZERO(*PointerPte))
            {
                ...
                if (LastProtoPte != NULL)
                {
                    ...
                    {
                        if ((ProtoPte < Subsection->SubsectionBase) || (ProtoPte >= LastProtoPte))
                        {
                ...
                Waited = MiDeletePte(PointerPte, (PVOID)Va, AddressSpaceDeletion, CurrentProcess, ProtoPte, &FlushList);
        ...
        if ((MI_GET_USED_PTES_FROM_HANDLE(UsedPageTableHandle) == 0) && (PointerPde->u.Long != 0))
        {
            ...
            MiDeletePte(PointerPde, TempVa, AddressSpaceDeletion, CurrentProcess, NULL, NULL);

#if (_MI_PAGING_LEVELS >= 3)
            if ((MI_GET_USED_PTES_FROM_HANDLE(UsedPageDirectoryHandle) == 0) && (PointerPpe->u.Long != 0))
            {
                ...
                MiDeletePte(PointerPpe, TempVa, AddressSpaceDeletion, CurrentProcess, NULL, NULL);

#if (_MI_PAGING_LEVELS >= 4)
                if ((MI_GET_USED_PTES_FROM_HANDLE(UsedPageDirectoryHandle) == 0) && (PointerPxe->u.Long != 0))
                {

                    TempVa = MiGetVirtualAddressMappedByPte(PointerPxe);
                    MiDeletePte(PointerPxe, TempVa, AddressSpaceDeletion, CurrentProcess, NULL, NULL);
                }
#endif
            }
#endif
...
#if DBG
                if ((LastProtoPte != NULL) && (Vad->u2.VadFlags2.ExtendableFile == 0))
                {
                    ...
                    if (Vad->u.VadFlags.ImageMap != 1)
                    {
                        if ((ProtoPte2 < Subsection->SubsectionBase) || (ProtoPte2 >= LastProtoPte2))
                        {
    ...
    if (PteContents.u.Hard.Valid == 1)
    {
        ...
        if (Pfn1->u3.e1.PrototypePte == 1)
        {
            ...
            if (PointerPde->u.Hard.Valid == 0)
            {
                ...
                {
#endif
                    KeBugCheckEx(MEMORY_MANAGEMENT, 0x61940, (ULONG_PTR)PointerPte, (ULONG_PTR)PointerPde->u.Long,
        ...
        else
        {
            ...
            MiDecrementShareCountInline(MI_PFN_ELEMENT(Pfn1->u4.PteFrame), Pfn1->u4.PteFrame);
        ...
        if (AddressSpaceDeletion == FALSE)
        {
            ...
            WorkingSetIndex = MiLocateWsle(VirtualAddress, MmWorkingSetList, WsPfnIndex);
            ...
            if ((Locked.LockedInWs == 1) || (Locked.LockedInMemory == 1))
            {
                ...
                if (WorkingSetIndex != MmWorkingSetList->FirstDynamic)
                {
                    ...
                    MiSwapWslEntries(Entry, WorkingSetIndex, &CurrentProcess->Vm);
                }
            }
            else
            {
            ...
            {
                KeFlushSingleTb(VirtualAddress, TRUE, FALSE, (PHARDWARE_PTE)PointerPte, ZeroPte.u.Flush);
            }
            else
            {
            ...
            if (CloneDescriptor != NULL)
            {
                ...
                if (MiDecrementCloneBlockReference(CloneDescriptor, CloneBlock, CurrentProcess))
                {
                    ...
                    do
                    {
                        ...
                        MiDoesPxeExistAndMakeValid(PointerPxe, CurrentProcess, TRUE, &Waited);

#if (_MI_PAGING_LEVELS >= 4)
                        Waited = 0;
#endif

                        MiDoesPpeExistAndMakeValid(PointerPpe, CurrentProcess, TRUE, &Waited);
                        ...
                        MiDoesPdeExistAndMakeValid(PointerPde, CurrentProcess, TRUE, &Waited);
                ...
                if (PrototypePte != MiPteToProto(PointerPte))
                {
                    ...
                    if (MiDecrementCloneBlockReference(CloneDescriptor, CloneBlock, CurrentProcess))
                    {
                        ...
                        do
                        {
                            ...
                            if (MiDoesPxeExistAndMakeValid(PointerPxe, CurrentProcess, TRUE, &Waited) == FALSE)
                            {
                            ...
                            if (MiDoesPpeExistAndMakeValid(PointerPpe, CurrentProcess, TRUE, &Waited) == FALSE)
                            {
                            ...
                            if (MiDoesPdeExistAndMakeValid(PointerPde, CurrentProcess, TRUE, &Waited) == FALSE)
                            {
                                return DroppedLocks;
                            }
...

{
    ...
    for (i = 0; i < MM_PAGING_FILE_MDLS; i += 1)
    {
        ...
        if ((MmPagingFile[PageFileNumber]->Entry[i]->Links.Flink != MM_IO_IN_PROGRESS) &&
            (MmPagingFile[PageFileNumber]->Entry[i]->CurrentList == &MmFreePagingSpaceLow))
        {
...

{
    ...
    if (count != 0)
    {
        if (count != 1)
        {
            ...
            {
                KeFlushMultipleTb(count, &PteFlushList->FlushVa[0], TRUE, (BOOLEAN)AllProcessors,
                                  &((PHARDWARE_PTE)PteFlushList->FlushPte[0]), FillPte.u.Flush);
            }
            else
            {
        ...
        {
            KeFlushSingleTb(PteFlushList->FlushVa[0], TRUE, (BOOLEAN)AllProcessors,
                            (PHARDWARE_PTE)PteFlushList->FlushPte[0], FillPte.u.Flush);
        }
