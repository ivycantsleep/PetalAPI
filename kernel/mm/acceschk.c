...
CCHAR MmReadWrite[32] = { 1, 10, 10, 10, 11, 11, 11, 11, 1, 10, 10, 10, 11, 11, 11, 11,
                          1, 10, 10, 10, 11, 11, 11, 11, 1, 10, 10, 10, 11, 11, 11, 11 };
...

{
    ...
    if (PteContents.u.Hard.Valid == 1)
    {
        ...
        if (WriteOperation != 0)
        {
            if ((PteContents.u.Hard.Write == 1) || (PteContents.u.Hard.CopyOnWrite == 1))
            {
    ...
    if (Protection & MM_GUARD_PAGE)
    {
        ...
        if ((PteContents.u.Soft.Transition == 1) && (PteContents.u.Soft.Prototype == 0))
        {
            ...
            if ((PteContents.u.Soft.Transition == 1) && (PteContents.u.Soft.Prototype == 0))
            {
                ...
                ASSERT((Pfn1->u3.e1.PrototypePte == 0) ||
                       (MiLocateCloneAddress(PsGetCurrentProcess(), Pfn1->PteAddress) != NULL));
                Pfn1->OriginalPte.u.Soft.Protection = Protection & ~MM_GUARD_PAGE;
            }
            if (CallerHoldsPfnLock == FALSE)
            {
                UNLOCK_PFN(OldIrql);
...

{
    ...
    try
    {
        ...
        if ((Teb->NtTib.StackBase <= FaultingAddress) && (Teb->DeallocationBStore > FaultingAddress))
        {
            ...
            if ((NextPage + PAGE_SIZE) >= (ULONG_PTR)PAGE_ALIGN(Teb->DeallocationBStore))
            {
                ...
                status = ZwAllocateVirtualMemory(NtCurrentProcess(), (PVOID *)&NextPage, 0, &RegionSize, MEM_COMMIT,
                                                 PAGE_READWRITE);
        ...
        else
        {
            ...
            if ((Teb->NtTib.StackBase <= FaultingAddress) || (DeallocationStack > FaultingAddress))
            {
            ...
            if ((NextPage - PageSize) <= (ULONG_PTR)DeallocationStack)
            {
                ...
                *StackLimit = (PVOID)((PUCHAR)NextPage);
                    ...
                    if (RevertExecuteFlag)
                    {
                        ...
                        status = ZwProtectVirtualMemory(NtCurrentProcess(), (PVOID *)&NextPage, &RegionSize,
                                                        StackProtection, &OldProtection);
        ...
        *StackLimit = (PVOID)((PUCHAR)(NextPage + PAGE_SIZE));
        ...
        if (NT_SUCCESS(status) || (status == STATUS_ALREADY_COMMITTED))
        {
            ...
            if (RevertExecuteFlag)
            {
                ...
                status = ZwProtectVirtualMemory(NtCurrentProcess(), (PVOID *)&NextPage, &RegionSize, ProtectionFlags,
                                                &OldProtection);
            }
