...
PFN_NUMBER
MiMakeOutswappedPageResident(IN PMMPTE ActualPteAddress, IN PMMPTE PointerTempPte, IN ULONG Global,
                             IN PFN_NUMBER ContainingPage);

NTSTATUS
MiCreatePebOrTeb(IN PEPROCESS TargetProcess, IN ULONG Size, OUT PVOID *Base);
...
PMMVAD
MiAllocateVad(IN ULONG_PTR StartingVirtualAddress, IN ULONG_PTR EndingVirtualAddress, IN LOGICAL Deletable);
...

{
    ...
    PointerPxe = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryIndex);
    ...
    PointerPxe = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryParentIndex);
...
#if (_MI_PAGING_LEVELS >= 4)
    PointerPpe = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageDirectoryIndex);
#endif
...
#if (_AMD64_)
    TempPte.u.Hard.PageFrameNumber = HyperSpaceIndex;
    PointerPde = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, HyperDirectoryIndex);
    ...
    PointerPxe = (PMMPTE)MiMapPageInHyperSpace(CurrentProcess, PageDirectoryParentIndex, &OldIrql);

    PointerFillPte = &PointerPxe[MiGetPxeOffset(KI_USER_SHARED_DATA)];
    RtlCopyMemory(PointerFillPte, CurrentAddressSpacePde,
                  ((1 + (MiGetPxeAddress(MM_SYSTEM_SPACE_END) - CurrentAddressSpacePde)) * sizeof(MMPTE)));
    ...
    if (MiMaximumSystemExtraSystemPdes)
    {
        ...
        RtlCopyMemory(PointerFillPte, CurrentAddressSpacePde, MiMaximumSystemExtraSystemPdes * sizeof(MMPTE));
    }
#endif
    ...
    if (MiMaximumSystemExtraSystemPdes != 0)
    {
        ...
        while (MaximumStart < MiUseMaximumSystemSpaceEnd)
        {
            ...
            RtlCopyMemory(PointerFillPte, CurrentAddressSpacePde, NumberOfPdes * sizeof(MMPTE));
...
MmInitializeProcessAddressSpace(IN PEPROCESS ProcessToInitialize, IN PEPROCESS ProcessToClone OPTIONAL,
                                IN PVOID SectionToMap OPTIONAL, OUT POBJECT_NAME_INFORMATION *AuditName OPTIONAL)
...

{
    ...
    for (i = 0; i < NumberOfPages; i += 1)
    {
        ...
        MI_MAKE_VALID_PTE(TempPte, VadBitMapPage, MM_READWRITE, PointerPte);
    ...
    MmWorkingSetList->CommittedPageTables = (PULONG)ExAllocatePoolWithTag(
        MmPagedPoolEnd != NULL ? PagedPool : NonPagedPool, (MM_USER_PAGE_TABLE_PAGES + 7) / 8, 'dPmM');
    ...
    MmWorkingSetList->CommittedPageDirectories = (PULONG)ExAllocatePoolWithTag(
        MmPagedPoolEnd != NULL ? PagedPool : NonPagedPool, (MM_USER_PAGE_DIRECTORY_PAGES + 7) / 8, 'dPmM');
    ...
    RtlZeroMemory(MmWorkingSetList->CommittedPageDirectories, (MM_USER_PAGE_DIRECTORY_PAGES + 7) / 8);
#endif

    RtlZeroMemory(MmWorkingSetList->CommittedPageTables, (MM_USER_PAGE_TABLE_PAGES + 7) / 8);
    ...
    if ((MmVirtualBias != 0) && (ProcessToClone == NULL))
    {
        ...
        if (SectionToMap != NULL)
        {
            ...
            if ((ImageInfo->ImageCharacteristics & IMAGE_FILE_LARGE_ADDRESS_AWARE) == 0)
            {
                ...
                VadReserve = MiAllocateVad(_2gb, (ULONG_PTR)MM_HIGHEST_VAD_ADDRESS, FALSE);
    ...
    if (ProcessToClone == NULL)
    {
        ...
        ASSERT(MiCheckForConflictingVad(ProcessToInitialize, WOW64_COMPATIBILITY_AREA_ADDRESS,
                                        MM_SHARED_USER_DATA_VA) == NULL);
        ...
        if (SectionToMap != NULL)
        {
                 ...
                 ImageInfo->Machine == IMAGE_FILE_MACHINE_I386))
            {
                ...
                VadReserve = MiAllocateVad(_2gb, (ULONG_PTR)MM_HIGHEST_USER_ADDRESS, TRUE);
    ...
    if (SectionToMap != NULL)
    {
        ...
        else
        {
            ...
            *Dst = '\0';
            ...
            ProcessToInitialize->SubSystemMajorVersion = (UCHAR)ImageInformation->SubSystemMajorVersion;
            ProcessToInitialize->SubSystemMinorVersion = (UCHAR)ImageInformation->SubSystemMinorVersion;

            BaseAddress = NULL;
            ViewSize = 0;
            ZERO_LARGE(SectionOffset);

            Status = MmMapViewOfSection((PSECTION)SectionToMap, ProcessToInitialize, &BaseAddress, 0, 0, &SectionOffset,
    ...
    if (ProcessToClone != NULL)
    {
        ...
        strcpy((PCHAR)ProcessToInitialize->ImageFileName, (PCHAR)ProcessToClone->ImageFileName);
        ...
        return MiCloneProcessAddressSpace(ProcessToClone, ProcessToInitialize,
#if (_MI_PAGING_LEVELS >= 4)
                                          PxePhysicalPage,
...
VOID MmInitializeHandBuiltProcess(IN PEPROCESS ProcessToInitialize, OUT PULONG_PTR DirectoryTableBase)
...

{
    ...
    if (MiSystemPaeVa.PteEntry[0].u.Long == 0)
    {
        ...
        PdeBase = (PMMPTE)MiMapSinglePage(NULL, PageFrameIndex, MmCached, HighPagePriority);

        if (PdeBase == NULL)
        {
            KeBugCheckEx(MEMORY_MANAGEMENT, 0x3452, (ULONG_PTR)CurrentProcess, (ULONG_PTR)ProcessToInitialize, 1);
        }
...
NTSTATUS
MmInitializeHandBuiltProcess2(IN PEPROCESS ProcessToInitialize)
...
--*/
{
    ...
    if ((MmVirtualBias != 0) && (MmHighestUserAddress > (PVOID)MM_SHARED_USER_DATA_VA))
    {
...

{
    ...
    if (Process->AddressSpaceInitialized == 2)
    {
        ...
        PointerPte = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex);
        ...
        ExtendedPageDirectoryParent = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex);
        ...
        PageDirectoryParent = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex3);
    ...
    else
    {
...
#if (_MI_PAGING_LEVELS >= 4)
        PageDirectoryParent = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex);
        ...
        PointerPte = (PMMPTE)MiMapPageInHyperSpaceAtDpc(CurrentProcess, PageFrameIndex2);
...
#else
    if ((PointerPpe->u.Hard.Valid == 1) && (PointerPde->u.Hard.Valid == 1) && (PointerPte->u.Hard.Valid == 1))
#endif
    {
        ...
        do
        {
            ...
            MiDeletePte(PointerPte, TempVa, AddressSpaceDeletion, Process, NULL, &PteFlushList);
            ...
            if ((PointerPte >= LastPte) ||
                ...
                ((MiGetPteAddress(PointerPte))->u.Hard.Valid == 0) || (PointerPte->u.Hard.Valid == 0))
            {
                FinalPte = TRUE;
            }
            else
            ...
            if ((Boundary == TRUE) || (FinalPte == TRUE))
            {
                ...
                {
                    MiDeletePte(PointerPde, PointerPte - 1, AddressSpaceDeletion, Process, NULL, NULL);
                    ...
                    if ((FinalPte == TRUE) || (MiIsPteOnPpeBoundary(PointerPte)))
                    {
                        ...
                        if (Pfn1->u2.ShareCount == 1 && Pfn1->u3.e2.ReferenceCount == 1)
                        {
                            MiDeletePte(PointerPpe, PointerPde, AddressSpaceDeletion, Process, NULL, NULL);
                            ...
                            if ((FinalPte == TRUE) || (MiIsPteOnPxeBoundary(PointerPte)))
                            {
                                ...
                                if (Pfn1->u2.ShareCount == 1 && Pfn1->u3.e2.ReferenceCount == 1)
                                {
                                    MiDeletePte(PointerPxe, PointerPpe, AddressSpaceDeletion, Process, NULL, NULL);
                                    CommittedPages += 1;
                                    Process->NumberOfPrivatePages += 1;
                                }
                            }
...

{
    ...
    if ((Process->Flags & PS_PROCESS_FLAGS_VM_DELETED) || (Process->AddressSpaceInitialized == 0))
    {
    ...
    while (Vad != (PMMVAD)NULL)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                MiDeleteVirtualAddresses(MI_VPN_TO_VA(Vad->StartingVpn), MI_VPN_TO_VA_ENDING(Vad->EndingVpn), FALSE,
                                         Vad);
    ...
    MM_TRACK_COMMIT(MM_DBG_COMMIT_RETURN_PROCESS_CLEAN_PAGETABLES, NumberOfCommittedPageTables);
    ...
    if (Process->NumberOfLockedPages != 0)
    {
        if (Process->LockedPagesList)
        {
            ...
            if ((LockedPagesHeader->Count != 0) && (MiTrackingAborted == FALSE))
            {
                ...
                KeBugCheckEx(DRIVER_LEFT_LOCKED_PAGES_IN_PROCESS, (ULONG_PTR)Tracker->CallingAddress,
                             (ULONG_PTR)Tracker->CallersCaller, (ULONG_PTR)Tracker->Mdl, Process->NumberOfLockedPages);
            }
        }

        KeBugCheckEx(PROCESS_HAS_LOCKED_PAGES, 0, (ULONG_PTR)Process, Process->NumberOfLockedPages,
...
PVOID
MmCreateKernelStack(IN BOOLEAN LargeStack, IN UCHAR PreferredNode)
...

{
    ...
    for (i = 0; i < NumberOfPages; i += 1)
    {
        ...
        PageFrameIndex = MiRemoveAnyPage(MI_GET_PAGE_COLOR_NODE(PreferredNode));
...
FreeStack:
    ...
    if (LargeStack)
    {
        PointerPte = MiGetPteAddress((PCHAR)PointerKernelStack + KERNEL_LARGE_BSTORE_SIZE);
    }
    else
    {
        PointerPte = MiGetPteAddress((PCHAR)PointerKernelStack + KERNEL_BSTORE_SIZE);
    }
...

{
    ...
    ASSERT(((PCHAR)Thread->Tcb.StackBase - (PCHAR)Thread->Tcb.StackLimit) <= ((LONG)MI_LARGE_STACK_SIZE + PAGE_SIZE));
    ...
    EndStack = MiGetPteAddress((PVOID)((PUCHAR)Thread->Tcb.StackBase - MI_LARGE_STACK_SIZE));
...
NTSTATUS
MmGrowKernelBackingStore(IN PVOID CurrentBackingStorePointer)
...

{
    ...
    ASSERT(((PCHAR)Thread->Tcb.BStoreLimit - (PCHAR)Thread->Tcb.StackBase) <= (KERNEL_LARGE_BSTORE_SIZE + PAGE_SIZE));

    BstoreLimit = MiGetPteAddress((PVOID)((PCHAR)Thread->Tcb.BStoreLimit - 1));

    ASSERT(BstoreLimit->u.Hard.Valid == 1);

    NewLimit = MiGetPteAddress((PVOID)((PUCHAR)CurrentBackingStorePointer + KERNEL_LARGE_BSTORE_COMMIT - 1));
    ...
    while (BstoreLimit <= NewLimit)
    {
        ...
        MI_MAKE_VALID_PTE(TempPte, PageFrameIndex, MM_READWRITE, BstoreLimit);
...

{
    ...
    ASSERT(((PCHAR)Thread->StackBase - (PCHAR)Thread->StackLimit) <= ((LONG)MI_LARGE_STACK_SIZE + PAGE_SIZE));
    ...
    *((PULONG_PTR)Thread->KernelStack - 1) = (ULONG_PTR)Thread;
    ...
    {
        KeFlushMultipleTb(Count, &FlushVa[0], TRUE, TRUE, NULL, *(PHARDWARE_PTE)&ZeroPte.u.Flush);
    }
    else
    {
...

{
    ...
    if (Thread->LargeStack)
    {
        ...
        BaseOfKernelStack =
            (PVOID)(((ULONG_PTR)Thread->InitialBStore + KERNEL_LARGE_BSTORE_COMMIT) & ~(ULONG_PTR)(PAGE_SIZE - 1));
    ...
    {
        KeBugCheckEx(KERNEL_STACK_INPAGE_ERROR, DiskRead, *((PULONG_PTR)Thread->KernelStack - 1), 0,
...

{
    ...
    if ((OutProcess->Vm.WorkingSetSize == MM_PROCESS_COMMIT_CHARGE) && (OutProcess->Vm.Flags.AllowWorkingSetAdjustment))
    {
...

{
    ...
    if (OutProcess->Flags & PS_PROCESS_FLAGS_OUTSWAPPED)
    {
        ...
        PageDirectoryPage = MiMakeOutswappedPageResident(MiGetPxeAddress(MmWorkingSetList), &TempPte, 0, PdePage);
        ...
        PageDirectoryParentMap[MiGetPxeOffset(PXE_BASE)].u.Flush = OutProcess->PageDirectoryPte;
        ...
        PageDirectoryPage = MiMakeOutswappedPageResident(MiGetPpeAddress(MmWorkingSetList), &TempPte, 0, PdePage);
        ...
        for (i = 0; i < PD_PER_SYSTEM - 1; i += 1)
        {
            MiMakeOutswappedPageResident(MiGetPteAddress(PDE_BASE + (i << PAGE_SHIFT)), &PageDirectoryPtes[i], 0,
                                         PdePage);
        ...
        HyperSpacePageTable = MiMakeOutswappedPageResident(MiGetPdeAddress(HYPER_SPACE2), &TempPte, 0, PdePage);
        ...
        HyperSpacePageTable = MiMakeOutswappedPageResident(MiGetPdeAddress(HYPER_SPACE), &TempPte, 0, PdePage);
        ...
        VadBitMapPage = MiMakeOutswappedPageResident(VadBitMapPte, &VadBitMapPteContents, 0, HyperSpacePageTable);
        ...
        WorkingSetPage = MiMakeOutswappedPageResident(WorkingSetListPte, &TempPte, 0, HyperSpacePageTable);
    ...
    if (PERFINFO_IS_GROUP_ON(PERF_MEMORY))
    {
        ...
        PerfInfoLogBytes(PERFINFO_LOG_TYPE_INSWAPPROCESS, &PerfInfoSwapProcess, sizeof(PerfInfoSwapProcess));
    }
    return;
}
...

{
    ...
    Status = MiFindEmptyAddressRangeDown(TargetProcess->VadRoot, ROUND_TO_PAGES(Size),
                                         ((PCHAR)MM_HIGHEST_VAD_ADDRESS + 1), PAGE_SIZE, Base);
    ...
    {
        MiProtectFor4kPage(*Base, ROUND_TO_PAGES(Size), MM_READWRITE, ALT_COMMIT, TargetProcess);
    }
#endif
...

{
    ...
    try
    {
        ...
        if ((InitialTeb->OldInitialTeb.OldStackBase == NULL) && (InitialTeb->OldInitialTeb.OldStackLimit == NULL))
        {
            ...
            TebBase->DeallocationBStore = (PCHAR)InitialTeb->StackBase + ((ULONG_PTR)InitialTeb->StackBase -
                                                                          (ULONG_PTR)InitialTeb->StackAllocationBase);
#endif
        }
        else
        {
        ...
        if (Wow64Process != NULL)
        {
            ...
            Teb32Base->ActivationContextStack.NextCookieSequenceNumber =
                TebBase->ActivationContextStack.NextCookieSequenceNumber;
        }
...
#define MI_INIT_PEB_FROM_IMAGE(Hdrs, ImgConfig)                                                           \
    {                                                                                                     \
        ...
        PebBase->ImageSubsystemMajorVersion = (Hdrs)->OptionalHeader.MajorSubsystemVersion;               \
        PebBase->ImageSubsystemMinorVersion = (Hdrs)->OptionalHeader.MinorSubsystemVersion;               \
                                                                                                          \
        /*                                                                   */                           \
        ...
        {                                                                                                 \
            PebBase->OSMajorVersion = (Hdrs)->OptionalHeader.Win32VersionValue & 0xFF;                    \
            PebBase->OSMinorVersion = ((Hdrs)->OptionalHeader.Win32VersionValue >> 8) & 0xFF;             \
            PebBase->OSBuildNumber = (USHORT)(((Hdrs)->OptionalHeader.Win32VersionValue >> 16) & 0x3FFF); \
            ...
            /*                                                                  */                        \
            PebBase->OSPlatformId = ((Hdrs)->OptionalHeader.Win32VersionValue >> 30) ^ 0x2;               \
        }                                                                                                 \
    }
...
#if defined(_WIN64)
NTSTATUS
MiInitializeWowPeb(IN PIMAGE_NT_HEADERS NtHeaders, IN PPEB PebBase, IN PEPROCESS TargetProcess)
...

{
    ...
    try
    {
        ImageConfigData32 = RtlImageDirectoryEntryToData(PebBase->ImageBaseAddress, TRUE,
                                                         IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &ReturnedSize);

        ProbeForReadSmallStructure((PVOID)ImageConfigData32, sizeof(*ImageConfigData32), sizeof(ULONG));
    ...
    Status = MiCreatePebOrTeb(TargetProcess, (ULONG)sizeof(PEB32), (PVOID)&PebBase32);
...

{
    ...
    Status = MmMapViewOfSection(InitNlsSectionPointer, TargetProcess, &ViewBase, 0L, 0L, &SectionOffset, &ViewSize,
                                ViewShare, MEM_TOP_DOWN | SEC_NO_CHANGE, PAGE_READONLY);
    ...
    if (NtHeaders != NULL)
    {
...
#endif
        {
            try
            {
                ImageConfigData = RtlImageDirectoryEntryToData(PebBase->ImageBaseAddress, TRUE,
                                                               IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &ReturnedSize);

                ProbeForReadSmallStructure((PVOID)ImageConfigData, sizeof(*ImageConfigData),
...

{
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        MiReturnPageTablePageCommitment(TebBase, EndingAddress, TargetProcess, PreviousVad, NextVad);
...
PFN_NUMBER
MiMakeOutswappedPageResident(IN PMMPTE ActualPteAddress, IN OUT PMMPTE PointerTempPte, IN ULONG Global,
                             IN PFN_NUMBER ContainingPage)
...
restart:
    ...
    else if (PointerTempPte->u.Soft.Transition == 1)
    {
        ...
        if ((MmAvailablePages == 0) || ((Pfn1->u4.InPageError == 1) && (Pfn1->u3.e1.ReadInProgress == 1)))
        {
    ...
    Refault:
        ...
        if ((!NT_SUCCESS(Status)) || (!NT_SUCCESS(IoStatus.Status)))
        {
            ...
            if ((MmIsRetryIoStatus(Status)) || (MmIsRetryIoStatus(IoStatus.Status)))
            {
...
UCHAR
MiSetMemoryPriorityProcess(IN PEPROCESS Process, IN UCHAR MemoryPriority)
