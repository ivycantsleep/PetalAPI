...
MiMapViewOfPhysicalSection(IN PCONTROL_AREA ControlArea, IN PEPROCESS Process, IN PVOID *CapturedBase,
                           ...
                           IN ULONG_PTR ZeroBits, IN ULONG AllocationType, IN BOOLEAN WriteCombined,
...

{
    ...
    if (*CapturedBase == NULL)
    {
        ...
        try
        {
            ...
            PhysicalViewSize =
                (SectionOffset->LowPart + *CapturedViewSize) - (ULONG)MI_64K_ALIGN(SectionOffset->LowPart);
        ...
        EndingAddress = (PVOID)(((ULONG)StartingAddress + PhysicalViewSize - 1L) | (PAGE_SIZE - 1L));
        StartingAddress = (PVOID)((ULONG)StartingAddress + (SectionOffset->LowPart & (X64K - 1)));
    ...
    else
    {
        ...
        PhysicalViewSize = (SectionOffset->LowPart + *CapturedViewSize) - (ULONG)MI_64K_ALIGN(SectionOffset->LowPart);
        StartingAddress = (PVOID)((ULONG)MI_64K_ALIGN(*CapturedBase) + (SectionOffset->LowPart & (X64K - 1)));
        EndingAddress = (PVOID)(((ULONG)StartingAddress + *CapturedViewSize - 1L) | (PAGE_SIZE - 1L));
    ...
    while (PointerPte <= LastPte)
    {
        ...
        if (AllocationType & MEM_LARGE_PAGES)
        {
            PagesTogether = AggregatePages(PointerPte, NextPfn, PagesToMap, &GranularityHint);
        }
        else
        {
            PagesTogether = 1;
...
ULONG
MaximumAlignment(IN ULONG Offset)
/*++
...
ULONG
AggregatePages(IN PMMPTE PointerPte, IN ULONG Pfn, IN ULONG Pages, OUT PULONG GranularityHint)
/*++
...
--*/
{
    ...
    MaxVirtualAlignment = MaximumAlignment((ULONG)MiGetVirtualAddressMappedByPte(PointerPte));
    ...
    MaxAlignment = (MaxVirtualAlignment > MaxPhysicalAlignment) ? MaxPhysicalAlignment : MaxVirtualAlignment;
    MaxAlignment = (MaxAlignment > MaxPageAlignment) ? MaxPageAlignment : MaxAlignment;
