...
VOID MiFillRemovedPages(IN ULONG StartPage, IN ULONG NumberOfPages);

ULONG
MiRemoveModuloPages(IN ULONG StartPage, IN ULONG LastPage);
...

{
    ...
    for (Page = StartPage; Page < LastPage; Page += 1)
    {
        ...
        while (HighPage <= MmHighestPhysicalPage)
        {
            ...
            if ((MmIsAddressValid(Pfn1)) && (MmIsAddressValid((PCHAR)Pfn1 + sizeof(MMPFN) - 1)) &&
                ((ULONG)Pfn1->u3.e1.PageLocation <= (ULONG)StandbyPageList) && (Pfn1->u1.Flink != 0) &&
                (Pfn1->u2.Blink != 0) && (Pfn1->u3.e2.ReferenceCount == 0) && (MmAvailablePages > 0))
            {
                ...
                TempVa = (PULONG)MiMapPageInHyperSpace(Process, HighPage, &OldIrql);
                RtlFillMemoryUlong(TempVa, PAGE_SIZE, (ULONG)HighPage | MI_LOWMEM_MAGIC_BIT);
...
VOID MiRemoveLowPages(ULONG RemovePhase)
...

{
            ...
            if (Page >= MiNoLowMemory)
            {
                ...
                else
                {
                    ...
                    ASSERT((MMLISTS)(MI_PFN_ELEMENT((MI_PFN_ELEMENT(Pfn1->u2.Blink)->u1.Flink)))->u4.PteFrame !=
                           MI_MAGIC_4GB_RECLAIM);
                    ASSERT((MMLISTS)(MI_PFN_ELEMENT((MI_PFN_ELEMENT(Pfn1->u2.Blink)->u1.Flink)))->u3.e1.PageLocation ==
                           FreePageList);
    ...
    if (RemovePhase == 1)
    {
        ...
        do
        {
            ...
            {
#if DBG
                DbgPrint("%08lx  %08lx %08lx\n", StartingRunIndex, BitMapIndex - 1, BitMapIndex - StartingRunIndex);
#endif
                ...
                ModuloRemoved += MiRemoveModuloPages(StartingRunIndex, BitMapIndex);
            }
            ...
            LengthOfClearRun = RtlFindNextForwardRunClear(MiLowMemoryBitMap, BitMapIndex, &StartingRunIndex);
        ...
        {
#if DBG
            DbgPrint("%08lx  %08lx %08lx\n", StartingRunIndex, StartingRunIndex + LengthOfClearRun - 1,
                     LengthOfClearRun);
#endif

            ModuloRemoved += MiRemoveModuloPages(StartingRunIndex, StartingRunIndex + LengthOfClearRun);
        }
...

{
    ...
    FindNext:
        ...
        if ((BitMapHint >= MiLowMemoryBitMap->SizeOfBitMap) || (BitMapHint + SizeInPages > HighestAcceptablePfn))
        {
            UNLOCK_PFN(OldIrql);
            return NULL;
        }
