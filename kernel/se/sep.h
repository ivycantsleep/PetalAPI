...
#define IF_SE_GLOBAL(FlagName) if (SeGlobalFlag & (SE_DIAG_##FlagName))
    ...
    ((PrivilegeSet) == NULL                                                                                 \
         ...
         : (((PrivilegeSet)->PrivilegeCount > 0)                                                            \
                ? ((ULONG)sizeof(PRIVILEGE_SET) +                                                           \
                   (((PrivilegeSet)->PrivilegeCount - ANYSIZE_ARRAY) * (ULONG)sizeof(LUID_AND_ATTRIBUTES))) \
...
#define EffectiveToken(SubjectSecurityContext)                                     \
    ((SubjectSecurityContext)->ClientToken ? (SubjectSecurityContext)->ClientToken \
...
#define SepBadImpersonationLevel(IL, SIR)                                                                         \
    ((((IL) == SecurityAnonymous) || ((IL) == SecurityIdentification) || ((SIR) && ((IL) != SecurityDelegation))) \
         ? TRUE                                                                                                   \
         : FALSE)
...
#define SEP_MAX_PRIVILEGE_COUNT (SE_MAX_WELL_KNOWN_PRIVILEGE - SE_MIN_WELL_KNOWN_PRIVILEGE + 1)

#define IsValidPrivilegeCount(count) ((count == 0) || (count <= SEP_MAX_PRIVILEGE_COUNT))
...
#define SepUnlockLsaQueue()                  \
    ExReleaseResourceLite(&SepLsaQueueLock); \
...
BOOLEAN
SepIdAssignableAsGroup(IN PACCESS_TOKEN Token, IN PSID Group);

BOOLEAN
SepCheckAcl(IN PACL Acl, IN ULONG Length);

BOOLEAN
SepAuditAlarm(IN PUNICODE_STRING SubsystemName, IN PVOID HandleId, IN PUNICODE_STRING ObjectTypeName,
              IN PUNICODE_STRING ObjectName, IN PSECURITY_DESCRIPTOR SecurityDescriptor, IN ACCESS_MASK DesiredAccess,
              IN BOOLEAN ObjectCreation, IN ACCESS_MASK GrantedAccess, OUT PBOOLEAN GenerateOnClose);

BOOLEAN
SepSinglePrivilegeCheck(LUID DesiredPrivilege, IN PACCESS_TOKEN EffectiveToken, IN KPROCESSOR_MODE PreviousMode);
...
BOOLEAN
SepTokenIsOwner(IN PACCESS_TOKEN Token, IN PSECURITY_DESCRIPTOR SecurityDescriptor, IN BOOLEAN TokenLocked);
...
BOOLEAN
SepSidInToken(IN PACCESS_TOKEN Token, IN PSID PrincipalSelfSid, IN PSID Sid, IN BOOLEAN DenyAce);
