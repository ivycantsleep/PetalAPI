...
static int UserHashVal(uname, domain) const char far *uname;
int domain;
{
...
void InitMemStuff()
{
    ...
    err = DosOpen(path, &Handle, &action, 0L, 0, FILE_OPEN, OPEN_ACCESS_READONLY | OPEN_SHARE_DENYNONE, 0L);
    if (err)
    {
...
void showentries()
{
    ...
    for (i = 0; i < USER_HASH_ENTRIES; i++)
    {
        ...
        while (pos)
        {
            ...
            relativeid = (((uo->uo_record.user.uc0_guid.guid_serial) << 16) + (uo->uo_record.user.uc0_guid.guid_uid)) ^
                         0x80000000;
            ...
            printf("\tUser = %ld %Fs %Fs\n", relativeid, (char FAR *)uo->uo_record.name, (char FAR *)groupnames);
            usercount++;
