...
#define NEEDS_IO_THREAD(Flags) (Flags & (WT_EXECUTEINIOTHREAD | WT_EXECUTEINUITHREAD | WT_EXECUTEINPERSISTENTIOTHREAD))
...
#define ACQUIRE_GLOBAL_TIMER_LOCK() RtlEnterCriticalSection(&TimerCriticalSection)

#define RELEASE_GLOBAL_TIMER_LOCK() RtlLeaveCriticalSection(&TimerCriticalSection)
...
#define RtlpShiftWaitArray(ThreadCB, SrcIndex, DstIndex, Count)                                             \
    {                                                                                                       \
                                                                                                            \
        RtlMoveMemory(&(ThreadCB)->ActiveWaitArray[DstIndex], &(ThreadCB)->ActiveWaitArray[SrcIndex],       \
                      sizeof(HANDLE) * (Count));                                                            \
                                                                                                            \
        RtlMoveMemory(&(ThreadCB)->ActiveWaitPointers[DstIndex], &(ThreadCB)->ActiveWaitPointers[SrcIndex], \
                      sizeof(HANDLE) * (Count));                                                            \
    }
...
#define CHECK_SIGNATURE(ptr) ASSERTMSG(InvalidSignatureMsg, ((ptr)->State & 0xffff0000) == 0xfedc0000)
#define SET_DEL_SIGNATURE(ptr) RtlInterlockedSetBitsDiscardReturn(&(ptr)->State, 0x0000b000)
#define CHECK_DEL_SIGNATURE(ptr) \
    ASSERTMSG(InvalidDelSignatureMsg, (((ptr)->State & 0xffff0000) == 0xfedc0000) && (!((ptr)->State & 0x0000f000)))
#define IS_DEL_SIGNATURE_SET(ptr) ((ptr)->State & 0x0000f000)
#define CLEAR_SIGNATURE(ptr) RtlInterlockedSetClearBits(&(ptr)->State, 0xcdef0000, 0xfedc0000 & ~(0xcdef0000))
...
NTSTATUS
NTAPI
RtlpStartThreadpoolThread(PUSER_THREAD_START_ROUTINE Function, PVOID Parameter, HANDLE *ThreadHandleReturn);
...
VOID RtlpAsyncWaitCallbackCompletion(IN PVOID Context);
...
NTSYSAPI
VOID NTAPI RtlpThreadCleanup(VOID);
...
NTSTATUS
RtlpWaitForEvent(HANDLE Event, HANDLE ThreadHandle);

NTSYSAPI
NTSTATUS
NTAPI
RtlThreadPoolCleanup(ULONG Flags);

VOID RtlpWaitOrTimerCallout(WAITORTIMERCALLBACKFUNC Function, PVOID Context, BOOLEAN TimedOut,
                            PACTIVATION_CONTEXT ActivationContext, HANDLE ImpersonationToken);

VOID RtlpApcCallout(APC_CALLBACK_FUNCTION Function, NTSTATUS Status, PVOID Context1, PVOID Context2);

VOID RtlpWorkerCallout(WORKERCALLBACKFUNC Function, PVOID Context, PACTIVATION_CONTEXT ActivationContext,
                       HANDLE ImpersonationToken);
