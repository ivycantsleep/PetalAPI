...
--*/
{
    ...
    RtlpExceptionLog =
        (PLAST_EXCEPTION_LOG)ExAllocatePoolWithTag(NonPagedPool, sizeof(LAST_EXCEPTION_LOG) * Entries, 'gbdE');
#else
...
--*/
{
    ...
    } while (LogIndex != (ULONG)InterlockedCompareExchange((PLONG)&RtlpExceptionLogCount,
                                                           ((LogIndex + 1) % MAX_EXCEPTION_LOG), LogIndex));
    ...
    RtlCopyMemory(RtlpExceptionLog[LogIndex].HandlerData, HandlerData, Size);
...
LONG NTAPI RtlUnhandledExceptionFilter2(IN struct _EXCEPTION_POINTERS *ExceptionInfo, IN CONST CHAR *Function)
/*++
...

{
    ...
    if (DebuggerPresent)
    {
        ...
        case STATUS_POSSIBLE_DEADLOCK:
        {
            ...
            CritSec = (PRTL_CRITICAL_SECTION)ExceptionInfo->ExceptionRecord->ExceptionInformation[0];
        ...
        case STATUS_ACCESS_VIOLATION:
            ...
            DbgPrint("The instruction at %p tried to %s ", ExceptionInfo->ExceptionRecord->ExceptionAddress,
    ...
    if (ExceptionInfo->ExceptionRecord->ExceptionCode == STATUS_POSSIBLE_DEADLOCK)
    {
        if (DebuggerPresent)
        {
            DbgPrint(" *** Restarting wait on critsec or resource at %p (in %ws:%s)\n\n",
                     ExceptionInfo->ExceptionRecord->ExceptionInformation[0], lpProcessName, Function);
        }
