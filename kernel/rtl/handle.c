...
void RtlInitializeHandleTable(IN ULONG MaximumNumberOfHandles, IN ULONG SizeOfHandleTableEntry,
...
NTSTATUS
RtlDestroyHandleTable(IN OUT PRTL_HANDLE_TABLE HandleTable)
{
    NTSTATUS Status;
    PVOID BaseAddress;
    SIZE_T ReserveSize;
    ...
    ReserveSize = (PUCHAR)(HandleTable->MaxReservedHandles) - (PUCHAR)(HandleTable->CommittedHandles);

    Status = NtFreeVirtualMemory(NtCurrentProcess(), &BaseAddress, &ReserveSize, MEM_RELEASE);
    return Status;
}

PRTL_HANDLE_TABLE_ENTRY
RtlAllocateHandle(IN PRTL_HANDLE_TABLE HandleTable, OUT PULONG HandleIndex OPTIONAL)
{
    NTSTATUS Status;
    PVOID BaseAddress;
    ULONG n;
    ...
    if (HandleTable->FreeHandles == NULL)
    {
        try
        {
            ...
            {
                ReserveSize = HandleTable->MaximumNumberOfHandles * HandleTable->SizeOfHandleTableEntry;
                BaseAddress = NULL;
                Status = NtAllocateVirtualMemory(NtCurrentProcess(), &BaseAddress, 0, &ReserveSize, MEM_RESERVE,
                                                 PAGE_READWRITE);
                ...
                if (NT_SUCCESS(Status))
                {
                    ...
                    HandleTable->MaxReservedHandles = (PRTL_HANDLE_TABLE_ENTRY)((PCHAR)BaseAddress + ReserveSize);
                }
            }
            else
            {
            ...
            if (NT_SUCCESS(Status))
            {
                ...
                else
                {
                    ...
                    Status = NtAllocateVirtualMemory(NtCurrentProcess(), (PVOID *)&p, 0, &CommitSize, MEM_COMMIT,
                                                     PAGE_READWRITE);
                    if (NT_SUCCESS(Status))
                    {
                        HandleTable->UnCommittedHandles = (PRTL_HANDLE_TABLE_ENTRY)((PCH)p + CommitSize);
                    }
                }
            }
        }
    ...
    {
        *HandleIndex = (ULONG)(((PCHAR)p - (PCHAR)HandleTable->CommittedHandles) / HandleTable->SizeOfHandleTableEntry);
    }
...
BOOLEAN
RtlFreeHandle(IN PRTL_HANDLE_TABLE HandleTable, IN PRTL_HANDLE_TABLE_ENTRY Handle)
{
#if DBG
...
BOOLEAN
RtlIsValidHandle(IN PRTL_HANDLE_TABLE HandleTable, IN PRTL_HANDLE_TABLE_ENTRY Handle)
{
    if (Handle == NULL || Handle < HandleTable->CommittedHandles || Handle >= HandleTable->UnCommittedHandles ||
        (ULONG_PTR)Handle & (HandleTable->SizeOfHandleTableEntry - 1) || !(Handle->Flags & RTL_HANDLE_ALLOCATED))
    {
        return FALSE;
    }
    else
...
BOOLEAN
RtlIsValidIndexHandle(IN PRTL_HANDLE_TABLE HandleTable, IN ULONG HandleIndex, OUT PRTL_HANDLE_TABLE_ENTRY *Handle)
{
    PRTL_HANDLE_TABLE_ENTRY p;

    p = (PRTL_HANDLE_TABLE_ENTRY)((PCHAR)HandleTable->CommittedHandles +
