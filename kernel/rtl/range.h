...
#define FOR_ALL_IN_LIST_SAFE(Type, Head, Current, Next)                          \
    for ((Current) = CONTAINING_RECORD((Head)->Flink, Type, ListEntry),          \
        (Next) = CONTAINING_RECORD((Current)->ListEntry.Flink, Type, ListEntry); \
         (Head) != &(Current)->ListEntry;                                        \
         (Current) = (Next), (Next) = CONTAINING_RECORD((Current)->ListEntry.Flink, Type, ListEntry))
...
#define FOR_REST_IN_LIST_SAFE(Type, Head, Current, Next)                                                           \
    for ((Next) = CONTAINING_RECORD((Current)->ListEntry.Flink, Type, ListEntry); (Head) != &(Current)->ListEntry; \
         (Current) = (Next), (Next) = CONTAINING_RECORD((Current)->ListEntry.Flink, Type, ListEntry))
...
#define FOR_ALL_IN_LIST_BACKWARDS(Type, Head, Current)                                                   \
    for ((Current) = CONTAINING_RECORD((Head)->Blink, Type, ListEntry); (Head) != &(Current)->ListEntry; \
         (Current) = CONTAINING_RECORD((Current)->ListEntry.Blink, Type, ListEntry))

#define FOR_ALL_IN_LIST_SAFE_BACKWARDS(Type, Head, Current, Next)                \
    for ((Current) = CONTAINING_RECORD((Head)->Blink, Type, ListEntry),          \
        (Next) = CONTAINING_RECORD((Current)->ListEntry.Blink, Type, ListEntry); \
         (Head) != &(Current)->ListEntry;                                        \
         (Current) = (Next), (Next) = CONTAINING_RECORD((Current)->ListEntry.Blink, Type, ListEntry))
...
#define FOR_REST_IN_LIST_SAFE_BACKWARDS(Type, Head, Current, Next)                                                 \
    for ((Next) = CONTAINING_RECORD((Current)->ListEntry.Blink, Type, ListEntry); (Head) != &(Current)->ListEntry; \
         (Current) = (Next), (Next) = CONTAINING_RECORD((Current)->ListEntry.Blink, Type, ListEntry))
...
#define RANGE_LIMITS_DISJOINT(s1, e1, s2, e2) (((s1) < (s2) && (e1) < (s2)) || ((s2) < (s1) && (e2) < (s1)))
...
#define FOR_REST_OF_RANGES(_Iterator, _Current, _Forward)                   \
    for ((_Current) = (PRTL_RANGE)(_Iterator)->Current; (_Current) != NULL; \
