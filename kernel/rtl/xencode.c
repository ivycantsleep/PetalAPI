...
#define tag_write_mask(p, ptr, n, b)                                                                                   \
    do                                                                                                                 \
    {                                                                                                                  \
        if ((v.stat.bits -= (b)) < 0)                                                                                  \
        {                                                                                                              \
            *(__unaligned tag_t *)v.temp.tag_ptr = (v.temp.tag_mask << ((b) + v.stat.bits)) + ((n) >> (-v.stat.bits)); \
...
#define BIOWR(mask, bits)                                                                                \
    {                                                                                                    \
        ...
        if ((Bits -= (bits)) < 0)                                                                        \
        {                                                                                                \
            *(__unaligned bitmask2 *)Ptr1 = (bitmask2)((Mask << (Bits + (bits))) + ((mask) >> (-Bits))); \
            Mask = (mask);                                                                               \
...
static void huffman_create_codes(huff_info *info, uxint *freq, xint n, uxint *mask, uchar *length, uxint maxbits,
                                 uchar *encoded, uxint *total)
{
    huff_node *p, *q, *r, *first_sorted, *first_free;
    xint i, k;
...
static void encode_pass1_progress(prs *p, encode_pass1_proc *encode_pass1,
...
int XPRESS_CALL XpressEncode(XpressEncodeStream stream, void *comp, int comp_size, const void *orig, int orig_size,
                             XpressProgressFn *ProgressFn, // NULL or progress callback
                             void *ProgressContext,        // user-defined context that will be passed to ProgressFn
                             int ProgressSize              // call ProgressFn each time ProgressSize bytes processed
)
{
    ...
    if ((unsigned)(orig_size - 1) > info->max_size || orig_size <= MIN_SIZE || comp_size < MIN_SIZE || comp == 0 ||
        orig == 0)
    {
        return (orig_size);
    }
...
XpressEncodeStream XPRESS_CALL XpressEncodeCreate(int orig_size, void *context, XpressAllocFn *AllocFn, int chain)
{
    xpress_info *info;
    prs *p;
    uchar *b;
...
void XPRESS_CALL XpressEncodeClose(XpressEncodeStream stream, void *context, XpressFreeFn *FreeFn)
{
    xpress_info *info = (xpress_info *)stream;
