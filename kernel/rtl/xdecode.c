...
#define GET_UINT16(x, p)            \
    x = *(__unaligned uint16 *)(p); \
    p += 2
...
#ifdef __alpha
#define COPY_BLOCK_FAST_8(dst, src) COPY_8_BYTES(dst, src)
#else
#if 0
...
#else
#define COPY_BLOCK_FAST_8(dst, src) ((__unaligned __int64 *)dst)[0] = ((__unaligned __int64 *)src)[0]
#endif
#endif


#define BIORD(bits) (Mask >> (sizeof(Mask) * 8 - (bits)))
...
int XPRESS_CALL XpressDecode(XpressDecodeStream stream, void *orig, int orig_size, int decode_size, const void *comp,
                             int comp_size)
{
    decode_info *info;
    const uchar *src;
    ...
    if (orig_size < comp_size || comp_size < 0 || orig_size <= MIN_SIZE || comp_size < MIN_SIZE)
        return (-1);
