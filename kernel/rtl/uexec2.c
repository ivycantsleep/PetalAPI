...
NTSTATUS
main(IN ULONG argc, IN PCH argv[], IN PCH envp[], IN ULONG DebugParameter OPTIONAL)
{
    NTSTATUS st;
    ...
    if (!Peb->Sm.StandardInput.FileHandle)
    {
        ...
        InitializeObjectAttributes(&ObjectAttributes, &ConfigFilePathname, OBJ_CASE_INSENSITIVE, NULL, NULL);

        st = NtOpenFile(&FileHandle, SYNCHRONIZE | FILE_READ_DATA, &ObjectAttributes, &IoStatus, FILE_SHARE_READ, 0);
        ...
        st = NtQueryInformationFile(FileHandle, &IoStatus, (PVOID)&FileInformation, sizeof(FileInformation),
        ...
        st = RtlCreateUserProcess(&ImagePathName, NULL, NULL, NULL, FALSE, NULL, NULL, NULL, &ProcessInformation,
    ...
    else
    {
        ...
        if (!Peb->Sm.StandardOutput.FileHandle)
        {
            ...
            st = NtQueryInformationFile(Peb->Sm.StandardInput.FileHandle, &IoStatus, (PVOID)&FileInformation,
        ...
        else
        {
            ...
            if (!Peb->Sm.StandardError.FileHandle)
            {
                ...
                st = NtQueryInformationFile(Peb->Sm.StandardOutput.FileHandle, &IoStatus, (PVOID)&FileInformation,
                                            sizeof(FileInformation), FileInternalInformation);
                ...
                st = RtlCreateUserProcess(&ImagePathName, NULL, NULL, NULL, FALSE, NULL, NULL, NULL,
                                          &ProcessInformation, &PebSessionInformation);
            ...
            else
            {
                ...
                st = NtQueryInformationFile(Peb->Sm.StandardError.FileHandle, &IoStatus, (PVOID)&FileInformation,
                                            sizeof(FileInformation), FileInternalInformation);
