...
NTSTATUS
LZNT1CompressChunk(IN PLZNT1_MATCH_FUNCTION MatchFunction, IN PUCHAR UncompressedBuffer,
                   IN PUCHAR EndOfUncompressedBufferPlus1, OUT PUCHAR CompressedBuffer,
                   IN PUCHAR EndOfCompressedBufferPlus1, OUT PULONG FinalCompressedChunkSize, IN PVOID WorkSpace);

NTSTATUS
LZNT1CompressChunkHiber(IN PUCHAR UncompressedBuffer, IN PUCHAR EndOfUncompressedBufferPlus1,
                        OUT PUCHAR CompressedBuffer, IN PUCHAR EndOfCompressedBufferPlus1,
                        OUT PULONG FinalCompressedChunkSize, IN PVOID WorkSpace);

NTSTATUS
LZNT1DecompressChunk(OUT PUCHAR UncompressedBuffer, IN PUCHAR EndOfUncompressedBufferPlus1, IN PUCHAR CompressedBuffer,
                     IN PUCHAR EndOfCompressedBufferPlus1, OUT PULONG FinalUncompressedChunkSize);

ULONG
LZNT1FindMatchStandard(IN PUCHAR ZivString, IN PLZNT1_STANDARD_WORKSPACE WorkSpace);

ULONG
LZNT1FindMatchMaximum(IN PUCHAR ZivString, IN PVOID WorkSpace);

NTSTATUS
RtlCompressBufferLZNT1_HIBER(IN USHORT Engine, IN PUCHAR UncompressedBuffer, IN ULONG UncompressedBufferSize,
...
NTSTATUS
RtlCompressWorkSpaceSizeLZNT1(IN USHORT Engine, OUT PULONG CompressBufferWorkSpaceSize,
...

{
         ...
         UncompressedChunk += MAX_UNCOMPRESSED_CHUNK_SIZE, CompressedChunk += CompressedChunkSize)
    {
        ...
        if (!NT_SUCCESS(Status = LZNT1CompressChunkHiber(UncompressedChunk, EndOfUncompressedBuffer, CompressedChunk,
                                                         EndOfCompressedBuffer, &CompressedChunkSize, WorkSpace)))
        {
...
NTSTATUS
RtlCompressBufferLZNT1(IN USHORT Engine, IN PUCHAR UncompressedBuffer, IN ULONG UncompressedBufferSize,
                       OUT PUCHAR CompressedBuffer, IN ULONG CompressedBufferSize, IN ULONG UncompressedChunkSize,
                       OUT PULONG FinalCompressedSize, IN PVOID WorkSpace)
...

{
         ...
         UncompressedChunk += MAX_UNCOMPRESSED_CHUNK_SIZE, CompressedChunk += CompressedChunkSize)
    {
        ...
        if (!NT_SUCCESS(Status = LZNT1CompressChunk(MatchFunction, UncompressedChunk, EndOfUncompressedBuffer,
                                                    CompressedChunk, EndOfCompressedBuffer, &CompressedChunkSize,
                                                    WorkSpace)))
        {
    ...
    while (TRUE)
    {
        ...
        if (ChunkHeader.Chunk.IsChunkCompressed)
        {
            ...
            if (!NT_SUCCESS(Status =
                                LZNT1DecompressChunk(UncompressedChunk, EndOfUncompressedBuffer,
                                                     CompressedChunk + sizeof(COMPRESSED_CHUNK_HEADER),
                                                     CompressedChunk + CompressedChunkSize, &UncompressedChunkSize)))
            {
        ...
        if (UncompressedChunkSize < SavedChunkSize)
        {
            ...
            if ((t2 = (UncompressedChunk + (t1 = (SavedChunkSize - UncompressedChunkSize)))) >= EndOfUncompressedBuffer)
            {
...
RtlDecompressFragmentLZNT1(OUT PUCHAR UncompressedFragment, IN ULONG UncompressedFragmentSize,
                           ...
                           OUT PULONG FinalUncompressedSize, IN PLZNT1_FRAGMENT_WORKSPACE WorkSpace)
    ...
    while (TRUE)
    {
        ...
        if (ChunkHeader.Chunk.IsChunkCompressed)
        {
            ...
            if ((FragmentOffset == 0) && (CopySize == UncompressedChunkSize))
            {
                ...
                if (!NT_SUCCESS(Status = LZNT1DecompressChunk(CurrentUncompressedFragment, EndOfUncompressedFragment,
                                                              CompressedChunk + sizeof(COMPRESSED_CHUNK_HEADER),
                                                              CompressedChunk + CompressedChunkSize, &CopySize)))
                {
            ...
            else
            {
                ...
                if (!NT_SUCCESS(Status = LZNT1DecompressChunk(
                                    (PUCHAR)WorkSpace, &WorkSpace->Buffer[0] + sizeof(LZNT1_FRAGMENT_WORKSPACE),
                                    CompressedChunk + sizeof(COMPRESSED_CHUNK_HEADER),
                                    CompressedChunk + CompressedChunkSize, &UncompressedChunkSize)))
                {
                ...
                if ((UncompressedChunkSize - FragmentOffset) < CopySize)
                {
                    ...
                    RtlCopyMemory(CurrentUncompressedFragment, &WorkSpace->Buffer[FragmentOffset],
                ...
                RtlCopyMemory(CurrentUncompressedFragment, &WorkSpace->Buffer[FragmentOffset], CopySize);
            }
        }
        else
        {
            ...
            RtlCopyMemory(CurrentUncompressedFragment,
                          CompressedChunk + sizeof(COMPRESSED_CHUNK_HEADER) + FragmentOffset, CopySize);
        }
...

{
    ...
    while (NT_SUCCESS(Status = RtlDescribeChunkLZNT1(&NextChunk, EndOfCompressedBufferPlus1, &DontCare, &Size)))
    {
        ...
        if (*CompressedBuffer <= (EndOfCompressedBufferPlus1 - ChunkSize))
        {
            ...
            if (ChunkSize == MAX_UNCOMPRESSED_CHUNK_SIZE)
            {
                ...
                if ((*CompressedBuffer + ChunkSize + Size) <= EndOfCompressedBufferPlus1)
                {
                    ...
                    SetCompressedChunkHeader(ChunkHeader, MAX_UNCOMPRESSED_CHUNK_SIZE + 2, FALSE);
            ...
            else if (ChunkSize == 0)
            {
                ...
                if ((*CompressedBuffer + ChunkSize + Size) <= EndOfCompressedBufferPlus1)
                {
                    ...
                    SetCompressedChunkHeader(ChunkHeader, 6, TRUE);
...
typedef union _LZNT1_COPY_TOKEN
{
    ...
    {
        USHORT Length : 12;
        USHORT Displacement : 4;
    } Fields412;
    struct
    {
        USHORT Length : 11;
        USHORT Displacement : 5;
    } Fields511;
    struct
    {
        USHORT Length : 10;
        USHORT Displacement : 6;
    } Fields610;
    struct
    {
        USHORT Length : 9;
        USHORT Displacement : 7;
    } Fields79;
    struct
    {
        USHORT Length : 8;
        USHORT Displacement : 8;
    } Fields88;
    struct
    {
        USHORT Length : 7;
        USHORT Displacement : 9;
    } Fields97;
    struct
    {
        USHORT Length : 6;
        USHORT Displacement : 10;
    } Fields106;
    struct
    {
        USHORT Length : 5;
        USHORT Displacement : 11;
    } Fields115;
    struct
    {
        USHORT Length : 4;
...
#define SetLZNT1(F, CT, L, D)                      \
    {                                              \
        if (F == FORMAT412)                        \
        {                                          \
            (CT).Fields412.Length = (L) - 3;       \
            (CT).Fields412.Displacement = (D) - 1; \
        }                                          \
        else if (F == FORMAT511)                   \
        {                                          \
            (CT).Fields511.Length = (L) - 3;       \
            (CT).Fields511.Displacement = (D) - 1; \
        }                                          \
        else if (F == FORMAT610)                   \
        {                                          \
            (CT).Fields610.Length = (L) - 3;       \
            (CT).Fields610.Displacement = (D) - 1; \
        }                                          \
        else if (F == FORMAT79)                    \
        {                                          \
            (CT).Fields79.Length = (L) - 3;        \
            (CT).Fields79.Displacement = (D) - 1;  \
        }                                          \
        else if (F == FORMAT88)                    \
        {                                          \
            (CT).Fields88.Length = (L) - 3;        \
            (CT).Fields88.Displacement = (D) - 1;  \
        }                                          \
        else if (F == FORMAT97)                    \
        {                                          \
            (CT).Fields97.Length = (L) - 3;        \
            (CT).Fields97.Displacement = (D) - 1;  \
        }                                          \
        else if (F == FORMAT106)                   \
        {                                          \
            (CT).Fields106.Length = (L) - 3;       \
            (CT).Fields106.Displacement = (D) - 1; \
        }                                          \
        else if (F == FORMAT115)                   \
        {                                          \
            (CT).Fields115.Length = (L) - 3;       \
            (CT).Fields115.Displacement = (D) - 1; \
        }                                          \
        else                                       \
        {                                          \
            (CT).Fields124.Length = (L) - 3;       \
            (CT).Fields124.Displacement = (D) - 1; \
        }                                          \
    }
...
LZNT1CompressChunkHiber(IN PUCHAR UncompressedBuffer, IN PUCHAR EndOfUncompressedBufferPlus1,
                        OUT PUCHAR CompressedBuffer, IN PUCHAR EndOfCompressedBufferPlus1,
                        OUT PULONG FinalCompressedChunkSize, IN PLZNT1_HIBER_WORKSPACE WorkSpace)
...

{
    ...
    if (OutputPointer < EndOfCompressedChunkPlus1Minus16)
    {
        ...
        while (InputPointer <= EndOfUncompressedBufferPlus1Minus3)
        {
            ...
            if ((MatchedOffset < InputOffset) &&
                ...
                && (MatchedString[1] == InputPointer[1]) && (MatchedString[2] == InputPointer[2]))
            {
                ...
                for (;;)
                {
                    ...
                    {
                        if (InputPointer[Length] != MatchedString[Length])
                            break;
                        Length++;
                        if (InputPointer[Length] != MatchedString[Length])
                            break;
    ...
    if (InputPointer < EndOfUncompressedBufferPlus1)
    {
        ...
        if ((CompressedBuffer + MAX_UNCOMPRESSED_CHUNK_SIZE + sizeof(COMPRESSED_CHUNK_HEADER)) <=
            EndOfCompressedBufferPlus1)
        {

            COMPRESSED_CHUNK_HEADER ChunkHeader;

            RtlCopyMemory(CompressedBuffer + sizeof(COMPRESSED_CHUNK_HEADER), UncompressedBuffer,
            ...
            *FinalCompressedChunkSize = MAX_UNCOMPRESSED_CHUNK_SIZE + sizeof(COMPRESSED_CHUNK_HEADER);
            ...
            SetCompressedChunkHeader(ChunkHeader, (USHORT)*FinalCompressedChunkSize, FALSE);
...

    {
        ...
        *FinalCompressedChunkSize = (ULONG)(OutputPointer - CompressedBuffer);
        ...
        SetCompressedChunkHeader(ChunkHeader, (USHORT)*FinalCompressedChunkSize, TRUE);
...

{
    ...
    while (InputPointer < EndOfUncompressedBufferPlus1)
    {
        ...
        if (!Length)
        {
            ...
            if (OutputPointer >= EndOfCompressedChunkPlus1)
            {
                break;
            }
        ...
        else
        {
            ...
            if ((OutputPointer + 1) >= EndOfCompressedChunkPlus1)
            {
                break;
            }
...

{
    ...
    if ((FirstEntry >= UncompressedBuffer) && //  is it within the uncompressed buffer?
        ...
        (FirstEntry[1] == ZivString[1]) && (FirstEntry[2] == ZivString[2]))
    {
    ...
    if ((SecondEntry >= UncompressedBuffer) && //  is it within the uncompressed buffer?
        ...
        (SecondEntry[1] == ZivString[1]) && (SecondEntry[2] == ZivString[2]))
    {
...
ULONG
LZNT1FindMatchMaximum(IN PUCHAR ZivString, IN PLZNT1_MAXIMUM_WORKSPACE WorkSpace)
