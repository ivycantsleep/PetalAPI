...
NTSTATUS
NTAPI
RtlpStartThread(PUSER_THREAD_START_ROUTINE Function, PVOID Parameter, HANDLE *ThreadHandleReturn)
{
    return RtlCreateUserThread(NtCurrentProcess(), // process handle
...
--*/
{
    ...
    try
    {
        ...
        if (NtCurrentTeb()->IsImpersonating)
        {
            ...
            Status = NtOpenThreadToken(NtCurrentThread(), MAXIMUM_ALLOWED, TRUE, &Token);
            if (!NT_SUCCESS(Status))
            {
                leave;
            }
        ...
        Status = RtlpStartThreadFunc(Function, Parameter, &ThreadHandle);
    ...
    finally
    {
        ...
        if (Token)
        {
            ...
            NtSetInformationThread(NtCurrentThread(), ThreadImpersonationToken, &Token, sizeof(HANDLE));
            NtClose(Token);
        }
    }
...
NTSTATUS
NTAPI
RtlpExitThread(NTSTATUS Status)
{
    ...
    else
    {
        ...
        if (Event)
        {
            ...
            else
            {
                ...
                NtSetInformationObject(Event->Handle, ObjectHandleFlagInformation, &HandleInfo, sizeof(HandleInfo));
            }
        }
    }
...
--*/
{
    ...
    {
        RtlInterlockedPushEntrySList(&EventCache, &Event->Link);
    }
}
...
--*/
{
    ...
    {
        NtSetInformationThread(NtCurrentThread(), ThreadImpersonationToken, (PVOID)&ImpersonationToken,
                               (ULONG)sizeof(HANDLE));
    }
    ...
    __finally
    {
        ...
        if (NtCurrentTeb()->IsImpersonating)
        {
            ...
            NtSetInformationThread(NtCurrentThread(), ThreadImpersonationToken, (PVOID)&NewToken,
                                   (ULONG)sizeof(HANDLE));
        }
    }
}
