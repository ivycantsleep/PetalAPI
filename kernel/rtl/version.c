...
ULONG
RtlpVerGetConditionMask(ULONGLONG ConditionMask, ULONG TypeMask);
...
RtlGetVersion(OUT PRTL_OSVERSIONINFOW lpVersionInformation)
{
    ...
    if (lpVersionInformation->dwOSVersionInfoSize == sizeof(RTL_OSVERSIONINFOEXW))
    {
        ...
        ((PRTL_OSVERSIONINFOEXW)lpVersionInformation)->wProductType =
    ...
    if (lpVersionInformation->dwOSVersionInfoSize == sizeof(OSVERSIONINFOEXW))
    {
        ...
        if (RtlGetNtProductType(&NtProductType))
        {
            ...
            if (NtProductType == VER_NT_WORKSTATION)
            {
                ...
                //
                ((POSVERSIONINFOEXW)lpVersionInformation)->wSuiteMask =
                    ((POSVERSIONINFOEXW)lpVersionInformation)->wSuiteMask & 0xffef;
            }
        }
    }
...
BOOLEAN
RtlpVerCompare(LONG Condition, LONG Value1, LONG Value2, BOOLEAN *Equal, int Flags)
{
...
NTSTATUS
RtlVerifyVersionInfo(IN PRTL_OSVERSIONINFOEXW VersionInfo, IN ULONG TypeMask, IN ULONGLONG ConditionMask)
...

{
    ...
    if (TypeMask & VER_MAJORVERSION)
    {
        ...
        if (RtlpVerCompare(Condition, VersionInfo->dwMajorVersion, CurrVersion.dwMajorVersion, &Equal, 0) == FALSE)
        {
            if (!Equal)
            {
                return STATUS_REVISION_MISMATCH;
    ...
    if (Equal)
    {
        ...
        if (TypeMask & VER_MINORVERSION)
        {
            ...
            if (RtlpVerCompare(Condition, VersionInfo->dwMinorVersion, CurrVersion.dwMinorVersion, &Equal,
        ...
        if (Equal)
        {
            if (TypeMask & VER_SERVICEPACKMAJOR)
            {
                ...
                if (RtlpVerCompare(Condition, VersionInfo->wServicePackMajor, CurrVersion.wServicePackMajor, &Equal,
                                   0) == FALSE)
                {
                    if (!Equal)
                    {
            ...
            if (Equal)
            {
                if (TypeMask & VER_SERVICEPACKMINOR)
                {
                    ...
                    if (RtlpVerCompare(Condition, (ULONG)VersionInfo->wServicePackMinor,
                                       (ULONG)CurrVersion.wServicePackMinor, &Equal, LEXICAL_COMPARISON) == FALSE)
                    {
                        return STATUS_REVISION_MISMATCH;
                    }
                }
    ...
    if ((TypeMask & VER_BUILDNUMBER) &&
        RtlpVerCompare(RTL_GET_CONDITION(ConditionMask, VER_BUILDNUMBER), VersionInfo->dwBuildNumber,
                       CurrVersion.dwBuildNumber, &Equal, 0) == FALSE)
    {
        return STATUS_REVISION_MISMATCH;
    }

    if ((TypeMask & VER_PLATFORMID) &&
        RtlpVerCompare(RTL_GET_CONDITION(ConditionMask, VER_PLATFORMID), VersionInfo->dwPlatformId,
                       CurrVersion.dwPlatformId, &Equal, 0) == FALSE)
    {
        return STATUS_REVISION_MISMATCH;
    }
    ...
    if ((TypeMask & VER_PRODUCT_TYPE) &&
        RtlpVerCompare(RTL_GET_CONDITION(ConditionMask, VER_PRODUCT_TYPE), VersionInfo->wProductType,
                       CurrVersion.wProductType, &Equal, 0) == FALSE)
    {
        return STATUS_REVISION_MISMATCH;
    }
