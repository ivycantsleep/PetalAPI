...
#if DBG
VOID RtlpDumpWaits(PRTLP_WAIT_THREAD_CONTROL_BLOCK ThreadCB)
{
    ULONG Lupe;

    DbgPrintEx(
        DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_VERBOSE_MASK,
        "Current active waits [Handle, Wait] for thread %x:", HandleToUlong(NtCurrentTeb()->ClientId.UniqueThread));
    for (Lupe = 0; Lupe < ThreadCB->NumActiveWaits; Lupe++)
    {
        DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_VERBOSE_MASK, "%s  [%p, %p]", Lupe % 4 ? "" : "\n",
                   ThreadCB->ActiveWaitArray[Lupe], ThreadCB->ActiveWaitPointers[Lupe]);
    }
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_VERBOSE_MASK, "\n%d (0x%x) total waits\n",
               ThreadCB->NumActiveWaits, ThreadCB->NumActiveWaits);
}
#endif

VOID RtlpAddWait(PRTLP_WAIT Wait, PRTLP_GENERIC_TIMER Timer, PVOID ApcArgument3)
/*++
...
--*/
{
    ...
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK,
               "<%d> Wait %p (Handle %p) inserted as index %d in thread %x\n", Wait->DbgId, Wait, Wait->WaitHandle,
               ThreadCB->NumActiveWaits, HandleToUlong(NtCurrentTeb()->ClientId.UniqueThread));
#endif
    ...
    RtlInterlockedSetBitsDiscardReturn(&Wait->State, STATE_REGISTERED | STATE_ACTIVE);
    Wait->RefCount = 1;
    ...
    if (Wait->Timeout != INFINITE_TIME)
    {
        ...
        Wait->Timer->Period = (Wait->Flags & WT_EXECUTEONLYONCE) ? 0
                              : Wait->Timeout == INFINITE_TIME   ? 0
                                                                 : Wait->Timeout;

        RtlInterlockedSetBitsDiscardReturn(&Wait->Timer->State, STATE_REGISTERED | STATE_ACTIVE);
...
VOID RtlpAsyncWaitCallbackCompletion(PVOID Context)
/*++
...
--*/
{
    ...
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK,
               ...
               Wait->WaitHandle, Wait->Function, Wait->Context, (ULONG)TimedOut,
               HandleToUlong(NtCurrentTeb()->ClientId.UniqueThread),
               HandleToUlong(NtCurrentTeb()->ClientId.UniqueProcess));
#endif

    RtlpWaitOrTimerCallout(Wait->Function, Wait->Context, TimedOut, Wait->ActivationContext, Wait->ImpersonationToken);
...
NTSTATUS
RtlpDeactivateWaitWithIndex(PRTLP_WAIT Wait, BOOLEAN OkayToFreeTheTimer, ULONG ArrayIndex)
/*++
...
--*/
{
    ...
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK,
               ...
               ArrayIndex, HandleToUlong(NtCurrentTeb()->ClientId.UniqueThread));
#endif
    ...
    RtlInterlockedClearBitsDiscardReturn(&Wait->State, STATE_ACTIVE);

    return STATUS_SUCCESS;
}

NTSTATUS
RtlpDeactivateWait(PRTLP_WAIT Wait, BOOLEAN OkayToFreeTheTimer)
/*++
...
--*/
{
    ...
    else
    {
        ...
        if (Wait->Timer)
        {
            ...
            if (RtlpReOrderDeltaList(&ThreadCB->TimerQueue.TimerList, Wait->Timer, TimeRemaining, &NewFiringTime,
                                     Wait->Timer->Period))
            {
    ...
    if (Wait->Flags & WT_EXECUTEINWAITTHREAD)
    {
        ...
        DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK,
                   ...
                   Wait, Wait->WaitHandle, Wait->Function, Wait->Context, FALSE,
                   HandleToUlong(NtCurrentTeb()->ClientId.UniqueThread),
                   HandleToUlong(NtCurrentTeb()->ClientId.UniqueProcess));
#endif

        RtlpWaitOrTimerCallout(Wait->Function, Wait->Context, FALSE, Wait->ActivationContext, Wait->ImpersonationToken);
    ...
    else
    {
        ...
        Status = RtlQueueWorkItem(RtlpAsyncWaitCallbackCompletion, Wait, Wait->Flags);
...
LONG RtlpWaitThread(PVOID Parameter)
/*++
...
#define WAIT_IDLE_TIMEOUT 400000
...
#if DBG
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK, "Starting wait thread\n");
#endif
    ...
    for (;;)
    {
        ...
        if (ThreadCB->NumActiveWaits == 1 && ThreadCB->NumRegisteredWaits == 0)
        {
        ...
        Status = NtWaitForMultipleObjects((CHAR)ThreadCB->NumActiveWaits, ThreadCB->ActiveWaitArray, WaitAny,
                                          TRUE, // Wait Alertably
                                          TimeOutPtr);
        ...
        else if (Status >= STATUS_ABANDONED_WAIT_0 && Status <= STATUS_ABANDONED_WAIT_63)
        {
            ...
            DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_ERROR_MASK,
                       ...
                       Status - STATUS_ABANDONED_WAIT_0, ThreadCB->ActiveWaitArray[Status - STATUS_ABANDONED_WAIT_0]);
#endif
            ...
            RtlpDeactivateWaitWithIndex(Wait, TRUE, Status - STATUS_ABANDONED_WAIT_0);
        }
        ...
        else
        {
            ...
            for (i = 0; i < ThreadCB->NumActiveWaits; i++)
            {
                ...
                Status = NtWaitForSingleObject(ThreadCB->ActiveWaitArray[i],
                ...
                else if (Status != STATUS_TIMEOUT)
                {
                    ...
                    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID,
                               Status == STATUS_ABANDONED_WAIT_0 ? RTLP_THREADPOOL_ERROR_MASK
                                                                 : RTLP_THREADPOOL_WARNING_MASK,
                               "<%d> %s: index:%d Handle:%p WaitEntry Ptr:%p\n", ThreadCB->ActiveWaitPointers[i]->DbgId,
                               Status == STATUS_ABANDONED_WAIT_0 ? "Abandoned wait" : "Deactivating invalid handle", i,
                               ThreadCB->ActiveWaitArray[i], ThreadCB->ActiveWaitPointers[i]);
#endif
                    RtlpDeactivateWaitWithIndex(ThreadCB->ActiveWaitPointers[i], TRUE, i);
...
// N.B. The WaitCriticalSection MUST be held when calling this function
{
    ...
    Status = NtCreateTimer(&WaitThreadTimer, TIMER_ALL_ACCESS, NULL, NotificationTimer);
...
NTSTATUS
RtlpFindWaitThread(PRTLP_WAIT_THREAD_CONTROL_BLOCK *ThreadCB)
/*++
...
VOID RtlpWaitReleaseWorker(ULONG Flags)
{
...
NTSTATUS
RtlRegisterWait(OUT PHANDLE WaitHandle, IN HANDLE Handle, IN WAITORTIMERCALLBACKFUNC Function, IN PVOID Context,
                IN ULONG Milliseconds, IN ULONG Flags)
...

{
    ...
    *WaitHandle = NULL;
    ...
    {
        Timer = RtlpAllocateTPHeap(sizeof(RTLP_GENERIC_TIMER), HEAP_ZERO_MEMORY);
        if (!Timer)
        {
            RtlpFreeTPHeap(Wait);
            RtlpWaitReleaseWorker(Flags);
    ...
    {
        Status = NtOpenThreadToken(NtCurrentThread(), MAXIMUM_ALLOWED, TRUE, &Wait->ImpersonationToken);
        if (!NT_SUCCESS(Status))
        {
            if (Timer)
            {
    ...
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK,
               ...
               Wait->WaitHandle, HandleToUlong(NtCurrentTeb()->ClientId.UniqueThread),
               HandleToUlong(NtCurrentTeb()->ClientId.UniqueProcess));
#endif

    // Queue an APC to the Wait Thread

    Status = NtQueueApcThread(ThreadCB->ThreadHandle, (PPS_APC_ROUTINE)RtlpAddWait, (PVOID)Wait, (PVOID)Timer, NULL);
...
NTSTATUS
RtlpDeregisterWait(PRTLP_WAIT Wait, HANDLE PartialCompletionEvent, PULONG RetStatusPtr)
/*++
...
--*/
{
    ...
    if (!(Wait->State & STATE_REGISTERED))
    {
        ...
        RtlInterlockedSetBitsDiscardReturn(&Wait->State, STATE_DELETE);
    ...
    RtlInterlockedClearBitsDiscardReturn(&Wait->State, STATE_REGISTERED);
    RtlInterlockedSetBitsDiscardReturn(&Wait->State, STATE_DELETE);
...
NTSTATUS
RtlDeregisterWaitEx(IN HANDLE WaitHandle, IN HANDLE Event)
/*++
...

{
    ...
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK,
               "<%d:%d> Wait %p (Handle %p) deregistering by thread:<%x:%x>\n", WaitDbgId, Wait->RefCount, Wait, Handle,
               CurrentThreadId, HandleToUlong(NtCurrentTeb()->ClientId.UniqueProcess));
#endif
    ...
    else
    {
        ...
        Status = NtQueueApcThread(Wait->ThreadCB->ThreadHandle, (PPS_APC_ROUTINE)RtlpDeregisterWait, (PVOID)Wait,
...
NTSTATUS
RtlDeregisterWait(IN HANDLE WaitHandle)
/*++
...
#if DBG
    DbgPrintEx(DPFLTR_RTLTHREADPOOL_ID, RTLP_THREADPOOL_TRACE_MASK, "<%d> Wait %p (Handle %p) deleted in thread:%x\n",
