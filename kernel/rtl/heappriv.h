...
#define RtlInitializeLockRoutine(L) \
    RtlInitializeCriticalSectionAndSpinCount((PRTL_CRITICAL_SECTION)(L), (PREALLOCATE_EVENT_MASK | 4000))
...
#define HEAP_DEBUG_FLAGS                                                                            \
    (HEAP_VALIDATE_PARAMETERS_ENABLED | HEAP_VALIDATE_ALL_ENABLED | HEAP_CAPTURE_STACK_BACKTRACES | \
     HEAP_CREATE_ENABLE_TRACING | HEAP_FLAG_PAGE_ALLOCS)
#define DEBUG_HEAP(F) ((F & HEAP_DEBUG_FLAGS) && !(F & HEAP_SKIP_VALIDATION_CHECKS))
#define SET_LAST_STATUS(S)                                                                                    \
    {                                                                                                         \
        NtCurrentTeb()->LastErrorValue = RtlNtStatusToDosError(NtCurrentTeb()->LastStatusValue = (ULONG)(S)); \
    }
...
#define HeapDebugPrint(_x_)                                                          \
    {                                                                                \
        ...
        _Entry = CONTAINING_RECORD(_Module, LDR_DATA_TABLE_ENTRY, InLoadOrderLinks); \
...
#define RtlpGetLookasideHeap(H) (((H)->FrontEndHeapType == HEAP_FRONT_LOOKASIDE) ? (H)->FrontEndHeap : NULL)

#define RtlpGetLowFragHeap(H) (((H)->FrontEndHeapType == HEAP_FRONT_LOWFRAGHEAP) ? (H)->FrontEndHeap : NULL)

#define RtlpIsFrontHeapUnlocked(H) ((H)->FrontHeapLockCount == 0)
...
#define HEAP_PERF_STOP_TIMER(H, OP)                                                             \
    {                                                                                           \
        ...
        {                                                                                       \
            ...
            {                                                                                   \
                HeapIndex->PerfData.OperationTime[OP] =                                         \
                    HeapIndex->PerfData.TempTime[OP] / (HeapIndex->PerfData.TempCount[OP] - 1); \
                                                                                                \
...
#define RtlpCheckLargeCache(H)                                                                      \
    {                                                                                               \
        PHEAP_INDEX HeapIndex = (PHEAP_INDEX)(H)->LargeBlocksIndex;                                 \
        if ((HeapIndex != NULL) && (HeapIndex->LargeBlocksCacheSequence >= RtlpSequenceNumberTest)) \
        {                                                                                           \
                                                                                                    \
...
#define RtlFindFirstSetRightMember(Set)                                                                               \
    (((Set) & 0xFFFF) ? (((Set) & 0xFF) ? RtlpBitsClearLow[(Set) & 0xFF] : RtlpBitsClearLow[((Set) >> 8) & 0xFF] + 8) \
                      : ((((Set) >> 16) & 0xFF) ? RtlpBitsClearLow[((Set) >> 16) & 0xFF] + 16                         \
...
#define RtlpInsertFreeBlockDirect(H, FB, SIZE)                                                                         \
    {                                                                                                                  \
        ...
        (FB)->Flags &= ~(HEAP_ENTRY_FILL_PATTERN | HEAP_ENTRY_EXTRA_PRESENT | HEAP_ENTRY_BUSY);                        \
                                                                                                                       \
        if ((H)->Flags & HEAP_FREE_CHECKING_ENABLED)                                                                   \
        {                                                                                                              \
                                                                                                                       \
            RtlFillMemoryUlong((PCHAR)((FB) + 1), ((SIZE) << HEAP_GRANULARITY_SHIFT) - sizeof(*(FB)), FREE_HEAP_FILL); \
                                                                                                                       \
        ...
        {                                                                                                              \
                                                                                                                       \
            _HEAD = &(H)->FreeLists[0];                                                                                \
            _NEXT = (H)->LargeBlocksIndex ? RtlpFindEntry(H, SIZE) : _HEAD->Flink;                                     \
                                                                                                                       \
            while (_HEAD != _NEXT)                                                                                     \
            {                                                                                                          \
                                                                                                                       \
    ...
    {                                                                                                   \
        ...
        {                                                                                               \
            ...
            cb = ((FB)->Size << HEAP_GRANULARITY_SHIFT) - sizeof(*(FB));                                \
                                                                                                        \
            if ((FB)->Flags & HEAP_ENTRY_EXTRA_PRESENT && cb > sizeof(HEAP_FREE_ENTRY_EXTRA))           \
            {                                                                                           \
                                                                                                        \
...
#define RtlpFastRemoveFreeBlock(H, FB)                                         \
    {                                                                          \
        ...
        _EX_Flink->Blink = _EX_Blink;                                          \
                                                                               \
        if ((_EX_Flink == _EX_Blink) && ((FB)->Size < HEAP_MAXIMUM_FREELISTS)) \
        {                                                                      \
                                                                               \
...
#define RtlpIsLowFragHeapEnabled() ((RtlpDisableHeapLookaside & HEAP_ENABLE_LOW_FRAG_HEAP) != 0)
...
ULONG
FORCEINLINE
RtlpGetAllocationUnits(PHEAP Heap, PHEAP_ENTRY Block)
{
...
VOID FORCEINLINE RtlpSetUnusedBytes(PHEAP Heap, PHEAP_ENTRY Block, SIZE_T UnusedBytes)
{
    if (UnusedBytes < 0xff)
    {
