...
int main(int argc, char *argv[])
{
    ...
    if (RtlNumberOfClearBits(BitMap) != 2048 * 8)
    {
        DbgPrint("Number of Clear bits error 1\n");
    }
    if (RtlNumberOfSetBits(BitMap) != 0)
    {
        DbgPrint("Number of Set bits error 1\n");
    }
    ...
    if (RtlNumberOfClearBits(BitMap) != 2048 * 8 - 68)
    {
        DbgPrint("Number of Clear bits error 2\n");
    }
    if (RtlNumberOfSetBits(BitMap) != 68)
    {
        DbgPrint("Number of Set bits error 2\n");
    }
    ...
    {
        DbgPrint("Number of Clear bits error 3\n");
    }
    if (RtlNumberOfSetBits(BitMap) != 2048 * 8)
    {
        DbgPrint("Number of Set bits error 3\n");
    }
    ...
    if (RtlNumberOfClearBits(BitMap) != 0)
    {
        DbgPrint("Number of Clear bits error 4\n");
    }
    if (RtlNumberOfSetBits(BitMap) != 2048 * 8)
    {
        DbgPrint("Number of Set bits error 4\n");
    }
    ...
    RtlClearAllBits(BitMap);
    if (!RtlAreBitsClear(BitMap, 0, 2048 * 8))
    {
        DbgPrint("RtlAreBitsClear  Error 0\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 8, 8))
    {
        DbgPrint("AreBitsClear Error 1\n");
    }
    RtlClearBits(BitMap, 9, 6);
    if (RtlAreBitsClear(BitMap, 8, 8))
    {
        DbgPrint("AreBitsClear Error 2\n");
    }
    RtlClearBits(BitMap, 8, 1);
    if (RtlAreBitsClear(BitMap, 8, 8))
    {
        DbgPrint("AreBitsClear Error 3\n");
    }
    RtlClearBits(BitMap, 15, 1);
    if (!RtlAreBitsClear(BitMap, 8, 8))
    {
        DbgPrint("AreBitsClear Error 4\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 8, 7))
    {
        DbgPrint("AreBitsClear Error 5\n");
    }
    RtlClearBits(BitMap, 9, 5);
    if (RtlAreBitsClear(BitMap, 8, 7))
    {
        DbgPrint("AreBitsClear Error 6\n");
    }
    RtlClearBits(BitMap, 8, 1);
    if (RtlAreBitsClear(BitMap, 8, 7))
    {
        DbgPrint("AreBitsClear Error 7\n");
    }
    RtlClearBits(BitMap, 14, 1);
    if (!RtlAreBitsClear(BitMap, 8, 7))
    {
        DbgPrint("AreBitsClear Error 8\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 9, 7))
    {
        DbgPrint("AreBitsClear Error 9\n");
    }
    RtlClearBits(BitMap, 10, 5);
    if (RtlAreBitsClear(BitMap, 9, 7))
    {
        DbgPrint("AreBitsClear Error 10\n");
    }
    RtlClearBits(BitMap, 9, 1);
    if (RtlAreBitsClear(BitMap, 9, 7))
    {
        DbgPrint("AreBitsClear Error 11\n");
    }
    RtlClearBits(BitMap, 15, 1);
    if (!RtlAreBitsClear(BitMap, 9, 7))
    {
        DbgPrint("AreBitsClear Error 12\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 9, 5))
    {
        DbgPrint("AreBitsClear Error 13\n");
    }
    RtlClearBits(BitMap, 10, 3);
    if (RtlAreBitsClear(BitMap, 9, 5))
    {
        DbgPrint("AreBitsClear Error 14\n");
    }
    RtlClearBits(BitMap, 9, 1);
    if (RtlAreBitsClear(BitMap, 9, 5))
    {
        DbgPrint("AreBitsClear Error 15\n");
    }
    RtlClearBits(BitMap, 13, 1);
    if (!RtlAreBitsClear(BitMap, 9, 5))
    {
        DbgPrint("AreBitsClear Error 16\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 8, 24))
    {
        DbgPrint("AreBitsClear Error 17\n");
    }
    RtlClearBits(BitMap, 9, 22);
    if (RtlAreBitsClear(BitMap, 8, 24))
    {
        DbgPrint("AreBitsClear Error 18\n");
    }
    RtlClearBits(BitMap, 8, 1);
    if (RtlAreBitsClear(BitMap, 8, 24))
    {
        DbgPrint("AreBitsClear Error 19\n");
    }
    RtlClearBits(BitMap, 31, 1);
    if (!RtlAreBitsClear(BitMap, 8, 24))
    {
        DbgPrint("AreBitsClear Error 20\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 8, 23))
    {
        DbgPrint("AreBitsClear Error 21\n");
    }
    RtlClearBits(BitMap, 9, 21);
    if (RtlAreBitsClear(BitMap, 8, 23))
    {
        DbgPrint("AreBitsClear Error 22\n");
    }
    RtlClearBits(BitMap, 8, 1);
    if (RtlAreBitsClear(BitMap, 8, 23))
    {
        DbgPrint("AreBitsClear Error 23\n");
    }
    RtlClearBits(BitMap, 30, 1);
    if (!RtlAreBitsClear(BitMap, 8, 23))
    {
        DbgPrint("AreBitsClear Error 24\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 9, 23))
    {
        DbgPrint("AreBitsClear Error 25\n");
    }
    RtlClearBits(BitMap, 10, 21);
    if (RtlAreBitsClear(BitMap, 9, 23))
    {
        DbgPrint("AreBitsClear Error 26\n");
    }
    RtlClearBits(BitMap, 9, 1);
    if (RtlAreBitsClear(BitMap, 9, 23))
    {
        DbgPrint("AreBitsClear Error 27\n");
    }
    RtlClearBits(BitMap, 31, 1);
    if (!RtlAreBitsClear(BitMap, 9, 23))
    {
        DbgPrint("AreBitsClear Error 28\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 9, 21))
    {
        DbgPrint("AreBitsClear Error 29\n");
    }
    RtlClearBits(BitMap, 10, 19);
    if (RtlAreBitsClear(BitMap, 9, 21))
    {
        DbgPrint("AreBitsClear Error 30\n");
    }
    RtlClearBits(BitMap, 9, 1);
    if (RtlAreBitsClear(BitMap, 9, 21))
    {
        DbgPrint("AreBitsClear Error 31\n");
    }
    RtlClearBits(BitMap, 29, 1);
    if (!RtlAreBitsClear(BitMap, 9, 21))
    {
        DbgPrint("AreBitsClear Error 32\n");
    }

    RtlSetAllBits(BitMap);
    if (RtlAreBitsClear(BitMap, 10, 1))
    {
        DbgPrint("AreBitsClear Error 33\n");
    }
    RtlClearBits(BitMap, 9, 1);
    if (RtlAreBitsClear(BitMap, 10, 1))
    {
        DbgPrint("AreBitsClear Error 34\n");
    }
    RtlClearBits(BitMap, 11, 1);
    if (RtlAreBitsClear(BitMap, 10, 1))
    {
        DbgPrint("AreBitsClear Error 35\n");
    }
    RtlClearBits(BitMap, 10, 1);
    if (!RtlAreBitsClear(BitMap, 10, 1))
    {
        DbgPrint("AreBitsClear Error 36\n");
    }


    RtlSetAllBits(BitMap);
    if (!RtlAreBitsSet(BitMap, 0, 2048 * 8))
    {
        DbgPrint("RtlAreBitsSet  Error 0\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 8, 8))
    {
        DbgPrint("AreBitsSet Error 1\n");
    }
    RtlSetBits(BitMap, 9, 6);
    if (RtlAreBitsSet(BitMap, 8, 8))
    {
        DbgPrint("AreBitsSet Error 2\n");
    }
    RtlSetBits(BitMap, 8, 1);
    if (RtlAreBitsSet(BitMap, 8, 8))
    {
        DbgPrint("AreBitsSet Error 3\n");
    }
    RtlSetBits(BitMap, 15, 1);
    if (!RtlAreBitsSet(BitMap, 8, 8))
    {
        DbgPrint("AreBitsSet Error 4\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 8, 7))
    {
        DbgPrint("AreBitsSet Error 5\n");
    }
    RtlSetBits(BitMap, 9, 5);
    if (RtlAreBitsSet(BitMap, 8, 7))
    {
        DbgPrint("AreBitsSet Error 6\n");
    }
    RtlSetBits(BitMap, 8, 1);
    if (RtlAreBitsSet(BitMap, 8, 7))
    {
        DbgPrint("AreBitsSet Error 7\n");
    }
    RtlSetBits(BitMap, 14, 1);
    if (!RtlAreBitsSet(BitMap, 8, 7))
    {
        DbgPrint("AreBitsSet Error 8\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 9, 7))
    {
        DbgPrint("AreBitsSet Error 9\n");
    }
    RtlSetBits(BitMap, 10, 5);
    if (RtlAreBitsSet(BitMap, 9, 7))
    {
        DbgPrint("AreBitsSet Error 10\n");
    }
    RtlSetBits(BitMap, 9, 1);
    if (RtlAreBitsSet(BitMap, 9, 7))
    {
        DbgPrint("AreBitsSet Error 11\n");
    }
    RtlSetBits(BitMap, 15, 1);
    if (!RtlAreBitsSet(BitMap, 9, 7))
    {
        DbgPrint("AreBitsSet Error 12\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 9, 5))
    {
        DbgPrint("AreBitsSet Error 13\n");
    }
    RtlSetBits(BitMap, 10, 3);
    if (RtlAreBitsSet(BitMap, 9, 5))
    {
        DbgPrint("AreBitsSet Error 14\n");
    }
    RtlSetBits(BitMap, 9, 1);
    if (RtlAreBitsSet(BitMap, 9, 5))
    {
        DbgPrint("AreBitsSet Error 15\n");
    }
    RtlSetBits(BitMap, 13, 1);
    if (!RtlAreBitsSet(BitMap, 9, 5))
    {
        DbgPrint("AreBitsSet Error 16\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 8, 24))
    {
        DbgPrint("AreBitsSet Error 17\n");
    }
    RtlSetBits(BitMap, 9, 22);
    if (RtlAreBitsSet(BitMap, 8, 24))
    {
        DbgPrint("AreBitsSet Error 18\n");
    }
    RtlSetBits(BitMap, 8, 1);
    if (RtlAreBitsSet(BitMap, 8, 24))
    {
        DbgPrint("AreBitsSet Error 19\n");
    }
    RtlSetBits(BitMap, 31, 1);
    if (!RtlAreBitsSet(BitMap, 8, 24))
    {
        DbgPrint("AreBitsSet Error 20\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 8, 23))
    {
        DbgPrint("AreBitsSet Error 21\n");
    }
    RtlSetBits(BitMap, 9, 21);
    if (RtlAreBitsSet(BitMap, 8, 23))
    {
        DbgPrint("AreBitsSet Error 22\n");
    }
    RtlSetBits(BitMap, 8, 1);
    if (RtlAreBitsSet(BitMap, 8, 23))
    {
        DbgPrint("AreBitsSet Error 23\n");
    }
    RtlSetBits(BitMap, 30, 1);
    if (!RtlAreBitsSet(BitMap, 8, 23))
    {
        DbgPrint("AreBitsSet Error 24\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 9, 23))
    {
        DbgPrint("AreBitsSet Error 25\n");
    }
    RtlSetBits(BitMap, 10, 21);
    if (RtlAreBitsSet(BitMap, 9, 23))
    {
        DbgPrint("AreBitsSet Error 26\n");
    }
    RtlSetBits(BitMap, 9, 1);
    if (RtlAreBitsSet(BitMap, 9, 23))
    {
        DbgPrint("AreBitsSet Error 27\n");
    }
    RtlSetBits(BitMap, 31, 1);
    if (!RtlAreBitsSet(BitMap, 9, 23))
    {
        DbgPrint("AreBitsSet Error 28\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 9, 21))
    {
        DbgPrint("AreBitsSet Error 29\n");
    }
    RtlSetBits(BitMap, 10, 19);
    if (RtlAreBitsSet(BitMap, 9, 21))
    {
        DbgPrint("AreBitsSet Error 30\n");
    }
    RtlSetBits(BitMap, 9, 1);
    if (RtlAreBitsSet(BitMap, 9, 21))
    {
        DbgPrint("AreBitsSet Error 31\n");
    }
    RtlSetBits(BitMap, 29, 1);
    if (!RtlAreBitsSet(BitMap, 9, 21))
    {
        DbgPrint("AreBitsSet Error 32\n");
    }

    RtlClearAllBits(BitMap);
    if (RtlAreBitsSet(BitMap, 10, 1))
    {
        DbgPrint("AreBitsSet Error 33\n");
    }
    RtlSetBits(BitMap, 9, 1);
    if (RtlAreBitsSet(BitMap, 10, 1))
    {
        DbgPrint("AreBitsSet Error 34\n");
    }
    RtlSetBits(BitMap, 11, 1);
    if (RtlAreBitsSet(BitMap, 10, 1))
    {
        DbgPrint("AreBitsSet Error 35\n");
    }
    RtlSetBits(BitMap, 10, 1);
    if (!RtlAreBitsSet(BitMap, 10, 1))
    {
        DbgPrint("AreBitsSet Error 36\n");
    }
