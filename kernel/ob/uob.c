...
VOID DumpObjectDirs(IN PCH DirName, IN ULONG Level)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, &Name, OBJ_OPENIF | OBJ_CASE_INSENSITIVE, NULL, NULL);
    ...
    {
        Status =
            NtQueryDirectoryObject(Handle, (PVOID)DirInfo, sizeof(DirInfoBuffer), TRUE, RestartScan, &Context, &Length);
        if (!NT_SUCCESS(Status))
        {
            break;
        }

        DbgPrint("%s%s%Z - %Z", DirName, Level ? "\\" : "", &DirInfo->Name, &DirInfo->TypeName);
        ...
        else if (RtlEqualString(&DirInfo->TypeName, &LinkTypeName, TRUE))
        {
            strcpy(SubDirName, DirName);
            if (Level)
            {
            ...
            InitializeObjectAttributes(&ObjectAttributes, &LinkName, 0, NULL, NULL);
...
main(int argc, char **argv, char **envp, int DebugFlag)
{
    ...
    if (argc == 1)
    {
        ...
        Parameters[RTL_USER_PROC_PARAMS_DEBUGFLAG] = DebugFlag ? "1" : "0";
        ...
        Status = RtlVectorsToProcessParameters(Arguments, envp, Parameters, ProcessParameters);
        if (!NT_SUCCESS(Status))
        {
        ...
        else
        {
            ...
            Status = RtlCreateUserProcess(&ImageName, NULL, NULL, NULL, TRUE, NULL, NULL, ProcessParameters,
            ...
            else
            {
                ...
                Status = NtWaitForSingleObject(ProcessInformation.Process, FALSE, (PLARGE_INTEGER)NULL);
                if (!NT_SUCCESS(Status))
                {
...
VOID TestParent(VOID)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, &SectionName, OBJ_OPENIF | OBJ_CASE_INSENSITIVE, NULL, NULL);

    SectionSize.LowPart = 0x1000;
    SectiinSize.HighPart = 0;
    Status = NtCreateSection(&SectionHandle, GENERIC_READ, &ObjectAttributes, &SectionSize, PAGE_READONLY, SEC_RESERVE,
                             NULL);
    if (!NT_SUCCESS(Status))
    {
    ...
    InitializeObjectAttributes(&ObjectAttributes, &DirectoryName, OBJ_CASE_INSENSITIVE | OBJ_PERMANENT, NULL,
                               (PSECURITY_DESCRIPTOR)1

    );
    ObjectAttributes.Length = 0;
    Status = NtCreateDirectoryObject(&DirectoryHandle, -1, &ObjectAttributes);
    if (!NT_SUCCESS(Status))
    {
    ...
    InitializeObjectAttributes(&ObjectAttributes, &DirectoryName, -1, NULL, (PSECURITY_DESCRIPTOR)1
    ...
    InitializeObjectAttributes(&ObjectAttributes, &DirectoryName, OBJ_CASE_INSENSITIVE | OBJ_PERMANENT, NULL, NULL
    ...
    Status = NtQueryObject(DirectoryHandle, ObjectBasicInformation, &ObjectBasicInfo, sizeof(ObjectBasicInfo),
                           &ReturnedLength);
    if (!NT_SUCCESS(Status))
    {
        DbgPrint("NtQueryObject( %lx, ObjectBasicInfo ) failed - Status == %X\n", DirectoryHandle, Status);
        NtTerminateProcess(NtCurrentProcess(), Status);
    }
    DbgPrint("NtQueryObject( %lx, ObjectBasicInfo ) returned %lx bytes\n", DirectoryHandle, ReturnedLength);
    ...
    Status = NtQueryObject(DirectoryHandle, ObjectNameInformation, ObjectNameInfo, sizeof(ObjectInfoBuffer),
                           &ReturnedLength);
    if (!NT_SUCCESS(Status))
    {
    ...
    Status = NtQueryObject(DirectoryHandle, ObjectTypeInformation, ObjectTypeInfo, sizeof(ObjectInfoBuffer),
                           &ReturnedLength);
    if (!NT_SUCCESS(Status))
    {
    ...
    DbgPrint("    TypeName = (%ld,%ld) '%Z'\n", ObjectTypeInfo->TypeName.MaximumLength, ObjectTypeInfo->TypeName.Length,
             &ObjectTypeInfo->TypeName);

    RtlInitString(&LinkName, "TestSymbolicLink");
    InitializeObjectAttributes(&ObjectAttributes, &LinkName, OBJ_CASE_INSENSITIVE, NULL, NULL);
    ObjectAttributes.RootDirectory = DirectoryHandle;
    RtlInitString(&LinkTarget, "\\Device\\FileSystem");
    Status = NtCreateSymbolicLinkObject(&LinkHandle, SYMBOLIC_LINK_ALL_ACCESS, &ObjectAttributes, &LinkTarget);
    ...
    InitializeObjectAttributes(&ObjectAttributes, &DirectoryName, OBJ_CASE_INSENSITIVE | OBJ_EXCLUSIVE, NULL, NULL
