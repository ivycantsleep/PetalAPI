...
ClientThread(LPVOID Context)
{
    ...
    {
        Status = SendRequest(1, ThreadName, PortHandle, Context, MsgLength, NULL, FALSE);
    ...
    fprintf(stderr, "%s %s\n", NT_SUCCESS(Status) ? "Exiting" : "ABORTING", ThreadName);
...
int _cdecl main(int argc, char *argv[])
{
    ...
    if (ClientNumber > MAX_CLIENT_PROCESSES || NumberOfThreads > MAX_CLIENT_THREADS)
    {
        Usage();
    }
    ...
    Status = NtCreateSection(&ClientView.SectionHandle, SECTION_MAP_READ | SECTION_MAP_WRITE, NULL, &MaximumSize,
    ...
    if (ShowHandleOrStatus(Status, ClientView.SectionHandle))
    {
        ...
        Status = NtConnectPort(&PortHandle, &PortName, &DynamicQos, &ClientView, &ServerView, (PULONG)&MaxMessageLength,
        ...
        if (ShowHandleOrStatus(Status, PortHandle))
        {
            ...
            fprintf(stderr, "    ConnectionInfo: (%ld) '%.*s'\n", ConnectionInformationLength,
                    ConnectionInformationLength, (PSZ)&ConnectionInformation[0]);
            fprintf(stderr, "    ClientView: Base=%lx, Size=%lx, RemoteBase: %lx\n", ClientView.ViewBase,
                    ClientView.ViewSize, ClientView.ViewRemoteBase);
            fprintf(stderr, "    ServerView: Base=%lx, Size=%lx\n", ServerView.ViewBase, ServerView.ViewSize);
    ...
    if (rc == NO_ERROR)
    {
        ...
        for (i = 1; i < NumberOfThreads; i++)
        {
            ...
            ClientThreadHandles[i] =
                CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ClientThread, (LPVOID)((ClientNumber << 4) | (i + 1)),
                             CREATE_SUSPENDED, &ClientThreadClientIds[i]);
