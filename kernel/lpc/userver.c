...
VOID ServerHandleConnectionRequest(IN PTLPC_PORTMSG Msg)
{
    ...
    fprintf(stderr, "    ConnectionInfo: (%ld) '%.*s'\n", ConnectionInformationLength, ConnectionInformationLength,
    ...
    if (AcceptConnection)
    {
        ...
        Status = NtCreateSection(&ServerView.SectionHandle, SECTION_MAP_READ | SECTION_MAP_WRITE, NULL, &MaximumSize,
    ...
    fprintf(stderr, "Server calling NtAcceptConnectPort( AcceptConnection = %ld )", AcceptConnection);
    ...
    Status = NtAcceptConnectPort(&ServerClientPortHandles[CountServerClientPortHandles],
                                 (PVOID)(CountServerClientPortHandles + 1), &Msg->h, AcceptConnection, &ServerView,
                                 &ClientView);
...
ServerThread(LPVOID Context)
{
    ...
    RtlIntegerToChar((ULONG)Teb->ClientId.UniqueProcess, 16, 9, ThreadName + strlen(ThreadName));
    strcat(ThreadName, ".");
    RtlIntegerToChar((ULONG)Teb->ClientId.UniqueThread, 16, 9, ThreadName + strlen(ThreadName));
    ...
    while (TRUE)
    {
        ...
        Status =
            NtReplyWaitReceivePort(ReplyPortHandle, (PVOID)&PortContext, (PPORT_MESSAGE)ReplyMsg, (PPORT_MESSAGE)&Msg);
        ...
        else if (PortContext >= CountServerClientPortHandles)
        {
            fprintf(stderr, "*** Invalid PortContext (%lx) received\n", PortContext);
        }
        else if (Msg.h.u2.s2.Type == LPC_PORT_CLOSED || Msg.h.u2.s2.Type == LPC_CLIENT_DIED)
        {
        ...
        else if (Msg.h.u2.s2.Type == LPC_REQUEST)
        {
            ...
            {
                Status =
                    SendRequest(1, ThreadName, ReplyPortHandle, Context, Msg.h.u1.s1.DataLength >> 1, ReplyMsg, TRUE);
            }
        }
    }
...
int _cdecl main(int argc, char *argv[])
{
    ...
    Status = NtCreatePort(&ServerConnectionPortHandle, &ObjectAttributes, 40, sizeof(TLPC_PORTMSG),
    ...
    if (rc == NO_ERROR)
    {
        ...
        for (i = 1; i < NumberOfThreads; i++)
        {
            ...
            rc = NO_ERROR;
            ServerThreadHandles[i] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ServerThread, (LPVOID)(i + 1),
                                                  CREATE_SUSPENDED, &ServerThreadClientIds[i]);
