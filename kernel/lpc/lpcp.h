...
#define LpcpGenerateMessageId() \
    LpcpNextMessageId++;        \
    if (LpcpNextMessageId == 0) \
        LpcpNextMessageId = 1;

#define LpcpGenerateCallbackId() \
    LpcpNextCallbackId++;        \
    if (LpcpNextCallbackId == 0) \
...
VOID LpcpMoveMessage(OUT PPORT_MESSAGE DstMsg, IN PPORT_MESSAGE SrcMsg, IN PVOID SrcMsgData, IN ULONG MsgType OPTIONAL,
...
#define LpcpReferencePortObject(PortHandle, PortAccess, PreviousMode, PortObject)                                   \
    ObReferenceObjectByHandle((PortHandle), (PortAccess), LpcPortObjectType, (PreviousMode), (PVOID *)(PortObject), \
                              NULL)
...
#define LpcpSetPortToThread(T, P) (T)->LpcWaitingOnPort = (PVOID)(((ULONG_PTR)P) | LPC_PORT_MASK_BIT);
...
#define LpcpPrint(_x_)                                                          \
    {                                                                           \
        DbgPrint("LPC[ %02x.%02x ]: ", PsGetCurrentThread()->Cid.UniqueProcess, \
                 PsGetCurrentThread()->Cid.UniqueThread);                       \
        DbgPrint _x_;                                                           \
    }

#define LpcpTrace(_x_)     \
    if (LpcpTraceMessages) \
    {                      \
        LpcpPrint(_x_);    \
    }
...
__forceinline PLPCP_MESSAGE LpcpAllocateFromPortZone(ULONG Size)
{
    PLPCP_MESSAGE Msg;
