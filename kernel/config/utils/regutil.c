...
NTSTATUS
RegReadMultiSzFile(IN PUNICODE_STRING FileName, OUT PVOID *ValueBuffer, OUT PULONG ValueLength)
{
    NTSTATUS Status;
    ...
    *ValueBuffer = RtlAllocateHeap(RtlProcessHeap(), 0, MultiSzFileSize);
    ...
    {
        MultiSource.Length = MultiSource.MaximumLength = (USHORT)MultiSzFileSize;
    }
    else
    {
    ...
    while (RegGetMultiString(&MultiSource, &MultiValue))
    {
        ...
        *ValueLength += MultiValue.Length;
        ...
        while (*s != L'"' && *s != L',' && ((s - MultiSource.Buffer) * sizeof(WCHAR)) < MultiSource.Length)
            s++;
...
RegReadBinaryFile(IN PUNICODE_STRING FileName, OUT PVOID *ValueBuffer, OUT PULONG ValueLength)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, &NtFileName, OBJ_CASE_INSENSITIVE, (HANDLE)NULL, NULL);

    Status = NtOpenFile(&File, SYNCHRONIZE | GENERIC_READ, &ObjectAttributes, &IoStatus,
    ...
    Status = NtQueryInformationFile(File, &IoStatus, (PVOID)&FileInformation, sizeof(FileInformation),
    ...
    *ValueBuffer = RtlAllocateHeap(RtlProcessHeap(), 0, *ValueLength);
    ...
    if (NT_SUCCESS(Status))
    {
        Status = NtReadFile(File, NULL, NULL, NULL, &IoStatus, *ValueBuffer, *ValueLength, NULL, NULL);
...
RegLoadAsciiFileAsUnicode(IN PUNICODE_STRING FileName, OUT PREG_UNICODE_FILE UnicodeFile)
{
    ...
    BufferBase = NULL;
    Status =
        NtAllocateVirtualMemory(NtCurrentProcess(), (PVOID *)&BufferBase, 0, &BufferSize, MEM_COMMIT, PAGE_READWRITE);
    if (NT_SUCCESS(Status))
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            if (NT_SUCCESS(Status))
            {
                ...
                {
                    Status = NtQueryInformationFile(File, &IoStatus, (PVOID)&FileDateTimeInfo, sizeof(FileDateTimeInfo),
    ...
    while (TRUE)
    {
        ...
        *IndentAmount = 0;
        while (*s <= L' ')
        {
            ...
            {
                *IndentAmount = ((*IndentAmount + 8) - (*IndentAmount % 8));
            }
        ...
        *FirstEqual = NULL;
        ...
        if (UnicodeFile->EndOfLine > UnicodeFile->BeginLine)
        {
            if (DebugOutput)
            {
                fprintf(stderr, "%02u  %.*ws\n", *IndentAmount, UnicodeFile->EndOfLine - UnicodeFile->BeginLine,
...
void RegDumpKeyValue(FILE *fh, PKEY_VALUE_FULL_INFORMATION KeyValueInformation, ULONG IndentLevel)
{
    ...
    case REG_DWORD:
        fprintf(fh, "REG_DWORD 0x%08lx", *((PULONG)((PCHAR)KeyValueInformation + KeyValueInformation->DataOffset)));
        break;
    ...
    case REG_LINK:
        fprintf(fh, "REG_LINK %ws", ((PWSTR)((PCHAR)KeyValueInformation + KeyValueInformation->DataOffset)));
        break;
    ...
    case REG_FULL_RESOURCE_DESCRIPTOR:
    {
        PCM_RESOURCE_LIST ResourceList =
            ((PCM_RESOURCE_LIST)((PCHAR)KeyValueInformation + KeyValueInformation->DataOffset));
        ...
        if (KeyValueInformation->Type == REG_RESOURCE_LIST)
        {
            ...
            fprintf(fh, "%.*sNumber of Full resource Descriptors = %d", IndentLevel,
                    "                                                                                  ",
                    ResourceList->Count);
        ...
        else
        {
            ...
            count = 1;
            FullDescriptor =
                ((PCM_FULL_RESOURCE_DESCRIPTOR)((PCHAR)KeyValueInformation + KeyValueInformation->DataOffset));
        }
        ...
        for (i = 0; i < count; i++)
        {
            ...
            case Eisa:
                ...
                break;
            case MicroChannel:
                TypeName = L"MicroChannel";
                break;
            case TurboChannel:
                TypeName = L"TurboChannel";
                break;
            case PCIBus:
                TypeName = L"PCI";
                break;
            ...
            for (j = 0; j < FullDescriptor->PartialResourceList.Count; j++)
            {
                ...
                case CmResourceTypePort:
                    fprintf(fh, "%.*sSTART 0x%08lx  LENGTH 0x%08lx\n", IndentLevel + 12,
                            "                                                                                  ",
                            PartialResourceDescriptor->u.Port.Start.LowPart, PartialResourceDescriptor->u.Port.Length);
                    break;

                case CmResourceTypeInterrupt:
                    fprintf(fh, "%.*sLEVEL %d  VECTOR %d  AFFINITY %d\n", IndentLevel + 12,
                            "                                                                                  ",
                            PartialResourceDescriptor->u.Interrupt.Level, PartialResourceDescriptor->u.Interrupt.Vector,
                ...
                case CmResourceTypeDma:
                    fprintf(fh, "%.*sCHANNEL %d  PORT %d\n", IndentLevel + 12,
                            "                                                                                  ",
                            PartialResourceDescriptor->u.Dma.Channel, PartialResourceDescriptor->u.Dma.Port);
                    break;
    ...
    default:
        ...
        {
            fprintf(fh, "*** Unknown registry type (%08lx)", KeyValueInformation->Type);
        }
        fprintf(fh, "%.*s", IndentLevel,
                "                                                                                  ");
...

{
    ...
    //
    while ((*(ValueString->Buffer) != L'"') && (ValueString->Length > 0))
    {
        ++ValueString->Buffer;
...
BOOLEAN
RegGetKeyValue(IN PUNICODE_STRING InitialKeyValue, IN OUT PREG_UNICODE_FILE UnicodeFile, OUT PULONG ValueType,
               OUT PVOID *ValueBuffer, OUT PULONG ValueLength)
{
    ...
    else if (!RtlPrefixUnicodeString(&RiRegKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_SZ;
        PrefixLength = 0;
    }
    else if (RtlPrefixUnicodeString(&RiRegNoneKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_NONE;
        PrefixLength = RiRegNoneKeyword.Length;
    }
    else if (RtlPrefixUnicodeString(&RiRegSzKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_SZ;
        PrefixLength = RiRegSzKeyword.Length;
    }
    else if (RtlPrefixUnicodeString(&RiRegExpandSzKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_EXPAND_SZ;
        PrefixLength = RiRegExpandSzKeyword.Length;
    }
    else if (RtlPrefixUnicodeString(&RiRegDwordKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_DWORD;
        PrefixLength = RiRegDwordKeyword.Length;
    }
    else if (RtlPrefixUnicodeString(&RiRegBinaryFileKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_BINARY;
        PrefixLength = RiRegBinaryFileKeyword.Length;
        GetDataFromBinaryFile = TRUE;
    }
    else if (RtlPrefixUnicodeString(&RiRegBinaryKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_BINARY;
        PrefixLength = RiRegBinaryKeyword.Length;
    }
    else if (RtlPrefixUnicodeString(&RiRegLinkKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_LINK;
        PrefixLength = RiRegLinkKeyword.Length;
    }
    else if (RtlPrefixUnicodeString(&RiRegMultiSzFileKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_MULTI_SZ;
        PrefixLength = RiRegMultiSzFileKeyword.Length;
        GetDataFromMultiSzFile = TRUE;
    }
    else if (RtlPrefixUnicodeString(&RiRegMultiSzKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_MULTI_SZ;
        PrefixLength = RiRegMultiSzKeyword.Length;
    }
    else if (RtlPrefixUnicodeString(&RiRegDateKeyword, &KeyValue, TRUE))
    {
        *ValueType = REG_BINARY;
        ParseDateTime = TRUE;
    ...
    {
        s = (PWSTR)((PCHAR)KeyValue.Buffer + PrefixLength);
    ...
    case REG_DWORD:
        *ValueBuffer = RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(ULONG));
        ...
        else if (RtlPrefixUnicodeString(&RiFalseKeyword, &KeyValue, TRUE) ||
...
#define NUMBER_DATE_TIME_FIELDS 6
            ...
            while (KeyValue.Length)
            {
                ...
                if (CurrentField == (NUMBER_DATE_TIME_FIELDS - 1))
                {
                    ...
                    else if (!_wcsnicmp(Field.Buffer, L"MON", 3))
                    {
                        FieldValue = 1;
                    }
                    else if (!_wcsnicmp(Field.Buffer, L"TUE", 3))
                    {
                        FieldValue = 2;
                    }
                    else if (!_wcsnicmp(Field.Buffer, L"WED", 3))
                    {
                        FieldValue = 3;
                    }
                    else if (!_wcsnicmp(Field.Buffer, L"THU", 3))
                    {
                        FieldValue = 4;
                    }
                    else if (!_wcsnicmp(Field.Buffer, L"FRI", 3))
                    {
                        FieldValue = 5;
                    }
                    else if (!_wcsnicmp(Field.Buffer, L"SAT", 3))
                    {
                        FieldValue = 6;
                    }
                    else
            ...
            else if (DateTimeFields.Year < 100)
            {
    ...
    *ValueBuffer = RtlAllocateHeap(RtlProcessHeap(), 0, *ValueLength);
    ...
    while (n--)
    {
        if (KeyValue.Length == 0)
        {
            ...
            KeyValue.Length = (USHORT)((PCHAR)UnicodeFile->EndOfLine - (PCHAR)UnicodeFile->BeginLine);
