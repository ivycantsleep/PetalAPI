...
NTSTATUS
RiInitializeRegistryFromAsciiFile(IN PUNICODE_STRING HiveName, IN PUNICODE_STRING FileName);
...
RiInitializeRegistryFromAsciiFile(IN PUNICODE_STRING HiveName, IN PUNICODE_STRING FileName)
{
    ...
    HiveHandle = EhOpenHive(HiveName, &KeyPath[0].Handle, &KeyPath[0].Name, TYPE_SIMPLE);
    if (HiveHandle == NULL)
    {
    ...
    Status = RegLoadAsciiFileAsUnicode(FileName, &UnicodeFile);
    ...
    while (RegGetNextLine(&UnicodeFile, &IndentAmount, &FirstEqual))
    {
        ...
        if (FirstEqual == NULL)
        {
            ...
            if (KeyPathLength == 0 || IndentAmount > CurrentKey->IndentAmount)
            {
                if (KeyPathLength == MAX_KEY_DEPTH)
                {
                    fprintf(stderr, "HIVEINI: %wZ key exceeded maximum depth (%u) of tree.\n", &KeyName, MAX_KEY_DEPTH);
            ...
            Status = EhCreateChild(HiveHandle, KeyPath[KeyPathLength - 1].Handle, &KeyName, &CurrentKey->Handle,
                                   &Disposition);
            ...
            if (NT_SUCCESS(Status))
            {
                if (DebugOutput)
                {
                    fprintf(stderr, "    Created key %02x %wZ (%08x)\n", CurrentKey->IndentAmount, &CurrentKey->Name,
                            CurrentKey->Handle);
                }
                KeyInformation = (PKEY_BASIC_INFORMATION)KeyInformationBuffer;
                Status = EhQueryKey(HiveHandle, CurrentKey->Handle, KeyBasicInformation, KeyInformation,
                                    sizeof(KeyInformationBuffer), &ResultLength);
                if (NT_SUCCESS(Status))
                {
                ...
                {
                    RtlZeroMemory(&CurrentKey->LastWriteTime, sizeof(CurrentKey->LastWriteTime));
                }
            ...
            {
                fprintf(stderr, "HIVEINI: CreateKey (%wZ) relative to handle (%lx) failed - %lx\n", &KeyName,
                        ObjectAttributes.RootDirectory, Status);
            }
        }
        else
        {
            ...
            while (IndentAmount <= CurrentKey->IndentAmount)
            {
                if (DebugOutput)
                {
                    fprintf(stderr, "    Popping from key %02x %wZ (%08x)\n", CurrentKey->IndentAmount,
                            &CurrentKey->Name, CurrentKey->Handle);
                }
                CurrentKey->Handle = NULL;
                CurrentKey--;
                if (--KeyPathLength == 1)
            ...
            if (DebugOutput)
            {
                fprintf(stderr, "    Adding value '%wZ = %wZ' to key %02x %wZ (%08x)\n", &KeyName, &KeyValue,
                        CurrentKey->IndentAmount, &CurrentKey->Name, CurrentKey->Handle);
            }

            if (RegGetKeyValue(&KeyValue, &UnicodeFile, &ValueType, &ValueBuffer, &ValueLength))
            {
                if (ValueBuffer == NULL)
                {
                    Status = EhDeleteValueKey(HiveHandle, KeyPath[KeyPathLength + 1].Handle, &KeyValue);
                    if (NT_SUCCESS(Status))
                    {
                ...
                {
                    if (UnicodeFile.LastWriteTime.QuadPart > CurrentKey->LastWriteTime.QuadPart)
                    {
                    ...
                    {
                        Status = EhQueryValueKey(HiveHandle, CurrentKey->Handle, &KeyName, KeyValueFullInformation,
                                                 OldValueInformation, OldValueBufferSize, &OldValueLength);
                        if (NT_SUCCESS(Status))
                        {
                            UpdateKeyValue = TRUE;
                        }
                    ...
                    if (!NT_SUCCESS(Status) || OldValueInformation->Type != ValueType ||
                        OldValueInformation->DataLength != ValueLength ||
                        !RtlEqualMemory((PCHAR)OldValueInformation + OldValueInformation->DataOffset, ValueBuffer,
                                        ValueLength))
                    {

                        Status = EhSetValueKey(HiveHandle, CurrentKey->Handle, &KeyName, 0, ValueType, ValueBuffer,
                                               ValueLength);
                        if (NT_SUCCESS(Status))
                        {
                            printf("%s value for Key: ", UpdateKeyValue ? "Updated" : "Created");
                        ...
                        {
                            fprintf(stderr, "HIVEINI: SetValueKey (%wZ) failed - %lx\n", &KeyName, Status);
                        }
                    }
            ...
            {
                fprintf(stderr, "HIVEINI: Invalid key (%wZ) value (%wZ)\n", &KeyName, &KeyValue);
            }
        }
    }
...
int __cdecl main(argc, argv) int argc;
char *argv[];
{
    int i;
    char *s;
    ...
    for (i = 1; i < argc; i++)
    {
            ...
            while (*++s)
            {
                ...
                case 'f':
                    if (++i < argc)
                    {
                        ...
                        RtlAnsiStringToUnicodeString(&DosHiveName, &AnsiString, TRUE);
                        RtlDosPathNameToNtPathName_U(DosHiveName.Buffer, &HiveName, NULL, NULL);
                        break;
                    }
        ...
        else
        {
            ...
            if (NT_SUCCESS(Status))
            {
                if (RtlDosPathNameToNtPathName_U(DosFileName.Buffer, &FileName, NULL, NULL))
                {
                    Status = RiInitializeRegistryFromAsciiFile(&HiveName, &FileName);
                    if (!NT_SUCCESS(Status))
                    {
                        fprintf(stderr, "HIVEINI: Failed to load from %wZ - Status == %lx\n", &FileName, Status);
                    }
                }
                else
                {
                    Status = STATUS_UNSUCCESSFUL;
                    fprintf(stderr, "HIVEINI: Unable to map Dos Name (%wZ) to NT name\n", &DosFileName);
                }
            }
            else
            {
                fprintf(stderr, "HIVEINI: Unable to convert %s to unicode - Status == %lx\n", &AnsiString, Status);
            }
        }
    }
