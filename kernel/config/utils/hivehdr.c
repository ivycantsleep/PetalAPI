...
void DoDump(PUCHAR Filename)
{
    ...
    {
        fprintf(stderr, "hivehdr: '%s' - cannot read full base block\n", Filename);
        return;
    }
    ...
    printf("    TimeStamp: %08lx:%08lx\n", bbp->TimeStamp.HighPart, bbp->TimeStamp.LowPart, (PUCHAR) & (bbp->Signature),
           validstring[valid]);
    ...
    valid = ((bbp->Type == HFILE_TYPE_PRIMARY) || (bbp->Type == HFILE_TYPE_ALTERNATE) || (bbp->Type == HFILE_TYPE_LOG));
    if (valid)
    {
        typeselect = bbp->Type;
    }
