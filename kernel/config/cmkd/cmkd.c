...
DECLARE_API(version)
{
    ...
    dprintf("%s Extension dll for Build %d debugging %s kernel for Build %d\n", DebuggerType, VER_PRODUCTBUILD,
            SavedMajorVersion == 0x0c ? "Checked" : "Free", SavedMinorVersion);
}
...
VOID CheckVersion(VOID)
{
    ...
    if ((SavedMajorVersion != 0x0c) || (SavedMinorVersion != VER_PRODUCTBUILD))
    {
        dprintf("\r\n*** Extension DLL(%d Checked) does not match target system(%d %s)\r\n\r\n", VER_PRODUCTBUILD,
                SavedMinorVersion, (SavedMajorVersion == 0x0f) ? "Free" : "Checked");
    }
#else
...
USHORT
GetKcbName(ULONG_PTR KcbAddr, PWCHAR NameBuffer, ULONG BufferSize)
/*++
...
--*/
{
    ...
    {
        ExitIfCtrlC() 0;
        if (!ReadMemory(TmpKcbAddr, &TmpKcb, sizeof(TmpKcb), &BytesRead))
        {
        ...
        if (!ReadMemory(NameBlockAddr, &NameBlock, sizeof(NameBlock), &BytesRead))
        {
    ...
    while (TmpKcbAddr)
    {
        ...
        if (!ReadMemory(NameBlockAddr + FIELD_OFFSET(CM_NAME_CONTROL_BLOCK, Name), Name, NameBlock.NameLength,
                        &BytesRead))
        {
...
DECLARE_API(childlist)
{
    ...
    if (!ReadMemory(Addr, &Index, sizeof(Index), &BytesRead))
    {
    ...
    else
    {
        ...
        if (Signature == CM_KEY_INDEX_ROOT)
        {
            ...
            for (u = 0; u < Count; u++)
            {
                if (!ReadMemory(Addr, &Cell, sizeof(Cell), &BytesRead))
                {
        ...
        else if (Signature == CM_KEY_FAST_LEAF)
        {
            ...
            for (u = 0; u < Count; u++)
            {
                ...
                else
                {
                    ...
                    Addr += sizeof(Cell);
                    if (!ReadMemory(Addr, NameHint, 4 * sizeof(UCHAR), &BytesRead))
                    {
                        dprintf("\tCould not read Index[%u]\n", u);
                    }
                    else
...

{
    ...
    if (!ReadMemory(KcbAddr, &Kcb, sizeof(Kcb), &BytesRead))
    {
    ...
    else
    {
        ...
        if (!(Kcb.ExtFlags & CM_KCB_INVALID_CACHED_INFO))
        {
            ...
            {
                if (!ReadMemory((ULONG_PTR)Kcb.IndexHint, &IndexHint, sizeof(IndexHint), &BytesRead))
                {
                    dprintf("Could not read Kcb\n");
                    return;
                }
...

{
    ...
    if (!ReadMemory(KnAddr, &KNode, sizeof(KNode), &BytesRead))
    {
    ...
    else
    {
        ...
        ReadMemory(KnAddr, KeyName, KNode.NameLength, &BytesRead);
...
//
ULONG_PTR
MyHvpGetCellPaged(PHHIVE Hive, HCELL_INDEX Cell)
/*++
...
--*/
{
    ...
    //
    ReadMemory((DWORD_PTR)Hive->Storage[Type].Map, &DirMap, sizeof(DirMap), &BytesRead);

    ReadMemory((DWORD_PTR)DirMap.Directory[Table], &MapTable, sizeof(MapTable), &BytesRead);
...
ULONG_PTR
MyHvpGetCellFlat(PHHIVE Hive, HCELL_INDEX Cell)
/*++
...
--*/
{
    ...
    ReadMemory((DWORD_PTR)Hive->BaseBlock, &BaseBlock, sizeof(BaseBlock), &BytesRead);
...

{
    ...
    if (!ReadMemory(HiveAddr, &CmHive, sizeof(CmHive), &BytesRead))
    {
...

{
    ...
    if (!ReadMemory(ValAddr, &KVal, sizeof(KVal), &BytesRead))
    {
    ...
    else
    {
        ...
        {
            ReadMemory(ValAddr, ValName, KVal.NameLength, &BytesRead);
...

{
    ...
    if (!ReadMemory(KBodyAddr, &KBody, sizeof(KBody), &BytesRead))
    {
...

{
    ...
    if (!ReadMemory(Address, &CmpCacheTable, sizeof(CmpCacheTable), &BytesRead))
    {
    ...
    if (!ReadMemory(Address, &CmpNameCacheTable, sizeof(CmpNameCacheTable), &BytesRead))
    {
...

{
    ...
    for (i = 0; i < CmpHashTableSize; i++)
    {
        ...
        ReadMemory(Address, &Current, sizeof(Current), &BytesRead);

        while (Current)
        {
            ExitIfCtrlC();
            ReadMemory(Current, &KeyHash, sizeof(KeyHash), &BytesRead);
...

{
    ...
    if (!ReadMemory(BaseAddr, &BaseBlock, sizeof(BaseBlock), &BytesRead))
    {
...

{
    ...
    {
        ExitIfCtrlC();
        if (!ReadMemory(Current, &KeyHash, sizeof(KeyHash), &BytesRead))
        {
...

{
    ...
    {
        ExitIfCtrlC();
        if (!ReadMemory(HiveAddr, &SecurityCacheEntry, sizeof(SecurityCacheEntry), &BytesRead))
        {
...

{
    ...
    dprintf("%4u  Pinned Views ; PinViewListHead = %p %p\n", Nr, (ULONG_PTR)CmHive.PinViewListHead.Flink,
            (ULONG_PTR)CmHive.PinViewListHead.Blink);
    if (Nr)
    {
        dprintf("------------------------------------------------------------------------------------------------------"
                "--------\n");
        dprintf("| ViewAddr |FileOffset|   Size   |ViewAddress|   Bcb    |    LRUViewList     |    PinViewList     | "
                "UseCount |\n");
        dprintf("------------------------------------------------------------------------------------------------------"
                "--------\n");
        ...
        {
            ViewAddr -= Offset;
            if (!ReadMemory(ViewAddr, &CmView, sizeof(CmView), &BytesRead))
            {
    ...
    dprintf("%4u  Mapped Views ; LRUViewListHead = %p %p\n", Nr, (ULONG_PTR)CmHive.LRUViewListHead.Flink,
            (ULONG_PTR)CmHive.LRUViewListHead.Blink);
    if (Nr)
    {
        dprintf("------------------------------------------------------------------------------------------------------"
                "--------\n");
...

{
    ...
    if (!ReadMemory(AnchorAddr, &CmpHiveListHead, sizeof(CmpHiveListHead), &BytesRead))
    {
    ...
    dprintf("----------------------------------------------------------------------------------------------------------"
            "---\n");
    dprintf("| HiveAddr |Stable Length|Stable Map|Volatile Length|Volatile Map|MappedViews|PinnedViews|U(Cnt)| "
            "BaseBlock | FileName \n");
    dprintf("----------------------------------------------------------------------------------------------------------"
            "---\n");
    while (HiveAddr != AnchorAddr)
    {
        ...
        if (!ReadMemory(Tmp, &BaseBlock, sizeof(BaseBlock), &BytesRead))
        {
...

{
    ...
    if (!ReadMemory(HiveAddr, &Hive, sizeof(Hive), &BytesRead))
    {
        dprintf("\tRead %lx bytes from %p\n", BytesRead, HiveAddr);
        return;
    }
    ...
    Nr = 0;
    AnchorAddr =
        HiveAddr + FIELD_OFFSET(HHIVE, Storage) + 5 * sizeof(ULONG) + HHIVE_FREE_DISPLAY_SIZE * sizeof(RTL_BITMAP);
    BinAddr = (ULONG_PTR)Hive.Storage[0].FreeBins.Flink;
    while (BinAddr != AnchorAddr)
    {
        ExitIfCtrlC();
        BinAddr -= Offset;
        if (!ReadMemory(BinAddr, &FreeBin, sizeof(FreeBin), &BytesRead))
        {
...

{
    ...
    for (i = 0; i < SizeOfBitmap; i++)
    {
        ...
        if (!(i % BitsPerULONG))
        {
            ...
            //
            if (!ReadMemory(DirtyBufferAddr, &DirtyBuffer, sizeof(DirtyBuffer), &BytesRead))
            {
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            */     \
            ...
            {                                                                  \
                Index = CmKDFindFirstSetLeft[Index] + HHIVE_FREE_DISPLAY_BIAS; \
            }                                                                  \
        }                                                                      \
    }
...

{
    ...
    if (!ReadMemory(BinAddr, &Bin, sizeof(Bin), &BytesRead))
    {
        dprintf("\tRead %lx bytes from %lx\n", BytesRead, BinAddr);
        return;
    }
    ...
    for (CurrIndex = 0; CurrIndex < HHIVE_FREE_DISPLAY_SIZE; CurrIndex++)
    {
        ...
        while (Offset < Bin.Size)
        {
            ...
            if (!ReadMemory(CurrentAddr, &Current, sizeof(Current), &BytesRead))
            {
...

{
    ...
    dprintf("\n%8s    %16s %16s %16s %16s", "Address", "64K (0x10000)", "64K (0x10000)", "64K (0x10000)",
            "64K (0x10000)");
