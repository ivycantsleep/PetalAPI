...
ULONG
CmpFindSubKeyInRoot(PHHIVE Hive, PCM_KEY_INDEX Index, PUNICODE_STRING SearchName, PHCELL_INDEX Child);

ULONG
CmpFindSubKeyInLeaf(PHHIVE Hive, PCM_KEY_INDEX Index, PUNICODE_STRING SearchName, PHCELL_INDEX Child);
...
HCELL_INDEX
CmpDoFindSubKeyByNumber(PHHIVE Hive, PCM_KEY_INDEX Index, ULONG Number);

HCELL_INDEX
CmpAddToLeaf(PHHIVE Hive, HCELL_INDEX LeafCell, HCELL_INDEX NewKey, PUNICODE_STRING NewName);

HCELL_INDEX
CmpSelectLeaf(PHHIVE Hive, PCM_KEY_NODE ParentKey, PUNICODE_STRING NewName, HSTORAGE_TYPE Type,
              PHCELL_INDEX *RootPointer);

HCELL_INDEX
CmpSplitLeaf(PHHIVE Hive, HCELL_INDEX RootCell, ULONG RootSelect, HSTORAGE_TYPE Type);

HCELL_INDEX
CmpFindSubKeyByHash(PHHIVE Hive, PCM_KEY_FAST_INDEX FastIndex, PUNICODE_STRING SearchName);
...
--*/
{
    ...
    for (i = 0; i < Hive->StorageTypeCount; i++)
    {
        if (Parent->SubKeyCounts[i] != 0)
        {
            ...
            {
                FoundIndex = CmpFindSubKeyInLeaf(Hive, IndexRoot, SearchName, &Child);
...
--*/
{
    ...
    if (Pcan->Flags & KEY_COMP_NAME)
    {
        ...
        Result = CmpCompareCompressedName(SearchName, Pcan->Name, Pcan->NameLength, 0);
    }
    else
    {
...
HCELL_INDEX
CmpFindSubKeyByNumber(PHHIVE Hive, PCM_KEY_NODE Node, ULONG Number)
/*++
            ...
            for (j = 0; j < Leaf->Count; j++)
            {
                if ((Leaf->Signature == CM_KEY_FAST_LEAF) || (Leaf->Signature == CM_KEY_HASH_LEAF))
                {
                    ...
                    if (FastIndex->List[j].Cell == Child)
                    {
                        ...
                        FastIndex->Count--;
                        RtlMoveMemory((PVOID) & (FastIndex->List[j]), (PVOID) & (FastIndex->List[j + 1]),
                ...
                else
                {
                    if (Leaf->List[j] == Child)
                    {
                        ...
                        HvMarkCellDirty(Hive, LeafCell);
                        Leaf->Count--;
                        RtlMoveMemory((PVOID) & (Leaf->List[j]), (PVOID) & (Leaf->List[j + 1]),
        ...
        for (j = 0; j < Index->Count; j++)
        {
            ...
            else
            {
                if (Index->List[j] == Child)
                {
                    ...
                    // found it!
                    //
                    RtlMoveMemory((PVOID) & (Index->List[j]), (PVOID) & (Index->List[j + 1]),
...
--*/
{
    ...
    if (pcell->Flags & KEY_COMP_NAME)
    {
        ...
        CmpCopyCompressedName(NewName.Buffer, NewName.MaximumLength, pcell->Name, pcell->NameLength);
    }
    else
    {
        IsCompressed = FALSE;
    ...
    if (pcell->SubKeyCounts[Type] == 0)
    {
        ...
        //
        WorkCell =
            HvAllocateCell(Hive, sizeof(CM_KEY_FAST_INDEX), Type, (HvGetCellType(Parent) == Type) ? Parent : HCELL_NIL);
    ...
    else
    {
        ...
        if ((Index->Signature == CM_KEY_FAST_LEAF) && (Index->Count >= (CM_MAX_FAST_INDEX)))
        {
            //
        ...
        else if (((Index->Signature == CM_KEY_INDEX_LEAF) || (Index->Signature == CM_KEY_HASH_LEAF)) &&
                 (Index->Count >= (CM_MAX_INDEX - 1)))
        {
            //
            // We must change flat entry to a root/leaf tree
            //
            WorkCell = HvAllocateCell(Hive,
                                      sizeof(CM_KEY_INDEX) + sizeof(HCELL_INDEX), // allow for 2
                                      Type, (HvGetCellType(Parent) == Type) ? Parent : HCELL_NIL);
            if (WorkCell == HCELL_NIL)
            {
                goto ErrorExit;
            }
...
--*/
{
    ...
    {
        ASSERT((Leaf->Signature == CM_KEY_FAST_LEAF) || (Leaf->Signature == CM_KEY_HASH_LEAF));
        FastLeaf = (PCM_KEY_FAST_INDEX)Leaf;
        EntrySize = sizeof(CM_INDEX);
    }
    OldSize = HvGetCellSize(Hive, Leaf);
    Size = OldSize - ((EntrySize * Leaf->Count) + FIELD_OFFSET(CM_KEY_INDEX, List));
    ...
    if (Select != Leaf->Count)
    {
        ...
        Result = CmpCompareInIndex(Hive, NewName, Select, Leaf, &Child);
        if (Result == 2)
        {
            //
            // couldn't map view inside; bail out
        ...
        if (Select != Leaf->Count)
        {
            ...
            if (FastLeaf != NULL)
            {
                RtlMoveMemory((PVOID) & (FastLeaf->List[Select + 1]), (PVOID) & (FastLeaf->List[Select]),
                              sizeof(CM_INDEX) * (FastLeaf->Count - Select));
            }
            else
            {
                RtlMoveMemory((PVOID) & (Leaf->List[Select + 1]), (PVOID) & (Leaf->List[Select]),
...
--*/
{
    ...
    Size = (ElemSize * NewCount) + FIELD_OFFSET(CM_KEY_INDEX, List) + 1; // +1 to assure room for add
    ...
    Size = Size - ((sizeof(HCELL_INDEX) * Root->Count) + FIELD_OFFSET(CM_KEY_INDEX, List));
...
#endif //DBG
        RtlMoveMemory((PVOID) & (NewLeaf->List[0]), (PVOID) & (FastLeaf->List[KeepCount]), ElemSize * NewCount);
    }
    else
    {
        RtlMoveMemory((PVOID) & (NewLeaf->List[0]), (PVOID) & (Leaf->List[KeepCount]), ElemSize * NewCount);
    }
    ...
    {
        RtlMoveMemory((PVOID) & (Root->List[RootSelect + 2]), (PVOID) & (Root->List[RootSelect + 1]),
...
--*/
{
    ...
    if (pcell->Flags & KEY_COMP_NAME)
    {
        ...
        CmpCopyCompressedName(SearchName.Buffer, SearchName.MaximumLength, pcell->Name, pcell->NameLength);
    }
    else
    {
        IsCompressed = FALSE;
...
--*/
{
    ...
    if (Leaf->Count == 0)
    {
        ...
        if (Root != NULL)
        {
            ...
            else if (RootSelect < (ULONG)(Root->Count))
            {
                ...
                //
                RtlMoveMemory((PVOID) & (Root->List[RootSelect]), (PVOID) & (Root->List[RootSelect + 1]),
    ...
    else if (LeafSelect < (ULONG)(Leaf->Count))
    {
        ...
        if (Leaf->Signature == CM_KEY_INDEX_LEAF)
        {
            RtlMoveMemory((PVOID) & (Leaf->List[LeafSelect]), (PVOID) & (Leaf->List[LeafSelect + 1]),
                          (Leaf->Count - LeafSelect) * sizeof(HCELL_INDEX));
        }
        else
        {
            FastIndex = (PCM_KEY_FAST_INDEX)Leaf;
            RtlMoveMemory((PVOID) & (FastIndex->List[LeafSelect]), (PVOID) & (FastIndex->List[LeafSelect + 1]),
...
Exit:
    ...
    if ((IsCompressed) && (SearchName.MaximumLength > sizeof(CompressedBuffer)))
    {
#if defined(_CM_LDR_)
        (Hive->Free)(SearchName.Buffer, SearchName.Length);
#else
