...
PCELL_DATA
CmpGetValueListFromCache(IN PHHIVE Hive, IN PCACHED_CHILD_LIST ChildList, OUT BOOLEAN *IndexCached)
/*++
...
--*/
{
    ...
    *IndexCached = TRUE;
    ...
    else
    {
        ...
        CachedValueIndex =
            (PCM_CACHED_VALUE_INDEX)ExAllocatePoolWithTag(PagedPool, AllocSize, CM_CACHE_VALUE_INDEX_TAG);
    ...
    *CellToRelease = HCELL_NIL;
    ...
    if (ChildList->Count != 0)
    {
        ...
        while (TRUE)
        {
            ...
            pchild =
                CmpGetValueKeyFromCache(Hive, List, Current, ContainingList, IndexCached, ValueCached, CellToRelease);
            if (pchild == NULL)
            {
                //
                // couldn't map view; bail out
            ...
            {
                CmKdPrintEx(
                    (DPFLTR_CONFIG_ID, CML_EXCEPTION, "CmpFindValueByNameFromCache: code:%08lx\n", GetExceptionCode()));
                //
