...
BOOLEAN
CmpCheckRecursionAndRecordThreadInfo(PCM_CALLBACK_CONTEXT_BLOCK CallbackBlock,
...
--*/
{
    ...
    CmCallbackContext =
        (PCM_CALLBACK_CONTEXT_BLOCK)ExAllocatePoolWithTag(PagedPool, sizeof(CM_CALLBACK_CONTEXT_BLOCK), 'bcMC');
...
--*/
{
    ...
    for (i = 0; i < CM_MAX_CALLBACKS; i++)
    {
        ...
        if (RoutineBlock != NULL)
        {
            ...
            {
                Status =
                    ExGetCallBackBlockRoutine(RoutineBlock)(CmCallbackContext->CallerContext, (PVOID)Type, Argument);
                //
...
--*/
{
    ...
    {
        CurrentThreadInfo = CONTAINING_RECORD(CurrentThreadInfo, CM_ACTIVE_NOTIFY_THREAD, ThreadList);
