...
--*/
{
    ...
    if (ChildList->Count != 0)
    {
        ...
        while (TRUE)
        {
            ...
            {
                Result = CmpCompareCompressedName(Name, pchild->Name, pchild->NameLength, 0);
            }
            else
            {
                ...
                Result = RtlCompareUnicodeString(Name, &Candidate, TRUE);
            }
    ...
    if (CmpIsHKeyValueBig(Hive, *realsize) == TRUE)
    {
        ...
        try
        {
            ...
            for (i = 0; i < BigData->Count; i++)
            {
                ...
                //
                RtlCopyMemory(WorkBuffer + CM_KEY_VALUE_BIG * i, PartialData,
                              (Length > CM_KEY_VALUE_BIG) ? CM_KEY_VALUE_BIG : Length);
...
NTSTATUS
CmpAddValueToList(IN PHHIVE Hive, IN HCELL_INDEX ValueCell, IN ULONG Index, IN ULONG Type, IN OUT PCHILD_LIST ChildList)
/*++
...
--*/
{
    ...
    if (count > 1)
    {
        ...
        NewCell = HvReallocateCell(Hive, ChildList->List, AllocateSize);
    }
    else
    {
...
--*/
{
    ...
    if (newcount > 0)
    {
        ...
        newcell = HvReallocateCell(Hive, ChildList->List, newcount * sizeof(HCELL_INDEX));
    ...
    if (CmpIsHKeyValueBig(Hive, DataSize) == TRUE)
    {
        ...
        *DataCell = HvAllocateCell(Hive, sizeof(CM_BIG_DATA), StorageType, ValueCell);
        ...
        for (; BigData->Count < Count; (BigData->Count)++)
        {
            ...
            {
                CmKdPrintEx(
                    (DPFLTR_CONFIG_ID, CML_EXCEPTION, "!!CmpSetValueDataNew: code:%08lx\n", GetExceptionCode()));
