...

{
    ...
    while (IsListEmpty(&CmpHiveListHead) == FALSE)
    {
        ...
        CmHive = (PCMHIVE)CONTAINING_RECORD(CmHive, CMHIVE, HiveList);
    ...
    while (PostBlock != (PCM_POST_BLOCK)AnchorAddr)
    {
        ...
        if (PostBlock->PostKeyBody)
        {
            ...
            DbgPrintEx(DPFLTR_CONFIG_ID, DPFLTR_ERROR_LEVEL, "Notification, PostBlock %p not triggered on KCB %p\n",
                       PostBlock, PostBlock->PostKeyBody->KeyBody->KeyControlBlock);
        }
...
--*/
{
    ...
    RtlInitUnicodeString(&TempName, CACHE_DMP_FILE_NAME);
    RtlAppendStringToString((PSTRING)&FileName, (PSTRING)&TempName);
    ...
    Status = ZwCreateFile(&FileHandle, FILE_READ_DATA | FILE_WRITE_DATA, &ObjectAttributes, &IoStatus,
                          NULL, // alloc size = none
                          FILE_ATTRIBUTE_NORMAL,
                          0, // share nothing
                          FILE_OPEN_IF, FILE_RANDOM_ACCESS,
                          NULL, // eabuffer
                          0     // ealength
    );
    ...
    Status = ZwWriteFile(FileHandle, NULL, NULL, NULL, &IoStatus, &KcbNo, sizeof(ULONG), &Offset, NULL);
    if (!NT_SUCCESS(Status))
    {
        goto Exit;
    }
    ...
    for (i = 0; i < CmpHashTableSize; i++)
    {
        ...
        while (Current)
        {
            ...
            if (Name)
            {
                ...
                Status = ZwWriteFile(FileHandle, NULL, NULL, NULL, &IoStatus, &Tmp, sizeof(ULONG), &Offset, NULL);
                if (!NT_SUCCESS(Status))
                {
                    goto Exit;
                }
                ...
                //
                Offset.LowPart = FileOffset;
                Status = ZwWriteFile(FileHandle, NULL, NULL, NULL, &IoStatus, Name->Buffer, Tmp, &Offset, NULL);
                if (!NT_SUCCESS(Status))
                {
                    goto Exit;
                }
...
--*/
{
    ...
    {
        Status = ObReferenceObjectByHandle(CmpRegistryRootHandle, KEY_READ, NULL, KernelMode, &RegistryRoot, NULL);
