...
--*/
{
    ...
    CmpDelayedCloseTable =
        ExAllocatePoolWithTag(PagedPool, CmpDelayedCloseSize * sizeof(CM_DELAYED_CLOSE_ENTRY), CM_DELAYCLOSE_TAG);
    ...
    for (i = 0; i < CmpDelayedCloseSize; i++)
    {
        ...
        InsertTailList(&CmpDelayedLRUListHead, &(CmpDelayedCloseTable[i].DelayedLRUList));
    }
}
...
--*/
{
    ...
    CmKdPrintEx((DPFLTR_CONFIG_ID, CML_FLOW,
                 "[CmpRemoveFromDelayedClose] : Removing kcb = %p from DelayedCloseTable; index = %lu\n", kcb,
                 (ULONG)i));
    //
...
--*/
{
    ...
    DelayedEntry = CONTAINING_RECORD(DelayedEntry, CM_DELAYED_CLOSE_ENTRY, DelayedLRUList);
    ...
    if (DelayedEntry->KeyControlBlock != NULL)
    {
        ...
        CmKdPrintEx(
            (DPFLTR_CONFIG_ID, CML_FLOW,
             "[CmpGetDelayedCloseIndex] : no index free; kicking kcb = %p index = %lu out of DelayedCloseTable\n",
             DelayedEntry->KeyControlBlock,
             (ULONG)(((PUCHAR)DelayedEntry - (PUCHAR)CmpDelayedCloseTable) / sizeof(CM_DELAYED_CLOSE_ENTRY))));
    ...
    {
        DbgPrint(
            "CmpGetDelayedCloseIndex: Bogus index %lx; DelayedEntry = %p; sizeof( CM_DELAYED_CLOSE_ENTRY ) = %lx\n",
...
--*/
{
    ...
    CmKdPrintEx((DPFLTR_CONFIG_ID, CML_FLOW,
                 "[CmpAddToDelayedClose] : Adding kcb = %p to DelayedCloseTable; index = %lu\n", kcb, DelayedIndex));
    //
    // move the entry on top of the LRU list
    //
    CmpRemoveEntryList(&(DelayedEntry->DelayedLRUList));
    InsertHeadList(&CmpDelayedLRUListHead, &(DelayedEntry->DelayedLRUList));
}
