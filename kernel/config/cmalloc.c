...
SearchFreeKcb:
    ...
    if (IsListEmpty(&CmpFreeKCBListHead) == FALSE)
    {
        ...
        kcb = CONTAINING_RECORD(kcb, CM_KEY_CONTROL_BLOCK, FreeListEntry);
    ...
    InsertTailList(&CmPageListHead, &(AllocPage->CmPageListEntry));
#endif //DBG
    ...
    {
        kcb =
            (PCM_KEY_CONTROL_BLOCK)((PUCHAR)AllocPage + FIELD_OFFSET(CM_ALLOC_PAGE, AllocPage) + j * CM_KCB_ENTRY_SIZE);
...
AllocFromPool:
    kcb = ExAllocatePoolWithTag(PagedPool, sizeof(CM_KEY_CONTROL_BLOCK), CM_KCB_TAG | PROTECTED_POOL);
...
--*/
{
    ...
    if (AllocPage->FreeCount == CM_KCBS_PER_PAGE)
    {
        ...
        for (j = 0; j < CM_KCBS_PER_PAGE; j++)
        {
            kcb = (PCM_KEY_CONTROL_BLOCK)((PUCHAR)AllocPage + FIELD_OFFSET(CM_ALLOC_PAGE, AllocPage) +
                                          j * CM_KCB_ENTRY_SIZE);
