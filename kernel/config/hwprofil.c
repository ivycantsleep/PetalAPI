...
NTSTATUS
CmDeleteKeyRecursive(HANDLE hKeyRoot, PWSTR Key, PVOID TemporaryBuffer, ULONG LengthTemporaryBuffer,
                     BOOLEAN ThisKeyToo);

NTSTATUS
CmpGetAcpiProfileInformation(IN HANDLE IDConfigDB, OUT PCM_HARDWARE_PROFILE_LIST *ProfileList,
                             OUT PCM_HARDWARE_PROFILE_ACPI_ALIAS_LIST *AliasList, IN PWCHAR NameBuffer,
                             IN PUCHAR ValueBuffer, IN ULONG Len);

NTSTATUS
CmpFilterAcpiDockingState(IN PPROFILE_ACPI_DOCKING_STATE NewDockState, IN ULONG CurrentDockingState,
                          IN PWCHAR CurrentAcpiSN, IN ULONG CurrentProfileNumber,
                          IN OUT PCM_HARDWARE_PROFILE_LIST ProfileList,
                          IN OUT PCM_HARDWARE_PROFILE_ACPI_ALIAS_LIST AliasList);

NTSTATUS
CmpMoveBiosAliasTable(IN HANDLE IDConfigDB, IN HANDLE CurrentInfo, IN ULONG CurrentProfileNumber,
                      IN ULONG NewProfileNumber, IN PWCHAR nameBuffer, IN PCHAR valueBuffer, IN ULONG bufferLen);
...
CmpGetAcpiProfileInformation(IN HANDLE IDConfigDB, OUT PCM_HARDWARE_PROFILE_LIST *ProfileList,
                             OUT PCM_HARDWARE_PROFILE_ACPI_ALIAS_LIST *AliasList, IN PWCHAR nameBuffer,
                             IN PUCHAR valueBuffer, IN ULONG bufferLen)
/*++
Routine Description:
...
--*/
{
    ...
    *AliasList = NULL;
    ...
    status = ZwOpenKey(&profiles, KEY_READ, &attributes);
    ...
    //
    status = ZwQueryKey(profiles, KeyFullInformation, &keyInfo, sizeof(keyInfo), &len);
    ...
    len = sizeof(CM_HARDWARE_PROFILE_LIST) + (sizeof(CM_HARDWARE_PROFILE) * (keyInfo.SubKeys - 1));
    ...
    *ProfileList = ExAllocatePool(PagedPool, len);
    ...
    for (i = 0; i < keyInfo.SubKeys; i++)
    {
        ...
        //
        status = ZwEnumerateKey(profiles, i, KeyBasicInformation, basicInfo,
        ...
        InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, profiles, NULL);
        status = ZwOpenKey(&entry, KEY_READ, &attributes);
        if (!NT_SUCCESS(status))
        {
            break;
        }
        ...
        status = NtQueryValueKey(entry, &name, KeyValueFullInformation, valueBuffer, bufferLen, &len);
        ...
        {
            TempProfile.PreferenceOrder = *(PULONG)((PUCHAR)value + value->DataOffset);
        }
        ...
        else
        {
            ...
            RtlCopyMemory(buffer, (PUCHAR)value + value->DataOffset, value->DataLength);
        }
    ...
    InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, IDConfigDB, NULL);
    status = ZwOpenKey(&acpiAlias, KEY_READ, &attributes);
    ...
    //
    status = ZwQueryKey(acpiAlias, KeyFullInformation, &keyInfo, sizeof(keyInfo), &len);
    ...
    *AliasList = ExAllocatePool(PagedPool, sizeof(CM_HARDWARE_PROFILE_LIST) +
    ...
    (*AliasList)->MaxAliasCount = (*AliasList)->CurrentAliasCount = keyInfo.SubKeys;
    ...
    for (i = 0; i < keyInfo.SubKeys; i++)
    {
        ...
        // Get the first key in the list.
        //
        status = ZwEnumerateKey(acpiAlias, i, KeyBasicInformation, basicInfo,
                                bufferLen - sizeof(UNICODE_NULL), // term 0
                                &len);
        ...
        InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, acpiAlias, NULL);
        status = ZwOpenKey(&entry, KEY_READ, &attributes);
        if (!NT_SUCCESS(status))
        {
            break;
        ...
        (*AliasList)->Alias[i].ProfileNumber = *(PULONG)((PUCHAR)value + value->DataOffset);
        ...
        (*AliasList)->Alias[i].DockState = *(PULONG)((PUCHAR)value + value->DataOffset);
        ...
        (*AliasList)->Alias[i].SerialNumber = (value->DataLength) ? ExAllocatePool(PagedPool, value->DataLength) : 0;
        ...
        if (value->DataLength)
        {
            RtlCopyMemory((*AliasList)->Alias[i].SerialNumber, (PUCHAR)value + value->DataOffset, value->DataLength);
        }
...
--*/
{
    ...
    while (aliasNumber < 200)
    {
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (PreventDuplication)
            {
                ...
                RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_DOCKING_STATE);
                status =
                    NtQueryValueKey(aliasEntry, &name, KeyValueFullInformation, valueBuffer, valueBufferLength, &len);
                ...
                if (NewDockState->DockingState != *(PULONG)((PUCHAR)keyInfo + keyInfo->DataOffset))
                {
                    //
                    // Not a dupe
                    //
                ...
                if (!RtlEqualMemory(NewDockState->SerialNumber, ((PUCHAR)keyInfo + keyInfo->DataOffset),
    ...
    {
        CmKdPrintEx(
            (DPFLTR_CONFIG_ID, CML_BUGCHECK, "CM: cmpCreateAcpiAliasEntry error finding new set %08lx\n", status));
        aliasEntry = 0;
        goto Exit;
    }
    ...
    {
        CmKdPrintEx(
            (DPFLTR_CONFIG_ID, CML_BUGCHECK, "CM: cmpCreateAcpiAliasEntry error creating new set %08lx\n", status));
        aliasEntry = 0;
        goto Exit;
    }
    ...
    //
    RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_ACPI_SERIAL_NUMBER);
    status = NtSetValueKey(aliasEntry, &name, 0, REG_BINARY, NewDockState->SerialNumber, NewDockState->SerialLength);
...
NTSTATUS
CmSetAcpiHwProfile(IN PPROFILE_ACPI_DOCKING_STATE NewDockState, IN PCM_ACPI_SELECTION_ROUTINE Select, IN PVOID Context,
...
--*/
{
    ...
    *ProfileChanged = FALSE;
    ...
    status = ZwOpenKey(&IDConfigDB, KEY_READ, &attributes);
    ...
    //
    status = CmpGetAcpiProfileInformation(IDConfigDB, &ProfileList, &AliasList, nameBuffer, valueBuffer,
                                          sizeof(valueBuffer));
    ...
    status = ZwOpenKey(&currentInfo, KEY_READ, &attributes);
    if (!NT_SUCCESS(status))
    {
        currentInfo = NULL;
        goto Clean;
    ...
    //
    RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_DOCKING_STATE);
    status = NtQueryValueKey(currentInfo, &name, KeyValueFullInformation, valueBuffer, sizeof(valueBuffer), &len);
    ...
    if (NT_SUCCESS(status) && (value->Type == REG_BINARY))
    {
        ...
        RtlCopyMemory(currentAcpiSN, (PUCHAR)value + value->DataOffset, value->DataLength);
    }
    else
    {
        currentAcpiSN = 0;
    ...
    status = NtQueryValueKey(IDConfigDB, &name, KeyValueFullInformation, valueBuffer, sizeof(valueBuffer), &len);
    ...
    //
    status = CmpFilterAcpiDockingState(NewDockState, currentDockingState, currentAcpiSN, currentProfileNumber,
                                       ProfileList, AliasList);
    ...
    if (flags & CM_HP_FLAGS_DUPLICATE)
    {
        ...
        status = CmpMoveBiosAliasTable(IDConfigDB, currentInfo, currentProfileNumber, profileNum, nameBuffer,
    ...
    if ((flags & CM_HP_FLAGS_PRISTINE) || (profileNum != currentProfileNumber))
    {
        ...
        *ProfileChanged = TRUE;
        ...
        if (flags & CM_HP_FLAGS_PRISTINE)
        {
            ...
            //
            ASSERT(!(flags & CM_HP_FLAGS_TRUE_MATCH));
            status = CmpCloneHwProfile(IDConfigDB, parent, HardwareProfile, profileNum, NewDockState->DockingState,
                                       &HardwareProfile, &profileNum);
            if (!NT_SUCCESS(status))
            {
                HardwareProfile = 0;
                goto Clean;
        ...
        status =
            NtCreateKey(&currentInfo, KEY_READ | KEY_WRITE, &attributes, 0, NULL, REG_OPTION_VOLATILE, &disposition);
        ...
        //
        RtlInitUnicodeString(&name, L"CurrentConfig");
        status = NtSetValueKey(IDConfigDB, &name, 0, REG_DWORD, &profileNum, sizeof(profileNum));
    ...
    RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_DOCKING_STATE);
    status = ZwSetValueKey(currentInfo, &name, 0, REG_DWORD, &i, sizeof(ULONG));

    //
    // Write the new ACPI information to the current Info key
    //
    RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_ACPI_SERIAL_NUMBER);
    status = ZwSetValueKey(currentInfo, &name, 0, REG_BINARY, NewDockState->SerialNumber, NewDockState->SerialLength);
    ...
    if (!(flags & CM_HP_FLAGS_TRUE_MATCH))
    {
        ...
        //
        status =
            CmpAddAcpiAliasEntry(IDConfigDB, NewDockState, profileNum, nameBuffer, valueBuffer, sizeof(valueBuffer),
    ...
    if (profileNum != currentProfileNumber)
    {
        ...
        //
        RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_CCS_CURRENT);
        InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE | OBJ_OPENLINK, NULL, NULL);

        status =
            NtCreateKey(&currentSymLink, KEY_CREATE_LINK, &attributes, 0, NULL, REG_OPTION_OPEN_LINK, &disposition);

        ASSERT(STATUS_SUCCESS == status);
        ASSERT(REG_OPENED_EXISTING_KEY == disposition);

        swprintf(nameBuffer, L"\\Registry\\Machine\\System\\CurrentControlSet\\Hardware Profiles\\%04d", profileNum);
        RtlInitUnicodeString(&name, nameBuffer);
        status = NtSetValueKey(currentSymLink, &CmSymbolicLinkValueName, 0, REG_LINK, name.Buffer, name.Length);
...
NTSTATUS
CmpFilterAcpiDockingState(IN PPROFILE_ACPI_DOCKING_STATE NewDockingState, IN ULONG CurrentDockState,
                          IN PWCHAR CurrentAcpiSN, IN ULONG CurrentProfileNumber,
                          IN OUT PCM_HARDWARE_PROFILE_LIST ProfileList,
                          IN OUT PCM_HARDWARE_PROFILE_ACPI_ALIAS_LIST AliasList)
/*++
...
--*/
{
    ...
    if (AliasList)
    {
        while (i < AliasList->CurrentAliasCount)
        {
            ...
            if (((alias->DockState & mask) != 0) &&
                ((alias->DockState & mask) != (NewDockingState->DockingState & mask)))
            {
            ...
            else if (alias->SerialLength != NewDockingState->SerialLength)
            {
                ...
                ;
                     ...
                     RtlCompareMemory(NewDockingState->SerialNumber, alias->SerialNumber, alias->SerialLength))
            {
                //
                ...
                for (j = 0; j < ProfileList->CurrentProfileCount; j++)
                {
                    if (ProfileList->Profile[j].Id == alias->ProfileNumber)
                    {
                        ...
                        //
                        ASSERT(!(ProfileList->Profile[j].Flags & CM_HP_FLAGS_PRISTINE));

                        ProfileList->Profile[j].Flags |= CM_HP_FLAGS_TRUE_MATCH;
                        trueMatch = TRUE;
                    }
                    if ((CurrentDockState == NewDockingState->DockingState) && (NULL == CurrentAcpiSN))
                    {
                        //
    ...
    if ((!dupDetect) && (NULL == CurrentAcpiSN) && (!trueMatch) && (CurrentDockState == NewDockingState->DockingState))
    {
    ...
    while (i < ProfileList->CurrentProfileCount)
    {
        ...
        else if (keepCurrent && (ProfileList->Profile[i].Id == CurrentProfileNumber))
        {
            //
            // Leave this one in the list
            //
        ...
        if (0 < len)
        {
            RtlMoveMemory(&ProfileList->Profile[i], &ProfileList->Profile[i + 1], sizeof(CM_HARDWARE_PROFILE) * len);
        }
...
--*/
{
    ...
    status = NtQueryValueKey(CurrentInfo, &name, KeyValueFullInformation, valueBuffer, bufferLen, &len);
    if (!NT_SUCCESS(status) || (value->Type != REG_DWORD))
    {
        status = STATUS_REGISTRY_CORRUPT;
        goto Clean;
    ...
    InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, IDConfigDB, NULL);
    status = ZwOpenKey(&alias, KEY_READ, &attributes);
    ...
    status = ZwQueryKey(alias, KeyFullInformation, &keyInfo, sizeof(keyInfo), &len);
    ...
    for (i = 0; i < keyInfo.SubKeys; i++)
    {
        ...
        // Get the first key in the list.
        //
        status = ZwEnumerateKey(alias, i, KeyBasicInformation, basicInfo,
                                bufferLen - sizeof(UNICODE_NULL), // term 0
                                &len);
        ...
        InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, alias, NULL);
        status = ZwOpenKey(&entry, KEY_READ | KEY_WRITE, &attributes);
        if (!NT_SUCCESS(status))
        {
            break;
        }
        ...
        RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_PROFILE_NUMBER);
        status = NtSetValueKey(entry, &name, 0, REG_DWORD, &NewProfileNumber, sizeof(NewProfileNumber));
        ...
        RtlInitUnicodeString(&name, nameBuffer);
        InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, hwprofile, NULL);
        ...
        if (*(PULONG)((PUCHAR)value + value->DataOffset))
        {
            ...
            status = CmDeleteKeyRecursive(hwprofile, nameBuffer, valueBuffer, bufferLen, TRUE);
...
--*/
{
    ...
    *NewProfileNumber = OldProfileNumber;
    ...
    while (*NewProfileNumber < 200)
    {
        ...
        InitializeObjectAttributes(&attributes, &newProfileName, OBJ_CASE_INSENSITIVE, Parent, NULL);

        status = NtOpenKey(NewProfile, KEY_READ | KEY_WRITE, &attributes);
    ...
    {
        CmKdPrintEx(
            (DPFLTR_CONFIG_ID, CML_BUGCHECK, "CM: CmpCloneHwProfile error finding new profile key %08lx\n", status));
        goto Exit;
    }
    ...
    status = NtQuerySecurityObject(OldProfile, DACL_SECURITY_INFORMATION, NULL, 0, &securityLength);
    ...
    if (STATUS_BUFFER_TOO_SMALL == status)
    {
        ...
        {
            status =
                NtQuerySecurityObject(OldProfile, DACL_SECURITY_INFORMATION, security, securityLength, &securityLength);
            if (!NT_SUCCESS(status))
            {
    ...
    // Create the new key
    //
    InitializeObjectAttributes(&attributes, &newProfileName, OBJ_CASE_INSENSITIVE, Parent, security);

    status = NtCreateKey(NewProfile, KEY_READ | KEY_WRITE, &attributes, 0, NULL, 0, &disposition);
    ...
    status = NtCreateKey(&IDConfigDBEntry, KEY_READ | KEY_WRITE, &attributes, 0, NULL, 0, &disposition);
    ...
    InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, IDConfigDB, NULL);
    status = ZwOpenKey(&hardwareProfiles, KEY_READ, &attributes);
    ...
    // Find the number of profile Sub Keys
    //
    status = ZwQueryKey(hardwareProfiles, KeyFullInformation, valueBuffer, sizeof(valueBuffer), &length);
    ...
    for (i = 0; i < profileSubKeys; i++)
    {
        ...
        //
        status = ZwEnumerateKey(hardwareProfiles, i, KeyBasicInformation, valueBuffer,
        ...
        //
        if ((!_wtoi(keyBasicInfo->Name)) || ((ULONG)(_wtoi(keyBasicInfo->Name)) == *NewProfileNumber))
        {
            continue;
        }
        ...
        InitializeObjectAttributes(&attributes, &name, OBJ_CASE_INSENSITIVE, hardwareProfiles, NULL);
        status = ZwOpenKey(&profileEntry, KEY_READ, &attributes);
        if (!NT_SUCCESS(status))
        {
            profileEntry = NULL;
            continue;
        ...
        RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_PREFERENCE_ORDER);
        status =
            NtQueryValueKey(profileEntry, &name, KeyValueFullInformation, valueBuffer, sizeof(valueBuffer), &length);
        ...
        //
        if (((*(PULONG)((PUCHAR)keyValueInfo + keyValueInfo->DataOffset)) > value) || (value == -1))
        {
    ...
    //
    RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_PREFERENCE_ORDER);
    status = NtSetValueKey(IDConfigDBEntry, &name, 0, REG_DWORD, &value, sizeof(value));

    //
    // Give the new profile a friendly name, based on the DockingState
    //
    status = CmpCreateHwProfileFriendlyName(IDConfigDB, DockingState, *NewProfileNumber, &friendlyName);

    if (NT_SUCCESS(status))
    {
        RtlInitUnicodeString(&name, CM_HARDWARE_PROFILE_STR_FRIENDLY_NAME);
        status = NtSetValueKey(IDConfigDBEntry, &name, 0, REG_SZ, friendlyName.Buffer,
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (NT_SUCCESS(status))
        {
            ...
            status = NtSetValueKey(IDConfigDBEntry, &name, 0, REG_SZ, guidStr.Buffer, guidStr.MaximumLength);
    ...
    //
    status =
        ObReferenceObjectByHandle(OldProfile, KEY_READ, CmpKeyObjectType, KernelMode, (PVOID *)(&oldProfileKey), NULL);
    ...
    status = ObReferenceObjectByHandle(*NewProfile, KEY_WRITE, CmpKeyObjectType, KernelMode, (PVOID *)(&newProfileKey),
                                       NULL);
    ...
    // new tree.
    //
    if (CmpCopyTree(oldProfileKey->KeyControlBlock->KeyHive, oldProfileKey->KeyControlBlock->KeyCell,
                    newProfileKey->KeyControlBlock->KeyHive, newProfileKey->KeyControlBlock->KeyCell))
    {
        ...
        SourceNode =
            (PCM_KEY_NODE)HvGetCell(oldProfileKey->KeyControlBlock->KeyHive, oldProfileKey->KeyControlBlock->KeyCell);
        if (SourceNode != NULL)
        {
            DestNode = (PCM_KEY_NODE)HvGetCell(newProfileKey->KeyControlBlock->KeyHive,
...
NTSTATUS
CmDeleteKeyRecursive(HANDLE hKeyRoot, PWSTR Key, PVOID TemporaryBuffer, ULONG LengthTemporaryBuffer, BOOLEAN ThisKeyToo)
/*++
...
--*/
{
    ...
    InitializeObjectAttributes(&Obja, &UnicodeString, OBJ_CASE_INSENSITIVE, hKeyRoot, NULL);
    ...
    while (1)
    {
        Status = ZwEnumerateKey(hKey, 0, KeyBasicInformation, TemporaryBuffer, LengthTemporaryBuffer, &ResultLength);
        if (!NT_SUCCESS(Status))
        {
            break;
        }
        ...
        //
        SubkeyName = ExAllocatePool(PagedPool, ((wcslen(KeyInfo->Name) + 1) * sizeof(WCHAR)));
        if (!SubkeyName)
        {
            Status = STATUS_INSUFFICIENT_RESOURCES;
            break;
        ...
        Status = CmDeleteKeyRecursive(hKey, SubkeyName, TemporaryBuffer, LengthTemporaryBuffer, TRUE);
...
--*/
{
    ...
    {
        dataTableEntry =
            CONTAINING_RECORD(KeLoaderBlock->LoadOrderListHead.Flink, KLDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
    }
    else if (PsLoadedModuleList.Flink)
    {
        dataTableEntry = CONTAINING_RECORD(PsLoadedModuleList.Flink, KLDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
    }
    else
    {
    ...
    // big.
    //
    if ((unicodeString.Length + 5 * sizeof(WCHAR) + sizeof(UNICODE_NULL)) > MAX_FRIENDLY_NAME_LENGTH)
    {
        status = STATUS_UNSUCCESSFUL;
        goto Clean;
    }
    ...
    InitializeObjectAttributes(&attributes, &keyName, OBJ_CASE_INSENSITIVE, IDConfigDB, NULL);
    status = ZwOpenKey(&hardwareProfiles, KEY_READ, &attributes);
    if (!NT_SUCCESS(status))
    {
        hardwareProfiles = NULL;
    ...
    //
    keyValueInfo = (PKEY_VALUE_FULL_INFORMATION)valueBuffer;
    status = ZwQueryValueKey(hardwareProfiles, &labelName, KeyValueFullInformation, valueBuffer, sizeof(valueBuffer),
                             &length);
    ...
    //
    status = ZwSetValueKey(hardwareProfiles, &labelName, 0, REG_DWORD, &index, sizeof(index));
    if (!NT_SUCCESS(status))
    {
        goto Clean;
    }
    ...
    {
        swprintf(friendlyNameBuffer, L"%s %u", unicodeString.Buffer, index);
    }
    else
    {
