...
void __cdecl main(int argc, char *argv[])
{
    ...
    HANDLE BaseHandle;
    ULONG Sizes[] = { sizeof(KEY_VALUE_BASIC_INFORMATION), sizeof(KEY_VALUE_FULL_INFORMATION) };
    ULONG ResultLength;
    ...
    status = NtQueryValueKey(BaseHandle, &WorkName2, InfoType, (PVOID) & (Buffer[0]), BufferSize, &ResultLength);
    ...
    case KeyValueFullInformation:
        ...
        printf("      Data: '%.*ws'\n", pfull->DataLength / 2, ((PUCHAR)pfull + pfull->DataOffset));
        break;
    ...
    case KeyValuePartialInformation:
        ...
        printf("      Type: %08lx\n", ppartial->Type);
        printf("DataLength: %08lx\n", ppartial->DataLength);
        printf("      Data: '%.*ws'\n", ppartial->DataLength / 2, ((PUCHAR) & (ppartial->Data)));
        break;
    }
...
void processargs(int argc, char *argv[])
{
    ...
    RtlAnsiStringToUnicodeString(&WorkName2, &temp, TRUE);
