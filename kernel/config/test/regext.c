...

{
    ...
    {
        (lpReadMem)(pHiveListEntry, HiveList, sizeof(HiveList), &BytesRead);
    }
    ...
    (lpReadMem)(&pCmpHiveListHead->Flink, &pNextHiveList, sizeof(pNextHiveList), &BytesRead);
    ...
    else
    {
        while (pNextHiveList != pCmpHiveListHead)
        {
            ...
            (lpReadMem)(&pNextHiveList->Flink, &pNextHiveList, sizeof(pNextHiveList), &BytesRead);
            if (BytesRead != sizeof(pNextHiveList))
            {
...
void poolDumpHive(IN PCMHIVE pHive)
{
    CMHIVE CmHive;
    ULONG BytesRead;
    ...
    (lpReadMem)(pHive, &CmHive, sizeof(CmHive), &BytesRead);
    ...
    if (SavePages)
    {
        fprintf(TempFile, "%ws %d %d\n", FileName, CmHive.Hive.Storage[Stable].Length,
                CmHive.Hive.Storage[Volatile].Length);
    }
    poolDumpMap(CmHive.Hive.Storage[Stable].Length, CmHive.Hive.Storage[Stable].Map);

    (lpPrint)("  Volatile Length = %lx\n", CmHive.Hive.Storage[Volatile].Length);
    poolDumpMap(CmHive.Hive.Storage[Volatile].Length, CmHive.Hive.Storage[Volatile].Map);
}
...
VOID poolDumpMap(IN ULONG Length, IN PHMAP_DIRECTORY Map)
{
    ...
    //
    (lpReadMem)(Map, &MapDirectory, Tables * sizeof(PHMAP_TABLE), &BytesRead);
    if (BytesRead < (Tables * sizeof(PHMAP_TABLE)))
    {
        (lpPrint)("Only read %lx/%lx bytes from %lx\n", BytesRead, Tables * sizeof(PHMAP_TABLE), Map);
        return;
    }
    ...
    for (i = 0; i < MapSlots; i++)
    {
        ...
        (lpReadMem)(&(MapTable->Table[i % HTABLE_SLOTS]), &MapEntry, sizeof(HMAP_ENTRY), &BytesRead);
        ...
        // probe the HBLOCK
        //
        (lpReadMem)(MapEntry.BlockAddress, &Garbage, sizeof(ULONG), &BytesRead);
        if (BytesRead > 0)
        {
            ++Present;
        }
...

{
    ...
    while (!feof(File))
    {
        ...
        {
            fscanf(File, "%lx\n", &Page);
            (lpReadMem)(Page, &Garbage, sizeof(ULONG), &BytesRead);
            if (BytesRead > 0)
            {
                ++Present;
            }
...

{
    ...
    (lpReadMem)(Root, &pKCB, sizeof(pKCB), &BytesRead);
...

{
    ...
    ++TotalKcbs;
    (lpReadMem)(pKcb, &kcb, sizeof(kcb), &BytesRead);
    ...
    if (Buffer != NULL)
    {
        (lpReadMem)(kcb.FullName.Buffer, Buffer, kcb.FullName.Length, &BytesRead);
