...
void __cdecl main(int argc, char *argv[])
{
    ...
    *p = '\0';
    ...
    status =
        NtCreateKey(&BaseHandle, MAXIMUM_ALLOWED, &ObjectAttributes, 0, &ClassName, REG_OPTION_VOLATILE, &Disposition);
    ...
    for (i = 0; i < NumberChildren; i++)
    {
        ...
        status = NtCreateKey(&WorkHandle, MAXIMUM_ALLOWED, &ObjectAttributes, 0, &ClassName, REG_OPTION_VOLATILE,
                             &Disposition);
        if (!NT_SUCCESS(status))
        {
