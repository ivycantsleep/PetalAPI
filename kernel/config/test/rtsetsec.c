...
void __cdecl main(int argc, char *argv[])
{
    ...
    Status = NtOpenKey(&KeyHandle, WRITE_DAC, &ObjectAttributes);
    if (!NT_SUCCESS(Status))
    {
    ...
    Status = NtSetSecurityObject(KeyHandle, DACL_SECURITY_INFORMATION, NewSecurityDescriptor);
    if (!NT_SUCCESS(Status))
    {
...
GenerateDescriptor(VOID)
{
    ...
    *(RtlSubAuthoritySid(WorldSid, 0)) = SECURITY_WORLD_RID;
    ...
    WorldAceLength = SeLengthSid(WorldSid) - sizeof(ULONG) + sizeof(ACCESS_ALLOWED_ACE);
    ...
    Status = RtlCopySid(SeLengthSid(WorldSid), &WorldAce->SidStart, WorldSid);
    if (!NT_SUCCESS(Status))
    {
    ...
    Status = RtlCopySid(SeLengthSid(CreatorSid), &OwnerAce->SidStart, CreatorSid);
    if (!NT_SUCCESS(Status))
    {
        printf("rtsetsec: RtlCopySid failed status %08lx\n", Status);
        exit(1);
    ...
    Status = RtlAddAce(Acl, ACL_REVISION, 0, WorldAce, WorldAceLength);
    if (!NT_SUCCESS(Status))
    {
        printf("rtsetsec: RtlAddAce (world ace) failed status %08lx\n", Status);
        exit(1);
    }
    Status = RtlAddAce(Acl, ACL_REVISION, 0, OwnerAce, OwnerAceLength);
    if (!NT_SUCCESS(Status))
    {
...
PSID GetMySid(VOID)
{
    ...
    Status = NtQueryInformationToken(Token, TokenOwner, Owner, 0, &Length);
    ...
    Status = NtQueryInformationToken(Token, TokenOwner, Owner, Length, &Length);
    if (!NT_SUCCESS(Status))
    {
        printf("rtsetsec: GetMySid() NtQueryInformationToken failed status %08lx\n", Status);
        exit(1);
