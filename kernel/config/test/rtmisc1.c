...
VOID NameClassAndTitle(KEY_NODE_INFORMATION *NodeInformation, UNICODE_STRING ClassName, ULONG TitleIndex,
                       UNICODE_STRING KeyName, LARGE_INTEGER CompTime,
...
void __cdecl main(int, char *)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, &KeyName, 0, BaseHandle, NULL);
    ObjectAttributes.Attributes |= OBJ_CASE_INSENSITIVE;
    ...
    status = NtCreateKey(&Testhand1, MAXIMUM_ALLOWED, &ObjectAttributes, TITLE_INDEX_1, &ClassName, 0, &Disposition);
    if (!NT_SUCCESS(status))
    {
    ...
    status = NtQueryKey(Testhand1, KeyNodeInformation, NodeInformation, bufsize, &ResultLength);
    if (!NT_SUCCESS(status))
    {
    ...
    NameClassAndTitle(NodeInformation, ClassName, TITLE_INDEX_1, KeyName, CompTime,
    ...
    status = NtOpenKey(&Testhand1, MAXIMUM_ALLOWED, &ObjectAttributes);
    if (!NT_SUCCESS(status))
    {
    ...
    status = NtSetValueKey(Testhand1, &ValueName, TITLE_INDEX_2, TYPE_1, datastring, strlen(datastring) + 1);
    if (!NT_SUCCESS(status))
    {
    ...
    status =
        NtQueryValueKey(Testhand1, &ValueName, KeyValueFullInformation, KeyValueInformation, bufsize, &ResultLength);
    if (!NT_SUCCESS(status))
    {
        printf("rtmisc1: t6: %08lx\n", status);
        goto punt;
    }
    expected = FIELD_OFFSET(KEY_VALUE_FULL_INFORMATION, Name) + ValueName.Length + strlen(datastring) + 1;
    if (ResultLength != expected)
    {
        printf("rtmisc1: t6a: expected = %08lx actual = %08lx", expected, ResultLength);
        failure++;
    }

    if ((KeyValueInformation->TitleIndex != TITLE_INDEX_2) || (KeyValueInformation->Type != TYPE_1) ||
    ...
    if (wcsncmp(ValueName.Buffer, tp, (ValueName.Length / sizeof(WCHAR))) != 0)
    {
        ...
        expectstring(ValueName.Buffer, (ValueName.Length / sizeof(WCHAR)), (PWSTR) & (KeyValueInformation->Name[0]),
    ...
    status = NtSetValueKey(Testhand1, &ValueName2, TITLE_INDEX_3, TYPE_2, datastring2, strlen(datastring2) + 1);
    if (!NT_SUCCESS(status))
    {
    ...
    status = NtQueryValueKey(Testhand1, &ValueName2, KeyValueBasicInformation, KeyValueBasic, bufsize, &ResultLength);
    if (!NT_SUCCESS(status))
    {
    ...
    expected = FIELD_OFFSET(KEY_VALUE_BASIC_INFORMATION, Name) + ValueName2.Length;
    if (ResultLength != expected)
    {
        printf("rtmisc1: t8a: expected = %08lx actual = %08lx", expected, ResultLength);
        failure++;
    }

    if ((KeyValueBasic->TitleIndex != TITLE_INDEX_3) || (KeyValueBasic->Type != TYPE_2) ||
    ...
    if (wcsncmp(ValueName2.Buffer, tp, (ValueName2.Length / sizeof(WCHAR))) != 0)
    {
        ...
        expectstring(ValueName2.Buffer, (ValueName2.Length / sizeof(WCHAR)), (PWSTR) & (KeyValueBasic->Name[0]),
    ...
    RtlZeroMemory(KeyValueBasic, bufsize);
    status = NtEnumerateValueKey(Testhand1,
                                 0, // Index
                                 KeyValueBasicInformation, KeyValueBasic, bufsize, &ResultLength);
    if (!NT_SUCCESS(status))
    {
    ...
    expected = FIELD_OFFSET(KEY_VALUE_BASIC_INFORMATION, Name) + ValueName.Length;
    if (ResultLength != expected)
    {
        printf("rtmisc1: t9a: expected = %08lx actual = %08lx", expected, ResultLength);
        failure++;
    }
    ...
    if (wcsncmp(ValueName.Buffer, tp, (ValueName.Length / sizeof(WCHAR))) != 0)
    {
        ...
        expectstring(ValueName.Buffer, (ValueName.Length / sizeof(WCHAR)), (PWSTR) & (KeyValueBasic->Name[0]),
                     (KeyValueBasic->NameLength / sizeof(WCHAR)));
        failure++;
    }
    ...
    if (ResultLength != expected)
    {
        printf("rtmisc1: t9e: expected = %08lx actual = %08lx", expected, ResultLength);
        failure++;
    }
    ...
    InitializeObjectAttributes(&ObjectAttributes, &KeyName2, 0, BaseHandle, NULL);
    ObjectAttributes.Attributes |= OBJ_CASE_INSENSITIVE;

    status = NtCreateKey(&Testhand1, MAXIMUM_ALLOWED, &ObjectAttributes, TITLE_INDEX_2, &ClassName2, 0, &Disposition);
    if (!NT_SUCCESS(status))
    {
    ...
    RtlZeroMemory(NodeInformation, bufsize);
    status = NtEnumerateKey(BaseHandle, 0, KeyNodeInformation, NodeInformation, bufsize, &ResultLength);
    if (!NT_SUCCESS(status))
    {
    ...
    RtlZeroMemory(NodeInformation, bufsize);
    status = NtEnumerateKey(BaseHandle, 1, KeyNodeInformation, NodeInformation, bufsize, &ResultLength);
    if (!NT_SUCCESS(status))
    {
    ...
    status = NtEnumerateKey(BaseHandle, 2, KeyNodeInformation, NodeInformation, bufsize, &ResultLength);
    if (status != STATUS_NO_MORE_ENTRIES)
    {
                       ...
                       PUCHAR TestName)
{
    ...
    {
        printf("%s wrong name\n", TestName);
        expectstring(KeyName.Buffer, (KeyName.Length / sizeof(WCHAR)), (PWSTR) & (NodeInformation->Name[0]),
