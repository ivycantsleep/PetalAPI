...
void __cdecl main(int argc, char *argv[])
{
    ...
    *p = '\0';
    ...
    status = NtCreateKey(&BaseHandle, MAXIMUM_ALLOWED, &ObjectAttributes, 0, &ClassName, CreateOption, &Disposition);
    ...
    for (i = 0; i < NumberChildren; i++)
    {
        ...
        status =
            NtCreateKey(&WorkHandle, MAXIMUM_ALLOWED, &ObjectAttributes, 0, &ClassName, CreateOption, &Disposition);
        if (!NT_SUCCESS(status))
        {
        ...
        for (j = 0; j < NumberValues; j++)
        {
            ...
            sprintf(formatbuffer, "This is a rtbatcr value for %s%d", BaseName, j);
...
void processargs(int argc, char *argv[])
{
    ...
    RtlAnsiStringToUnicodeString(&KeyName, &temp, TRUE);
