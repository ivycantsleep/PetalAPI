...
VOID CmpDumpKeyBodyList(IN PCM_KEY_CONTROL_BLOCK kcb, IN PULONG Count);
...
BOOLEAN
CmpRehashKcbSubtree(PCM_KEY_CONTROL_BLOCK Start, PCM_KEY_CONTROL_BLOCK End);
#endif //NT_RENAME_KEY
...
VOID CmpDumpKeyBodyList(IN PCM_KEY_CONTROL_BLOCK kcb, IN PULONG Count)
{
    ...
    {
        KeyBody = CONTAINING_RECORD(KeyBody, CM_KEY_BODY, KeyBodyList);
        //
...
ULONG
CmpSearchForOpenSubKeys(PCM_KEY_CONTROL_BLOCK KeyControlBlock, SUBKEY_SEARCH_TYPE SearchType)
/*++
Routine Description:
...
CmpGetNameControlBlock(PUNICODE_STRING NodeName)
{
    ...
    while (CurrentName)
    {
        ...
        if ((NameConvKey == CurrentName->ConvKey) && (NameSize == Ncb->NameLength))
        {
            //
    ...
    if (NameFound == FALSE)
    {
        ...
        Ncb = ExAllocatePoolWithTag(PagedPool, Size, CM_NAME_TAG | PROTECTED_POOL);
...
--*/
{
    ...
    FullName = (PUNICODE_STRING)ExAllocatePoolWithTag(PagedPool, size, CM_NAME_TAG | PROTECTED_POOL);
    ...
    if (FullName)
    {
        ...
        while (TmpKcb)
        {
            if ((TmpKcb->KeyHive == NULL) || (TmpKcb->KeyCell == HCELL_NIL) ||
...
--*/
{
    ...
    for (i = 0; i < CmpHashTableSize; i++)
    {
        ...
        while (*Prev)
        {
            Current = CONTAINING_RECORD(*Prev, CM_KEY_CONTROL_BLOCK, KeyHash);
            ASSERT_KCB(Current);
...
VOID CmpDereferenceKeyControlBlockWithLock(PCM_KEY_CONTROL_BLOCK KeyControlBlock)
{
    ...
    if (--KeyControlBlock->RefCount == 0)
    {
        ...
        //
        if ((CmpHoldLazyFlush && (!(KeyControlBlock->ExtFlags & CM_KCB_SYM_LINK_FOUND)) &&
             (!(KeyControlBlock->Flags & KEY_SYM_LINK))) ||
...

{
    ...
    while (Current)
    {
        ...
        if ((KeyHash->ConvKey == Current->ConvKey) && (KeyHash->KeyCell == Current->KeyCell) &&
            (KeyHash->KeyHive == Current->KeyHive))
        {
            //
            // Found a match
            //
            return (CONTAINING_RECORD(Current, CM_KEY_CONTROL_BLOCK, KeyHash));
        }
    ...
    //
    {
        ...
        for (DbgIndex = 0; DbgIndex < CmpHashTableSize; DbgIndex++)
        {
            ...
            while (Current)
            {
                ...
                ASSERT_KEY_HASH(Current);
                ASSERT((KeyHash->KeyHive != Current->KeyHive) || FakeKey || (kcb->ExtFlags & CM_KCB_KEY_NON_EXIST) ||
...
VOID CmpInitializeCache()
{
    ...
    CmpCacheTable = ExAllocatePoolWithTag(PagedPool, TotalCmCacheSize, 'aCMC');
    ...
    CmpNameCacheTable = ExAllocatePoolWithTag(PagedPool, TotalCmCacheSize, 'aCMC');
