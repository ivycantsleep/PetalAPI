...
#define IS_VALID_DISK_INFO(_Disk)                                                                                      \
    ((_Disk) != NULL && (_Disk)->DeviceObject != NULL && (_Disk)->SectorSize != 0 && (_Disk)->ScratchBuffer != NULL && \
     (_Disk)->SectorCount != 0)


#define ROUND_TO(_val, _factor) ((((_val) + (_factor) - 1) / (_factor)) * (_factor))

#define max(a, b) (((a) > (b)) ? (a) : (b))

#define IS_NULL_GUID(_guid)                                                                              \
    ((_guid).Data1 == 0 && (_guid).Data2 == 0 && (_guid).Data3 == 0 && (_guid).Data4[0] == 0 &&          \
     (_guid).Data4[1] == 0 && (_guid).Data4[2] == 0 && (_guid).Data4[3] == 0 && (_guid).Data4[4] == 0 && \
     (_guid).Data4[5] == 0 && (_guid).Data4[6] == 0 && (_guid).Data4[7] == 0)
...
NTSTATUS
FstubWriteEntryEFI(IN PDISK_INFORMATION Disk, IN ULONG PartitionEntryBlockCount, IN ULONG EntryNumber,
                   IN PEFI_PARTITION_ENTRY PartitionEntry, IN ULONG Partition, IN BOOLEAN Flush,
                   IN OUT ULONG32 *PartialCheckSum);

NTSTATUS
FstubWriteHeaderEFI(IN PDISK_INFORMATION Disk, IN ULONG PartitionEntryBlockCount, IN GUID DiskGUID,
