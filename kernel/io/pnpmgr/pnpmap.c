...
const CLASSDATA Class1Descriptions[] = { { 0x0000, L"SCSI Controller" },   { 0x0100, L"IDE Controller" },
                                         { 0x0200, L"Floppy Controller" }, { 0x0300, L"IPI Controller" },
                                         { 0x0400, L"RAID Controller" },   { 0x8000, L"Other Mass Storage" } };

const CLASSDATA Class2Descriptions[] = { { 0x0000, L"Ethernet" },
                                         { 0x0100, L"Token ring" },
                                         { 0x0200, L"FDDI" },
                                         { 0x0300, L"ATM" },
...
const CLASSDATA Class4Descriptions[] = { { 0x0000, L"Video device" },
                                         { 0x0100, L"Audio device" },
                                         { 0x8000, L"Other multimedia" } };

const CLASSDATA Class5Descriptions[] = { { 0x0000, L"RAM memory" },
                                         { 0x0100, L"Flash memory" },
                                         { 0x8000, L"Other memory" } };

const CLASSDATA Class6Descriptions[] = { { 0x0000, L"HOST / PCI" },  { 0x0100, L"PCI / ISA" },
                                         { 0x0200, L"PCI / EISA" },  { 0x0300, L"PCI / MCA" },
                                         { 0x0400, L"PCI / PCI" },   { 0x0500, L"PCI / PCMCIA" },
                                         { 0x0600, L"NuBus" },       { 0x0700, L"Cardbus" },
                                         { 0x8000, L"Other bridge" } };

const CLASSDATA Class7Descriptions[] = { { 0x0000, L"XT Serial" },     { 0x0001, L"16450" },
                                         { 0x0002, L"16550" },         { 0x0100, L"Parallel output only" },
                                         { 0x0101, L"BiDi Parallel" }, { 0x0102, L"ECP 1.x parallel" },
                                         { 0x8000, L"Other comm" } };

const CLASSDATA Class8Descriptions[] = { { 0x0000, L"Generic 8259" }, { 0x0001, L"ISA PIC" },
                                         { 0x0002, L"EISA PIC" },     { 0x0100, L"Generic 8237" },
                                         { 0x0101, L"ISA DMA" },      { 0x0102, L"EISA DMA" },
                                         { 0x0200, L"Generic 8254" }, { 0x0201, L"ISA timer" },
                                         { 0x0202, L"EISA timer" },   { 0x0300, L"Generic RTC" },
                                         { 0x0301, L"ISA RTC" },      { 0x8000, L"Other system device" } };
...
} const ClassDescriptionsList[] = { { NULL, 0 },
...
NTSTATUS
PbBiosResourcesToNtResources(IN ULONG BusNumber, IN ULONG SlotNumber, IN OUT PUCHAR *BiosData,
...
NTSTATUS
PnPBiosIoResourceListToCmResourceList(IN PIO_RESOURCE_REQUIREMENTS_LIST IoResourceList,
                                      OUT PCM_RESOURCE_LIST *CmResourceList, OUT ULONG *CmResourceListSize);

NTSTATUS
PnPBiosExtractCompatibleIDs(IN PUCHAR *DevNodeData, IN ULONG DevNodeDataLength, OUT PWSTR *CompatibleIDs,
                            OUT ULONG *CompatibleIDsLength);

NTSTATUS
PnPBiosTranslateInfo(IN VOID *BiosInfo, IN ULONG BiosInfoLength, OUT PBIOS_DEVNODE_INFO *DevNodeInfoList,
                     OUT ULONG *NumberNodes);

LONG PnPBiosFindMatchingDevNode(IN PWCHAR MapperName, IN PCM_RESOURCE_LIST ResourceList,
                                IN PBIOS_DEVNODE_INFO DevNodeInfoList, IN ULONG NumberNodes);

NTSTATUS
PnPBiosEliminateDupes(IN PBIOS_DEVNODE_INFO DevNodeInfoList, IN ULONG NumberNodes);

PWCHAR
PnPBiosGetDescription(IN PBIOS_DEVNODE_INFO DevNodeInfoEntry);

NTSTATUS
PnPBiosWriteInfo(IN PBIOS_DEVNODE_INFO DevNodeInfoList, IN ULONG NumberNodes);

VOID PnPBiosCopyIoDecode(IN HANDLE EnumRootKey, IN PBIOS_DEVNODE_INFO DevNodeInfo);

NTSTATUS
PnPBiosFreeDevNodeInfo(IN PBIOS_DEVNODE_INFO DevNodeInfoList, IN ULONG NumberNodes);

NTSTATUS
PnPBiosCheckForHardwareDisabled(IN PIO_RESOURCE_REQUIREMENTS_LIST IoResourceList, IN OUT PBOOLEAN Disabled);

BOOLEAN
PnPBiosCheckForExclusion(IN EXCLUDED_PNPNODE const *ExclusionArray, IN ULONG ExclusionCount, IN PWCHAR PnpDeviceName,
...
#ifdef ALLOC_PRAGMA
BOOLEAN
PnPBiosIgnoreNode(PWCHAR PnpID, PWCHAR excludeNodes);
PKEY_VALUE_FULL_INFORMATION
PnPGetBiosInfoValue(PWCHAR ValueName);
NTSTATUS
PnPBiosCopyDeviceParamKey(IN HANDLE EnumRootKey, IN PWCHAR SourcePath, IN PWCHAR DestinationPath);
...
--*/
{
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Could not open %S, status = %8.8X\n", MULTIFUNCTION_KEY_NAME, status));
    ...
    for (index = 0;; index++)
    {
        ...
        status = ZwEnumerateKey(multifunctionKey, // handle of key to enumerate
                                ...
                                KeyBasicInformation, keyBasicInfo, keyBasicInfoLength, &returnedLength);
        ...
        if (!NT_SUCCESS(status))
        {
            ...
            if (status != STATUS_NO_MORE_ENTRIES)
            {
                ...
                IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Could not enumerate under key %S, status = %8.8X\n",
                             MULTIFUNCTION_KEY_NAME, status));
            }
        ...
        if (!NT_SUCCESS(status))
        {
            ...
            IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Could not open registry key %S\\%S, status = %8.8X\n",
        ...
        status = ZwQueryValueKey(biosKey, &valueName, KeyValuePartialInformation, valueInfo, valueInfoLength,
                                 &returnedLength);
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (wcscmp((PWSTR)valueInfo->Data, L"PNP BIOS") == 0)
            {
                ...
                if (!NT_SUCCESS(status))
                {
                    ...
                    if (status == STATUS_BUFFER_TOO_SMALL || status == STATUS_BUFFER_OVERFLOW)
                    {
                        ...
                        if (valueInfo != NULL)
                        {
                            ...
                            status = ZwQueryValueKey(biosKey, &valueName, KeyValuePartialInformation, valueInfo,
                                                     valueInfoLength, &returnedLength);
                        }
                        else
                        {
                ...
                if (NT_SUCCESS(status))
                {
                    ...
                    *BiosInfoLength =
                        biosValue->PartialResourceList.PartialDescriptors[0].u.DeviceSpecificData.DataSize;
                    *BiosInfo = ExAllocatePool(PagedPool, *BiosInfoLength);

                    if (*BiosInfo != NULL)
                    {

                        RtlCopyMemory(*BiosInfo, &biosValue->PartialResourceList.PartialDescriptors[1],
                                      *BiosInfoLength);
...
BOOLEAN
PnPBiosCheckForExclusion(IN EXCLUDED_PNPNODE const *Exclusions, IN ULONG ExclusionCount, IN PWCHAR PnpDeviceName,
                         IN PWCHAR PnpCompatIds)
{
    PWCHAR idPtr;
    ...
    for (exclusionIndex = 0; exclusionIndex < ExclusionCount; exclusionIndex++)
    {
        ...
        if (RtlCompareMemory(idPtr, Exclusions[exclusionIndex].Id, Exclusions[exclusionIndex].IdLength) !=
            Exclusions[exclusionIndex].IdLength)
        {
...
--*/
{
    ...
    *CmResourceList = ExAllocatePool(PagedPool, *CmResourceListSize);
    ...
    for (descIndex = 0; descIndex < IoResourceList->List[0].Count; descIndex++)
    {
        ...
        default:
            IopDbgPrint(
                (IOP_MAPPER_ERROR_LEVEL, "Unexpected ResourceType (%d) in I/O Descriptor\n", ioDescriptor->Type));
...
--*/
{
    ...
    for (numNodes = 0; lengthRemaining > sizeof(CM_PNP_BIOS_DEVICE_NODE); numNodes++)
    {
        ...
        if (devNodeHeader->Size > lengthRemaining)
        {
            ...
            IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Node # %d, invalid size (%d), length remaining (%d)\n",
    ...
    if (fullValueInfo)
    {
        ...
        if (fullValueInfo->Type == REG_DWORD && fullValueInfo->DataLength == sizeof(ULONG) &&
    ...
    for (nodeIndex = 0; nodeIndex < numNodes; nodeIndex++)
    {
        ...
        // for completeness.
        //
        RtlCopyMemory(&devNodeInfo[nodeIndex].TypeCode, devNodeHeader->DeviceType,
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (tempResReqList != NULL)
            {
                ...
                // CM_RESOURCE_LIST.
                //
                status = PnPBiosIoResourceListToCmResourceList(tempResReqList, &devNodeInfo[nodeIndex].BootConfig,
        ...
        status = PnPBiosExtractCompatibleIDs(&configPtr, // BiosData
                                             (ULONG)remainingNodeLength, &devNodeInfo[nodeIndex].CompatibleIDs,
...
--*/
{
    ...
    if (bestMatch != -1)
    {
        ...
        if (bestScore == possibleScore)
        {
            ...
            IopDbgPrint((IOP_MAPPER_INFO_LEVEL, "Perfect match, score = %4.4X, possible = %4.4X, index = %d\n",
...
--*/
{
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        IopDbgPrint(
            (IOP_MAPPER_ERROR_LEVEL, "Could not open registry key %S, status = %8.8X\n", ENUMROOT_KEY_NAME, status));
    ...
    if (deviceBasicInfo != NULL && instanceBasicInfo != NULL && valueInfo != NULL)
    {
        ...
        for (deviceIndex = 0;; deviceIndex++)
        {
            ...
            status = ZwEnumerateKey(enumRootKey, deviceIndex, KeyBasicInformation, deviceBasicInfo,
                                    deviceBasicInfoLength, &returnedLength);
            ...
            for (instanceIndex = 0;; instanceIndex++)
            {
                ...
                status = ZwEnumerateKey(deviceKey, instanceIndex, KeyBasicInformation, instanceBasicInfo,
                                        instanceBasicInfoLength, &returnedLength);
                ...
                if (!NT_SUCCESS(status))
                {
                    ...
                    {
                        IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Could not enumerate under key %S\\%S, status = %8.8X\n",
                                     ENUMROOT_KEY_NAME, deviceBasicInfo->Name, status));
                    }
                    else
                    {
                ...
                if (RtlCompareMemory(instanceBasicInfo->Name, INSTANCE_ID_PREFIX,
                ...
                RtlCopyMemory(&logConfStr[instanceBasicInfo->NameLength / 2 + 1], REGSTR_KEY_LOGCONF,
                ...
                if (!NT_SUCCESS(status))
                {
                    ...
                    IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Could not open registry key %S\\%S\\%S, status = %8.8X\n",
                ...
                status = ZwQueryValueKey(logConfKey, &valueName, KeyValuePartialInformation, valueInfo, valueInfoLength,
                                         &returnedLength);
                ...
                if (!NT_SUCCESS(status))
                {
                    ...
                    if (status == STATUS_BUFFER_TOO_SMALL || status == STATUS_BUFFER_OVERFLOW)
                    {
                        ...
                        if (valueInfo != NULL)
                        {
                            ...
                            status = ZwQueryValueKey(logConfKey, &valueName, KeyValuePartialInformation, valueInfo,
                                                     valueInfoLength, &returnedLength);
                        }
                        else
                        {
                ...
                if (NT_SUCCESS(status))
                {
                    ...
                    matchingIndex =
                        PnPBiosFindMatchingDevNode(deviceBasicInfo->Name, resourceList, DevNodeInfoList, NumberNodes);

                    if (matchingIndex != -1)
                    {

                        DevNodeInfoList[matchingIndex].Replaces =
                            ExAllocatePool(PagedPool, deviceBasicInfo->NameLength + instanceBasicInfo->NameLength +
                                                          2 * sizeof(UNICODE_NULL));

                        if (DevNodeInfoList[matchingIndex].Replaces != NULL)
                        {

                            RtlCopyMemory(DevNodeInfoList[matchingIndex].Replaces, deviceBasicInfo->Name,
                            ...
                            RtlCopyMemory(&DevNodeInfoList[matchingIndex].Replaces[deviceBasicInfo->NameLength / 2 + 1],
                                          instanceBasicInfo->Name, instanceBasicInfo->NameLength);

                            DevNodeInfoList[matchingIndex]
                                .Replaces[(deviceBasicInfo->NameLength + instanceBasicInfo->NameLength) / 2 + 1] = '\0';

                            IopDbgPrint((IOP_MAPPER_INFO_LEVEL, "Match found: %S\\%S%d replaces %S\n",
                                         DevNodeInfoList[matchingIndex].ProductId, INSTANCE_ID_PREFIX,
                                         DevNodeInfoList[matchingIndex].Handle,
                                         DevNodeInfoList[matchingIndex].Replaces));
                        }
                        else
                        {
                            IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Error allocating memory for %S\\%S%d\\Replaces\n",
                                         DevNodeInfoList[matchingIndex].ProductId, INSTANCE_ID_PREFIX,
                                         DevNodeInfoList[matchingIndex].Handle));
                        }
                    }
                ...
                {
                    IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Error retrieving %S\\%S\\%S\\BootConfig, status = %8.8X\n",
                                 ENUMROOT_KEY_NAME, deviceBasicInfo->Name, logConfStr, status));
                }
...
--*/
{
    ...
    status = IopOpenDeviceParametersSubkey(&destinationDeviceParamKey, EnumRootKey, &destinationInstanceKeyName,
                                           KEY_ALL_ACCESS);
    ...
    for (index = 0;; index++)
    {
        status = ZwEnumerateValueKey(sourceDeviceParamKey, index, KeyValueFullInformation, valueFullInfo,
        ...
        if (NT_SUCCESS(status))
        {
            ...
            status =
                ZwSetValueKey(destinationDeviceParamKey, &valueName, valueFullInfo->TitleIndex, valueFullInfo->Type,
                              (PUCHAR)valueFullInfo + valueFullInfo->DataOffset, valueFullInfo->DataLength);
        }
        else
        {
...
--*/
{
    ...
    for (nodeIndex = 0; nodeIndex < NumberNodes; nodeIndex++)
    {
        ...
        if (excludeList && PnPBiosIgnoreNode(&DevNodeInfoList[nodeIndex].ProductId[1],
        ...
        if (PnPBiosCheckForExclusion(ExcludedDevices, EXCLUDED_DEVICES_COUNT, DevNodeInfoList[nodeIndex].ProductId,
        ...
        if (DevNodeInfoList[nodeIndex].FirmwareDisabled &&
            PnPBiosCheckForExclusion(ExcludeIfDisabled, EXCLUDE_DISABLED_COUNT, DevNodeInfoList[nodeIndex].ProductId,
                                     NULL))
        {
            continue;
        }

        swprintf(instanceNameStr, L"%s\\%s%d", DevNodeInfoList[nodeIndex].ProductId, INSTANCE_ID_PREFIX,
                 DevNodeInfoList[nodeIndex].Handle);

        RtlInitUnicodeString(&instanceKeyName, instanceNameStr);

        status = IopCreateRegistryKeyEx(&instanceKey, enumRootKey, &instanceKeyName, KEY_ALL_ACCESS,
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (disposition != REG_CREATED_NEW_KEY)
            {
                ...
                status = IopGetRegistryValue(instanceKey, REGSTR_VALUE_MIGRATED, &keyValueInformation);
                if (NT_SUCCESS(status))
                {
            ...
            if (isNewDevice)
            {
                ...
                status = ZwSetValueKey(instanceKey, &valueName, 0, REG_MULTI_SZ, DevNodeInfoList[nodeIndex].ProductId,
                ...
                if (DevNodeInfoList[nodeIndex].CompatibleIDs != NULL)
                {
                    ...
                    status = ZwSetValueKey(instanceKey, &valueName, 0, REG_MULTI_SZ,
                                           DevNodeInfoList[nodeIndex].CompatibleIDs,
            ...
            if (DevNodeInfoList[nodeIndex].Replaces != NULL)
            {
                ...
                status = ZwSetValueKey(instanceKey, &valueName, 0, REG_SZ, DevNodeInfoList[nodeIndex].Replaces,
            ...
            status = IopCreateRegistryKeyEx(&controlKey, instanceKey, &controlKeyName, KEY_ALL_ACCESS,
            ...
            else
            {
                ...
                IopDbgPrint((IOP_MAPPER_ERROR_LEVEL,
                             ...
                             DevNodeInfoList[nodeIndex].ProductId, instanceNameStr, status));
            ...
            status = IopCreateRegistryKeyEx(&logConfKey, instanceKey, &logConfKeyName, KEY_ALL_ACCESS,
            ...
            if (NT_SUCCESS(status))
            {
                ...
                if (DevNodeInfoList[nodeIndex].BootConfig != NULL)
                {
                    ...
                    PiWstrToUnicodeString(&valueName, REGSTR_VAL_BOOTCONFIG);
                    status = ZwSetValueKey(logConfKey, &valueName, 0, REG_RESOURCE_LIST,
                ...
                if (DevNodeInfoList[nodeIndex].BasicConfig != NULL)
                {
                    ...
                    status = ZwSetValueKey(logConfKey, &valueName, 0, REG_RESOURCE_REQUIREMENTS_LIST,
...
#define DECODE_FLAGS                                                                                    \
    (CM_RESOURCE_PORT_10_BIT_DECODE | CM_RESOURCE_PORT_12_BIT_DECODE | CM_RESOURCE_PORT_16_BIT_DECODE | \
    ...
    status = IopCreateRegistryKeyEx(&logConfKey, EnumRootKey, &logConfKeyName, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE,
                                    NULL);
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        if (status == STATUS_BUFFER_TOO_SMALL || status == STATUS_BUFFER_OVERFLOW)
        {
            ...
            else
            {
                ...
                IopDbgPrint((IOP_MAPPER_ERROR_LEVEL, "Could not allocate memory for BootConfig value\n"));
...
VOID PpFilterNtResource(IN PWCHAR PnpDeviceName, PIO_RESOURCE_REQUIREMENTS_LIST ResReqList)
{
    ...
    if (RtlCompareMemory(PnpDeviceName, L"*PNP06", sizeof(L"*PNP06") - sizeof(WCHAR)) ==
