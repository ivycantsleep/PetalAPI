...
BOOLEAN
IovpValidateDeviceObject(IN PDEVICE_OBJECT DeviceObject);
VOID IovFreeIrpPrivate(IN PIRP Irp);

NTSTATUS
IovpUnloadDriver(PDRIVER_OBJECT DriverObject);

BOOLEAN
IovpBuildDriverObjectList(IN PVOID Object, IN PUNICODE_STRING ObjectName, IN ULONG HandleCount, IN ULONG PointerCount,
                          IN PVOID Parameter);

NTSTATUS
IovpLocalCompletionRoutine(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context);
...
IovpValidateDeviceObject(IN PDEVICE_OBJECT DeviceObject)
{
    if ((DeviceObject->Type != IO_TYPE_DEVICE) || (DeviceObject->DriverObject == NULL) ||
...
VOID IovFreeIrpPrivate(IN PIRP Irp)
{
    ...
    if (IopVerifierOn)
    {
        ...
        {
            KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_FREE_IRP_TYPE_INVALID, (ULONG_PTR)Irp, 0, 0);
        }
        if (!IsListEmpty(&(Irp)->ThreadListEntry))
        {
            KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_FREE_IRP_NOT_ASSOCIATED_WITH_THREAD, (ULONG_PTR)Irp, 0,
                         0);
        }
    }
...
IovCallDriver(IN PDEVICE_OBJECT DeviceObject, IN OUT PIRP Irp)
{
    ...
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_CALL_DRIVER_INVALID_DEVICE_OBJECT, (ULONG_PTR)DeviceObject,
                     0, 0);
    }
    ...
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_CALL_DRIVER_IRQL_NOT_EQUAL, (ULONG_PTR)DeviceObject,
                     saveIrql, KeGetCurrentIrql());
    }
                                    ...
                                    IN PIO_STATUS_BLOCK IoStatusBlock OPTIONAL)
{
    ...
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_BUILD_FSD_REQUEST_EXCEPTION, (ULONG_PTR)DeviceObject,
                     (ULONG_PTR)MajorFunction, GetExceptionCode());
    }
    return (Irp);
}
                                    ...
                                    OUT PIO_STATUS_BLOCK IoStatusBlock)
{
    ...
    try
    {
        Irp = IoBuildDeviceIoControlRequest(IoControlCode, DeviceObject, InputBuffer, InputBufferLength, OutputBuffer,
                                            OutputBufferLength, InternalDeviceIoControl, Event, IoStatusBlock);
    }
    except(EXCEPTION_EXECUTE_HANDLER)
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_BUILD_IOCTL_REQUEST_EXCEPTION, (ULONG_PTR)DeviceObject,
                     (ULONG_PTR)IoControlCode, GetExceptionCode());
    }
...
IovInitializeTimer(IN PDEVICE_OBJECT DeviceObject, IN PIO_TIMER_ROUTINE TimerRoutine, IN PVOID Context)
{
    ...
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_REINITIALIZING_TIMER_OBJECT, (ULONG_PTR)DeviceObject, 0,
                     0);
    }
...
VOID FASTCALL IovCompleteRequest(IN PIRP Irp, IN CCHAR PriorityBoost)
{
    ...
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_COMPLETE_REQUEST_CANCEL_ROUTINE_SET,
                     (ULONG_PTR)Irp->CancelRoutine, (ULONG_PTR)Irp, 0);
    }

    if (Irp->IoStatus.Status == STATUS_PENDING || Irp->IoStatus.Status == 0xffffffff)
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_COMPLETE_REQUEST_INVALID_STATUS, Irp->IoStatus.Status,
                     (ULONG_PTR)Irp, 0);
    }

    if (KeGetCurrentIrql() > DISPATCH_LEVEL)
    {
        KeBugCheckEx(DRIVER_VERIFIER_IOMANAGER_VIOLATION, IO_COMPLETE_REQUEST_INVALID_IRQL, KeGetCurrentIrql(),
                     (ULONG_PTR)Irp, 0);
    }
    ...
    IovpKdPrint(("Hook:Irp 0x%x StackCount %d currentlocation %d stackpointer 0%x\n", Irp, Irp->StackCount,
                 currentLocation, IoGetCurrentIrpStackLocation(Irp)));
...
BOOLEAN
IovCancelIrp(PIRP Irp, BOOLEAN *returnValue)
{
                          ...
                          IN PVOID Parameter)
{
    ...
    {
        driverListEntry = ExAllocatePoolWithTag(NonPagedPool, sizeof(IOV_DRIVER_LIST_ENTRY), 'ovI');
        if (!driverListEntry)
        {
            return FALSE;
        }
...
IovUnloadDrivers(VOID)
{
    ...
    while (TRUE)
    {
        ...
        {
            IovpKdPrint(("Unload of driver %wZ (%p) pended\n", &driverListEntry->DriverObject->DriverName,
                         driverListEntry->DriverObject));
