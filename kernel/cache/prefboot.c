...

{
    ...
    if (PERFINFO_IS_GROUP_ON(PERF_LOADER))
    {
        ...
        PerfInfoLogBytes(PERFINFO_LOG_TYPE_BOOT_PHASE_START, &LogEntry, sizeof(LogEntry));
    }
    ...
    case PfSystemDriverInitPhase:
        ...
        KeInitializeEvent(&BootPrefetcher->SystemDriversPrefetchingDone, NotificationEvent, FALSE);
        KeInitializeEvent(&BootPrefetcher->PreSmssPrefetchingDone, NotificationEvent, FALSE);
        KeInitializeEvent(&BootPrefetcher->VideoInitPrefetchingDone, NotificationEvent, FALSE);
        KeInitializeEvent(&BootPrefetcher->VideoInitStarted, NotificationEvent, FALSE);
        ...
        Status =
            PsCreateSystemThread(&ThreadHandle, THREAD_ALL_ACCESS, NULL, NULL, NULL, CcPfBootWorker, BootPrefetcher);
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            Status = ObReferenceObjectByHandle(ThreadHandle, THREAD_SET_INFORMATION, PsThreadType, KernelMode, &Thread,
                                               NULL);
    ...
    case PfSessionManagerInitPhase:
        ...
        if (BootPrefetcher)
        {
            KeWaitForSingleObject(&BootPrefetcher->PreSmssPrefetchingDone, Executive, KernelMode, FALSE, NULL);
        }
    ...
    case PfPostVideoInitPhase:
        ...
        Status = CcPfSetParameter(CcPfGlobals.Parameters.ParametersKey, CCPF_VIDEO_INIT_TIME_VALUE_NAME, REG_DWORD,
                                  &VideoInitTime, sizeof(VideoInitTime));
...

{
    ...
    Status = CcPfQueryScenarioInformation(PrefetchHeader.Scenario, CcPfBasicScenarioInformation, &ScenarioInfo,
                                          sizeof(ScenarioInfo), &RequiredSize);
    ...
    Status = CcPfQueryScenarioInformation(PrefetchHeader.Scenario, CcPfBootScenarioInformation, &BootScenarioInfo,
                                          sizeof(BootScenarioInfo), &RequiredSize);
    ...
    Status = CcPfGetParameter(CcPfGlobals.Parameters.ParametersKey, CCPF_VIDEO_INIT_TIME_VALUE_NAME, REG_DWORD,
                              &VideoInitTime, &ValueSize);
    ...
    Status = CcPfGetParameter(CcPfGlobals.Parameters.ParametersKey, CCPF_VIDEO_INIT_PAGES_PER_SECOND_VALUE_NAME,
                              REG_DWORD, &VideoInitPagesPerSecond, &ValueSize);
    ...
    for (BootPhaseIdx = CcPfBootScenMaxPhase - 1;
         RemainingVideoInitPages && (BootPhaseIdx >= CcPfBootScenSystemProcInitPhase); BootPhaseIdx--)
    {
    ...
    if (PERFINFO_IS_GROUP_ON(PERF_DISK_IO))
    {
        ...
        PerfInfoLogBytes(PERFINFO_LOG_TYPE_BOOT_PREFETCH_INFORMATION, &LogEntry, sizeof(LogEntry));
    }
    ...
    for (PrefetchPhaseIdx = 0; PrefetchPhaseIdx < LastPrefetchPhaseIdx; PrefetchPhaseIdx++)
    {
        ...
        {
            KeWaitForSingleObject(&BootPrefetcher->VideoInitStarted, Executive, KernelMode, FALSE, NULL);
        }
        ...
        for (CursorIdx = 0; CursorIdx < MaxCursor; CursorIdx++)
        {
            ...
            if (NumPagesToPrefetch)
            {
                ...
                Status = CcPfPrefetchSections(&PrefetchHeader, Cursor->PrefetchType, &Cursor->StartCursor,
                                              NumPagesToPrefetch, &NumPagesPrefetched, &Cursor->EndCursor);
