...
FORCEINLINE
LOGICAL
KiIsKernelStackSwappable(IN KPROCESSOR_MODE WaitMode, IN PKTHREAD Thread)
...

{
    ...
    return ((WaitMode != KernelMode) && (Thread->EnableStackSwap != FALSE) &&
...
NTSTATUS
KiCallUserMode(IN PVOID *OutputBuffer, IN PULONG OutputLength);
...
ULONG
KiComputeTimerTableIndex(IN LARGE_INTEGER Interval, IN LARGE_INTEGER CurrentCount, IN PKTIMER Timer);
...
KCONTINUE_STATUS
KiSetDebugProcessor(IN PKTRAP_FRAME TrapFrame, IN PKEXCEPTION_FRAME ExceptionFrame, IN KPROCESSOR_MODE PreviousMode);
...
PKTHREAD
FASTCALL
KiFindReadyThread(IN ULONG Processor, KPRIORITY LowPriority);
...
__inline VOID KiSetTbFlushTimeStampBusy(VOID)
...
__inline VOID KiClearTbFlushTimeStampBusy(VOID)
...
VOID KiSuspendNop(IN struct _KAPC *Apc, IN OUT PKNORMAL_ROUTINE *NormalRoutine, IN OUT PVOID *NormalContext,
...
VOID KiUserApcDispatcher(IN PVOID NormalContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2,
...
BOOLEAN
FASTCALL
KiSwapContext(IN PKTHREAD Thread);

VOID FASTCALL KiWaitSatisfyAll(IN PKWAIT_BLOCK WaitBlock);
            ...
            {                                                                                      \
                ...
                {                                                                                  \
                    ...
                    (_Thread_)->WaitStatus = STATUS_ABANDONED;                                     \
                                                                                                   ...
                                                                                                   \
                InsertHeadList((_Thread_)->MutantListHead.Blink, &(_Object_)->MutantListEntry);    \
            }                                                                                      \
        }                                                                                          \
    }
...
NTSTATUS
Ki386CheckDivideByZeroTrap(IN PKTRAP_FRAME Frame);
...
__inline VOID KiSetSwapEvent(VOID)
