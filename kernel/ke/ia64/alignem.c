...

{
    ...
    try
    {
        ...
        case LDCCLRACQ_OP:
            ...
            {
                ProbeForRead(EffectiveAddress, 1 << OpSize, sizeof(UCHAR));
            }
        ...
        case LDFCNC_OP:
            ...
            if (Operand1 >= 32)
                Operand1 = 32 + (Rrbfr + Operand1 - 32) % 96;
            if (Operand2 >= 32)
                Operand2 = 32 + (Rrbfr + Operand2 - 32) % 96;
            if (Operand3 >= 32)
                Operand3 = 32 + (Rrbfr + Operand3 - 32) % 96;
            ...
            if (FaultInstruction.u.i_field.x == 1)
            {
                ...
                if (PreviousMode != KernelMode)
                {
                    ...
                    ProbeForRead(EffectiveAddress, Length << 1, sizeof(UCHAR));
                }
                ...
                KiSetFloatRegisterValue(Operand1, FloatData, ExceptionFrame, TrapFrame);
                ...
                KiEmulateLoadFloat(EffectiveAddress, OpSize, &FloatData);
                KiSetFloatRegisterValue(Operand2, FloatData, ExceptionFrame, TrapFrame);
            }
            else
            {
        ...
        case ST_OP:
            ...
            if (PreviousMode != KernelMode)
            {
                ProbeForWrite(EffectiveAddress, 1 << OpSize, sizeof(UCHAR));
            }
        ...
        case STF_OP:
            ...
            if (Operand2 >= 32)
                Operand2 = 32 + (Rrbfr + Operand2 - 32) % 96;
            FloatData = KiGetFloatRegisterValue(Operand2, ExceptionFrame, TrapFrame);
