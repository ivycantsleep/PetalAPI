...
struct
{
    ULONG Flags;
    PUCHAR String;
} ActFlags[] = { POWER_ACTION_QUERY_ALLOWED,
                 "QueryApps",
...
PUCHAR
SysPower(IN SYSTEM_POWER_STATE State)
{
    static UCHAR line[50];
    PUCHAR p;
...
PUCHAR
Action(IN PBOOLEAN CapFlag, IN PPOWER_ACTION_POLICY Act)
{
...
VOID SetTimerTime(IN HANDLE h, IN PUCHAR Text, IN ULONG DueTimeInMin)
{
    ...
    sprintf(s, "%d. Current time is:  %d:%d:%d, ", Iterations, TimeFields.wHour, TimeFields.wMinute,
            TimeFields.wSecond);
    printf(s);
    DbgPrint("SHD: %s", s);
    ...
    sprintf(s, "timer set for %d:%d:%d (%d min)  %s", TimeFields.wHour, TimeFields.wMinute, TimeFields.wSecond,
            DueTimeInMin, Text);
    printf(s);
    DbgPrint(s);
    ...
    Status = SetWaitableTimer(h, &DueTime, 0, NULL, NULL, TRUE);
...
char *argv[];
{
    ...
    if (argc == 1)
    {
        printf("shd shutdown|off|reset|hiber|sleep|doze [sync qapp ui oapp diswake critical] [loop] [maxloop <Int>] "
               "[waitTime <InMinutes>] [delayTime <InMinutes>]\n");
        exit(1);
    }
    ...
    while (argc)
    {
        ...
        if (_stricmp(p, "shutdown") == 0)
            Act.Action = PowerActionShutdown;
        if (_stricmp(p, "off") == 0)
            Act.Action = PowerActionShutdownOff;
        if (_stricmp(p, "reset") == 0)
            Act.Action = PowerActionShutdownReset;
        if (_stricmp(p, "hiber") == 0)
            Act.Action = PowerActionHibernate;
        if (_stricmp(p, "sleep") == 0)
            Act.Action = PowerActionSleep;


        if (_stricmp(p, "qapp") == 0)
            Act.Flags |= POWER_ACTION_QUERY_ALLOWED;
        if (_stricmp(p, "ui") == 0)
            Act.Flags |= POWER_ACTION_UI_ALLOWED;
        if (_stricmp(p, "oapp") == 0)
            Act.Flags |= POWER_ACTION_OVERRIDE_APPS;
        if (_stricmp(p, "diswake") == 0)
            Act.Flags |= POWER_ACTION_DISABLE_WAKES;
        if (_stricmp(p, "critical") == 0)
            Act.Flags |= POWER_ACTION_CRITICAL;
        if (_stricmp(p, "sync") == 0)
            Asynchronous = FALSE;

        if (_stricmp(p, "loop") == 0)
            SleepLoop = TRUE;
    ...
    if (!SleepLoop)
    {
        ...
        Status = NtInitiatePowerAction(Act.Action, MinSystemState, Act.Flags, Asynchronous);
        goto exit_main;
    }
    ...
    while (1)
    {
        ...
        printf(" %s\n", Action(NULL, &Act));
        DbgPrint(" %s\n", Action(NULL, &Act));
        Status = NtInitiatePowerAction(Act.Action, MinSystemState, Act.Flags, FALSE);
