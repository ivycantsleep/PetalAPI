...
ULONG
SimpleCheck(IN ULONG PartialSum, IN PVOID SourceVa, IN ULONG Length)
{
    PUSHORT Source;
...
VOID ReadPage(IN ULONG PageNo, IN PUCHAR Buffer)
{
...
BOOLEAN
CheckZeroPage(IN PULONG Buffer)
{
    ULONG i;
    ...
    Status = RtlCompressBuffer(COMPRESSION_FORMAT_LZNT1, Buffer, PAGE_SIZE, NewBuffer, PAGE_SIZE * 2, PAGE_SIZE,
                               &NewBufferSize, CompBuffer);
        ...
        for (Index = 1; Index <= Table[0].Link.EntryCount; Index++)
        {
            ...
            if (Verify && Check != Table[Index].Range.CheckSum)
            {
                printf("Hit on range %08x - %08x. Tbl %08x %08x, File %08x %08x\n", Table[Index].Range.StartPage,
                       Table[Index].Range.EndPage, TablePage, Table[Index].Range.CheckSum, Table[Index].Range.PageNo,
                       Check);
            }
        }
