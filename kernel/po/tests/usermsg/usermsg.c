...
LPTSTR szDebugPBT[] = { TEXT("WM_POWERBROADCAST, PBT_APMQUERYSUSPEND"),
...
BOOL CALLBACK DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_POWERBROADCAST:
        ...
        {
            SendDlgItemMessage(hWnd, IDC_STATUSLIST, LB_ADDSTRING, 0, (LPARAM)szDebugPBT[wParam]);
        }
        else
        {
            SendDlgItemMessage(hWnd, IDC_STATUSLIST, LB_ADDSTRING, 0,
                               (LPARAM)TEXT("WM_POWERBROADCAST, unknown wParam"));
        }
    ...
    case WM_DEVICECHANGE:
        SendDlgItemMessage(hWnd, IDC_STATUSLIST, LB_ADDSTRING, 0, (LPARAM)TEXT("WM_DEVICECHANGE"));
        break;
    }
    return FALSE;
}
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpAnsiCmdLine, INT cmdShow)
{
    DWORD dwThreadId;

#ifdef SEND_TEST_MESSAGES
    // Spin a thread to send update messages
    g_hNotifyMsg = CreateThread(NULL, STACKSIZE, (LPTHREAD_START_ROUTINE)NotifyMsgThread, NULL, 0, &dwThreadId);
#endif
