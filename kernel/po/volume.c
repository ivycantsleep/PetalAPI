...
--*/
{
    ...
    while (i)
    {
        i -= 1;
        Status =
            PsCreateSystemThread(&Thread, THREAD_ALL_ACCESS, &ObjectAttributes, 0L, NULL, PopFlushVolumeWorker, &Flush);
    ...
    InitializeObjectAttributes(&ObjectAttributes, &RegistryName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
...
--*/
{
    ...
    while (!IsListEmpty(&Flush->List))
    {
        ...
        Status = ObQueryNameString(Dope->DeviceObject, ObName, sizeof(Buffer), &len);
        ...
        if (NT_SUCCESS(Status) && ObName->Name.Buffer)
        {
            ...
            Status =
                ZwCreateFile(&handle, SYNCHRONIZE | FILE_READ_DATA | FILE_WRITE_DATA, &objA, &IoStatus, NULL,
                             GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN, 0, NULL, 0);
