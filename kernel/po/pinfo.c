...
VOID PopVerifyPowerPolicy(IN BOOLEAN Ac, IN PSYSTEM_POWER_POLICY InputPolicy, OUT PSYSTEM_POWER_POLICY PowerPolicy);

VOID PopVerifyProcessorPowerPolicy(IN BOOLEAN Ac, IN PPROCESSOR_POWER_POLICY InputPolicy,
                                   OUT PPROCESSOR_POWER_POLICY PowerPolicy);

VOID PopVerifyThrottle(IN PUCHAR Throttle, IN UCHAR Min);

VOID PopApplyPolicy(IN BOOLEAN UpdateRegistry, IN BOOLEAN AcPolicy, IN PSYSTEM_POWER_POLICY NewPolicy,
                    IN ULONG PolicyLength);

VOID PopApplyProcessorPolicy(IN BOOLEAN UpdateRegistry, IN BOOLEAN AcPolicy, IN PPROCESSOR_POWER_POLICY NewPolicy,
                             IN ULONG PolicyLength);

VOID PopFilterCapabilities(IN PSYSTEM_POWER_CAPABILITIES SourceCapabilities,
...
NTSTATUS
NtPowerInformation(IN POWER_INFORMATION_LEVEL InformationLevel, IN PVOID InputBuffer OPTIONAL,
...
--*/
{
    ...
    try
    {
        ...
        if (PreviousMode != KernelMode)
        {
            ...
            if (InputBuffer)
            {
                if (InformationLevel != VerifySystemPolicyAc && InformationLevel != VerifySystemPolicyDc &&
        ...
        case SystemPowerPolicyAc:
            ...
            if (InputBuffer)
            {
                ...
                PopApplyPolicy(TRUE, TRUE, (PSYSTEM_POWER_POLICY)InputBuffer, InputBufferLength);
            }
        ...
        case SystemPowerPolicyDc:
            ...
            if (InputBuffer)
            {
                ...
                PopApplyPolicy(TRUE, FALSE, (PSYSTEM_POWER_POLICY)InputBuffer, InputBufferLength);
            }
        ...
        case ProcessorPowerPolicyAc:
            ...
            if (InputBuffer)
            {
                ...
                PopApplyProcessorPolicy(TRUE, TRUE, (PPROCESSOR_POWER_POLICY)InputBuffer, InputBufferLength);
            }
        ...
        case ProcessorPowerPolicyDc:
            ...
            if (InputBuffer)
            {
                ...
                PopApplyProcessorPolicy(TRUE, FALSE, (PPROCESSOR_POWER_POLICY)InputBuffer, InputBufferLength);
            }
        ...
        case AdministratorPowerPolicy:
            ...
            if (InputBuffer)
            {
                ...
                if (PopUserIsAdmin())
                {
                    ...
                    PopApplyAdminPolicy(TRUE, (PADMINISTRATOR_POWER_POLICY)InputBuffer, InputBufferLength);
        ...
        case VerifyProcessorPowerPolicyAc:
            ...
            if (InputBuffer && OutputBuffer)
            {
                ...
                if (InputBufferLength >= sizeof(PROCESSOR_POWER_POLICY))
                {
                    ...
                    PopVerifyProcessorPowerPolicy(TRUE, InputBuffer, &Buf.ProcessorPolicy);
        ...
        case VerifyProcessorPowerPolicyDc:
            ...
            if (InputBuffer && OutputBuffer)
            {
                ...
                if (InputBufferLength >= sizeof(PROCESSOR_POWER_POLICY))
                {
                    ...
                    PopVerifyProcessorPowerPolicy(FALSE, InputBuffer, &Buf.ProcessorPolicy);
                    ReturnBuffer = &Buf.ProcessorPolicy;
                    ReturnBufferLength = sizeof(Buf.ProcessorPolicy);
                }
                else
        ...
        case SystemPowerCapabilities:
            ...
            PopCapabilities.DiskSpinDown = PopAttributes[POP_DISK_SPINDOWN_ATTRIBUTE].Count ? TRUE : FALSE;
        ...
        case SystemPowerStateNotifyHandler:
            ...
            if (PopPowerStateNotifyHandler.Handler && ((PPOWER_STATE_NOTIFY_HANDLER)InputBuffer)->Handler)
            {
            ...
            RtlCopyMemory(&PopPowerStateNotifyHandler, InputBuffer, sizeof(POWER_STATE_NOTIFY_HANDLER));
        ...
        case SystemReserveHiberFile:
            ...
            if (PreviousMode != KernelMode)
            {
                ...
                Status = ZwPowerInformation(SystemReserveHiberFile, &Enable, sizeof(Enable), NULL, 0);
                PopAcquirePolicyLock();
                break;
            }
...
VOID PopApplyAdminPolicy(IN BOOLEAN UpdateRegistry, IN PADMINISTRATOR_POWER_POLICY NewPolicy, IN ULONG PolicyLength)
/*++
...
--*/
{
    ...
    if (Policy.MinSleep < PowerSystemSleeping1 || Policy.MinSleep > PowerSystemHibernate ||
        Policy.MinSleep < PowerSystemSleeping1 || Policy.MaxSleep > PowerSystemHibernate ||
        Policy.MinSleep > Policy.MaxSleep || Policy.MinVideoTimeout > Policy.MaxVideoTimeout ||
    ...
    if (UpdateRegistry)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            ZwSetValueKey(handle, &UnicodeString, 0L, REG_BINARY, &Policy, sizeof(ADMINISTRATOR_POWER_POLICY));
...
--*/
{
    ...
    if (UpdateRegistry)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            ZwSetValueKey(handle, &UnicodeString, 0L, REG_BINARY, &OrigPolicy, sizeof(SYSTEM_POWER_POLICY));
...
--*/
{
    ...
    if (SystemPolicy == PopProcessorPolicy)
    {
        ...
        // Changing the active policy
        //
        PopSetNotificationWork(PO_NOTIFY_PROCESSOR_POLICY | PO_NOTIFY_PROCESSOR_POLICY_CALLBACK);
    ...
    if (UpdateRegistry)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            RtlInitUnicodeString(&UnicodeString, RegName);
            ZwSetValueKey(handle, &UnicodeString, 0L, REG_BINARY, &OrigPolicy, sizeof(PROCESSOR_POWER_POLICY));
            ZwClose(handle);
        }
    }
}
...
--*/
{
    ...
    PopVerifySystemPowerState(&PowerPolicy->LidOpenWake, SubstituteLightestOverallDownwardBounded);
    PopVerifySystemPowerState(&PowerPolicy->MinSleep, SubstituteLightestOverallDownwardBounded);
    PopVerifySystemPowerState(&PowerPolicy->MaxSleep, SubstituteLightestOverallDownwardBounded);
    PopVerifySystemPowerState(&PowerPolicy->ReducedLatencySleep, SubstituteLightestOverallDownwardBounded);
    for (i = 0; i < PO_NUM_POWER_LEVELS; i++)
    {
        ...
        if (DPolicy->Enable)
        {
            ...
            PopVerifyPowerActionPolicy(&PowerPolicy->DischargePolicy[i].PowerPolicy);
            PopVerifySystemPowerState(&PowerPolicy->DischargePolicy[i].MinSystemState,
    ...
    //
    if ((PowerPolicy->Idle.Action == PowerActionHibernate) && (!PopCapabilities.SystemS5))
    {
    ...
    if (PowerPolicy->IdleTimeout && PowerPolicy->IdleTimeout < PO_MIN_IDLE_TIMEOUT)
    {
    ...
    if ((PowerPolicy->IdleTimeout > 0) && (PowerPolicy->IdleSensitivity == 0))
    {
    ...
    if (PowerPolicy->FanThrottleTolerance != PO_NO_FAN_THROTTLE || PowerPolicy->ForcedThrottle != PO_NO_FORCED_THROTTLE)
    {
...
--*/
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (pPolicy->DemotePercent == 0 && pPolicy->AllowPromotion && pPolicy->TimeCheck < pPolicy->PromoteLimit)
        {
...
BOOLEAN
PopVerifyPowerActionPolicy(IN PPOWER_ACTION_POLICY Action)
/*++
...
--*/
{
    ...
    if (Action->Flags & !(POWER_ACTION_QUERY_ALLOWED | POWER_ACTION_UI_ALLOWED | POWER_ACTION_OVERRIDE_APPS |
                          POWER_ACTION_LOCK_CONSOLE | POWER_ACTION_DISABLE_WAKES | POWER_ACTION_CRITICAL))
    {
    ...
    // are allowed at all.
    //
    if ((Action->Action == PowerActionSleep) || (Action->Action == PowerActionHibernate))
    {
...
VOID PopAdvanceSystemPowerState(IN OUT PSYSTEM_POWER_STATE PowerState, IN POP_SUBSTITUTION_POLICY SubstitutionPolicy,
                                IN SYSTEM_POWER_STATE LightestSystemState, IN SYSTEM_POWER_STATE DeepestSystemState)
/*++
...
--*/
{
    ...
    if ((*PowerState != PowerSystemWorking) &&
        ((*PowerState < LightestSystemState) || (*PowerState > DeepestSystemState)))
    {
...
--*/
{
    ...
    case SubstituteLightenSleep:
        ...
        if (State == PowerSystemHibernate && (!PopCapabilities.SystemS4 || !PopCapabilities.HiberFilePresent))
        {
    ...
    case SubstituteDeepenSleep:
        ...
        if (State == PowerSystemHibernate &&
            (!HibernateAllowed || !PopCapabilities.SystemS4 || !PopCapabilities.HiberFilePresent))
        {
...
VOID PopResetCurrentPolicies(VOID)
/*++
...
NTSTATUS
PopNotifyPolicyDevice(IN PVOID Notification, IN PVOID Context)
/*++
...
--*/
{
    ...
    // Open the device
    //
    InitializeObjectAttributes(&ObjA, DriverName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, 0, 0);
    Status = ZwOpenFile(&DriverHandle, SYNCHRONIZE | FILE_READ_DATA | FILE_WRITE_DATA,
                        &ObjA,                              // Object
    ...
    case PolicyDeviceSystemButton:
        SwitchDevice = ExAllocatePoolWithTag(NonPagedPool, sizeof(*SwitchDevice), POP_PSWT_TAG);
        if (!SwitchDevice)
        {
    ...
    case PolicyDeviceThermalZone:
        ...
        // New thermal zone
        //
        ThermalZone = ExAllocatePoolWithTag(NonPagedPool, sizeof(*ThermalZone), POP_THRM_TAG);
        if (!ThermalZone)
        {
        ...
        KeInitializeDpc(&ThermalZone->PassiveDpc, PopThermalZoneDpc, ThermalZone);
        ...
        //
        ExInterlockedInsertTailList(&PopThermal, &ThermalZone->Link, &PopThermalLock);
...
POWER_ACTION
PopMapInternalActionToIrpAction(IN POWER_ACTION Action, IN SYSTEM_POWER_STATE SystemPowerState,
...
--*/
{
    ...
    if (Action != PowerActionWarmEject)
    {
        ...
        return (SystemPowerState != PowerSystemHibernate) ? Action : PowerActionHibernate;
    }
    ...
    if (UnmapWarmEject)
    {
        ...
        return (SystemPowerState != PowerSystemHibernate) ? PowerActionSleep : PowerActionHibernate;
    }
    ...
    ASSERT((SystemPowerState >= PowerSystemSleeping1) && (SystemPowerState <= PowerSystemHibernate));
...

{
    ...
    if (NT_SUCCESS(Status))
    {
        if (VetoType != PNP_VetoTypeUnknown)
        {
            ...
            PoPrint(PO_WARN, ("PopFilterCapabilities: disabling sleep states due to legacy %s: %ws\n",
                              (VetoType == PNP_VetoLegacyDriver) ? "driver" : "device", VetoList));
