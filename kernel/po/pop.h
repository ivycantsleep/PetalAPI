...
#define POP_NONO                                 \
    'ONON' // freed structure, compare with pool \
...
#define PopSetDoSystemPowerState(doe, value)            \
    {                                                   \
        ...
        doe->PowerFlags |= (value & POPF_SYSTEM_STATE); \
    }

#define PopGetDoSystemPowerState(doe) (doe->PowerFlags & POPF_SYSTEM_STATE)

#define PopSetDoDevicePowerState(doe, value)                   \
    {                                                          \
        doe->PowerFlags &= ~POPF_DEVICE_STATE;                 \
        doe->PowerFlags |= ((value << 4) & POPF_DEVICE_STATE); \
    }

#define PopGetDoDevicePowerState(doe) ((doe->PowerFlags & POPF_DEVICE_STATE) >> 4)
...
#define PopLockIrpSerialList(OldIrql) KeAcquireSpinLock(&PopIrpSerialLock, OldIrql);

#define PopUnlockIrpSerialList(OldIrql) KeReleaseSpinLock(&PopIrpSerialLock, OldIrql);
...
#define PopLockWorkerQueue(OldIrql) KeAcquireSpinLock(&PopWorkerLock, OldIrql);

#define PopUnlockWorkerQueue(OldIrql) KeReleaseSpinLock(&PopWorkerLock, OldIrql);
...
#define PopLockDopeGlobal(OldIrql) KeAcquireSpinLock(&PopDopeGlobalLock, OldIrql)

#define PopUnlockDopeGlobal(OldIrql) KeReleaseSpinLock(&PopDopeGlobalLock, OldIrql)
...
#define SIZEOF_EJECT_PARTIAL_INFO SIZEOF_PARTIAL_INFO_HEADER + sizeof(UNDOCK_POWER_RESTRICTIONS)
...
#define InterlockedOrAffinity(Target, Set)                                 \
    {                                                                      \
        ...
        {                                                                  \
            _i = _j;                                                       \
            _j = InterlockedCompareExchange64((Target), (_i | (Set)), _i); \
        } while (_i != _j);                                                \
    }
...
ULONG
PopGatherMemoryForHibernate(IN PPOP_HIBER_CONTEXT HiberContext, IN PFN_NUMBER NoPages, IN PMDL *Mdl, IN BOOLEAN Wait);
...
PopExceptionFilter(IN PEXCEPTION_POINTERS ExceptionInformation, IN BOOLEAN AllowRaisedException);
...
NTSTATUS
PopIdleSwitchIdleHandler(IN PPOP_IDLE_HANDLER NewHandler, IN ULONG NumElements);

NTSTATUS
PopIdleSwitchIdleHandlers(IN PPOP_IDLE_HANDLER NewHandler, IN ULONG NumElements);

NTSTATUS
PopIdleUpdateIdleHandler(IN PPOP_IDLE_HANDLER NewHandler, IN PPOP_IDLE_HANDLER OldHandler, IN ULONG NumElements);

NTSTATUS
PopIdleUpdateIdleHandlers(VOID);

NTSTATUS
PopIdleVerifyIdleHandlers(IN PPOP_IDLE_HANDLER NewHandler, IN ULONG NumElements);

VOID PopProcessorInformation(OUT PPROCESSOR_POWER_INFORMATION ProcInfo, IN ULONG ProcInfoLength,
...
VOID PopRequestPowerChange(IN PDEVOBJ_EXTENSION PowerExtension, IN POWER_STATE SystemPowerState,
...
NTSTATUS
PopSetPowerComplete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context);

#define PopIsStateDatabaseIdle()                                                        \
    (IsListEmpty(&PopStateChangeInProgress) && IsListEmpty(&PopSyncStateChangeQueue) && \
