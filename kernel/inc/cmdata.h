...
#define CM_MAX_LEAF_SIZE ((sizeof(HCELL_INDEX) * CM_MAX_INDEX) + (FIELD_OFFSET(CM_KEY_INDEX, List)))
...
#define CM_MAX_FAST_LEAF_SIZE ((sizeof(CM_INDEX) * CM_MAX_FAST_INDEX) + (FIELD_OFFSET(CM_KEY_FAST_INDEX, List)))
...
#define KEY_VOLATILE                          \
    0x0001 // This key (and all its children) \
           // is volatile.

#define KEY_HIVE_EXIT                             \
    0x0002 // This key marks a bounary to another \
...
#define KEY_HIVE_ENTRY                             \
    0x0004 // This key is the root of a particular \
           // hive.
...
#define KEY_COMP_NAME                              \
    0x0020 // The name for this key is stored in a \
           // compressed form.
#define KEY_PREDEF_HANDLE                        \
    0x0040 // There is no real key backing this, \
...
#define KEY_USER_FLAGS_VALID_MASK                               \
    0x000F // we only allow 4 bits for the user defined flags   \
...
#define VALUE_COMP_NAME                              \
    0x0001 // The name for this value is stored in a \
           // compressed form.
...
#define CmpIsHKeyValueSmall(realsize, size)                                                      \
    ((size >= CM_KEY_VALUE_SPECIAL_SIZE) ? ((realsize) = size - CM_KEY_VALUE_SPECIAL_SIZE, TRUE) \
                                         : ((realsize) = size, FALSE))

#define CmpIsHKeyValueBig(Hive, size) \
    ((Hive->Version >= HSYS_WHISTLER_BETA1) && ((size) < CM_KEY_VALUE_SPECIAL_SIZE) && ((size) > CM_KEY_VALUE_BIG))
...
#define ASSERT_BIG_DATA(BigData) \
    ASSERT(((BigData)->Signature == CM_BIG_DATA_SIGNATURE) && ((BigData)->Count > 0) && ((BigData)->List != HCELL_NIL));
...
#define CMP_GET_CACHED_CELLDATA(Cell) \
    (&(((PCM_CACHED_VALUE_INDEX)(((ULONG_PTR)(Cell)) & ~CMP_CELL_CACHED_MASK))->Data.CellData))
...
#define CM_KCB_SUBKEY_ONE                          \
    0x0002 // This key has only one subkey and the \
           // first 4 char                         \
           //
...
#define CM_KCB_CACHE_MASK (CM_KCB_NO_SUBKEY | CM_KCB_KEY_NON_EXIST | CM_KCB_SUBKEY_ONE | CM_KCB_SUBKEY_HINT)
...
typedef struct _CM_KEY_CONTROL_BLOCK
{
    ...
    union
    {                                   // The hint is always stored in uppercase.
