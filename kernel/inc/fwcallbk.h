...
typedef ULONG (*PVEN_EISA_READ_REGISTER_BUFFER_UCHAR_ROUTINE)(IN ULONG BusNumber, IN ULONG Offset, OUT PVOID Buffer,
                                                              IN ULONG Length);

typedef ULONG (*PVEN_EISA_WRITE_REGISTER_BUFFER_UCHAR_ROUTINE)(IN ULONG BusNumber, IN ULONG Offset, OUT PVOID Buffer,
                                                               IN ULONG Length);
...
typedef BOOLEAN (*PVEN_TRANSLATE_BUS_ADDRESS_ROUTINE)(IN INTERFACE_TYPE InterfaceType, IN ULONG BusNumber,
...
typedef ULONG (*PVEN_GET_BUS_DATA_BY_OFFSET_ROUTINE)(IN BUS_DATA_TYPE BusDataType, IN ULONG BusNumber,
                                                     IN ULONG SlotNumber, IN PVOID Buffer, IN ULONG Offset,
                                                     IN ULONG Length);

typedef ULONG (*PVEN_SET_BUS_DATA_BY_OFFSET_ROUTINE)(IN BUS_DATA_TYPE BusDataType, IN ULONG BusNumber,
                                                     IN ULONG SlotNumber, IN PVOID Buffer, IN ULONG Offset,
                                                     IN ULONG Length);

typedef PADAPTER_OBJECT (*PVEN_GET_ADAPTER_ROUTINE)(IN PDEVICE_DESCRIPTION DeviceDescription,
                                                    IN OUT PULONG NumberOfMapRegisters);

typedef PVOID (*PVEN_ALLOCATE_COMMON_BUFFER_ROUTINE)(IN PADAPTER_OBJECT AdapterObject, IN ULONG Length,
                                                     OUT PPHYSICAL_ADDRESS LogicalAddress, IN BOOLEAN CacheEnabled);

typedef VOID (*PVEN_FREE_COMMON_BUFFER_ROUTINE)(IN PADAPTER_OBJECT AdapterObject, IN ULONG Length,
                                                IN PHYSICAL_ADDRESS LogicalAddress, IN PVOID VirtualAddress,
                                                IN BOOLEAN CacheEnabled);

typedef ARC_STATUS (*PVEN_RETURN_ARC_INFORMATION_ROUTINE)(OUT PARC_INFORMATION ArcInformation);

typedef ARC_STATUS (*PVEN_ISSUE_SRB_DIRECT_ROUTINE)(UCHAR ScsiAdapterId, UCHAR PathId, UCHAR TargetId, UCHAR LunId,
                                                    PVOID Srb, PVOID BufferAddress, ULONG BufferLength,
                                                    BOOLEAN WriteToDevice);

typedef ARC_STATUS (*PVEN_READ_WRITE_ERROR_FRAME_ROUTINE)(ULONG ReadWrite, ULONG FrameType, PVOID FrameAddress,
                                                          PLONG FrameSize, ULONG FrameNumber);
...
ARC_STATUS
HalpVenReadWriteErrorFrame(IN ULONG ReadWrite, IN ULONG FrameType, IN OUT PVOID FrameAddress, IN OUT PLONG FrameSize,
...
#define VenReturnLanguageId ((PVEN_RETURN_LANGUAGE_ID_ROUTINE)(SYSTEM_BLOCK->VendorVector[ReturnLanguageIdRoutine]))

#define VenWPrint ((PVEN_WIDE_PRINT_ROUTINE)(SYSTEM_BLOCK->VendorVector[WidePrintRoutine]))

#define VenPrint ((PVEN_PRINT_ROUTINE)(SYSTEM_BLOCK->VendorVector[PrintRoutine]))
...
#define VenReturnExtendedSystemInformation(x)                       \
    ((PVEN_RETURN_EXTENDED_SYSTEM_INFORMATION_ROUTINE)(SYSTEM_BLOCK \
                                                           ->VendorVector[ReturnExtendedSystemInformationRoutine]))(x)
...
#define VenEISAWriteRegisterBufferUCHAR(BusNumber, Offset, Buffer, Length)                                      \
    ((PVEN_EISA_WRITE_REGISTER_BUFFER_UCHAR_ROUTINE)(SYSTEM_BLOCK                                               \
                                                         ->VendorVector[EISAWriteRegisterBufferUCHARRoutine]))( \
        (BusNumber), (Offset), (Buffer), (Length))
...
#define VenEISAWritePortUCHAR(BusNumber, Offset, Datum)                                                        \
    ((PVEN_EISA_WRITE_PORT_UCHAR_ROUTINE)(SYSTEM_BLOCK->VendorVector[EISAWritePortUCHARRoutine]))((BusNumber), \
                                                                                                  (Offset), (Datum))

#define VenEISAWritePortUSHORT(BusNumber, Offset, Datum)                                                         \
    ((PVEN_EISA_WRITE_PORT_USHORT_ROUTINE)(SYSTEM_BLOCK->VendorVector[EISAWritePortUSHORTRoutine]))((BusNumber), \
                                                                                                    (Offset), (Datum))

#define VenEISAWritePortULONG(BusNumber, Offset, Datum)                                                        \
    ((PVEN_EISA_WRITE_PORT_ULONG_ROUTINE)(SYSTEM_BLOCK->VendorVector[EISAWritePortULONGRoutine]))((BusNumber), \
                                                                                                  (Offset), (Datum))
...
#define VenCallBios(InterruptNumber, BiosArguments) \
    ((PVEN_CALL_BIOS_ROUTINE)(SYSTEM_BLOCK->VendorVector[CallBiosRoutine]))((InterruptNumber), (BiosArguments))
#endif
...
#define VenGetAdapter(DeviceDescription, NumberOfMapRegisters)                                       \
    ((PVEN_GET_ADAPTER_ROUTINE)(SYSTEM_BLOCK->VendorVector[GetAdapterRoutine]))((DeviceDescription), \
...
__inline ARC_STATUS VenReadWriteErrorFrame(IN ULONG ReadWrite, IN ULONG FrameType, IN OUT PVOID FrameAddress,
                                           IN OUT PLONG FrameSize, IN ULONG FrameNumber)

{
    ...
    KIRQL OldIrql = FwAcquireFirmwareLock();
    Status = HalpVenReadWriteErrorFrame(ReadWrite, FrameType, FrameAddress, FrameSize, FrameNumber);
