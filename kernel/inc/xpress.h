...
#define XPRESS_CALL __stdcall
    ...
    typedef void *XPRESS_CALL XpressAllocFn(void *context, // user-defined context (as passed to XpressEncodeCreate)
                                            int size       // size of memory block to allocate
    );

    // user-supplied callback function that releases memory
    typedef void XPRESS_CALL XpressFreeFn(void *context, // user-defined context (as passed to XpressEncodeClose)
    ...
    typedef void XPRESS_CALL XpressProgressFn(void *context, // user-defined context
    ...
    int XPRESS_CALL XpressEncode(XpressEncodeStream stream,         // encoder's workspace
    ...
    void XPRESS_CALL XpressEncodeClose(XpressEncodeStream stream,          // encoder's workspace
    ...
    typedef struct
    {
        int XpressDecodeDummy;
    } *XpressDecodeStream;

    // allocate memory for decoder. Returns NULL if not enough memory.
    XpressDecodeStream XPRESS_CALL XpressDecodeCreate(
        void *context,         // user-defined context info (will  be passed to AllocFn)
        XpressAllocFn *AllocFn // memory allocation callback
    );

    // decode compressed block. Returns # of decoded bytes or -1 otherwise
    int XPRESS_CALL XpressDecode(XpressDecodeStream stream,        // decoder's workspace
    ...
    void XPRESS_CALL XpressDecodeClose(XpressDecodeStream stream, // encoder's workspace
    ...
    int XPRESS_CALL XpressCrc32(const void *data, // beginning of data block
