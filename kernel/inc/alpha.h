...
#define MAXIMUM_VECTOR 256
    ...
    typedef BOOLEAN (*PKBUS_ERROR_ROUTINE)(IN struct _EXCEPTION_RECORD *ExceptionRecord,
...
#define ILLEGAL_LEVEL ((1 << 0) | (1 << APC_LEVEL) | (1 << DISPATCH_LEVEL) | (1 << CLOCK_LEVEL) | (1 << IPI_LEVEL))
    //
...
#define SANITIZE_PSR(psr, mode) (((mode) == KernelMode) ? (psr & 0x3f) : (0x3))
...
#define PRCB_BUILD_UNIPROCESSOR 0x0002
    ...
    NTHALAPI
    VOID READ_REGISTER_BUFFER_USHORT(PUSHORT Register, PUSHORT Buffer, ULONG Count);

    NTHALAPI
    VOID READ_REGISTER_BUFFER_ULONG(PULONG Register, PULONG Buffer, ULONG Count);
    ...
    NTHALAPI
    VOID WRITE_REGISTER_BUFFER_UCHAR(PUCHAR Register, PUCHAR Buffer, ULONG Count);

    NTHALAPI
    VOID WRITE_REGISTER_BUFFER_USHORT(PUSHORT Register, PUSHORT Buffer, ULONG Count);

    NTHALAPI
    VOID WRITE_REGISTER_BUFFER_ULONG(PULONG Register, PULONG Buffer, ULONG Count);
...
#define ExInterlockedExchangeUlong(Target, Value, Lock) ExAlphaInterlockedExchangeUlong(Target, Value)
    ...
    NTKERNELAPI
    INTERLOCKED_RESULT
    ExAlphaInterlockedIncrementLong(IN PLONG Addend);

    NTKERNELAPI
    INTERLOCKED_RESULT
    ExAlphaInterlockedDecrementLong(IN PLONG Addend);

    NTKERNELAPI
    ULONG
    ExAlphaInterlockedExchangeUlong(IN PULONG Target, IN ULONG Value);
...
#define InterlockedExchange64 _InterlockedExchange64
    ...
    LONGLONG
    InterlockedCompareExchange64(IN OUT PLONGLONG Destination, IN LONGLONG ExChange, IN LONGLONG Comperand);

    PVOID
    InterlockedExchangePointer(IN OUT PVOID *Target, IN PVOID Value);

    PVOID
    InterlockedCompareExchangePointer(IN OUT PVOID *Destination, IN PVOID ExChange, IN PVOID Comperand);

    LONGLONG
    InterlockedExchange64(IN OUT PLONGLONG Target, IN LONGLONG Value);
...
#define InterlockedCompareExchangePointer(Destination, ExChange, Comperand) \
    ...
    PVOID
    _InterlockedCompareExchange(IN OUT PVOID *Destination, IN PVOID ExChange, IN PVOID Comperand);
    ...
    NTKERNELAPI
    LONGLONG
    ExpInterlockedCompareExchange64(IN OUT PLONGLONG Destination, IN PLONGLONG Exchange, IN PLONGLONG Comperand);
...
#define KeYieldProcessor()
    ...
    NTKERNELAPI
    VOID KeProfileInterrupt(VOID);

    // begin_ntosp

    NTKERNELAPI
    VOID KeProfileInterruptWithSource(IN KPROFILE_SOURCE ProfileSource);

    // end_ntosp

    NTKERNELAPI
    VOID KeUpdateRunTime(VOID);
...
#define KeFillEntryTb(Pte, Virtual, Invalid)               \
    ...
    BOOLEAN
    KiEmulateByteWord(IN OUT PEXCEPTION_RECORD ExceptionRecord, IN OUT struct _KEXCEPTION_FRAME *ExceptionFrame,
    ...
    KiEmulateFloating(IN OUT PEXCEPTION_RECORD ExceptionRecord, IN OUT struct _KEXCEPTION_FRAME *ExceptionFrame,
                      IN OUT struct _KTRAP_FRAME *TrapFrame, IN OUT PSW_FPCR SoftwareFpcr);
    ...
    BOOLEAN
    KiFloatingException(IN OUT PEXCEPTION_RECORD ExceptionRecord, IN OUT struct _KEXCEPTION_FRAME *ExceptionFrame,
                        IN OUT struct _KTRAP_FRAME *TrapFrame, IN BOOLEAN ImpreciseTrap, OUT PULONG SoftFpcrCopy);

    ULONGLONG
    KiGetRegisterValue(IN ULONG Register, IN struct _KEXCEPTION_FRAME *ExceptionFrame,
                       IN struct _KTRAP_FRAME *TrapFrame);
