...
NTSTATUS
PspLookupSystemDllEntryPoint(IN PSZ EntryPointName, OUT PVOID *EntryPointAddress);

NTSTATUS
PspLookupKernelUserEntryPoints(VOID);

USHORT
PspNameToOrdinal(IN PSZ EntryPointName, IN ULONG DllBase, IN ULONG NumberOfNames, IN PULONG NameTableBase,
...
VOID PspNullSpecialApc(IN PKAPC Apc, IN OUT PKNORMAL_ROUTINE *NormalRoutine, IN OUT PVOID *NormalContext,
...
VOID PspExitSpecialApc(IN PKAPC Apc, IN OUT PKNORMAL_ROUTINE *NormalRoutine, IN OUT PVOID *NormalContext,
                       IN OUT PVOID *SystemArgument1, IN OUT PVOID *SystemArgument2);
...
VOID PspGetSetContextSpecialApc(IN PKAPC Apc, IN OUT PKNORMAL_ROUTINE *NormalRoutine, IN OUT PVOID *NormalContext,
                                IN OUT PVOID *SystemArgument1, IN OUT PVOID *SystemArgument2);
...
NTSTATUS
PspSetPrimaryToken(IN HANDLE ProcessHandle, IN PEPROCESS ProcessPointer OPTIONAL, IN HANDLE TokenHandle OPTIONAL,
                   IN PACCESS_TOKEN TokenPointer OPTIONAL, IN BOOLEAN PrivilegeChecked);
...
NTSTATUS
PspTerminateProcess(PEPROCESS Process, NTSTATUS Status);
...
//
#define PspLockProcessSecurityExclusive(xProcess, xCurrentThread) PspLockProcessExclusive(xProcess, xCurrentThread)

#define PspLockProcessSecurityShared(xProcess, xCurrentThread) PspLockProcessShared(xProcess, xCurrentThread)

#define PspUnlockProcessSecurityShared(xProcess, xCurrentThread) PspUnlockProcessShared(xProcess, xCurrentThread)

#define PspUnlockProcessSecurityExclusive(xProcess, xCurrentThread) PspUnlockProcessExclusive(xProcess, xCurrentThread)
