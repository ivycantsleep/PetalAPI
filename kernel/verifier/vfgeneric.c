...
const PCHAR IrpMajorNames[] = { "IRP_MJ_CREATE",                   // 0x00
...
{
    VfMajorRegisterHandlers(IRP_MJ_ALL_MAJORS, VfGenericDumpIrpStack, VfGenericVerifyNewRequest,
                            VfGenericVerifyIrpStackDownward, VfGenericVerifyIrpStackUpward, NULL, NULL,
                            VfGenericIsValidIrpStatus, VfGenericIsNewRequest, VfGenericVerifyNewIrp,
                            VfGenericVerifyFinalIrpStack, NULL);
}
                                        ...
                                        IN PIOV_STACK_LOCATION StackLocationData, IN PVOID CallerAddress OPTIONAL)
{
    ...
    if ((IovPacket->Flags & TRACKFLAG_IO_ALLOCATED) && (!(IovPacket->Flags & TRACKFLAG_WATERMARKED)))
    {
        ...
        if (VfMajorIsSystemRestrictedIrp(IrpSp))
        {
            ...
            //
            WDM_FAIL_ROUTINE(
                (DCERROR_RESTRICTED_IRP, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, IovPacket->TrackedIrp));
        }
    }
}
                                              ...
                                              IN PIOV_STACK_LOCATION StackLocationData, IN PVOID CallerAddress OPTIONAL)
{
    ...
    default:
        ...
        if ((IovPacket->DepartureIrql >= DISPATCH_LEVEL) && (!(IovPacket->Flags & TRACKFLAG_PASSED_AT_BAD_IRQL)))
        {

            WDM_FAIL_ROUTINE((DCERROR_DISPATCH_CALLED_AT_BAD_IRQL, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
    ...
    //
    if ((IovPacket->Flags & TRACKFLAG_BOGUS) && (!(RequestHeadLocationData->Flags & STACKFLAG_BOGUS_IRP_TOUCHED)))
    {
        ...
        if (newRequest && (!firstRequest))
        {
            ...
            WDM_FAIL_ROUTINE((DCERROR_BOGUS_FUNC_TRASHED, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
        }
        ...
        if (statusChanged)
        {
            ...
            if (IrpSp->MinorFunction == 0xFF)
            {
                ...
                WDM_FAIL_ROUTINE(
                    (DCERROR_BOGUS_MINOR_STATUS_TRASHED, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
            }
            else
            {

                WDM_FAIL_ROUTINE((DCERROR_BOGUS_STATUS_TRASHED, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
            }
        }
        ...
        if (infoChanged)
        {
            ...
            WDM_FAIL_ROUTINE((DCERROR_BOGUS_INFO_TRASHED, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
        }
    }
    ...
    if (!VfMajorIsValidIrpStatus(IrpSp, currentStatus))
    {
        ...
        WDM_FAIL_ROUTINE((DCERROR_INVALID_STATUS, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
    }
}
                                            ...
                                            IN BOOLEAN RequestFinalized)
{
    ...
    if ((IovPacket->Flags & TRACKFLAG_BOGUS) && (!(RequestHeadLocationData->Flags & STACKFLAG_BOGUS_IRP_TOUCHED)))
    {
        ...
        if (statusChanged)
        {
            ...
            if (IrpSp->MinorFunction == 0xFF)
            {
                ...
                WDM_FAIL_ROUTINE((DCERROR_BOGUS_MINOR_STATUS_TRASHED, DCPARAM_IRP + DCPARAM_ROUTINE, routine, irp));
            }
            else
            {

                WDM_FAIL_ROUTINE((DCERROR_BOGUS_STATUS_TRASHED, DCPARAM_IRP + DCPARAM_ROUTINE, routine, irp));
            }
        }
        ...
        if (infoChanged)
        {
            ...
            WDM_FAIL_ROUTINE((DCERROR_BOGUS_INFO_TRASHED, DCPARAM_IRP + DCPARAM_ROUTINE, routine, irp));
        }
    }
    ...
    if (!VfMajorIsValidIrpStatus(IrpSp, currentStatus))
    {
        ...
        WDM_FAIL_ROUTINE((DCERROR_INVALID_STATUS, DCPARAM_IRP + DCPARAM_ROUTINE, routine, irp));
    }
    ...
    if (irp->CancelRoutine)
    {
        ...
        if (VfSettingsIsOptionEnabled(IovPacket->VerifierSettings, VERIFIER_OPTION_VERIFY_CANCEL_LOGIC))
        {
            ...
            WDM_FAIL_ROUTINE((DCERROR_CANCELROUTINE_AFTER_COMPLETION, DCPARAM_IRP + DCPARAM_ROUTINE, routine, irp));
        }
    }
}
...
--*/
{
    return (BOOLEAN)((IrpLastSp == NULL) || (IrpSp->MajorFunction != IrpLastSp->MajorFunction) ||
                                    ...
                                    IN PIOV_STACK_LOCATION StackLocationData, IN PVOID CallerAddress OPTIONAL)
{
    ...
    if (Irp->UserIosb || Irp->UserEvent)
    {
        ...
        //
        queuesApc = (BOOLEAN)(!((Irp->Flags & (IRP_PAGING_IO | IRP_CLOSE_OPERATION)) &&
        ...
        if (queuesApc)
        {
            ...
            for (index = Irp->CurrentLocation - 1; index <= Irp->StackCount; index++)
            {
