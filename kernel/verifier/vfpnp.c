...
VOID VfPnpInit(VOID)
{
    VfMajorRegisterHandlers(IRP_MJ_PNP, VfPnpDumpIrpStack, VfPnpVerifyNewRequest, VfPnpVerifyIrpStackDownward,
                            VfPnpVerifyIrpStackUpward, VfPnpIsSystemRestrictedIrp, VfPnpAdvanceIrpStatus, NULL, NULL,
                            NULL, NULL, VfPnpTestStartedPdoStack);
}


VOID FASTCALL VfPnpVerifyNewRequest(IN PIOV_REQUEST_PACKET IovPacket, IN PDEVICE_OBJECT DeviceObject,
    ...
    if (IrpSp->MinorFunction == IRP_MN_QUERY_CAPABILITIES)
    {
        ...
        if (VfUtilIsMemoryRangeReadable(deviceCapabilities, sizeof(DEVICE_CAPABILITIES), VFMP_INSTANT_NONPAGED))
        {
            ...
            if (deviceCapabilities->Version < 1)
            {
                ...
                //
                WDM_FAIL_ROUTINE(
                    (DCERROR_PNP_QUERY_CAP_BAD_VERSION, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
            }
            ...
            if (deviceCapabilities->Address != (ULONG)-1)
            {
                ...
                //
                WDM_FAIL_ROUTINE(
                    (DCERROR_PNP_QUERY_CAP_BAD_ADDRESS, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, irp));
            }
...
VOID FASTCALL VfPnpVerifyIrpStackDownward(IN PIOV_REQUEST_PACKET IovPacket, IN PDEVICE_OBJECT DeviceObject,
                                          IN PIO_STACK_LOCATION IrpLastSp OPTIONAL, IN PIO_STACK_LOCATION IrpSp,
                                          ...
                                          IN PIOV_STACK_LOCATION StackLocationData, IN PVOID CallerAddress OPTIONAL)
{
    ...
    ViPnpVerifyMinorWasProcessedProperly(irp, IrpSp, devObjType, iovSessionData->VerifierSettings, howProcessed,
                                         CallerAddress);
}


VOID FASTCALL VfPnpVerifyIrpStackUpward(IN PIOV_REQUEST_PACKET IovPacket, IN PIO_STACK_LOCATION IrpSp,
                                        IN PIOV_STACK_LOCATION RequestHeadLocationData,
                                        ...
                                        IN BOOLEAN RequestFinalized)
{
    ...
    if (IsNewlyCompleted)
    {
        ...
        if (mustPassDown)
        {
            ...
            else if (NT_SUCCESS(currentStatus))
            {
                ...
                WDM_FAIL_ROUTINE(
                    (DCERROR_SUCCESSFUL_PNP_IRP_NOT_FORWARDED, DCPARAM_IRP + DCPARAM_ROUTINE, routine, irp));
            }
            else if (currentStatus == STATUS_NOT_SUPPORTED)
            {

                WDM_FAIL_ROUTINE(
                    (DCERROR_UNTOUCHED_PNP_IRP_NOT_FORWARDED, DCPARAM_IRP + DCPARAM_ROUTINE, routine, irp));
            }
        }
    }
    ...
    if (IsNewlyCompleted && isPdo)
    {
        ...
        ViPnpVerifyMinorWasProcessedProperly(irp, IrpSp, VF_DEVOBJ_PDO, iovSessionData->VerifierSettings, howProcessed,
                                             routine);
    }
    ...
    if (IsNewlyCompleted && (RequestHeadLocationData->Flags & STACKFLAG_CHECK_FOR_REFERENCE))
    {
        ...
        ASSERT((IrpSp->MajorFunction == IRP_MJ_PNP) && (IrpSp->MinorFunction == IRP_MN_QUERY_DEVICE_RELATIONS) &&
               (IrpSp->Parameters.QueryDeviceRelations.Type == TargetDeviceRelation));

        ASSERT(RequestHeadLocationData->ReferencingObject);
        ASSERT(IovPacket->RefTrackingCount);

        referencesTaken = ObvUtilStopObRefMonitoring(RequestHeadLocationData->ReferencingObject,
    ...
    //
    if ((currentStatus == STATUS_NOT_SUPPORTED) && (!(RequestHeadLocationData->Flags & STACKFLAG_FAILURE_FORWARDED)) &&
    ...
    case IRP_MN_QUERY_DEVICE_RELATIONS:
        ...
        if ((RequestHeadLocationData == StackLocationData) &&
            ...
            VfSettingsIsOptionEnabled(iovSessionData->VerifierSettings, VERIFIER_OPTION_SCRAMBLE_RELATIONS))
        {
            ...
            if (NT_SUCCESS(currentStatus) && irp->IoStatus.Information)
            {
                ...
                touchable = VfUtilIsMemoryRangeReadable(deviceRelations, (sizeof(DEVICE_RELATIONS) - sizeof(PVOID)),
                ...
                touchable = VfUtilIsMemoryRangeReadable(
                    deviceRelations, (sizeof(DEVICE_RELATIONS) - (deviceRelations->Count - 1) * sizeof(PVOID)),
                    VFMP_INSTANT_NONPAGED);
    ...
    case IRP_MN_SURPRISE_REMOVAL:
        if (VfSettingsIsOptionEnabled(iovSessionData->VerifierSettings, VERIFIER_OPTION_MONITOR_REMOVES))
        {
            ...
            else if (!IovUtilIsPdo(IrpSp->DeviceObject))
            {
                ...
                WDM_FAIL_ROUTINE((DCERROR_DETACHED_IN_SURPRISE_REMOVAL, DCPARAM_IRP + DCPARAM_ROUTINE + DCPARAM_DEVOBJ,
                                  routine, iovSessionData->BestVisibleIrp, IrpSp->DeviceObject));
            }
            ...
            if (IovUtilIsDeviceObjectMarked(IrpSp->DeviceObject, MARKTYPE_DELETED))
            {
                ...
                WDM_FAIL_ROUTINE((DCERROR_DELETED_IN_SURPRISE_REMOVAL, DCPARAM_IRP + DCPARAM_ROUTINE + DCPARAM_DEVOBJ,
                                  routine, iovSessionData->BestVisibleIrp, IrpSp->DeviceObject));
            }
        }
...
VOID FASTCALL VfPnpDumpIrpStack(IN PIO_STACK_LOCATION IrpSp)
{
    DbgPrint("IRP_MJ_PNP.");
    ...
    case IRP_MN_READ_CONFIG:
        DbgPrint("(WhichSpace=%x, Buffer=%x, Offset=%x, Length=%x)", IrpSp->Parameters.ReadWriteConfig.WhichSpace,
                 IrpSp->Parameters.ReadWriteConfig.Buffer, IrpSp->Parameters.ReadWriteConfig.Offset,
...
BOOLEAN
FASTCALL
VfPnpIsSystemRestrictedIrp(IN PIO_STACK_LOCATION IrpSp)
{
    switch (IrpSp->MinorFunction)
    {
...
BOOLEAN
FASTCALL
VfPnpAdvanceIrpStatus(IN PIO_STACK_LOCATION IrpSp, IN NTSTATUS OriginalStatus, IN OUT NTSTATUS *StatusToAdvance)
/*++
...
--*/
{
    ...
    if (VfSettingsIsOptionEnabled(NULL, VERIFIER_OPTION_RELATION_IGNORANCE_TEST))
    {
        ...
        irpSp.MinorFunction = IRP_MN_QUERY_DEVICE_RELATIONS;
        irpSp.Parameters.QueryDeviceRelations.Type = (DEVICE_RELATION_TYPE)-1;
        VfIrpSendSynchronousIrp(PhysicalDeviceObject, &irpSp, TRUE, STATUS_NOT_SUPPORTED, (ULONG_PTR)-1, NULL, NULL);
    }
...
*/
    ...
    if (VfIrpSendSynchronousIrp(PhysicalDeviceObject, &irpSp, FALSE, STATUS_NOT_SUPPORTED, 0,
                                (ULONG_PTR *)&targetDeviceRelationList, &status))
    {
    ...
    irpSp.Parameters.QueryInterface.Interface = &interface;
    irpSp.Parameters.QueryInterface.InterfaceSpecificData = (PVOID)-1;
    VfIrpSendSynchronousIrp(PhysicalDeviceObject, &irpSp, TRUE, STATUS_SUCCESS, 0, NULL, NULL);
                                          ...
                                          IN HOW_PROCESSED HowProcessed, IN PVOID CallerAddress)
{
    ...
    case IRP_MN_SURPRISE_REMOVAL:
        ...
        if ((HowProcessed != NOT_PROCESSED) ||
            (!VfSettingsIsOptionEnabled(VerifierSnapshot, VERIFIER_OPTION_EXTENDED_REQUIRED_IRPS)))
        {
    ...
    case IRP_MN_CANCEL_STOP_DEVICE:
        ...
        else if ((HowProcessed == DEFINITELY_PROCESSED) && (!NT_SUCCESS(Irp->IoStatus.Status)) &&
                 (VfSettingsIsOptionEnabled(VerifierSnapshot, VERIFIER_OPTION_EXTENDED_REQUIRED_IRPS)))
        {
        ...
        case TargetDeviceRelation:
            ...
            else
            {
                ...
                if (HowProcessed == NOT_PROCESSED)
                {
                    ...
                    WDM_FAIL_ROUTINE(
                        (DCERROR_PNP_IRP_NEEDS_PDO_HANDLING, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, Irp));
                }
                else if (NT_SUCCESS(Irp->IoStatus.Status))
                {
                    ...
                    if (Irp->IoStatus.Information == (ULONG_PTR)NULL)
                    {
                        ...
                        WDM_FAIL_ROUTINE(
                            (DCERROR_TARGET_RELATION_LIST_EMPTY, DCPARAM_IRP + DCPARAM_ROUTINE, CallerAddress, Irp));
                    }
        ...
        case EjectionRelations:
            ...
            for (index = 0; index < deviceRelations->Count; index++)
            {
                ...
                if (relationPdo != relationObject)
                {
                    ...
                    //
                    WDM_FAIL_ROUTINE((DCERROR_NON_PDO_RETURNED_IN_RELATION,
                                      DCPARAM_IRP + DCPARAM_ROUTINE + DCPARAM_DEVOBJ, CallerAddress, Irp,
                                      relationObject));
                }
