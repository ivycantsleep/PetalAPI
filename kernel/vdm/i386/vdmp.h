...
NTSTATUS
VdmpEnterIcaLock(IN PRTL_CRITICAL_SECTION pIcaLock);

NTSTATUS
VdmpLeaveIcaLock(IN PRTL_CRITICAL_SECTION pIcaLock);

BOOLEAN
VdmpDispatchableIntPending(IN ULONG EFlags);
...
BOOLEAN
VdmPrinterStatus(ULONG iPort, ULONG cbInstructionSize, PKTRAP_FRAME TrapFrame);
BOOLEAN
VdmPrinterWriteData(ULONG iPort, ULONG cbInstructionSize, PKTRAP_FRAME TrapFrame);
NTSTATUS
...
NTSTATUS
VdmpSetLdtEntries(ULONG Selector0, ULONG Entry0Low, ULONG Entry0Hi, ULONG Selector1, ULONG Entry1Low, ULONG Entry1Hi);

NTSTATUS
VdmpSetProcessLdtInfo(PPROCESS_LDT_INFORMATION ProcessInformation, ULONG ProcessInformationLength);

VOID VdmTraceEvent(USHORT Type, USHORT wData, USHORT lData, PKTRAP_FRAME TrapFrame);

NTSTATUS
VdmpPrinterInitialize(PVOID ServiceData);
