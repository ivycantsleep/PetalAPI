...
VOID NTFastDOSIO(PKTRAP_FRAME TrapFrame, ULONG IoType)
{
    ...
    if (CONSOLE_HANDLE(hFile) || hFile == (HANDLE)STD_INPUT_HANDLE || hFile == (HANDLE)STD_OUTPUT_HANDLE ||
    ...
    if (!(TrapFrame->EFlags & EFLAGS_ZF))
    {
        ...
        Status = NtSetInformationFile(hFile, IoStatusBlock, CurrentPosition, sizeof(FILE_POSITION_INFORMATION),
    ...
    if (IoType == SVC_DEMFASTREAD)
    {
        ...
        Status = NtReadFile(hFile, NULL, NULL, NULL, IoStatusBlock, (PVOID)lpBuf, CountToIO, NULL, NULL);
    }
    else
    {
        ...
        if (CountToIO == 0)
        {
            ...
            Status = NtQueryInformationFile(hFile, IoStatusBlock, CurrentPosition, sizeof(FILE_POSITION_INFORMATION),
                                            FilePositionInformation);
            ...
            Status = NtSetInformationFile(hFile, IoStatusBlock, EndOfFile, sizeof(FILE_END_OF_FILE_INFORMATION),
