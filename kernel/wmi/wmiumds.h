...
#define WmipReferenceEntry(Entry) InterlockedIncrement(&((PENTRYHEADER)(Entry))->RefCount)
...
#define VERIFY_DPCTXHANDLE(DsCtxHandle) \
    (((DsCtxHandle) == NULL) || (((PBDATASOURCE)(DsCtxHandle))->Signature == DS_SIGNATURE))
...
#define WmipUnreferenceDS(DataSource)                                                                                  \
    {                                                                                                                  \
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: %p.%p Unref DS %x (%x) at %s %d\n",              \
                          PsGetCurrentProcessId(), PsGetCurrentThreadId(), DataSource, DataSource->RefCount, __FILE__, \
                          __LINE__));                                                                                  \
...
#define WmipUnreferenceGE(GuidEntry)                                                                                 \
    {                                                                                                                \
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: %p.%p Unref GE %x (%x) at %s %d\n",            \
                          PsGetCurrentProcessId(), PsGetCurrentThreadId(), GuidEntry, GuidEntry->RefCount, __FILE__, \
                          __LINE__));                                                                                \
...
#define WmipUnreferenceIS(InstanceSet)                                                                         \
    {                                                                                                          \
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: %p.%p Unref IS %x (%x) at %s %d\n",      \
                          PsGetCurrentProcessId(), PsGetCurrentThreadId(), InstanceSet, InstanceSet->RefCount, \
                          __FILE__, __LINE__));                                                                \
...
#define WmipUnreferenceMR(MofResource)                                                                         \
    {                                                                                                          \
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: %p.%p Unref MR %x (%x) at %s %d\n",      \
                          PsGetCurrentProcessId(), PsGetCurrentThreadId(), MofResource, MofResource->RefCount, \
                          __FILE__, __LINE__));                                                                \
...
#else
#define WmipUnreferenceDS(DataSource) WmipUnreferenceEntry(&WmipDSChunkInfo, (PENTRYHEADER)DataSource)

#define WmipReferenceDS(DataSource) WmipReferenceEntry((PENTRYHEADER)DataSource)

#define WmipUnreferenceGE(GuidEntry) WmipUnreferenceEntry(&WmipGEChunkInfo, (PENTRYHEADER)GuidEntry)

#define WmipReferenceGE(GuidEntry) WmipReferenceEntry((PENTRYHEADER)GuidEntry)

#define WmipUnreferenceIS(InstanceSet) WmipUnreferenceEntry(&WmipISChunkInfo, (PENTRYHEADER)InstanceSet)

#define WmipReferenceIS(InstanceSet) WmipReferenceEntry((PENTRYHEADER)InstanceSet)

#define WmipUnreferenceDC(DataConsumer) WmipUnreferenceEntry(&WmipDCChunkInfo, (PENTRYHEADER)DataConsumer)

#define WmipReferenceDC(DataConsumer) WmipReferenceEntry((PENTRYHEADER)DataConsumer)

#define WmipUnreferenceMR(MofResource) WmipUnreferenceEntry(&WmipMRChunkInfo, (PENTRYHEADER)MofResource)

#define WmipReferenceMR(MofResource) WmipReferenceEntry((PENTRYHEADER)MofResource)
...
#define WmipAllocString(Size) WmipAlloc((Size) * sizeof(WCHAR))

#define WmipFreeString(Ptr) WmipFree(Ptr)
...
#define WmipAlloc(Size) ExAllocatePoolWithTag(PagedPool, Size, 'pimW')

#define WmipAllocWithTag(Size, Tag) ExAllocatePoolWithTag(PagedPool, Size, Tag)

#define WmipFree(Ptr) ExFreePool(Ptr)

#endif

#define WmipAllocNP(Size) ExAllocatePoolWithTag(NonPagedPool, Size, 'pimW')

#define WmipAllocNPWithTag(Size, Tag) ExAllocatePoolWithTag(NonPagedPool, Size, Tag)
