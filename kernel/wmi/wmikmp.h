...
_inline NTSTATUS WmipEnterCritSection(BOOLEAN AllowAPC)
{
    NTSTATUS status;

    do
    {
        status = KeWaitForMutexObject(&WmipSMMutex, Executive, KernelMode, AllowAPC, NULL);
...
{
    KeReleaseMutex(&WmipSMMutex, FALSE);
}
...
_inline NTSTATUS WmipEnterTraceCritSection(BOOLEAN AllowAPC)
{
    NTSTATUS status;

    do
    {
        status = KeWaitForMutexObject(&WmipTLMutex, Executive, KernelMode, AllowAPC, NULL);
    } while ((status == STATUS_ALERTED) || (status == STATUS_USER_APC));
...
{
    KeReleaseMutex(&WmipTLMutex, FALSE);
}
...
#define WmipValidateWnodeHeader(Wnode, BufSize, BufferSizeMin, RequiredFlags, ProhibitedFlags) \
    (((BufSize < BufferSizeMin) || ((Wnode->Flags & RequiredFlags) != RequiredFlags) ||        \
      (BufSize != Wnode->BufferSize) || ((Wnode->Flags & ProhibitedFlags) != 0))               \
         ? STATUS_UNSUCCESSFUL                                                                 \
         : STATUS_SUCCESS)

#define WmipIsAligned(Value, Alignment) ((((Value) + (Alignment - 1)) & ~(Alignment - 1)) == (Value))
...
typedef NTSTATUS (*PQUERY_WMI_REGINFO)(IN PDEVICE_OBJECT DeviceObject, OUT PULONG RegFlags, OUT PUNICODE_STRING Name,
...
typedef NTSTATUS (*PQUERY_WMI_DATABLOCK)(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG GuidIndex,
                                         IN ULONG InstanceIndex, IN ULONG InstanceCount,
                                         IN OUT PULONG InstanceLengthArray, IN ULONG BufferAvail, OUT PUCHAR Buffer);
/*++
...
typedef NTSTATUS (*PSET_WMI_DATABLOCK)(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG GuidIndex,
...
typedef NTSTATUS (*PSET_WMI_DATAITEM)(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG GuidIndex,
...
typedef NTSTATUS (*PEXECUTE_WMI_METHOD)(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG GuidIndex,
                                        IN ULONG InstanceIndex, IN ULONG MethodId, IN ULONG InBufferSize,
                                        IN ULONG OutBufferSize, IN OUT PUCHAR Buffer);
/*++
...
typedef NTSTATUS (*PWMI_FUNCTION_CONTROL)(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG GuidIndex,
...
NTSTATUS
IoWMIFireEvent(IN PWMILIB_INFO WmiLibInfo, IN PDEVICE_OBJECT DeviceObject, IN ULONG GuidIndex, IN ULONG EventDataSize,
               IN PVOID EventData);
...
NTSTATUS WmipOpenBlock(IN ULONG Ioctl, IN KPROCESSOR_MODE AccessMode, IN POBJECT_ATTRIBUTES ObjectAttributes,
...
NTSTATUS WmipQuerySingleMultiple(IN PIRP Irp, IN KPROCESSOR_MODE AccessMode, IN OUT PUCHAR BufferPtr,
                                 ...
                                 IN PWMIGUIDOBJECT *ObjectList, IN PUNICODE_STRING InstanceNames,
...
NTSTATUS WmipEnableDisableTrace(ULONG Ioctl, PWMITRACEENABLEDISABLEINFO TraceEnableInfo);
...
NTSTATUS
WmipDisableTraceProviders(ULONG StopLoggerId, PLIST_ENTRY GMHeadPtr);
...
#define WmipReferenceRegEntry(RegEntry)                                                                         \
    {                                                                                                           \
        ...
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: Ref RegEntry %p -> 0x%x in %s line %d\n", \
...
void WmipTranslatePDOInstanceNames(IN OUT PIRP Irp, IN UCHAR MinorFunction, IN ULONG BufferSize,
...
#define WmipUnreferenceRegEntry(RegEntry)                                                                         \
    {                                                                                                             \
        WmipDebugPrintEx((DPFLTR_WMICORE_ID, DPFLTR_INFO_LEVEL, "WMI: UnRef RegEntry %p -> 0x%x in %s line %d\n", \
                          (RegEntry), (RegEntry)->RefCount, __FILE__, __LINE__));                                 \
...
void WmipEventNotification(PVOID Context);

BOOLEAN WmipIsValidRegEntry(PREGENTRY CheckRegEntry);
...
NTSTATUS WmipProbeWmiOpenGuidBlock(POBJECT_ATTRIBUTES CapturedObjectAttributes, PUNICODE_STRING CapturedGuidString,
                                   PWCHAR CapturedGuidBuffer, PULONG DesiredAccess, PWMIOPENGUIDBLOCK InBlock,
...
NTSTATUS WmipOpenGuidObject(IN POBJECT_ATTRIBUTES ObjectAttributes, IN ACCESS_MASK DesiredAccess,
