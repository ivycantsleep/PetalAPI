...
NTSTATUS
WmipLookupLoggerIdByName(IN PUNICODE_STRING Name, OUT PULONG LoggerId);

PWMI_LOGGER_CONTEXT
WmipInitContext();

NTSTATUS
WmipAllocateTraceBufferPool(IN PWMI_LOGGER_CONTEXT LoggerContext);

NTSTATUS
WmipFreeTraceBufferPool(IN PWMI_LOGGER_CONTEXT LoggerContext);
...
NTSTATUS
WmipStartLogger(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo)
/*++
...

{
    ...
    if ((LogFileMode & EVENT_TRACE_FILE_MODE_SEQUENTIAL) && (LogFileMode & EVENT_TRACE_FILE_MODE_CIRCULAR))
    {
        return STATUS_INVALID_PARAMETER;
    }

    if ((LogFileMode & EVENT_TRACE_USE_GLOBAL_SEQUENCE) && (LogFileMode & EVENT_TRACE_USE_LOCAL_SEQUENCE))
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if ((LogFileMode & EVENT_TRACE_FILE_MODE_CIRCULAR) && (LogFileMode & EVENT_TRACE_FILE_MODE_APPEND))
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if ((LoggerInfo->LogFileHandle != NULL) || (LogFileMode & EVENT_TRACE_DELAY_OPEN_FILE_MODE))
    ...
    {
        if ((LoggerInfo->LogFileName.Buffer == NULL) || (LogFileMode & EVENT_TRACE_FILE_MODE_CIRCULAR) ||
    ...
    if (LoggerInfo->LoggerName.Length > 0)
    {
        ...
        try
        {
            ...
            {
                ProbeForRead(LoggerInfo->LoggerName.Buffer, LoggerInfo->LoggerName.Length, sizeof(UCHAR));
            }
            if (!RtlCreateUnicodeString(&LoggerName, LoggerInfo->LoggerName.Buffer))
            {
    ...
    { // other loggers requested
        for (LoggerId = 2; LoggerId < MAXLOGGERS; LoggerId++)
        {
            if (InterlockedCompareExchangePointer(&ContextTable[LoggerId], ContextTable, NULL) == NULL)
                break; // mark the slot as busy by putting in ServiceInfo
        }

        if (LoggerId >= MAXLOGGERS)
        { // could not find any more slots
    ...
    if (NT_SUCCESS(Status) && (!IsEqualGUID(&InstanceGuid, &EventTraceGuid)))
    {
        ...
        //
        Status = WmipCheckGuidAccess((LPGUID)&InstanceGuid, DesiredAccess);
    }
    ...
    { // minimum is 15 minutes
        LoggerContext->BufferAgeLimit.QuadPart = max(DEFAULT_AGE_LIMIT, LoggerInfo->AgeLimit) * OneSecond.QuadPart * 60;
    }
    ...
    if (EnableKernel)
    {
        ...
        //
        WmipFileTable = (PFILE_OBJECT *)WmipExtendBase(LoggerContext, MAX_FILE_TABLE_SIZE * sizeof(PVOID));

        Status = (WmipFileTable == NULL) ? STATUS_NO_MEMORY : STATUS_SUCCESS;
        if (NT_SUCCESS(Status))
        {
            if (!RtlCreateUnicodeString(&LoggerContext->LoggerName, KERNEL_LOGGER_CAPTION))
            {
                Status = STATUS_NO_MEMORY;
            }
        }
    ...
    if (LoggerInfo->MinimumBuffers >= 2 && LoggerInfo->MinimumBuffers <= LoggerContext->MaximumBuffers)
    {
    ...
    try
    {
        ...
        {
            ULONG SizeNeeded = sizeof(WNODE_HEADER) + sizeof(TRACE_LOGFILE_HEADER);
            if (RequestorMode != KernelMode)
            {
                ProbeForRead(LoggerInfo->Checksum, SizeNeeded, sizeof(UCHAR));
            }
            LoggerContext->LoggerHeader = ExAllocatePoolWithTag(PagedPool, SizeNeeded, TRACEPOOLTAG);
            if (LoggerContext->LoggerHeader != NULL)
            {
                RtlCopyMemory(LoggerContext->LoggerHeader, LoggerInfo->Checksum, SizeNeeded);
            }
        }
        if (LoggerContext->KernelTraceOn)
        {
            if (RequestorMode != KernelMode)
            {
                ProbeForWrite(LoggerInfo->LoggerName.Buffer, LoggerContext->LoggerName.Length + sizeof(WCHAR),
                              sizeof(UCHAR));
            }
            RtlCopyUnicodeString(&LoggerInfo->LoggerName, &LoggerContext->LoggerName);
        }
        if (LoggerInfo->LogFileName.Length > 0)
        {
            if (RequestorMode != KernelMode)
            ...
            if (!RtlCreateUnicodeString(&FileName, LoggerInfo->LogFileName.Buffer))
            {
                Status = STATUS_NO_MEMORY;
            }
        }
    ...
    if (LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_NEWFILE)
    {
        ...
        Status = WmipGenerateFileName(&LoggerContext->LogFilePattern, &LoggerContext->FileCounter,
    ...
    LoggerInfo->AgeLimit = (ULONG)(LoggerContext->BufferAgeLimit.QuadPart / OneSecond.QuadPart / 60);
    LoggerInfo->BufferSize = LoggerContext->BufferSize / 1024;

    WmiSetLoggerId(LoggerId, (PTRACE_ENABLE_CONTEXT)&LoggerInfo->Wnode.HistoricalContext);
    ...
    KeInitializeEvent(&LoggerContext->LoggerEvent, NotificationEvent, FALSE);
    KeInitializeEvent(&LoggerContext->FlushEvent, NotificationEvent, FALSE);
    ...
    if (NT_SUCCESS(Status))
    {
        Status = PsCreateSystemThread(&ThreadHandle, THREAD_ALL_ACCESS, NULL, NULL, NULL, WmipLogger, LoggerContext);
    }

    if (NT_SUCCESS(Status))
    { // if SystemThread is started
        ZwClose(ThreadHandle);

        // Wait for Logger to start up properly before proceeding
        //
        KeWaitForSingleObject(&LoggerContext->LoggerEvent, Executive, KernelMode, FALSE, &TimeOut);
        ...
        if (NT_SUCCESS(LoggerContext->LoggerStatus))
        {
            ...
            while (pNext != pHead)
            {
                pBuffer = (PWMI_BUFFER_HEADER)CONTAINING_RECORD(pNext, WMI_BUFFER_HEADER, Entry);
            ...
            {
                LoggerInfo->LoggerThreadId = LoggerContext->LoggerThread->Cid.UniqueThread;
            }
            ...
            if (IsGlobalForKernel)
            {
                Status = PerfInfoStartLog(PerfGroupMasks, PERFINFO_START_LOG_FROM_GLOBAL_LOGGER);
            }
...
NTSTATUS
WmipQueryLogger(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo, IN PWMI_LOGGER_CONTEXT LoggerContext)
/*++
...

{
...
#if DBG
        Status = WmipVerifyLoggerInfo(LoggerInfo, &LoggerContext, "WmipQueryLogger");
#else
...
#else
    LoggerInfo->FlushTimer = (ULONG)(LoggerContext->FlushTimer.QuadPart / OneSecond.QuadPart);
#endif //WMI_NON_BLOCKING
    ...
    try
    {
        if (LoggerContext->LogFileName.Length > 0 && LoggerInfo->LogFileName.MaximumLength > 0)
        {
            if (RequestorMode != KernelMode)
            {
                ProbeForWrite(LoggerInfo->LogFileName.Buffer, LoggerContext->LogFileName.Length + sizeof(WCHAR),
                              sizeof(UCHAR));
            }
            RtlCopyUnicodeString(&LoggerInfo->LogFileName, &LoggerContext->LogFileName);
        }
        if (LoggerContext->LoggerName.Length > 0 && LoggerInfo->LoggerName.MaximumLength > 0)
        {
            if (RequestorMode != KernelMode)
            {
                ProbeForWrite(LoggerInfo->LoggerName.Buffer, LoggerContext->LoggerName.Length + sizeof(WCHAR),
...
NTSTATUS
WmipStopLoggerInstance(IN PWMI_LOGGER_CONTEXT LoggerContext)
{
    LONG LoggerOn;
    ...
    if (LoggerContext->KernelTraceOn)
    {
        ...
        LONG PerfLogInTransition = InterlockedCompareExchange(&LoggerContext->PerfLogInTransition,
                                                              PERF_LOG_STOP_TRANSITION, PERF_LOG_NO_TRANSITION);
    ...
    {
        WmipDisableKernelTrace(EVENT_TRACE_FLAG_PROCESS | EVENT_TRACE_FLAG_THREAD | EVENT_TRACE_FLAG_IMAGE_LOAD);
    ...
    //
    WmipLoggerContext[LoggerContext->LoggerId] = (PWMI_LOGGER_CONTEXT)&WmipLoggerContext[0];
...
NTSTATUS
WmipVerifyLoggerInfo(IN PWMI_LOGGER_INFORMATION LoggerInfo,
#if DBG
                     OUT PWMI_LOGGER_CONTEXT *pLoggerContext, IN LPSTR Caller
#else
    ...
    *pLoggerContext = NULL;
    ...
    try
    {
        if (LoggerInfo->LoggerName.Length > 0)
        {
            ...
            RtlCreateUnicodeString(&LoggerName, LoggerInfo->LoggerName.Buffer);
        }
    }
    except(EXCEPTION_EXECUTE_HANDLER)
    {
...
PVOID
WmipExtendBase(IN PWMI_LOGGER_CONTEXT Base, IN ULONG Size)
{
    //
...
VOID WmipFreeLoggerContext(IN PWMI_LOGGER_CONTEXT LoggerContext)
{
    ULONG LoggerId;
    ULONG i;
    LONG RefCount;
    ...
    {
        count++;
        Status = KeWaitForSingleObject(&LoggerContext->LoggerEvent, Executive, KernelMode, FALSE, &Timeout);
        KeResetEvent(&LoggerContext->LoggerEvent);
...
#if DBG
    TraceDebug(
        (2, "WmipFreeLoggerContext: Freeing pool %X %d %d->%d\n", LoggerContext, LoggerId, RefCount + 1, RefCount));
    if (LoggerContext->CollectionOn)
    {
        TraceDebug((1, "WmipFreeLoggerContext: %X %d still active\n", LoggerContext, LoggerId));
        //        DbgBreakPoint();
    }
#ifndef WMI_MUTEX_FREE
    if (LoggerContext->MutexCount >= 1)
...

{
    ...
    LoggerContext = (PWMI_LOGGER_CONTEXT)ExAllocatePoolWithTag(NonPagedPool, CONTEXT_SIZE, TRACEPOOLTAG);
    ...
    LoggerContext->EndPageMarker =
        (PUCHAR)LoggerContext + ALIGN_TO_POWER2(sizeof(WMI_LOGGER_CONTEXT), DEFAULT_TRACE_ALIGNMENT);
    ...
    LoggerContext->MaximumBuffers = LoggerContext->MinimumBuffers + DEFAULT_BUFFERS + 20;
...

{
    ...
    Max_Buffers = (ULONG)(MmMaximumNonPagedPoolInBytes / TRACE_MAXIMUM_NP_POOL_USAGE / LoggerContext->BufferSize);
    ...
    {
        Max_Buffers = max(LoggerContext->MaximumBuffers, NumberProcessors + DEFAULT_BUFFERS + 20);
    }

    LoggerContext->MinimumBuffers = max(LoggerContext->MinimumBuffers, NumberProcessors + DEFAULT_BUFFERS);
...
//
    ...
    AllocatedBuffers = WmipAllocateFreeBuffers(LoggerContext, NumberOfBuffers);
    ...
    LoggerContext->ProcessorBuffers =
        (SLIST_HEADER *)WmipExtendBase(LoggerContext, sizeof(SLIST_HEADER) * NumberProcessors);
    ...
    {
        Buffer = (PWMI_BUFFER_HEADER)ExAllocatePoolWithTag(LoggerContext->PoolType, BufferSize, TRACEPOOLTAG);

        if (Buffer == NULL)
        { // need to free previously allocated buffers
            WmipFreeTraceBufferPool(LoggerContext);
            return STATUS_NO_MEMORY;
        }
        //
        ...
        TraceDebug(
            (2, "WmipAllocateTraceBuffer: %d Allocated %X Entry %X\n", LoggerContext->LoggerId, Buffer, Buffer->Entry));
    }
    ...
    LoggerContext->ProcessorBuffers =
        (PWMI_BUFFER_HEADER *)WmipExtendBase(LoggerContext, sizeof(PWMI_BUFFER_HEADER) * NumberProcessors);
    ...
    for (i = 0; i < (LONG)NumberProcessors; i++)
    {
        ...
        InterlockedPushEntrySList(&LoggerContext->ProcessorBuffers[i], (PSINGLE_LIST_ENTRY)&Buffer->SlistEntry);
...
WmipFreeTraceBufferPool(IN PWMI_LOGGER_CONTEXT LoggerContext)
{
    ...
    if (ProcessorBuffers != NULL)
    {
        for (i = 0; i < (ULONG)KeNumberProcessors; i++)
        {
            while (Entry = InterlockedPopEntrySList(&ProcessorBuffers[i]))
            {
                ...
                TraceDebug((2,
                            "WmipFreeTraceBufferPool (CPU %2d): %2d, %p, Free: %d, InUse: %d, Dirty: %d, Total: %d\n",
                            i, LoggerContext->LoggerId, Buffer, LoggerContext->BuffersAvailable,
...
#else
    while (Entry = ExInterlockedRemoveHeadList(&LoggerContext->FreeList, &LoggerContext->BufferSpinLock))
    {
        PWMI_BUFFER_HEADER Buffer;
    ...
    for (i = 0; i < (ULONG)LoggerContext->NumberOfBuffers; i++)
    {
        ...
        TraceDebug((1, "WmipFreeTraceBufferPool: Move entry %X from flush to free\n", Entry));
    }
    ...
    if (Buffers != NULL)
    {
        for (i = 0; i < (ULONG)KeNumberProcessors; i++)
        {
            if (Buffers[i] != NULL)
            {
                TraceDebug((1, "WmipFreeTraceBufferPool: Freeing buffer %X for CPU%d\n", Buffers[i], i));
...
WmipLookupLoggerIdByName(IN PUNICODE_STRING Name, OUT PULONG LoggerId)
{
    ...
    {
        if (ContextTable[i] == NULL || ContextTable[i] == (PWMI_LOGGER_CONTEXT)ContextTable)
            continue;
...
NTSTATUS
WmipShutdown(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
//
...
//
{
    ...
    if (WmipLoggerCount > 0)
    {
        ...
        for (i = 0; i < MAXLOGGERS; i++)
        {
            ...
            if ((LoggerContext != NULL) && (LoggerContext != (PWMI_LOGGER_CONTEXT)&WmipLoggerContext[0]))
            {
...
#if DBG
            else if (LoggerContext == (PWMI_LOGGER_CONTEXT)&WmipLoggerContext[0])
            {
...
NTSTATUS
WmipFlushLogger(IN OUT PWMI_LOGGER_CONTEXT LoggerContext, IN ULONG Wait)
{
    LARGE_INTEGER TimeOut = { (ULONG)(-20 * 1000 * 1000 * 10), -1 };
    NTSTATUS Status;
...
NTSTATUS
FASTCALL
WmipNotifyLogger(IN PWMI_LOGGER_CONTEXT LoggerContext)
...
PVOID
WmipGetTraceBuffer(IN PWMI_LOGGER_CONTEXT LoggerContext, IN HANDLE LogFileHandle, IN PWMI_BUFFER_HEADER Buffer,
                   IN ULONG GroupType, IN ULONG RequiredSize, OUT PULONG GuidMapBuffers)
{
    ...
    if (RequiredSize > (LoggerContext->BufferSize - Buffer->Offset))
    {
        ...
        {
            RtlFillMemory((char *)Buffer + Buffer->Offset, LoggerContext->BufferSize - Buffer->Offset, 0xFF);
        }

        Status = ZwWriteFile(LogFileHandle, NULL, NULL, NULL, &IoStatus, Buffer, LoggerContext->BufferSize,
...
ULONG
WmipDumpGuidMaps(IN PWMI_LOGGER_CONTEXT LoggerContext, IN PLIST_ENTRY TraceGMHeadPtr)
{
    PWMI_BUFFER_HEADER Buffer;
    ...
    if ((LoggerContext->LoggerMode & EVENT_TRACE_REAL_TIME_MODE) &&
        ((LoggerContext->LogFileName.Buffer == NULL) || (LoggerContext->LogFileName.Length == 0)))
    {
    ...
    Buffer = ExAllocatePoolWithTag(PagedPool, BufferSize, TRACEPOOLTAG);
    if (Buffer == NULL)
    {
    ...
    Status = WmipCreateNtFileName(LoggerContext->LogFileName.Buffer, &LogFileName);
    ...
    Status = WmipCreateDirectoryFile(LogFileName, FALSE, &LogFileHandle, TRUE);

    if (NT_SUCCESS(Status))
    {
        PULONG AuxInfo;
        if ((LoggerContext->LoggerMode & EVENT_TRACE_FILE_MODE_PREALLOCATE) &&
            (LoggerContext->MaximumFileSize >
             (((LONGLONG)LoggerContext->BuffersWritten * (LONGLONG)LoggerContext->BufferSize) / (1024 * 1024))))
        {
            LoggerContext->ByteOffset.QuadPart =
                ((LONGLONG)LoggerContext->BufferSize) * ((LONGLONG)LoggerContext->BuffersWritten);
        }
        else
        {
            FILE_STANDARD_INFORMATION FileSize;

            Status = ZwQueryInformationFile(LogFileHandle, &IoStatus, &FileSize, sizeof(FILE_STANDARD_INFORMATION),
        ...
        while (GuidMapList != TraceGMHeadPtr)
        {
            ...
            AuxInfo = (PULONG)WmipGetTraceBuffer(LoggerContext, LogFileHandle, Buffer,
                                                 EVENT_TRACE_GROUP_HEADER + EVENT_TRACE_TYPE_GUIDMAP,
                                                 sizeof(TRACEGUIDMAP), &GuidMapBuffers);
...
NTSTATUS
WmipNtDllLoggerInfo(IN OUT PWMINTDLLLOGGERINFO Buffer)
{
...
VOID WmipValidateClockType(IN OUT PWMI_LOGGER_INFORMATION LoggerInfo)
/*++
