...
NTSTATUS
WmipOpenRegistryKeyEx(OUT PHANDLE Handle, IN HANDLE BaseHandle OPTIONAL, IN PUNICODE_STRING KeyName,
...
NTSTATUS
WmipCreateRegistryKeyEx(OUT PHANDLE Handle, IN HANDLE BaseHandle OPTIONAL, IN PUNICODE_STRING KeyName,
...
NTSTATUS WmipReadValueKey(IN HANDLE Key, IN PUNICODE_STRING ValueName, IN ULONG ValueType,
                          OUT PKEY_VALUE_PARTIAL_INFORMATION *PartialInfoPtr, OUT PULONG InfoSizePtr)
{
    ...
    Status = ZwQueryValueKey(Key, ValueName, KeyValuePartialInformation, &PartialInfo, InfoSize, &InfoSize);
    ...
    else
    {
        ...
        //
        Buffer = ExAllocatePoolWithTag(PagedPool, InfoSize, WMIPSCHEDPOOLTAG);

        if (Buffer != NULL)
        {
            Status = ZwQueryValueKey(Key, ValueName, KeyValuePartialInformation, Buffer, InfoSize, &InfoSize);
...
NTSTATUS WmipOpenDiagRegKey(IN PDEVICE_OBJECT DeviceObject, IN PUNICODE_STRING SubKey, IN ULONG DesiredAccess,
                            IN BOOLEAN CreateIfNeeded, OUT PHANDLE Key)
{
    HANDLE BaseKey;
    NTSTATUS Status;
    ULONG Disposition;
    ...
    if (NT_SUCCESS(Status))
    {
        Status = IoOpenDeviceRegistryKey(PDO, PLUGPLAY_REGKEY_DEVICE, DesiredAccess, &BaseKey);
        ...
        if (NT_SUCCESS(Status))
        {
            if (SubKey != NULL)
            {
                if (CreateIfNeeded)
                {
                    Status = WmipCreateRegistryKeyEx(Key, BaseKey, SubKey, DesiredAccess, REG_OPTION_NON_VOLATILE,
                                                     &Disposition);
                }
                else
                {
                    Status = WmipOpenRegistryKeyEx(Key, BaseKey, SubKey, DesiredAccess);
                }
                ZwClose(BaseKey);
            }
            else
...
BOOLEAN WmipDoesSigMatchDiag(IN PSCHEDULEDDIAG Diag, IN UCHAR IrpMn, IN LPGUID Guid, IN ULONG InstanceContextSize,
                             IN PUCHAR InstanceContext, IN ULONG InstanceIndex, IN ULONG MethodId)
/*++
...
--*/
{
    ...
    if ((Diag->IsValid) && (Diag->IrpMn == IrpMn) && (IsEqualGUID(&Diag->Guid, Guid)) &&
        ...
        ((IrpMn != IRP_MN_EXECUTE_METHOD) || (Diag->MethodId == MethodId)))
    {
        ...
        //
        if ((InstanceContext == NULL) && (Diag->InstanceContextOffset == 0))
        {
        ...
        {
            DiagInstanceContext = OffsetToPtr(Diag, Diag->InstanceContextOffset);

            if (RtlCompareMemory(DiagInstanceContext, InstanceContext, InstanceContextSize) == InstanceContextSize)
            {
                //
...
PSCHEDULEDDIAG WmipFindDiagInBuffer(IN PUCHAR DiagList, IN ULONG DiagBufferSize, IN UCHAR IrpMn, IN LPGUID Guid,
                                    IN ULONG InstanceContextSize, IN PUCHAR InstanceContext, IN ULONG InstanceIndex,
                                    IN ULONG MethodId)
/*++
...
--*/
{
    ...
    while (Offset < DiagBufferSize)
    {
        ...
        if (WmipDoesSigMatchDiag(Diag, IrpMn, Guid, InstanceContextSize, InstanceContext, InstanceIndex, MethodId))
        {
            //
...
NTSTATUS WmipUpdateOrAppendDiag(IN PDEVICE_OBJECT DeviceObject, IN PUNICODE_STRING DiagType, IN PUNICODE_STRING DiagSet,
                                IN UCHAR IrpMn, IN LPGUID Guid, IN ULONG InstanceContextSize, IN PUCHAR InstanceContext,
                                IN ULONG InstanceIndex, IN ULONG MethodId, IN ULONG DataSize, IN PUCHAR Data)
/*++
...
--*/
{
    ...
    //
    Status = WmipOpenDiagRegKey(DeviceObject, DiagType, KEY_WRITE | KEY_READ, TRUE, &Key);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        //
        InfoSize = sizeof(KEY_VALUE_PARTIAL_INFORMATION);
        Status = ZwQueryValueKey(Key, DiagSet, KeyValuePartialInformation, &PartialInfo, InfoSize, &InfoSize);
        if (((Status != STATUS_BUFFER_OVERFLOW) && (!NT_SUCCESS(Status))) || (PartialInfo.Type != REG_BINARY))
        {
            //
        ...
        DiagBuffer = ExAllocatePoolWithTag(PagedPool, SizeNeeded, WMIPSCHEDPOOLTAG);
        ...
        if (DiagBuffer != NULL)
        {
            ...
            if (InfoSize != 0)
            {
                Status = ZwQueryValueKey(Key, DiagSet, KeyValuePartialInformation, DiagBuffer, InfoSize, &InfoSize);
                ...
                if (NT_SUCCESS(Status))
                {
                    ...
                    // diag signature
                    //
                    Diag = WmipFindDiagInBuffer(RegDataPtr, DiagPartialInfo->DataLength, IrpMn, Guid,
                                                InstanceContextSize, InstanceContext, InstanceIndex, MethodId);
            ...
            if (NT_SUCCESS(Status))
            {
                ...
                //
                Status = ZwSetValueKey(Key, DiagSet, 0, REG_BINARY, RegDataPtr, RegDataSize);
            }
...
NTSTATUS IoWMIScheduleDiagnostic(IN PDEVICE_OBJECT DeviceObject, IN PUNICODE_STRING DiagSet, IN UCHAR IrpMn,
                                 IN LPGUID Guid, IN ULONG InstanceContextSize, IN PUCHAR InstanceContext,
                                 IN ULONG InstanceIndex, IN ULONG MethodId, IN ULONG DataSize, IN PUCHAR Data)
{
    NTSTATUS Status;
    UNICODE_STRING Scheduled;
    ...
    Status = WmipUpdateOrAppendDiag(DeviceObject, &Scheduled, DiagSet, IrpMn, Guid, InstanceContextSize,
                                    InstanceContext, InstanceIndex, MethodId, DataSize, Data);

    return (Status);
}

NTSTATUS IoWMICancelDiagnostic(IN PDEVICE_OBJECT DeviceObject, IN PUNICODE_STRING DiagSet, IN UCHAR IrpMn,
                               IN LPGUID Guid, IN ULONG InstanceContextSize, IN PUCHAR InstanceContext,
                               IN ULONG InstanceIndex, IN ULONG MethodId)
{
    ...
    Status = WmipOpenDiagRegKey(DeviceObject, &Scheduled, KEY_WRITE | KEY_READ, TRUE, &Key);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (((Status == STATUS_BUFFER_OVERFLOW) || NT_SUCCESS(Status)) && (PartialInfo.Type == REG_BINARY))
        {
            ...
            //
            DiagBuffer = ExAllocatePoolWithTag(PagedPool, InfoSize, WMIPSCHEDPOOLTAG);
            ...
            if (DiagBuffer != NULL)
            {
                ...
                if (NT_SUCCESS(Status))
                {
                    ...
                    // diag signature
                    //
                    Diag = WmipFindDiagInBuffer(DiagList, DiagListSize, IrpMn, Guid, InstanceContextSize,
                                                InstanceContext, InstanceIndex, MethodId);
                    ...
                    if (Diag != NULL)
                    {
                        ...
                        // Write diag buffer back to registry
                        //
                        Status = ZwSetValueKey(Key, DiagSet, 0, REG_BINARY, DiagList, DiagListSize);
                    }
                    else
                    {
...
NTSTATUS WmipSendMethodDiagRequest(PDEVICE_OBJECT DeviceObject, PSCHEDULEDDIAG Diag, PWNODE_METHOD_ITEM *WnodeMethodPtr)
{
    ...
    SizeNeeded = sizeof(WNODE_METHOD_ITEM) + Diag->InstanceContextSize +
                 ((Diag->OutDataSize > Diag->DataSize) ? Diag->OutDataSize : Diag->DataSize);


    Looping = TRUE;
    while (Looping)
    {
        WnodeMethod = ExAllocatePoolWithTag(NonPagedPool, SizeNeeded, WMIPSCHEDPOOLTAG);
        ...
        if (WnodeMethod != NULL)
        {
            ...
            InstanceOffset = FIELD_OFFSET(WNODE_SINGLE_INSTANCE, VariableData);
            DataOffset = (InstanceOffset + Diag->InstanceContextSize + sizeof(USHORT) + 7) & ~7;
            ...
            WnodeMethod->WnodeHeader.Flags = WNODE_FLAG_METHOD_ITEM | WNODE_FLAG_DIAG_REQUEST;
            ...
            Status = WmipSendWmiIrp(IRP_MN_EXECUTE_METHOD, ProviderId, &WnodeMethod->WnodeHeader.Guid, SizeNeeded,
                                    WnodeMethod, &Iosb);
...
NTSTATUS WmipSendQSIDiagRequest(PDEVICE_OBJECT DeviceObject, PSCHEDULEDDIAG Diag, PWNODE_SINGLE_INSTANCE *WnodeSIPtr)
{
    ...
    SizeNeeded = sizeof(WNODE_SINGLE_INSTANCE) + Diag->InstanceContextSize + (Diag->OutDataSize > Diag->DataSize)
                     ? Diag->OutDataSize
                     : Diag->DataSize;
    Looping = TRUE;
    while (Looping)
    {
        WnodeSI = ExAllocatePoolWithTag(NonPagedPool, SizeNeeded, WMIPSCHEDPOOLTAG);
        ...
        if (WnodeSI != NULL)
        {
            ...
            WnodeSI->WnodeHeader.Flags = WNODE_FLAG_SINGLE_INSTANCE | WNODE_FLAG_DIAG_REQUEST;
            ...
            Status = WmipSendWmiIrp(IRP_MN_QUERY_SINGLE_INSTANCE, ProviderId, &WnodeSI->WnodeHeader.Guid, SizeNeeded,
                                    WnodeSI, &Iosb);
...
NTSTATUS IoWMIStartScheduledDiagnostics(IN PDEVICE_OBJECT DeviceObject, IN PUNICODE_STRING DiagSet)
{
    ...
    Status = WmipOpenDiagRegKey(DeviceObject, &Scheduled, KEY_READ, FALSE, &Key);

    if (NT_SUCCESS(Status))
    {
        Status = WmipReadValueKey(Key, DiagSet, REG_BINARY, &PartialInfo, &InfoSize);
            ...
            while (Offset < DiagSize)
            {
                ...
                if (Diag->IsValid)
                {
                    ...
                    case IRP_MN_QUERY_SINGLE_INSTANCE:
                    {
                        ...
                        Status2 = WmipSendQSIDiagRequest(DeviceObject, Diag, &WnodeSI);
                        if (NT_SUCCESS(Status2))
                        {
                            if (Diag->InstanceContextOffset != 0)
                            {
                                InstanceContext = OffsetToPtr(Diag, Diag->InstanceContextOffset);
                            }
                            else
                            {
                            ...
                            Data = OffsetToPtr(WnodeSI, WnodeSI->DataBlockOffset);
                            Status2 = WmipUpdateOrAppendDiag(
                                DeviceObject, &Checkpoint, DiagSet, Diag->IrpMn, &Diag->Guid, Diag->InstanceContextSize,
                                InstanceContext, Diag->InstanceIndex, Diag->MethodId, WnodeSI->SizeDataBlock, Data);
                    ...
                    case IRP_MN_EXECUTE_METHOD:
                    {
                        ...
                        Status2 = WmipSendMethodDiagRequest(DeviceObject, Diag, &WnodeMethod);
                        if (NT_SUCCESS(Status2))
                        {
                            if (Diag->InstanceContextOffset != 0)
                            {
                            ...
                            Data = OffsetToPtr(WnodeMethod, WnodeMethod->DataBlockOffset);

                            Status2 = WmipUpdateOrAppendDiag(
                                DeviceObject, &Checkpoint, DiagSet, Diag->IrpMn, &Diag->Guid, Diag->InstanceContextSize,
                                InstanceContext, Diag->InstanceIndex, Diag->MethodId, WnodeMethod->SizeDataBlock, Data);
...
NTSTATUS IoWMIGetDiagnosticResult(IN PDEVICE_OBJECT DeviceObject, IN PUNICODE_STRING DiagSet, IN UCHAR IrpMn,
                                  IN LPGUID Guid, IN ULONG InstanceContextSize, IN PUCHAR InstanceContext,
                                  IN ULONG InstanceIndex, IN ULONG MethodId, IN OUT ULONG *DataSize, OUT PUCHAR Data)
{
    NTSTATUS Status;
    UNICODE_STRING Checkpoint;
    HANDLE Key;
    ...
    RtlInitUnicodeString(&Checkpoint, REGSTR_CHECKPOINT);
    Status = WmipOpenDiagRegKey(DeviceObject, &Checkpoint, KEY_READ, FALSE, &Key);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            DiagSize = PartialInfo->DataLength;
            Diag = WmipFindDiagInBuffer(DiagList, DiagSize, IrpMn, Guid, InstanceContextSize, InstanceContext,
                                        InstanceIndex, MethodId);
            if (Diag != NULL)
            {
...
NTSTATUS IoWMISaveDiagnosticResult(IN PDEVICE_OBJECT DeviceObject, IN PUNICODE_STRING DiagSet, IN UCHAR IrpMn,
                                   IN LPGUID Guid, IN ULONG InstanceContextSize, IN PUCHAR InstanceContext,
                                   IN ULONG InstanceIndex, IN ULONG MethodId, IN ULONG DataSize, IN PUCHAR Data)
{
    NTSTATUS Status;
    ...
    //
    RtlInitUnicodeString(&Checkpoint, REGSTR_CHECKPOINT);
    Status = WmipUpdateOrAppendDiag(DeviceObject, &Checkpoint, DiagSet, IrpMn, Guid, InstanceContextSize,
                                    InstanceContext, InstanceIndex, MethodId, DataSize, Data);
