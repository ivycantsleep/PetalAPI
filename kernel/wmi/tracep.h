...
//
#define WmipGetLoggerContext(LoggerId) (LoggerId < MAXLOGGERS) ? WmipLoggerContext[LoggerId] : NULL

#define WmipIsValidLogger(L) (((L) != NULL) && ((L) != (PWMI_LOGGER_CONTEXT) & WmipLoggerContext[0]) ? TRUE : FALSE)

#define WmipInitializeMutex(x) KeInitializeMutex((x), 0)
#define WmipAcquireMutex(x) KeWaitForSingleObject((x), Executive, KernelMode, FALSE, NULL)
...
//
__inline __int64 WmipGetSystemTime(VOID)
{
    LARGE_INTEGER Time;
...
__inline __int64 WmipGetPerfCounter(VOID)
{
    LARGE_INTEGER Time;
...
#if DBG
NTSTATUS
WmipVerifyLoggerInfo(IN PWMI_LOGGER_INFORMATION LoggerInfo, OUT PWMI_LOGGER_CONTEXT *LoggerContext, LPSTR Caller);
#else
NTSTATUS
WmipVerifyLoggerInfo(IN PWMI_LOGGER_INFORMATION LoggerInfo, OUT PWMI_LOGGER_CONTEXT *LoggerContext);
#endif
...
NTSTATUS
WmipFlushActiveBuffers(IN PWMI_LOGGER_CONTEXT, IN ULONG FlushAll);

PSYSTEM_TRACE_HEADER
FASTCALL
WmiReserveWithSystemHeader(IN ULONG LoggerId, IN ULONG AuxSize, IN PETHREAD Thread, OUT PVOID *BufferResource);
...
ULONG
FASTCALL
WmipReleaseTraceBuffer(IN PWMI_BUFFER_HEADER Buffer, IN PWMI_LOGGER_CONTEXT LoggerContext);
...
NTSTATUS
WmipDelayCreate(OUT PHANDLE FileHandle, IN OUT PUNICODE_STRING FileName, IN ULONG Append);
...
NTSTATUS
WmipQueryGLRegistryRoutine(IN PWSTR ValueName, IN ULONG ValueType, IN PVOID ValueData, IN ULONG ValueLength,
                           IN PVOID Context, IN PVOID EntryContext);

NTSTATUS
WmipAddLogHeader(IN PWMI_LOGGER_CONTEXT LoggerContext, IN OUT PWMI_BUFFER_HEADER Buffer);

NTSTATUS
WmipCreateDirectoryFile(IN PWCHAR DirFileName, IN ULONG IsDirectory, OUT PHANDLE FileHandle, ULONG Append);

NTSTATUS
WmipCreateNtFileName(IN PWCHAR strFileName, OUT PWCHAR *strNtFileName);
...
#ifdef NTPERF
NTSTATUS
WmipSwitchPerfmemBuffer(PWMI_SWITCH_PERFMEM_BUFFER_INFORMATION SwitchBufferInfo);
#endif //NTPERF

NTSTATUS
WmipNtDllLoggerInfo(PWMINTDLLLOGGERINFO Buffer);
