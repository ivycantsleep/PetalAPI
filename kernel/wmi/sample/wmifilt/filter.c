...
--*/
{
    ...
    FilterRegistryPath.Buffer = ExAllocatePoolWithTag(PagedPool, RegistryPath->Length + 2, FILTER_TAG);
...
NTSTATUS VA_AddDevice(IN PDRIVER_OBJECT driverObj, IN PDEVICE_OBJECT physicalDevObj)
/*++
...

{
    ...
    status = IoCreateDevice(driverObj, sizeof(struct DEVICE_EXTENSION),
...

{
    ...
    DBGOUT(("VA_Dispatch: majorFunc=%d, minorFunc=%d", (ULONG)majorFunc, (ULONG)minorFunc));
