...
NTSTATUS
ThreadThatExits(IN PVOID ThreadParameter)
{
...
NTSTATUS
ThreadThatExcepts(IN PVOID ThreadParameter)
{
...
NTSTATUS
ThreadThatSpins(IN PVOID ThreadParameter)
{
    for (;;)
        ;
...
UdbgTest1()
{
    ...
    st = RtlCreateUserThread(DebugProcess, NULL, TRUE, 0L, 0L, 0L, ThreadThatExits, (PVOID)STATUS_ABANDONED,
                             &ExitThread, &ExitClientId);
    ASSERT(NT_SUCCESS(st));
...
UdbgTest2()
{
    ...
    st = RtlCreateUserThread(DebugProcess, NULL, TRUE, 0L, 0L, 0L, ThreadThatExcepts, (PVOID)STATUS_ABANDONED,
