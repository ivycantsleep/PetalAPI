...
VOID ACPIWakeCompleteRequestQueue(IN PLIST_ENTRY RequestList, IN NTSTATUS Status)
/*++
...
--*/
{
    ...
    while (listEntry != RequestList)
    {
        ...
        //
        ACPIDevPrint((ACPI_PRINT_WAKE, powerRequest->DeviceExtension,
...
NTSTATUS
ACPIWakeDisableAsync(IN PDEVICE_EXTENSION DeviceExtension, IN PLIST_ENTRY RequestList, IN PFNACB CallBack,
                     IN PVOID Context)
/*++
...
--*/
{
    ...
    //
    ACPIDevPrint((ACPI_PRINT_WAKE, DeviceExtension, "ACPIWakeDisableAsync - ReferenceCount: %lx - %lx = %lx\n",
    ...
    //
    InsertTailList(&(DeviceExtension->PowerInfo.WakeSupportList), &(pswContext->ListEntry));
    ...
    if (runPsw)
    {
        ...
        //
        status = AMLIAsyncEvalObject(pswObject, NULL, 1, &pswData, ACPIWakeEnableDisableAsyncCallBack, pswContext);

        //
        // What Happened
        //
        ACPIDevPrint((ACPI_PRINT_WAKE, DeviceExtension, "ACPIWakeDisableAsync = 0x%08lx (P)\n", status));


        if (status != STATUS_PENDING)
        {

            ACPIWakeEnableDisableAsyncCallBack(pswObject, status, NULL, pswContext);
        }
    ...
    else
    {
        ...
        ACPIDevPrint((ACPI_PRINT_WAKE, DeviceExtension, "ACPIWakeEnableDisableAsync = 0x%08lx (Q)\n", STATUS_PENDING));
...
ACPIWakeDisableAsyncExit:
    ...
    // What happened
    //
    ACPIDevPrint((ACPI_PRINT_WAKE, DeviceExtension, "ACPIWakeDisableAsync = 0x%08lx\n", status));

    //
    // Call the specified callback ourselves
    //
    (*CallBack)(pswObject, status, NULL, Context);
    return STATUS_PENDING;
}

NTSTATUS
ACPIWakeEmptyRequestQueue(IN PDEVICE_EXTENSION DeviceExtension)
/*++
...
NTSTATUS
ACPIWakeEnableDisableAsync(IN PDEVICE_EXTENSION DeviceExtension, IN BOOLEAN Enable, IN PFNACB CallBack,
                           IN PVOID Context)
/*++
...
--*/
{
    ...
    if (Enable)
    {
        ...
        ACPIDevPrint((ACPI_PRINT_WAKE, DeviceExtension, "ACPIWakeEnableDisableAsync - Count: %d (+)\n",
                      DeviceExtension->PowerInfo.WakeSupportCount));
    ...
    if (runPsw)
    {
        ...
        // the _PSW
        //
        if ((DeviceExtension->Flags & DEV_PROP_HAS_PME) && pswContext->Enable == FALSE)
        {
        ...
        // What Happened
        //
        ACPIDevPrint((ACPI_PRINT_WAKE, DeviceExtension, "ACPIWakeEnableDisableAsync = 0x%08lx (P)\n", status));
...
ACPIWakeEnableDisableAsyncExit:
    ...
    // What happened
    //
    ACPIDevPrint((ACPI_PRINT_WAKE, DeviceExtension, "ACPIWakeEnableDisableAsync = 0x%08lx\n", status));
...
VOID EXPORT ACPIWakeEnableDisableAsyncCallBack(IN PNSOBJ AcpiObject, IN NTSTATUS Status, IN POBJDATA ObjData,
                                               IN PVOID Context)
/*++
...
--*/
{
    ...
    ACPIDevPrint((ACPI_PRINT_WAKE, deviceExtension, "ACPIWakeEnableDisableAsyncCallBack = %08lx (C)\n", Status));
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        ACPIDevPrint((ACPI_PRINT_WAKE, deviceExtension,
                      "ACPIWakeEnableDisableAsyncCallBack - RefCount: %lx %s %lx = %lx\n",
                      deviceExtension->PowerInfo.WakeSupportCount, (pswContext->Enable ? "-" : "+"), pswContext->Count,
                      (pswContext->Enable ? deviceExtension->PowerInfo.WakeSupportCount - pswContext->Count
    ...
    // the _PSW
    //
    if ((deviceExtension->Flags & DEV_PROP_HAS_PME) && pswContext->Enable == TRUE)
    {

        ACPIWakeEnableDisablePciDevice(deviceExtension, pswContext->Enable);
    }
    ...
    if (!IsListEmpty(&(deviceExtension->PowerInfo.WakeSupportList)))
    {
        ...
        runPsw = TRUE;
        nextContext =
            CONTAINING_RECORD(deviceExtension->PowerInfo.WakeSupportList.Flink, ACPI_WAKE_PSW_CONTEXT, ListEntry);
    }
    ...
    //
    (*pswContext->CallBack)(AcpiObject, Status, ObjData, (pswContext->Context));
    ...
    if (runPsw)
    {
        ...
        // the _PSW
        //
        if ((deviceExtension->Flags & DEV_PROP_HAS_PME) && nextContext->Enable == FALSE)
        {
        ...
        // Call the interpreter
        //
        status = AMLIAsyncEvalObject(AcpiObject, NULL, 1, &pswData, ACPIWakeEnableDisableAsyncCallBack, nextContext);

        ACPIDevPrint((ACPI_PRINT_WAKE, nextContext->DeviceExtension,
        ...
        if (status != STATUS_PENDING)
        {
            ...
            // Ugh - Recursive
            //
            ACPIWakeEnableDisableAsyncCallBack(AcpiObject, status, NULL, nextContext);
        }
    }
}

VOID ACPIWakeEnableDisablePciDevice(IN PDEVICE_EXTENSION DeviceExtension, IN BOOLEAN Enable)
/*++
...
--*/
{
    ...
    PciPmeInterface->UpdateEnable(DeviceExtension->PhysicalDeviceObject, Enable);
...
NTSTATUS
ACPIWakeEnableDisableSync(IN PDEVICE_EXTENSION DeviceExtension, IN BOOLEAN Enable)
/*++
...
--*/
{
    ...
    ASSERT(DeviceExtension != NULL && DeviceExtension->Signature == ACPI_SIGNATURE);
    ...
    //
    status = ACPIWakeEnableDisableAsync(DeviceExtension, Enable, ACPIWakeEnableDisableSyncCallBack, &syncContext);
    if (status == STATUS_PENDING)
    {
...
VOID EXPORT ACPIWakeEnableDisableSyncCallBack(IN PNSOBJ AcpiObject, IN NTSTATUS Status, IN POBJDATA ObjData,
                                              IN PVOID Context)
/*++
...
VOID ACPIWakeEnableWakeEvents(VOID)
/*++
...
--*/
{
    ...
    for (gpeRegister = 0; gpeRegister < AcpiInformation->GpeSize; gpeRegister++)
    {
        ...
        // events
        //
        GpeCurEnable[gpeRegister] |= (GpeWakeEnable[gpeRegister] & ~GpePending[gpeRegister]);
    }
...
NTSTATUS
ACPIWakeInitializePciDevice(IN PDEVICE_OBJECT DeviceObject)
/*++
...
--*/
{
    ...
    //
    PciPmeInterface->GetPmeInformation(deviceExtension->PhysicalDeviceObject, &pmeSupported, &pmeStatus, &pmeEnable);
    ...
    if (pmeSupported == TRUE)
    {
        ...
        //
        ACPIInternalUpdateFlags(&(deviceExtension->Flags), (DEV_PROP_HAS_PME), FALSE);
...
NTSTATUS
ACPIWakeInitializePmeRouting(IN PDEVICE_OBJECT DeviceObject)
/*++
...
--*/
{
    ...
    //
    interface = ExAllocatePoolWithTag(NonPagedPool, sizeof(PCI_PME_INTERFACE), ACPI_ARBITER_POOLTAG);
    if (interface == NULL)
    {
    ...
    //
    status = ACPIInternalSendSynchronousIrp(DeviceObject, &irpSp, &dummy);
    if (!NT_SUCCESS(status))
    {

        PDEVICE_EXTENSION deviceExtension = ACPIInternalGetDeviceExtension(DeviceObject);

        ACPIDevPrint((ACPI_PRINT_CRITICAL, deviceExtension, " - ACPIWakeInitializePmeRouting = %08lx\n", status));
...
VOID ACPIWakeRemoveDevicesAndUpdate(IN PDEVICE_EXTENSION TargetExtension, OUT PLIST_ENTRY ListHead)
/*++
...
--*/
{
    ...
    for (gpeRegister = 0; gpeRegister < AcpiInformation->GpeSize; gpeRegister++)
    {
        ...
        // now as well
        //
        GpeCurEnable[gpeRegister] &=
            (GpeSpecialHandler[gpeRegister] | ~(GpeWakeEnable[gpeRegister] | GpeWakeHandler[gpeRegister]));
    }
    ...
    while (listEntry != &AcpiPowerWaitWakeList)
    {
        ...
        else
        {
            ...
            //
            byteIndex = ACPIGpeIndexToByteIndex(deviceExtension->PowerInfo.WakeBit);
            ...
            // at most a UCHAR
            //
            gpeRegister = ACPIGpeIndexToGpeRegister(deviceExtension->PowerInfo.WakeBit);
...
NTSTATUS
ACPIWakeRestoreEnables(IN PACPI_BUILD_CALLBACK CallBack, IN PVOID CallBackContext)
/*++
...
--*/
{
    ...
    status = ACPIBuildRunMethodRequest(
        RootDeviceExtension, CallBack, CallBackContext, PACKED_PSW,
        (RUN_REQUEST_CHECK_STATUS | RUN_REQUEST_RECURSIVE | RUN_REQUEST_CHECK_WAKE_COUNT), TRUE);
...
VOID ACPIWakeRestoreEnablesCompletion(IN PDEVICE_EXTENSION DeviceExtension, IN PVOID Context, IN NTSTATUS Status)
/*++
...
--*/
{
    ...
    //
    ACPIDeviceCompleteGenericPhase(NULL, Status, NULL, Context);
}

NTSTATUS
ACPIWakeWaitIrp(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    //
    if (deviceExtension->PowerInfo.SystemWakeLevel < irpStack->Parameters.WaitWake.PowerState)
    {

        //
        // The system level is not the one we are currently at
        //
        ACPIDevPrint((ACPI_PRINT_WAKE, deviceExtension, "(0x%08lx): ACPIWakeWaitIrp ->S%d < Irp->S%d\n", Irp,
                      deviceExtension->PowerInfo.SystemWakeLevel - 1, irpStack->Parameters.WaitWake.PowerState - 1));
    ...
    // to support this functionality
    //
    if (deviceExtension->PowerInfo.DeviceWakeLevel < deviceExtension->PowerInfo.PowerState)
    {

        //
        // We are too much powered off to wake the computer
        //
        ACPIDevPrint((ACPI_PRINT_WAKE, deviceExtension, "(0x%08lx): ACPIWakeWaitIrp  Device->D%d Max->D%d\n", Irp,
                      deviceExtension->PowerInfo.DeviceWakeLevel - 1, deviceExtension->PowerInfo.PowerState - 1));
