...
PCCHAR ACPIDispatchPnpTableNames[ACPIDispatchPnpTableSize] = { "IRP_MN_START_DEVICE",
...
PCCHAR ACPIDispatchPowerTableNames[ACPIDispatchPowerTableSize] = { "IRP_MN_WAIT_WAKE", "IRP_MN_POWER_SEQUENCE",
                                                                   "IRP_MN_SET_POWER", "IRP_MN_QUERY_POWER",
                                                                   "UNKNOWN POWER MINOR CODE" };

PCCHAR ACPIDispatchUnknownTableName[1] = { "IRP_MN_????" };
#endif
...
// SP3
VOID _ACPIInternalErrorEx(IN ULONG Bugcode0, IN ULONG_PTR Bugcode1, IN ULONG_PTR Bugcode2)
{
...
#if DBG
VOID ACPIDebugPrint(ULONG DebugPrintLevel, PCCHAR DebugMessage, ...)
/*++
...
VOID ACPIDebugDevicePrint(ULONG DebugPrintLevel, PVOID DebugExtension, PCCHAR DebugMessage, ...)
/*++
...
---*/
{
    ...
    if (deviceExtension->Flags & DEV_PROP_HID)
    {
        ...
        if (deviceExtension->Flags & DEV_PROP_UID)
        {
            ...
            DbgPrintEx(DPFLTR_ACPI_ID, DebugPrintLevel, "%p %s-%s ", deviceExtension, deviceExtension->DeviceID,
                       deviceExtension->InstanceID);
        }
        else
        {

            DbgPrintEx(DPFLTR_ACPI_ID, DebugPrintLevel, "%p %s ", deviceExtension, deviceExtension->DeviceID);
        }
    }
    else if (deviceExtension->Flags & DEV_PROP_ADDRESS)
    {

        DbgPrintEx(DPFLTR_ACPI_ID, DebugPrintLevel, "%p %x ", deviceExtension, deviceExtension->Address);
    }
    else
    {

        DbgPrintEx(DPFLTR_ACPI_ID, DebugPrintLevel, "%p ", deviceExtension);
    }
...
PCCHAR
ACPIDebugGetIrpText(UCHAR MajorFunction, UCHAR MinorFunction)
/*++
...
--*/
{
    ...
    case CmResourceTypePort:
        ACPIPrint((ACPI_PRINT_RESOURCES_1,
                   "[%2d] [%2d] %-9s  MininumAddress = %#08lx  MaximumAddress = %#08lx\n"
                   "                     Length         = %#08lx  Alignment      = %#08lx\n",
                   ListCount, DescCount, "Port", Descriptor->u.Port.MinimumAddress.LowPart,
                   Descriptor->u.Port.MaximumAddress.LowPart, Descriptor->u.Port.Length, Descriptor->u.Port.Alignment));
        break;
    case CmResourceTypeMemory:
        ACPIPrint((ACPI_PRINT_RESOURCES_1,
                   "[%2d] [%2d] %-9s  MinimumAddress = %#08lx  MaximumAddress = %#08lx\n"
                   "                     Length         = %#08lx  Alignment      = %#08lx\n",
                   ListCount, DescCount, "Memory", Descriptor->u.Memory.MinimumAddress.LowPart,
                   Descriptor->u.Memory.MaximumAddress.LowPart, Descriptor->u.Memory.Length,
                   Descriptor->u.Memory.Alignment));
        break;
    case CmResourceTypeInterrupt:
        ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] %-9s  MinimumVector  = %#08lx  MaximumVector  = %#08lx\n",
                   ListCount, DescCount, "Interrupt", Descriptor->u.Interrupt.MinimumVector,
                   Descriptor->u.Interrupt.MaximumVector));
        break;
    case CmResourceTypeDma:
        ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] %-9s  MinimumChannel = %#08lx  MaximumChannel = %#08lx\n",
                   ListCount, DescCount, "Dma", Descriptor->u.Dma.MinimumChannel, Descriptor->u.Dma.MaximumChannel));
        break;
    default:
        ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] Type = (%d)\n", ListCount, DescCount, Descriptor->Type));
    ...
    //
    ACPIPrint((ACPI_PRINT_RESOURCES_1, "                     Option,Share   = %#04lx%#04lx  Flags          = %#08lx\n",
               ListCount, DescCount, Descriptor->Option, Descriptor->ShareDisposition));
...
--*/
{
    ...
    ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d]      %#04x       Version        = %#08lx  Revision       = %#08lx\n",
               Count, List->Count, List->Version, List->Revision));
...
--*/
{
    ...
    ACPIDevPrint((ACPI_PRINT_RESOURCES_1, DeviceExtension, "IoResourceRequirementsList @ %x\n", List));
    ...
    ACPIPrint((ACPI_PRINT_RESOURCES_1, "%x size: %xb alternatives: %x bus type: %x bus number %x\n", List,
               List->ListSize, List->AlternativeLists, List->InterfaceType, List->BusNumber));
...
--*/
{
    ...
    ACPIDevPrint((ACPI_PRINT_RESOURCES_1, DeviceExtension, "CmResourceList @ %x\n", List));
    ...
    if (List->Count == 0)
    {
        ...
        ACPIDevPrint((ACPI_PRINT_RESOURCES_1, DeviceExtension, "There are no full resource descriptors in the list\n"));
        return;
    }
    ...
    for (i = 0; i < List->Count; i++)
    {
        ...
        size = sizeof(CM_FULL_RESOURCE_DESCRIPTOR) +
               (fullDesc->PartialResourceList.Count - 1) * sizeof(CM_PARTIAL_RESOURCE_DESCRIPTOR);
        ...
        ACPIPrint((ACPI_PRINT_RESOURCES_1,
                   ...
                   i, fullDesc->BusNumber, fullDesc->InterfaceType, i, fullDesc->PartialResourceList.Count,
                   fullDesc->PartialResourceList.Version, fullDesc->PartialResourceList.Revision));
        ...
        for (j = 0; j < fullDesc->PartialResourceList.Count; j++)
        {
            ...
            case CmResourceTypePort:
                ...
                ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] %12s  Start: %#08lx  Length: %#08lx\n", i, j, "Port",
                           partDesc->u.Port.Start.LowPart, partDesc->u.Port.Length));
                break;

            case CmResourceTypeInterrupt:

                ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] %12s  Level: %#02x  Vector: %#02x  Affinity: %#08lx\n",
                           i, j, "Interrupt", partDesc->u.Interrupt.Level, partDesc->u.Interrupt.Vector,
            ...
            case CmResourceTypeMemory:
                ...
                ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] %12s  Start: %#08lx  Length: %#08lx\n", i, j, "Memory",
                           partDesc->u.Memory.Start.LowPart, partDesc->u.Memory.Length));
                break;

            case CmResourceTypeDma:

                ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] %12s  Channel: %#02x  Port: %#02x  Reserved: %#02x\n",
                           i, j, "Dma", partDesc->u.Dma.Channel, partDesc->u.Dma.Port, partDesc->u.Dma.Reserved1));
                break;

            default:

                ACPIPrint((ACPI_PRINT_RESOURCES_1, "[%2d] [%2d] Type: %2d   1: %#08lx  2: %#08lx  3: %#08lx\n", i, j,
                           partDesc->Type, partDesc->u.DeviceSpecificData.DataSize,
                           partDesc->u.DeviceSpecificData.Reserved1, partDesc->u.DeviceSpecificData.Reserved2));
                break;
            }
            ...
            // Dump ancillary info
            //
            ACPIPrint((ACPI_PRINT_RESOURCES_1, "                        Flags: %#08lx  Share: %#08lx\n",
                       partDesc->Flags, partDesc->ShareDisposition));
        }
...
VOID ACPIDebugDeviceCapabilities(IN PDEVICE_EXTENSION DeviceExtension, IN PDEVICE_CAPABILITIES DeviceCapabilities,
                                 IN PUCHAR Message)
/*++
...
--*/
{
    ...
    ACPIDevPrint((ACPI_PRINT_SXD, DeviceExtension, " - %s - Capabilities @ %08lx\n", Message, DeviceCapabilities));
    ACPIDevPrint((ACPI_PRINT_SXD, DeviceExtension, " -"));
    ...
    for (index = PowerSystemWorking; index < PowerSystemMaximum; index++)
    {
        ...
        if (DeviceCapabilities->DeviceState[index] == PowerSystemUnspecified)
        {
            ...
            ACPIPrint((ACPI_PRINT_SXD, " S%d -> None", (index - 1)));
        }
        else
        {

            ACPIPrint((ACPI_PRINT_SXD, " S%d -> D%x", (index - 1), (DeviceCapabilities->DeviceState[index] - 1)));
        }
    }
    ACPIPrint((ACPI_PRINT_SXD, "\n"));
    ACPIDevPrint((ACPI_PRINT_SXD, DeviceExtension, " -"));

    if (DeviceCapabilities->SystemWake == PowerSystemUnspecified)
    {

        ACPIPrint((ACPI_PRINT_SXD, " SystemWake = None"));
    }
    else
    {

        ACPIPrint((ACPI_PRINT_SXD, " SystemWake = S%d", (DeviceCapabilities->SystemWake - 1)));
    }

    if (DeviceCapabilities->DeviceWake == PowerDeviceUnspecified)
    {

        ACPIPrint((ACPI_PRINT_SXD, " DeviceWake = None"));
    }
    else
    {

        ACPIPrint((ACPI_PRINT_SXD, " DeviceWake = D%d", (DeviceCapabilities->DeviceWake - 1)));
    }

    if (DeviceCapabilities->DeviceD1)
    {

        ACPIPrint((ACPI_PRINT_SXD, " DeviceD1"));
    }
    if (DeviceCapabilities->DeviceD2)
    {

        ACPIPrint((ACPI_PRINT_SXD, " DeviceD2"));
    }
    if (DeviceCapabilities->WakeFromD0)
    {

        ACPIPrint((ACPI_PRINT_SXD, " WakeD0"));
    }
    if (DeviceCapabilities->WakeFromD1)
    {

        ACPIPrint((ACPI_PRINT_SXD, " WakeD1"));
    }
    if (DeviceCapabilities->WakeFromD2)
    {

        ACPIPrint((ACPI_PRINT_SXD, " WakeD2"));
    }
    if (DeviceCapabilities->WakeFromD3)
    {

        ACPIPrint((ACPI_PRINT_SXD, " WakeD3"));
    }
...
--*/
{
    ...
    ACPIDevPrint((ACPI_PRINT_SXD, DeviceExtension, " - %s - Internal Capabilities\n", Message));
    ACPIDevPrint((ACPI_PRINT_SXD, DeviceExtension, " -"));
    ...
    for (index = PowerSystemWorking; index < PowerSystemMaximum; index++)
    {
        ...
        else
        {
            ...
            ACPIPrint((ACPI_PRINT_SXD, " S%d -> D%x", (index - 1), (powerInfo->DevicePowerMatrix[index] - 1)));
        }
    }
    ...
    else
    {
        ...
        ACPIPrint((ACPI_PRINT_SXD, " SystemWake = S%d", (powerInfo->SystemWakeLevel - 1)));
    }
    ...
    else
    {
        ...
        ACPIPrint((ACPI_PRINT_SXD, " DeviceWake = D%d", (powerInfo->DeviceWakeLevel - 1)));
    }
...
VOID ACPIDebugThermalPrint(ULONG DebugPrintLevel, PVOID DebugExtension, ULONGLONG DebugTime, PCCHAR DebugMessage, ...)
/*++
...
---*/
{
    ...
    DbgPrintEx(DPFLTR_ACPI_ID, DebugPrintLevel, "%d:%02d:%02d.%03d ", exCurTime.Hour, exCurTime.Minute,
               exCurTime.Second, exCurTime.Milliseconds);
