...
NTSTATUS
ACPIBusAndFilterIrpEject(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context, IN BOOLEAN ProcessingFilterIrp)
/*++
...
--*/
{
    ...
    if ((deviceExtension->DeviceState != Inactive) && (deviceExtension->DeviceState != Stopped))
    {

        //
        // We got called to eject a live node! Yuck, how did this happen?!
        //
        ACPIDevPrint((ACPI_PRINT_FAILURE, deviceExtension, "(0x%08lx): ACPIBusAndFilterIrpEject: Active node!\n", Irp));
    ...
    // Bye bye card.
    //
    ACPIGetNothingEvalIntegerSync(deviceExtension, PACKED_EJ0, 1);
    ...
    if (NT_SUCCESS(status) && (!ProcessingFilterIrp) && (!(deviceExtension->Flags & DEV_TYPE_NOT_PRESENT)))
    {
...
ACPIBusAndFilterIrpEjectExit:
    ...
    // done
    //
    ACPIDevPrint((ACPI_PRINT_IRP, deviceExtension, "(%#08lx): %s = %#08lx\n", Irp,
...
NTSTATUS
ACPIBusAndFilterIrpQueryCapabilities(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context,
...
--*/
{
    ...
    if ((deviceExtension->Flags & DEV_CAP_NO_REMOVE_OR_EJECT) == 0)
    {
        ...
        if (rmvObject != NULL)
        {
            ...
            if (NSGETOBJTYPE(rmvObject) == OBJTYPE_METHOD)
            {
                ...
                //
                status = ACPIGetIntegerSyncValidate(deviceExtension, PACKED_RMV, &rmvValue, NULL);
    ...
    if (ACPIAmliGetNamedChild(acpiObject, PACKED_SUN) != NULL)
    {
        ...
        //
        status = ACPIGetIntegerSync(deviceExtension, PACKED_SUN, &slotUniqueNumber, NULL);
    ...
    if (ACPIAmliGetNamedChild(acpiObject, PACKED_ADR) != NULL)
    {
        ...
        status = ACPIGetAddressSync(deviceExtension, &(capabilities->Address), NULL);
        if (!NT_SUCCESS(status))
        {

            ACPIDevPrint((ACPI_PRINT_CRITICAL, deviceExtension, " - Could query device address - %08lx", status));
...
NTSTATUS
ACPIBusAndFilterIrpQueryEjectRelations(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp,
                                       IN OUT PDEVICE_RELATIONS *DeviceRelations)
{
    ...
    status = ACPIDetectEjectDevices(deviceExtension, DeviceRelations, additionalExtension);
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        // That's not nice..
        //
        ACPIDevPrint((ACPI_PRINT_CRITICAL, deviceExtension, "ACPIBusAndFilterIrpQueryEjectRelations: enum = 0x%08lx\n",
                      Irp, status));
    }
...
NTSTATUS
ACPIBusAndFilterIrpQueryPnpDeviceState(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context,
                                       IN BOOLEAN ProcessingFilterIrp)
/*++
...
--*/
{
    ...
    if (!(deviceExtension->Flags & DEV_PROP_NO_OBJECT))
    {
        ...
        nsObj = ACPIAmliGetNamedChild(deviceExtension->AcpiObject, PACKED_STA);
    }
    ...
    //
    if (!(deviceExtension->Flags & DEV_PROP_NO_OBJECT) && !(deviceExtension->Flags & DEV_CAP_PROCESSOR) &&
        !(deviceExtension->Flags & DEV_CAP_THERMAL_ZONE) && !(deviceExtension->Flags & DEV_CAP_BUTTON))
    {
        ...
        if (!ProcessingFilterIrp)
        {
            ...
            //
            nsObj = ACPIAmliGetNamedChild(deviceExtension->AcpiObject, PACKED_DIS);
            if (nsObj == NULL)
            {

                nsObj = ACPIAmliGetNamedChild(deviceExtension->AcpiObject, PACKED_PS3);
            }
            if (nsObj == NULL)
            {

                nsObj = ACPIAmliGetNamedChild(deviceExtension->AcpiObject, PACKED_PR0);
            }
...
ACPIBusAndFilterIrpQueryPnpDeviceStateExit:
    ...
    ACPIDevPrint((ACPI_PRINT_PNP_STATE, deviceExtension, ":%s%s%s%s%s%s\n",
...
NTSTATUS
ACPIBusAndFilterIrpSetLock(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context,
                           IN BOOLEAN ProcessingFilterIrp)
/*++
...
NTSTATUS
ACPIBusIrpCancelRemoveOrStopDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIBusIrpDeviceUsageNotification(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    if (deviceExtension->ParentExtension != NULL)
    {
        ...
        //
        status = ACPIInternalSendSynchronousIrp(parentObject, irpSp, NULL);
    }
...
NTSTATUS
ACPIBusIrpEject(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    return ACPIIrpInvokeDispatchRoutine(DeviceObject, Irp, NULL, ACPIBusAndFilterIrpEject, FALSE, TRUE);
}

NTSTATUS
ACPIBusIrpQueryBusInformation(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    //
    busInfo = ExAllocatePoolWithTag(PagedPool, sizeof(PNP_BUS_INFORMATION), ACPI_MISC_POOLTAG);
    if (busInfo != NULL)
    {
    ...
    else
    {
        ...
        ACPIDevPrint((ACPI_PRINT_CRITICAL, DeviceObject->DeviceExtension,
...
NTSTATUS
ACPIBusIrpQueryBusRelations(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, OUT PDEVICE_RELATIONS *DeviceRelations)
/*++
...
--*/
{
    ...
    else
    {
        ...
        // Load the filters
        //
        filterStatus = ACPIDetectFilterDevices(DeviceObject, *DeviceRelations);
...
NTSTATUS
ACPIBusIrpQueryCapabilities(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    return ACPIIrpInvokeDispatchRoutine(DeviceObject, Irp, NULL, ACPIBusAndFilterIrpQueryCapabilities, TRUE, TRUE);

    ACPIDebugExit("ACPIBusIrpQueryCapabilities");
}

NTSTATUS
ACPIBusIrpQueryDeviceRelations(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIBusIrpQueryId(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    case BusQueryCompatibleIDs:
        ...
        //
        status = ACPIGetCompatibleIDSyncWide(deviceExtension, &baseBuffer, &baseBufferSize);
        ...
        else if (!NT_SUCCESS(status))
        {
            ...
            ACPIDevPrint((ACPI_PRINT_FAILURE, deviceExtension, " (0x%08lx): IRP_MN_QUERY_ID( %d - CID) = 0x%08lx\n",
                          Irp, type, status));
            break;
        }
    ...
    case BusQueryInstanceID:
        ...
        //
        status = ACPIGetInstanceIDSyncWide(deviceExtension, &baseBuffer, &baseBufferSize);
        if (status == STATUS_OBJECT_NAME_NOT_FOUND)
        {
        ...
        else if (!NT_SUCCESS(status))
        {
            ...
            ACPIDevPrint((ACPI_PRINT_FAILURE, deviceExtension, " (0x%08lx): IRP_MN_QUERY_ID( %d - UID) = 0x%08lx\n",
                          Irp, type, status));
            break;
        }
    ...
    case BusQueryDeviceID:
        ...
        //
        status = ACPIGetDeviceIDSyncWide(deviceExtension, &baseBuffer, &baseBufferSize);
        if (status == STATUS_OBJECT_NAME_NOT_FOUND)
        {
        ...
        else if (!NT_SUCCESS(status))
        {
            ...
            ACPIDevPrint((ACPI_PRINT_FAILURE, deviceExtension, " (0x%08lx): IRP_MN_QUERY_ID( %d - HID) = 0x%08lx\n",
                          Irp, type, status));
            break;
        }
    ...
    case BusQueryHardwareIDs:
        ...
        //
        status = ACPIGetHardwareIDSyncWide(deviceExtension, &baseBuffer, &baseBufferSize);
        if (status == STATUS_OBJECT_NAME_NOT_FOUND)
        {
...
NTSTATUS
ACPIBusIrpQueryInterface(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    else if (CompareGuid(interfaceType, (PVOID)&GUID_BUS_INTERFACE_STANDARD))
    {
        ...
        if (deviceExtension->ParentExtension != NULL)
        {
            ...
            PDEVICE_OBJECT parentObject = deviceExtension->ParentExtension->DeviceObject;
            ...
            if (parentObject != NULL)
            {
                ...
                Irp->IoStatus.Status = ACPIInternalSendSynchronousIrp(parentObject, irpStack, NULL);
            }
        }
    }
...
NTSTATUS
ACPIBusIrpQueryPnpDeviceState(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    return ACPIIrpInvokeDispatchRoutine(DeviceObject, Irp, NULL, ACPIBusAndFilterIrpQueryPnpDeviceState, TRUE, TRUE);
}

NTSTATUS
ACPIBusIrpQueryPower(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIBusIrpQueryRemoveOrStopDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIBusIrpQueryResources(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    if (!(deviceExtension->Flags & DEV_PROP_DEVICE_ENABLED))
    {
        ...
        //
        ACPIDevPrint(
            (ACPI_PRINT_WARNING, deviceExtension, "(0x%08lx) : ACPIBusIrpQueryResources - Device not Enabled\n", Irp));
    ...
    if (!(deviceExtension->Flags & DEV_CAP_CONTAINER))
    {
        ...
        //
        status = ACPIGetBufferSync(deviceExtension, PACKED_CRS, &crsBuf, &crsBufSize);
    }
    else
    {
    ...
    status = PnpBiosResourcesToNtResources(
        crsBuf, (deviceExtension->Flags & DEV_CAP_PCI ? PNP_BIOS_TO_IO_NO_CONSUMED_RESOURCES : 0), &ioList);
    ...
    if (deviceExtension->Flags & DEV_CAP_PCI)
    {
        ...
        status = ACPIRangeSubtract(&ioList, RootDeviceExtension->ResourceList);
        if (!NT_SUCCESS(status))
        {
    ...
    else if (deviceExtension->Flags & DEV_CAP_PIC_DEVICE)
    {
        ...
        //
        status = ACPIRangeFilterPICInterrupt(ioList);
        if (!NT_SUCCESS(status))
        {
...
ACPIBusIrpQueryResourcesExit:
    ...
    // will not boot.
    //
    if (!NT_SUCCESS(status) && status != STATUS_INSUFFICIENT_RESOURCES && (deviceExtension->Flags & DEV_CAP_PCI))
    {

        KeBugCheckEx(ACPI_BIOS_ERROR, ACPI_ROOT_PCI_RESOURCE_FAILURE, (ULONG_PTR)deviceExtension, 0, (ULONG_PTR)Irp);
    }
...
NTSTATUS
ACPIBusIrpQueryResourceRequirements(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    if (!(deviceExtension->Flags & DEV_CAP_CONTAINER))
    {
        ...
        //
        crsStat = ACPIGetBufferSync(deviceExtension, PACKED_CRS, &crsBuf, &crsBufSize);
        prsStat = ACPIGetBufferSync(deviceExtension, PACKED_PRS, &prsBuf, &prsBufSize);
    }
    else
    {
    ...
    else if (!NT_SUCCESS(prsStat))
    {
        ...
        //
        if (prsStat == STATUS_INSUFFICIENT_RESOURCES || crsStat == STATUS_INSUFFICIENT_RESOURCES)
        {
    ...
    if (NT_SUCCESS(prsStat))
    {
        ...
        // information...
        //
        status = PnpBiosResourcesToNtResources(prsBuf, 0, &resList);

        ASSERT(NT_SUCCESS(status));

        ACPIDevPrint((ACPI_PRINT_IRP, deviceExtension, "(0x%08lx): %s - ResourcesToNtResources =  0x%08lx\n", Irp,
                      ACPIDebugGetIrpText(IRP_MJ_PNP, minorFunction), status));
    ...
    if (!NT_SUCCESS(status) && NT_SUCCESS(crsStat))
    {
        ...
        status = PnpBiosResourcesToNtResources(
            crsBuf, (deviceExtension->Flags & DEV_CAP_PCI ? PNP_BIOS_TO_IO_NO_CONSUMED_RESOURCES : 0), &resList);
        //ASSERT(NT_SUCCESS(status));
    }
    ...
    if (deviceExtension->Flags & DEV_CAP_PCI)
    {
        ...
        //
        status = ACPIRangeSubtract(&resList, RootDeviceExtension->ResourceList);
        ASSERT(NT_SUCCESS(status));
        if (!NT_SUCCESS(status))
        {
    ...
    else if (deviceExtension->Flags & DEV_CAP_PIC_DEVICE)
    {
        ...
        // Strip out the PIC resources
        //
        status = ACPIRangeFilterPICInterrupt(resList);
        if (!NT_SUCCESS(status))
        {
    ...
    //
    Irp->IoStatus.Information = (ULONG_PTR)(NT_SUCCESS(status) ? resList : NULL);
...
ACPIBusIrpQueryResourceRequirementsExit:
    ...
    if (!NT_SUCCESS(status) && status != STATUS_INSUFFICIENT_RESOURCES && (deviceExtension->Flags & DEV_CAP_PCI))
    {
        ...
        ASSERT(NT_SUCCESS(status));
        ACPIDevPrint((ACPI_PRINT_CRITICAL, deviceExtension, "(0x%08lx): %s = 0x%08lx\n", Irp,
                      ACPIDebugGetIrpText(IRP_MJ_PNP, minorFunction), status));

        KeBugCheckEx(ACPI_BIOS_ERROR, ACPI_ROOT_PCI_RESOURCE_FAILURE, (ULONG_PTR)deviceExtension, 1, (ULONG_PTR)Irp);
    }
...
NTSTATUS
ACPIBusIrpQueryTargetRelation(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, OUT PDEVICE_RELATIONS *DeviceRelations)
/*++
...
--*/
{
    ...
    //
    *DeviceRelations = ExAllocatePoolWithTag(NonPagedPool, sizeof(DEVICE_RELATIONS), ACPI_IRP_POOLTAG);
...
NTSTATUS
ACPIBusIrpRemoveDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    //
    ACPIDevPrint((ACPI_PRINT_REMOVE, deviceExtension, "(0x%08lx): %s = 0x%08lx\n", Irp,
...
NTSTATUS
ACPIBusIrpSetLock(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    return ACPIIrpInvokeDispatchRoutine(DeviceObject, Irp, NULL, ACPIBusAndFilterIrpSetLock, TRUE, TRUE);
}

NTSTATUS
ACPIBusIrpSetDevicePower(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PIO_STACK_LOCATION IrpStack)
/*++
...
NTSTATUS
ACPIBusIrpSetPower(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIBusIrpSetSystemPower(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PIO_STACK_LOCATION IrpStack)
/*++
...
--*/
{
    ...
    if (IrpStack->Parameters.Power.ShutdownType == PowerActionWarmEject)
    {
        ...
        //
        status = ACPIDeviceIrpWarmEjectRequest(deviceExtension, Irp, ACPIDeviceIrpCompleteRequest, FALSE);
    ...
    ACPIDevPrint((ACPI_PRINT_POWER, deviceExtension, "(0x%08lx) ACPIBusIrpSetSystemPower: send D%d irp!\n", Irp,
    ...
    // Make the request
    //
    PoRequestPowerIrp(DeviceObject, IRP_MN_SET_POWER, powerState, ACPIBusIrpSetSystemPowerComplete, Irp, NULL);
...
NTSTATUS
ACPIBusIrpSetSystemPowerComplete(IN PDEVICE_OBJECT DeviceObject, IN UCHAR MinorFunction, IN POWER_STATE PowerState,
...
NTSTATUS
ACPIBusIrpStartDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    status = ACPIInitStartDevice(DeviceObject, irpStack->Parameters.StartDevice.AllocatedResources,
                                 ACPIBusIrpStartDeviceCompletion, Irp, Irp);
    if (NT_SUCCESS(status))
    {
...
VOID ACPIBusIrpStartDeviceCompletion(IN PDEVICE_EXTENSION DeviceExtension, IN PVOID Context, IN NTSTATUS Status)
/*++
...
VOID ACPIBusIrpStartDeviceWorker(IN PVOID Context)
/*++
...
NTSTATUS
ACPIBusIrpStopDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIBusIrpSurpriseRemoval(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIBusIrpUnhandled(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
VOID SmashInterfaceQuery(IN OUT PIRP Irp)
{
    GUID *interfaceType;
    PIO_STACK_LOCATION irpStack = IoGetCurrentIrpStackLocation(Irp);
