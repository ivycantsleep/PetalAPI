...
NTSTATUS
ACPIRootIrpCancelRemoveOrStopDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIRootIrpCompleteRoutine(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
/*++
...
--*/
{
    ...
    if (deviceExtension != NULL)
    {
        ...
        //
        ACPIDevPrint((ACPI_PRINT_IRP, deviceExtension, "(%#08lx): %s = %#08lx (Complete)\n", Irp,
                      ACPIDebugGetIrpText(IRP_MJ_PNP, irpStack->MinorFunction), Irp->IoStatus.Status));
    }
#endif
...
NTSTATUS
ACPIRootIrpQueryBusRelations(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, OUT PDEVICE_RELATIONS *PdeviceRelations)
/*++
...
--*/
{
    ...
    if (acpiObject == NULL)
    {
        ...
        ACPIDevPrint((ACPI_PRINT_WARNING, deviceExtension, "(%#08lx): %s - Invalid ACPI Object %#08lx\n", Irp,
                      ACPIDebugGetIrpText(IRP_MJ_PNP, minorFunction), acpiObject));
    ...
    //
    detectStatus = ACPIDetectPdoDevices(DeviceObject, PdeviceRelations);
    ...
    if (!NT_SUCCESS(detectStatus))
    {
        ...
        ACPIDevPrint((ACPI_PRINT_WARNING, deviceExtension, "(%#08lx): %s - Enum Failed %#08lx\n", Irp,
    ...
    if (NT_SUCCESS(detectStatus))
    {
        ...
        detectStatus = ACPIDetectDockDevices(deviceExtension, PdeviceRelations);
        ...
        if (!NT_SUCCESS(detectStatus))
        {
            ...
            ACPIDevPrint((ACPI_PRINT_WARNING, deviceExtension,
                          ...
                          "%#08lx\n",
                          Irp, ACPIDebugGetIrpText(IRP_MJ_PNP, minorFunction), detectStatus));
        }
    }
...
NTSTATUS
ACPIRootIrpQueryCapabilities(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        ACPIDevPrint((ACPI_PRINT_CRITICAL, deviceExtension, "(0x%08lx): %s - InitializeRootMapping = %08lx\n", Irp,
                      ACPIDebugGetIrpText(IRP_MJ_PNP, minorFunction), status));
        goto ACPIRootIrpQueryCapabilitiesExit;
    }
...
NTSTATUS
ACPIRootIrpQueryDeviceRelations(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    default:
        ...
        ACPIDevPrint((ACPI_PRINT_WARNING, deviceExtension, "(%#08lx): %s - Unhandled Type %d\n", Irp,
    ...
    //
    if ((NT_SUCCESS(status) || (status == STATUS_NOT_SUPPORTED)) && checkForFilters == TRUE)
    {

        //
        // Grab the device relations
        //
        detectStatus = ACPIDetectFilterDevices(DeviceObject, deviceRelations);
...
NTSTATUS
ACPIRootIrpQueryInterface(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    // any other GUID
    //
    if ((CompareGuid(interfaceType, (PVOID)&GUID_ARBITER_INTERFACE_STANDARD)) && (resource == CmResourceTypeInterrupt))
    {
        ...
        //
        RtlCopyMemory(irpStack->Parameters.QueryInterface.Interface, &ArbiterTable, count);
...
NTSTATUS
ACPIRootIrpQueryPower(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    case PowerSystemWorking:
        objectName = PACKED_S0;
        break;
    case PowerSystemSleeping1:
        objectName = PACKED_S1;
        break;
    case PowerSystemSleeping2:
        objectName = PACKED_S2;
        break;
    case PowerSystemSleeping3:
        objectName = PACKED_S3;
        break;
    ...
    //
    object = ACPIAmliGetNamedChild(deviceExtension->AcpiObject->pnsParent, objectName);
    if (object != NULL)
    {
...
ACPIRootIrpQueryPowerExit:
    ...
    //
    Irp->IoStatus.Status = status;
    ACPIDevPrint((ACPI_PRINT_IRP, deviceExtension, "(0x%08lx): ACPIRootIrpQueryPower = %08lx\n", Irp, status));
...
NTSTATUS
ACPIRootIrpQueryRemoveOrStopDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIRootIrpRemoveDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIRootIrpSetPower(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    //
    Irp->IoStatus.Status = STATUS_SUCCESS;
    ACPIDevPrint((ACPI_PRINT_IRP, deviceExtension, "(0x%08lx): ACPIRootIrpSetPower - S%d\n", Irp,
    ...
    // Queue the request
    //
    status = ACPIDeviceIrpSystemRequest(DeviceObject, Irp, ACPIDeviceIrpForwardRequest);
...
NTSTATUS
ACPIRootIrpStartDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    //
    ACPIDevPrint((ACPI_PRINT_IRP, deviceExtension, "(0x%08lx): ACPIRootIrpStartDevice\n", Irp));
    ...
    deviceExtension->ResourceList =
        (irpStack->Parameters.StartDevice.AllocatedResourcesTranslated == NULL)
            ? NULL
            : RtlDuplicateCmResourceList(NonPagedPool, irpStack->Parameters.StartDevice.AllocatedResourcesTranslated,
                                         ACPI_RESOURCE_POOLTAG);
    if (deviceExtension->ResourceList == NULL)
    {

        ACPIDevPrint((ACPI_PRINT_CRITICAL, deviceExtension, " - Did not find a resource list!\n"));
        KeBugCheckEx(ACPI_BIOS_ERROR, ACPI_ROOT_RESOURCES_FAILURE, (ULONG_PTR)deviceExtension, 0, 0);
    }
...
NTSTATUS
ACPIRootIrpStopDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
NTSTATUS
ACPIRootIrpUnhandled(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    //
    ACPIDevPrint((ACPI_PRINT_WARNING, deviceExtension, "(%#08lx): %s - Unhandled\n", Irp,
...
--*/
{
    ...
    num = AMLIMaxCTObjs;
    KeReleaseSpinLock(&gdwGContextSpinLock, oldIrql);
    OSWriteRegValue("AMLIMaxCTObjs", pKey, &num, sizeof(num));
    ...
    //
    OSWriteRegValue("FixedEventMask", wKey, &(AcpiInformation->pm1_wake_mask), sizeof(AcpiInformation->pm1_wake_mask));

    //
    // Store the PM1 Fixed Register Status
    //
    OSWriteRegValue("FixedEventStatus", wKey, &(AcpiInformation->pm1_wake_status),
                    sizeof(AcpiInformation->pm1_wake_status));

    //
    // Store the GPE Mask
    //
    OSWriteRegValue("GenericEventMask", wKey, GpeSavedWakeMask, AcpiInformation->GpeSize);

    //
    // Store the GPE Status
    //
    OSWriteRegValue("GenericEventStatus", wKey, GpeSavedWakeStatus, AcpiInformation->GpeSize);
...
NTSTATUS
ACPIRootUpdateRootResourcesWithBusResources(VOID)
/*++
...
--*/
{
    ...
    //
    status = PnpCmResourceListToIoResourceList(RootDeviceExtension->ResourceList, &globalList);
    if (!NT_SUCCESS(status))
    {
    ...
    //
    deviceExtension =
        CONTAINING_RECORD(RootDeviceExtension->ChildDeviceList.Flink, DEVICE_EXTENSION, SiblingDeviceList);
    ...
    while (deviceExtension != NULL)
    {
        ...
        // node
        //
        if (!(deviceExtension->Flags & DEV_MASK_BUS) && !(deviceExtension->Flags & DEV_PROP_NO_OBJECT))
        {

            //
            // At this point, see if there is a _CRS
            //
            ACPIGetBufferSync(deviceExtension, PACKED_CRS, &crsBuf, NULL);
            if (crsBuf != NULL)
            {
                ...
                //
                status = PnpBiosResourcesToNtResources(crsBuf, 0, &currentList);
                ...
                if (NT_SUCCESS(status))
                {
                    ...
                    //
                    status = ACPIRangeAdd(&globalList, currentList);
                ...
                ACPIDevPrint((ACPI_PRINT_RESOURCES_1, deviceExtension, "ACPIRootUpdateResources = %08lx\n", status));
        ...
        //
        if (deviceExtension->SiblingDeviceList.Flink != &(RootDeviceExtension->ChildDeviceList))
        {
            ...
            //
            deviceExtension =
                CONTAINING_RECORD(deviceExtension->SiblingDeviceList.Flink, DEVICE_EXTENSION, SiblingDeviceList);
        ...
        //
        oldReferenceCount = InterlockedDecrement(&(oldExtension->ReferenceCount));
...
NTSTATUS
ACPIRootUpdateRootResourcesWithHalResources(VOID)
/*++
...
#define INVALID_HANDLE (HANDLE) - 1
    ...
    //
    u.buffer = ExAllocatePoolWithTag(PagedPool, bufferSize, ACPI_MISC_POOLTAG);
    if (u.buffer == NULL)
    {
    ...
    //
    globalResList = NULL;
    status = ACPIRangeAddCmList(&globalResList, RootDeviceExtension->ResourceList);
    if (!NT_SUCCESS(status))
    {
    ...
    //
    for (translatedLength = 0; rgzTranslated[translatedLength]; translatedLength++)
        ;
    for (busTranslatedLength = 0; rgzBusTranslated[busTranslatedLength]; busTranslatedLength++)
        ;
    ...
    //
    InitializeObjectAttributes(&resourceObject, &keyName, OBJ_CASE_INSENSITIVE, NULL, NULL);
    status = ZwOpenKey(&resourceMap, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &resourceObject);
    if (!NT_SUCCESS(status))
    {
        ...
        // Failed:
        //
        ACPIPrint((ACPI_PRINT_CRITICAL, "ACPIRootUpdateRootResourcesWithHalResources: ZwOpenKey = 0x%08lx\n", status));
        ExFreePool(u.buffer);
        return status;
    }
    ...
    while (NT_SUCCESS(status))
    {
        ...
        //
        status = ZwEnumerateKey(resourceMap, classKeyIndex++, KeyBasicInformation, u.keyBasicInf, bufferSize, &temp);
        if (!NT_SUCCESS(status))
        {
        ...
        InitializeObjectAttributes(&resourceObject, &keyName, OBJ_CASE_INSENSITIVE, resourceMap, NULL);
        status = ZwOpenKey(&classKeyHandle, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &resourceObject);
        if (!NT_SUCCESS(status))
        {
        ...
        while (NT_SUCCESS(status))
        {
            ...
            //
            status =
                ZwEnumerateKey(classKeyHandle, driverKeyIndex++, KeyBasicInformation, u.keyBasicInf, bufferSize, &temp);
            if (!NT_SUCCESS(status))
            {
            ...
            keyName.Length = (USHORT)u.keyBasicInf->NameLength;
            keyName.MaximumLength = (USHORT)u.keyBasicInf->NameLength;
            InitializeObjectAttributes(&resourceObject, &keyName, OBJ_CASE_INSENSITIVE, classKeyHandle, NULL);
            status = ZwOpenKey(&driverKeyHandle, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &resourceObject);
            if (!NT_SUCCESS(status))
            {
            ...
            //
            status = ZwQueryKey(driverKeyHandle, KeyFullInformation, u.keyFullInf, bufferSize, &temp);
            if (!NT_SUCCESS(status))
            {
            ...
            // How long is the key?
            //
            length = sizeof(KEY_VALUE_FULL_INFORMATION) + u.keyFullInf->MaxValueNameLen +
                     u.keyFullInf->MaxValueDataLen + sizeof(UNICODE_NULL);
            if (length > bufferSize)
            {
                ...
                // Grow the buffer
                //
                tempBuffer = ExAllocatePoolWithTag(PagedPool, length, ACPI_MISC_POOLTAG);
                if (tempBuffer == NULL)
                {
            ...
            for (;;)
            {
                ...
                status = ZwEnumerateValueKey(driverKeyHandle, driverValueIndex++, KeyValueFullInformation,
                                             u.valueKeyFullInf, bufferSize, &temp);
                if (!NT_SUCCESS(status))
                {
                ...
                if (i < translatedLength || RtlCompareMemory(((PUCHAR)u.valueKeyFullInf->Name) + i - translatedLength,
                                                             rgzTranslated, translatedLength) != translatedLength)
                {
                ...
                if (i >= busTranslatedLength &&
                    RtlCompareMemory(((PUCHAR)u.valueKeyFullInf->Name) + i - busTranslatedLength, rgzBusTranslated,
                ...
                //
                cmResList = (PCM_RESOURCE_LIST)((PUCHAR)u.valueKeyFullInf + u.valueKeyFullInf->DataOffset);
                ...
                for (temp = i = 0; i < cmResList->Count; i++)
                {
                    ...
                    //
                    cmFullResList =
                        (PCM_FULL_RESOURCE_DESCRIPTOR)((PUCHAR)cmFullResList + sizeof(CM_FULL_RESOURCE_DESCRIPTOR) +
                ...
                // much space
                //
                tempCmList = ExAllocatePool(PagedPool, sizeof(CM_RESOURCE_LIST) +
                                                           (temp - 1) * sizeof(CM_PARTIAL_RESOURCE_DESCRIPTOR));
                if (tempCmList == NULL)
                {
                ...
                //
                RtlCopyMemory(tempCmList, cmResList, sizeof(CM_RESOURCE_LIST) - sizeof(CM_PARTIAL_RESOURCE_DESCRIPTOR));
                tempCmList->Count = 1;
                ...
                for (temp = i = 0; i < cmResList->Count; i++)
                {
                    ...
                    //
                    RtlCopyMemory(&(tempCmList->List->PartialResourceList.PartialDescriptors[temp]),
                                  cmFullResList->PartialResourceList.PartialDescriptors,
                                  cmFullResList->PartialResourceList.Count * sizeof(CM_PARTIAL_RESOURCE_DESCRIPTOR));
                ...
                //
                status = ACPIRangeAddCmList(&globalResList, tempCmList);
                if (!NT_SUCCESS(status))
                {
