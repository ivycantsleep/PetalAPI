...
WMIGUIDREGINFO ACPIThermalGuidList = { &THERMAL_ZONE_GUID, 1, 0 };
...
VOID ACPIThermalCalculateProcessorMask(IN PNSOBJ ProcessorObject, IN PTHRM_INFO Thrm)
/*++
...
VOID ACPIThermalCancelRequest(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    ACPIThermalPrint((ACPI_PRINT_THERMAL, deviceExtension, currentTime, "ACPIThermalCancelRequest: Irp %08lx\n", Irp));
#endif
...
VOID EXPORT ACPIThermalComplete(IN PNSOBJ AcpiObject, IN NTSTATUS Status, IN POBJDATA Result OPTIONAL,
...
BOOLEAN
ACPIThermalCompletePendingIrps(IN PDEVICE_EXTENSION DeviceExtension, IN PTHRM_INFO Thermal)
/*++
...
--*/
{
    ...
    while (listEntry != &doneList)
    {
        ...
        ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, KeQueryInterruptTime(), "Completing Irp 0x%x\n", irp));
...
NTSTATUS
ACPIThermalDeviceControl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
#if DBG
        ACPIThermalPrint((ACPI_PRINT_THERMAL, deviceExtension, currentTime,
                          "%08x - THERMAL_QUERY_INFORMATION: %x - %x\n", Irp, thermalInfo->ThermalStamp,
...
#if DBG
        ACPIThermalPrint(
            (ACPI_PRINT_THERMAL, deviceExtension, currentTime, "%08x - SET_COOLING_POLICY: %x\n", Irp, Thrm->Mode));
#endif
...
#if DBG
        ACPIThermalPrint((ACPI_PRINT_THERMAL, deviceExtension, currentTime, "%08x - ACTIVE_COOLING_LEVEL: %x\n", Irp,
                          Thrm->CoolingLevel));
#endif
...
VOID ACPIThermalEvent(IN PDEVICE_OBJECT DeviceObject, IN ULONG EventData)
/*++
...
--*/
{
    ...
    ACPIThermalPrint(
        (ACPI_PRINT_THERMAL, deviceExtension, KeQueryInterruptTime(), "ACPIThermalEvent - Notify(%x)\n", EventData));
...
NTSTATUS
ACPIThermalFanStartDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    // Let the world know
    //
    ACPIDevPrint((ACPI_PRINT_THERMAL, deviceExtension, "(0x%08lx): %s = 0x%08lx\n", Irp,
...
VOID ACPIThermalLoop(IN PDEVICE_EXTENSION DeviceExtension, IN ULONG Clear)
/*++
    ...
    if (!(DeviceExtension->Thermal.Flags & THRM_IN_SERVICE_LOOP))
    {
        ...
        for (;;)
        {
            ...
            if (!(DeviceExtension->Thermal.Flags & THRM_INITIALIZE))
            {
                ...
                ACPISetDeviceWorker(DeviceExtension, THRM_COOLING_LEVEL | THRM_INITIALIZE);
                continue;
            }
            ...
            if (!(DeviceExtension->Thermal.Flags & THRM_MODE))
            {
                ...
                status = ACPIGetNothingEvalIntegerAsync(DeviceExtension, PACKED_SCP, thermal->Mode, ACPIThermalComplete,
                                                        DeviceExtension);
                if (status != STATUS_PENDING)
                {

                    ACPIThermalComplete(NULL, status, NULL, DeviceExtension);
                }
                continue;
            }
            ...
            // the loop
            //
            if ((DeviceExtension->Thermal.Flags & THRM_WAIT_FOR_NOTIFY) && (DeviceExtension->Thermal.Flags & THRM_TEMP))
            {
            ...
            if (!(DeviceExtension->Thermal.Flags & THRM_TEMP))
            {
                ...
                status = AMLIAsyncEvalObject(thermal->TempMethod, &thermal->Temp, 0, NULL, ACPIThermalTempatureRead,
                                             DeviceExtension);

                if (status != STATUS_PENDING)
                {

                    ACPIThermalTempatureRead(thermal->TempMethod, status, &thermal->Temp, DeviceExtension);
                }
                continue;
            }
            ...
            // any requests
            //
            doneRequests = ACPIThermalCompletePendingIrps(DeviceExtension, thermal);
            if (doneRequests)
            {
...
VOID ACPIThermalPowerCallback(IN PDEVICE_EXTENSION DeviceExtension, IN PVOID Context, IN NTSTATUS Status)
/*++
...
--*/
{
    if (!NT_SUCCESS(Status))
    {
        ...
        ACPIDevPrint(
            (ACPI_PRINT_CRITICAL, DeviceExtension, "ACPIThermalPowerCallBack: failed power setting %x\n", Status));
    }
}

NTSTATUS
ACPIThermalQueryWmiDataBlock(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN ULONG GuidIndex, IN ULONG InstanceIndex,
...
NTSTATUS
ACPIThermalQueryWmiRegInfo(IN PDEVICE_OBJECT DeviceObject, OUT ULONG *RegFlags, OUT PUNICODE_STRING InstanceName,
                           OUT PUNICODE_STRING *RegistryPath, OUT PUNICODE_STRING MofResourceName,
...
NTSTATUS
ACPIThermalStartDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
/*++
...
--*/
{
    ...
    ACPIDevPrint((ACPI_PRINT_IRP, deviceExtension, "(0x%08lx): IRP_MN_START_DEVICE\n", Irp));
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        ACPIDevPrint(
            (ACPI_PRINT_FAILURE, deviceExtension, "ACPIThermalStartDevice -> SetDeviceInterface = 0x%08lx\n", status));
        goto ACPIThermalStartDeviceExit;
    }

    ACPIRegisterForDeviceNotifications(DeviceObject, (PDEVICE_NOTIFY_CALLBACK)ACPIThermalEvent, (PVOID)DeviceObject);

    //
    // Initialize device object for WMILIB
    //
    wmilibContext = ExAllocatePoolWithTag(PagedPool, sizeof(WMILIB_CONTEXT), ACPI_THERMAL_POOLTAG);
    ...
    //        that case
    //
    status = ACPIDeviceInternalDeviceRequest(deviceExtension, PowerDeviceD0, NULL, NULL, 0);
    if (status == STATUS_PENDING)
    {
...
VOID EXPORT ACPIThermalTempatureRead(IN PNSOBJ AcpiObject, IN NTSTATUS Status, IN POBJDATA Result OPTIONAL,
                                     IN PVOID Context)
/*++
...
--*/
{
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        ACPIThermalPrint((ACPI_PRINT_THERMAL, deviceExtension, KeQueryInterruptTime(),
...
VOID ACPIThermalWorker(IN PDEVICE_EXTENSION DeviceExtension, IN ULONG Events)
/*++
...
--*/
{
    ...
    if (Events & THRM_INITIALIZE)
    {
        ...
        ULONG names[10] = {
            PACKED_AL0, PACKED_AL1, PACKED_AL2, PACKED_AL3, PACKED_AL4,
            PACKED_AL5, PACKED_AL6, PACKED_AL7, PACKED_AL8, PACKED_AL9,
        };
        ...
        for (Level = 0; Level < 10; Level++)
        {
            ...
            //
            ALobj = ACPIAmliGetNamedChild(ThrmObj, names[Level]);
            if (ALobj == NULL)
            {
    ...
    if ((Events & THRM_COOLING_LEVEL))
    {
        ...
        for (Level = 0; Level < 10; Level++)
        {
            ...
            //
            Status = AMLIEvalNameSpaceObject(ALobj, &ALPackage, 0, NULL);
            if (!NT_SUCCESS(Status))
            {
            ...
            for (Index = 0; Index < PackageSize; Index += 1)
            {
                ...
                Status = AMLIEvalPkgDataElement(&ALPackage, Index, &ALElement);
                if (!NT_SUCCESS(Status))
                {
                ...
                //
#if DBG
                ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, currentTime, "ACPIThermalWorker: Turn %s %s\n",
                                  TurnOn ? "on " : "off", ALElement.pbDataBuff));
#endif

                //
                // Find this device of this name
                //
                Status = AMLIGetNameSpaceObject(ALElement.pbDataBuff, ThrmObj, &ACDevObj, 0);
                ...
                // Turn it on/off
                //
                ACPIDeviceInternalDeviceRequest((PDEVICE_EXTENSION)ACDevObj->Context,
                                                TurnOn ? PowerDeviceD0 : PowerDeviceD3, ACPIThermalPowerCallback, NULL,
                                                0);
            }
    ...
    if (Events & THRM_TRIP_POINTS)
    {
        ...
        ULONG names[10] = {
            PACKED_AC0, PACKED_AC1, PACKED_AC2, PACKED_AC3, PACKED_AC4,
            PACKED_AC5, PACKED_AC6, PACKED_AC7, PACKED_AC8, PACKED_AC9,
        };

        //
        // Get the thermal constants, passive & critical values
        //
        ACPIGetIntegerSync(DeviceExtension, PACKED_TC1, &Thrm->Info.ThermalConstant1, NULL);
#if DBG
        ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, currentTime,
                          "ACPIThermalWorker - ThermalConstant1 = %x\n", Thrm->Info.ThermalConstant1));
#endif
        ACPIGetIntegerSync(DeviceExtension, PACKED_TC2, &Thrm->Info.ThermalConstant2, NULL);
#if DBG
        ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, currentTime,
                          "ACPIThermalWorker - ThermalConstant2 = %x\n", Thrm->Info.ThermalConstant2));
#endif
        ACPIGetIntegerSync(DeviceExtension, PACKED_PSV, &Thrm->Info.PassiveTripPoint, NULL);
#if DBG
        ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, currentTime,
                          "ACPIThermalWorker - PassiveTripPoint = %d.%dK\n", (Thrm->Info.PassiveTripPoint / 10),
                          (Thrm->Info.PassiveTripPoint % 10)));
#endif
        ACPIGetIntegerSync(DeviceExtension, PACKED_CRT, &Thrm->Info.CriticalTripPoint, NULL);
#if DBG
        ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, currentTime,
                          "ACPIThermalWorker - CriticalTripPoint = %d.%dK\n", (Thrm->Info.CriticalTripPoint / 10),
                          (Thrm->Info.CriticalTripPoint % 10)));
#endif
        ACPIGetIntegerSync(DeviceExtension, PACKED_TSP, &Thrm->Info.SamplingPeriod, NULL);
#if DBG
        ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, currentTime, "ACPIThermalWorker - SamplingPeriod = %x\n",
        ...
        for (Level = 0; Level < 10; Level++)
        {
            ...
            Status = ACPIGetIntegerSync(DeviceExtension, names[Level], &Thrm->Info.ActiveTripPoint[Level], NULL);
            if (!NT_SUCCESS(Status))
            {
            ...
            ACPIThermalPrint((ACPI_PRINT_THERMAL, DeviceExtension, currentTime,
                              ...
                              (Thrm->Info.ActiveTripPoint[Level] / 10), (Thrm->Info.ActiveTripPoint[Level] % 10)));
#endif
        }
        ...
        //
        ALobj = ACPIAmliGetNamedChild(ThrmObj, PACKED_PSL);
        if (ALobj != NULL)
        {
            ...
            for (Index = 0; Index < PackageSize; Index++)
            {
                ...
                // Find this device of this name
                //
                Status = AMLIGetNameSpaceObject(ALElement.pbDataBuff, NULL, &ACDevObj, 0);
...
NTSTATUS
ACPIThermalWmi(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    NTSTATUS status;
    PDEVICE_EXTENSION deviceExtension = ACPIInternalGetDeviceExtension(DeviceObject);
    PIO_STACK_LOCATION irpSp;
