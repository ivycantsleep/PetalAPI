...
#else
#define AMLI_WARN(x)              \
    ConPrintf(MODNAME "_WARN: "); \
    ConPrintf x;                  \
    ConPrintf("\n");
#define AMLI_ERROR(x)              \
    ConPrintf(MODNAME "_ERROR: "); \
    ConPrintf x;                   \
    ConPrintf("\n");               \
    KdBreakPoint();                \
    CHKDEBUGGERREQ();
...
#define RESETERR()                              \
    {                                           \
        gDebugger.rcLastError = STATUS_SUCCESS; \
        gDebugger.szLastError[0] = '\0';        \
    }
#define CHKDEBUGGERREQ()                                \
    if (gDebugger.dwfDebugger & DBGF_DEBUGGER_REQ)      \
    {                                                   \
        ConPrintf("\nProcess AML Debugger Request.\n"); \
        gDebugger.dwfDebugger &= ~DBGF_DEBUGGER_REQ;    \
        AMLIDebugger(FALSE);                            \
    }
...
#define MALLOC_PAGED(n, t) (++gdwcMemObjs, ALLOCPOOLWITHTAG((gdwfAMLI & AMLIF_LOCKED) ? NonPagedPool : PagedPool, n, t))
#define MALLOC_LOCKED(n, t) (++gdwcMemObjs, ALLOCPOOLWITHTAG(NonPagedPool, n, t))
#else
#define MALLOC_PAGED(n, t)                                                                                    \
    (++gdwcMemObjs, ExAllocatePoolWithTagPriority((gdwfAMLI & AMLIF_LOCKED) ? NonPagedPool : PagedPool, n, t, \
                                                  HighPoolPrioritySpecialPoolOverrun))
#define MALLOC_LOCKED(n, t) \
    (++gdwcMemObjs, ExAllocatePoolWithTagPriority(NonPagedPool, n, t, HighPoolPrioritySpecialPoolOverrun))
#endif
...
#define ARGTYPE_SNAME2                          \
    's'                    //supername argument \
