...
BOOLEAN
SetSystemIndicator(SYSTEM_INDICATORS SystemIndicators, ULONG Value)
{
    ...
    default:
        ACPIPrint((ACPI_PRINT_FAILURE, "SetSystemIndicator: Unknown Indicator\n"));
    ...
    if (AMLIERR(AMLIGetNameSpaceObject(IndicatorName, pnssi, &pns, NSF_LOCAL_SCOPE)) == AMLIERR_NONE)
    {
        ...
        else
        {
            ...
            ACPIPrint(
                (ACPI_PRINT_FAILURE, "Attempt to Eval %s %x failed using objdata %x\n", IndicatorName, pns, &Arg0));
            ACPIBreakPoint();
        }
    }
    else
